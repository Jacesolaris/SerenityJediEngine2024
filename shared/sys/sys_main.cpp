...
char* Sys_GetClipboardData()
{
	...
	const auto buf = static_cast<char*>(Z_Malloc(len, TAG_CLIPBOARD));
...
void* Sys_LoadDll(const char* name, const qboolean useSystemLib)
{
	...
	for (const char* libDir : searchPaths)
	{
		if (!libDir[0])
			continue;
...
void* Sys_LoadLegacyGameDll(const char* name, VMMainProc** vmMain, SystemCallProc* systemcalls)
{
	...
	const auto dllEntry = static_cast<DllEntryProc*>(Sys_LoadFunction(libHandle, "dllEntry"));
