...
constexpr auto QCONSOLE_HISTORY = 32;
...
{
	const int pos = qconsole_history_pos < 1 ? QCONSOLE_HISTORY - 1 : qconsole_history_pos - 1;
...
static void CON_HistNext(void)
{
	...
	const int pos = qconsole_history_pos >= QCONSOLE_HISTORY - 1 ? 0 : qconsole_history_pos + 1;
...
static void CON_Show(void)
{
	...
	write_area.Left = 0;
	write_area.Top = binfo.dwCursorPosition.Y;
	write_area.Bottom = binfo.dwCursorPosition.Y;
	write_area.Right = MAX_EDIT_LINE;

	// set color to white
	WORD attrib = CON_ColorCharToAttrib(COLOR_WHITE);
...
{
	const int realLen = qconsole_linelen;
...
void CON_Init(void)
{
	...
	// initialize history
	for (int i = 0; i < QCONSOLE_HISTORY; i++)
...
char* CON_Input(void)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		const WORD key = buff[i].Event.KeyEvent.wVirtualKeyCode;
		...
		{
			const char c = buff[i].Event.KeyEvent.uChar.AsciiChar;

			if (key == VK_BACK)
			{
				if (qconsole_cursor > 0)
				{
					const int newlen = qconsole_linelen > 0 ? qconsole_linelen - 1 : 0;
					if (qconsole_cursor < qconsole_linelen)
					{
