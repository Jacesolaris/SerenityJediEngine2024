...
static qboolean IN_IsConsoleKey(fakeAscii_t key, const int character)
{
	...
	if (cl_consoleKeys->modified)
	{
		...
		while (numConsoleKeys < MAX_CONSOLE_KEYS)
		{
			...
			else
			{
				...
				c->u.key = static_cast<fakeAscii_t>(Key_StringToKeynum(token));
	...
	{
		const consoleKey_t* c = &consoleKeys[i];
...
static fakeAscii_t IN_TranslateSDLToJKKey(SDL_Keysym* keysym, const qboolean down)
{
	...
	if (keysym->sym >= A_LOW_A && keysym->sym <= A_LOW_Z)
		key = static_cast<fakeAscii_t>(A_CAP_A + (keysym->sym - A_LOW_A));
	else if (keysym->sym >= A_LOW_AGRAVE && keysym->sym <= A_LOW_THORN && keysym->sym != A_DIVIDE)
		key = static_cast<fakeAscii_t>(A_CAP_AGRAVE + (keysym->sym - A_LOW_AGRAVE));
	else if (keysym->sym >= SDLK_SPACE && keysym->sym < SDLK_DELETE)
		key = static_cast<fakeAscii_t>(keysym->sym);
	else
	{
	...
	if (cl_consoleUseScanCode->integer)
	{
		...
		{
			const SDL_Keycode translated = SDL_GetKeyFromScancode(SDL_SCANCODE_GRAVE);
...
void IN_Init(void* windowData)
{
	...
	const int appState = SDL_GetWindowFlags(SDL_window);
...
uint8_t ConvertUTF32ToExpectedCharset(const uint32_t utf32)
{
	...
	default: return static_cast<uint8_t>(utf32);
	}
}
...
static void IN_ProcessEvents()
{
	...
	while (SDL_PollEvent(&e))
	{
		...
		case SDL_KEYDOWN:
			...
			if (key == A_BACKSPACE)
				Sys_QueEvent(0, SE_CHAR, CTRL('h'), qfalse, 0, nullptr);
			else if (kg.keys[A_CTRL].down && key >= A_CAP_A && key <= A_CAP_Z)
				Sys_QueEvent(0, SE_CHAR, CTRL(tolower(key)), qfalse, 0, nullptr);
			...
			if (lastKeyDown != A_CONSOLE)
			{
				...
				while (*c)
				{
					...
					if (utf32 != 0)
					{
						...
						{
							Sys_QueEvent(0, SE_KEY, A_CONSOLE, qtrue, 0, nullptr);
							Sys_QueEvent(0, SE_KEY, A_CONSOLE, qfalse, 0, nullptr);
						}
						else
						{
							const uint8_t encoded = ConvertUTF32ToExpectedCharset(utf32);
							Sys_QueEvent(0, SE_CHAR, encoded, 0, 0, nullptr);
						}
					}
				}
			}
		...
		case SDL_MOUSEMOTION:
			if (mouseActive)
			{
				if (!e.motion.xrel && !e.motion.yrel)
					break;
				Sys_QueEvent(0, SE_MOUSE, e.motion.xrel, e.motion.yrel, 0, nullptr);
			}
			break;
		...
		case SDL_MOUSEWHEEL:
			if (e.wheel.y > 0)
			{
				Sys_QueEvent(0, SE_KEY, A_MWHEELUP, qtrue, 0, nullptr);
				Sys_QueEvent(0, SE_KEY, A_MWHEELUP, qfalse, 0, nullptr);
			}
			else if (e.wheel.y < 0)
			{
				Sys_QueEvent(0, SE_KEY, A_MWHEELDOWN, qtrue, 0, nullptr);
				Sys_QueEvent(0, SE_KEY, A_MWHEELDOWN, qfalse, 0, nullptr);
			}
			break;
...
static void IN_JoyMove()
{
	...
	if (total > 0)
	{
		...
		if (balldx || balldy)
		{
			...
			if (abs(balldy) > 1)
				balldy *= 2;
			Sys_QueEvent(0, SE_MOUSE, balldx, balldy, 0, nullptr);
		}
	}
		...
		for (i = 0; i < 4; i++)
		{
			if (((Uint8*)&hats)[i] != ((Uint8*)&stick_state.oldhats)[i])
			{
				...
				case SDL_HAT_UP:
					...
					break;
				case SDL_HAT_RIGHT:
					Sys_QueEvent(0, SE_KEY, hat_keys[4 * i + 1], qfalse, 0, nullptr);
					break;
				case SDL_HAT_DOWN:
					Sys_QueEvent(0, SE_KEY, hat_keys[4 * i + 2], qfalse, 0, nullptr);
					break;
				case SDL_HAT_LEFT:
					Sys_QueEvent(0, SE_KEY, hat_keys[4 * i + 3], qfalse, 0, nullptr);
					break;
				...
				case SDL_HAT_UP:
					...
					break;
				case SDL_HAT_RIGHT:
					Sys_QueEvent(0, SE_KEY, hat_keys[4 * i + 1], qtrue, 0, nullptr);
					break;
				case SDL_HAT_DOWN:
					Sys_QueEvent(0, SE_KEY, hat_keys[4 * i + 2], qtrue, 0, nullptr);
					break;
				case SDL_HAT_LEFT:
					Sys_QueEvent(0, SE_KEY, hat_keys[4 * i + 3], qtrue, 0, nullptr);
					break;
				case SDL_HAT_RIGHTUP:
					Sys_QueEvent(0, SE_KEY, hat_keys[4 * i + 0], qtrue, 0, nullptr);
					Sys_QueEvent(0, SE_KEY, hat_keys[4 * i + 1], qtrue, 0, nullptr);
	...
	if (total > 0)
	{
		if (in_joystickUseAnalog->integer)
		{
			...
			for (i = 0; i < total; i++)
			{
				...
				const float f = static_cast<float>(abs(axis)) / 32767.0f;

				if (f < in_joystickThreshold->value) axis = 0;

				if (axis != stick_state.oldaaxes[i])
				{
					Sys_QueEvent(0, SE_JOYSTICK_AXIS, i, axis, 0, nullptr);
