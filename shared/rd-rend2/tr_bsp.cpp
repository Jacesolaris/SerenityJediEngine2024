...
static void ParseFace(const world_t* worldData, dsurface_t* ds, drawVert_t* verts, packedTangentSpace_t* tangentSpace, float* hdrVertColors, msurface_t* surf, int* indexes) {
	...
	srfBspSurface_t* cv;
	glIndex_t* tri;
	int			num_verts, num_indexes, badTriangles;
	...
	num_verts = LittleLong(ds->num_verts);
	...
	cv->indexes = (glIndex_t*)Hunk_Alloc(num_indexes * sizeof(cv->indexes[0]), h_low);

	cv->num_verts = num_verts;
	cv->verts = (srfVert_t*)Hunk_Alloc(num_verts * sizeof(cv->verts[0]), h_low);
...
void R_MovePatchSurfacesToHunk(world_t* worldData) {
	...
	for (i = 0; i < worldData->numsurfaces; i++) {
		...
		hunkgrid->indexes = (glIndex_t*)Hunk_Alloc(grid->num_indexes * sizeof(glIndex_t), h_low);
		Com_Memcpy(hunkgrid->indexes, grid->indexes, grid->num_indexes * sizeof(glIndex_t));

		hunkgrid->num_verts = grid->num_verts;
		hunkgrid->verts = (srfVert_t*)Hunk_Alloc(grid->num_verts * sizeof(srfVert_t), h_low);
		Com_Memcpy(hunkgrid->verts, grid->verts, grid->num_verts * sizeof(srfVert_t));
...
static void R_CreateWorldVBOs(world_t* worldData)
{
	...
	for (surface = &worldData->surfaces[0]; surface < &worldData->surfaces[worldData->numsurfaces]; surface++)
	{
		...
		if (!bspSurf->num_indexes || !bspSurf->num_verts)
			continue;
	...
	for (firstSurf = lastSurf = surfacesSorted; firstSurf < &surfacesSorted[numSortedSurfaces]; firstSurf = lastSurf)
	{
		...
		while (currVboSize < maxVboSize && currIboSize < maxIboSize && lastSurf < &surfacesSorted[numSortedSurfaces])
		{
			...
			for (currSurf = lastSurf; currSurf < &surfacesSorted[numSortedSurfaces] && (*currSurf)->shader->sortedIndex == currShaderIndex; currSurf++)
			{
				...
				addVboSize += bspSurf->num_verts * sizeof(srfVert_t);
		...
		for (currSurf = firstSurf; currSurf < lastSurf; currSurf++)
		{
			...
			num_verts += bspSurf->num_verts;
			num_indexes += bspSurf->num_indexes;
			numSurfaces++;
		}

		ri.Printf(PRINT_ALL, "...calculating world VBO %d ( %i verts %i tris )\n", k, num_verts, num_indexes / 3);

		// create arrays
		verts = (packedVertex_t*)Hunk_AllocateTempMemory(num_verts * sizeof(packedVertex_t));
		indexes = (glIndex_t*)Hunk_AllocateTempMemory(num_indexes * sizeof(glIndex_t));
		...
		for (currSurf = firstSurf; currSurf < lastSurf; currSurf++)
		{
			...
			bspSurf->minIndex = num_verts + bspSurf->indexes[0];
			bspSurf->maxIndex = num_verts + bspSurf->indexes[0];

			for (i = 0, surf_index = bspSurf->indexes; i < bspSurf->num_indexes; i++, surf_index++)
			{
				indexes[num_indexes++] = num_verts + *surf_index;
				bspSurf->minIndex = MIN(bspSurf->minIndex, num_verts + *surf_index);
				bspSurf->maxIndex = MAX(bspSurf->maxIndex, num_verts + *surf_index);
			}

			bspSurf->firstVert = num_verts;

			for (i = 0; i < bspSurf->num_verts; i++)
			{
				packedVertex_t& vert = verts[num_verts++];
		...
		vbo = R_CreateVBO((byte*)verts, sizeof(packedVertex_t) * num_verts, VBO_USAGE_STATIC);
...
static void R_MergeLeafSurfaces(world_t* worldData)
{
	...
	for (i = 0; i < numWorldSurfaces; i++)
	{
		...
		if (num_verts == 0 || num_indexes == 0 || numSurfsToMerge < 2)
		{
			continue;
		}
