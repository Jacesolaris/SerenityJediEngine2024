...
int CTaskManager::GetVector(const int ent_id, CBlock* block, int& member_num, vec3_t& value, CIcarus* icarus)
{
	...
	if (Check(CIcarus::ID_TAG, block, member_num))
	{
		...
		member_num++;
		ICARUS_VALIDATE(Get(ent_id, block, member_num, &tagName, icarus));
		ICARUS_VALIDATE(GetFloat(ent_id, block, member_num, tagLookup, icarus));

		if (icarus->GetGame()->GetTag(ent_id, tagName, static_cast<int>(tagLookup), value) == false)
		{
...
int CTaskManager::Get(const int ent_id, CBlock* block, int& member_num, char** value, CIcarus* icarus)
{
	...
	if (Check(CIcarus::ID_TAG, block, member_num))
	{
		...
		if (icarus->GetGame()->GetTag(ent_id, tagName, static_cast<int>(tagLookup), vector) == false)
		{
			icarus->GetGame()->DebugPrint(IGameInterface::WL_ERROR, "Unable to find tag \"%s\"!\n", tagName);
			assert(0 && "Unable to find tag");
			return false;
	...
	if (bm->GetID() == CIcarus::TK_VECTOR)
	{
		...
		for (float& i : vval)
		{
			if (GetFloat(ent_id, block, member_num, i, icarus) == false)
				return false;
		}
