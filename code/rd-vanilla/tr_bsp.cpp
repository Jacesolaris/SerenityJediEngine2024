...
static	void R_LoadSurfaces(const lump_t* surfs, const lump_t* verts, const lump_t* index_lump, world_t& world_data, const int index) {
	...
	for (i = 0; i < count; i++, in++)
	{
		...
		case MST_PLANAR:
			...
			int sfaceSize = reinterpret_cast<intptr_t>(&static_cast<srfSurfaceFace_t*>(nullptr)->points[LittleLong in->num_verts]);
			sfaceSize += sizeof(int) * LittleLong in->num_indexes;
