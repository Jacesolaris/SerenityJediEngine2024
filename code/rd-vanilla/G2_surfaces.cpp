...
void G2_FindRecursiveSurface(const model_t* currentModel, int surface_num, surfaceInfo_v& root_list, int* active_surfaces)
{
	...
	const mdxmSurfHierarchy_t* surf_info = reinterpret_cast<mdxmSurfHierarchy_t*>((byte*)surfIndexes + surfIndexes->offsets[surface->thisSurfaceIndex]);
...
int G2_GetParentSurface(const CGhoul2Info* ghlInfo, const int index)
{
	...
	const mdxmSurfHierarchy_t* surf_info = reinterpret_cast<mdxmSurfHierarchy_t*>((byte*)surfIndexes + surfIndexes->offsets[surf->thisSurfaceIndex]);
