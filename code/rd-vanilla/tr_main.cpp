...
int R_CullPointAndRadius(const vec3_t pt, float radius)
{
	...
	// check against frustum planes
#ifdef JK2_MODE
	// They used 4 frustrum planes in JK2, and 5 in JKA
	for (i = 0; i < 4; i++)
	{
...
{
	entvec[0] = -worldvec[0] * preTransEntMatrix[0] - worldvec[1] * preTransEntMatrix[4] + worldvec[2] *
		preTransEntMatrix[8];
	entvec[1] = -worldvec[0] * preTransEntMatrix[1] - worldvec[1] * preTransEntMatrix[5] + worldvec[2] *
		preTransEntMatrix[9];
...
void R_SetupProjection()
{
	...
	const float zFar = tr.viewParms.zFar;

	const float ymax = zNear * tan(tr.refdef.fov_y * M_PI / 360.0f);
	const float ymin = -ymax;

	const float xmax = zNear * tan(tr.refdef.fov_x * M_PI / 360.0f);
...
void R_PlaneForSurface(surfaceType_t* surfType, cplane_t* plane)
{
	...
	{
	case SF_FACE:
		*plane = reinterpret_cast<srfSurfaceFace_t*>(surfType)->plane;
		return;
	case SF_TRIANGLES:
		tri = reinterpret_cast<srfTriangles_t*>(surfType);
		...
		return;
	case SF_POLY:
		poly = reinterpret_cast<srfPoly_t*>(surfType);
		PlaneFromPoints(plane4, poly->verts[0].xyz, poly->verts[1].xyz, poly->verts[2].xyz);
		VectorCopy(plane4, plane->normal);
		plane->dist = plane4[3];
		return;
	case SF_GRID:
		grid = reinterpret_cast<srfGridMesh_t*>(surfType);
		v1 = &grid->verts[0];
		v2 = &grid->verts[1];
		v3 = &grid->verts[2];
...
static qboolean IsMirror(const drawSurf_t* drawSurf, const int entity_num)
{
	...
	for (int i = 0; i < tr.refdef.num_entities; i++)
	{
		...
		const float d = DotProduct(e->e.origin, originalPlane.normal) - originalPlane.dist;
		if (d > 64 || d < -64)
		{
			continue;
		}
...
static qboolean SurfIsOffscreen(const drawSurf_t* drawSurf, vec4_t clipDest[128])
{
	...
	if (IsMirror(drawSurf, entity_num))
	{
		return qfalse;
	}
...
qboolean R_MirrorViewBySurface(drawSurf_t* drawSurf, int entity_num)
{
	vec4_t clipDest[128];
	...
	if (!R_GetPortalOrientations(drawSurf, entity_num, &surface, &camera,
...
static QINLINE void R_Radix(const int byte, const int size, drawSurf_t* source, drawSurf_t* dest)
{
	...
	sortKey = reinterpret_cast<unsigned char*>(&source[0].sort) + byte;
...
void R_SortDrawSurfs(drawSurf_t* drawSurfs, int numDrawSurfs)
{
	...
	{
		R_DecomposeSort((drawSurfs + i)->sort, &entity_num, &shader, &fogNum, &dlighted);
		...
		if (R_MirrorViewBySurface(drawSurfs + i, entity_num))
		{
