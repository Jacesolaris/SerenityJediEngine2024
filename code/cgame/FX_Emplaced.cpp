...
void FX_EmplacedProjectileThink(centity_t* cent, const weaponInfo_s* weapon)
{
	...
	{
		if (cent->gent && cent->gent->owner && cent->gent->owner->activator && cent->gent->owner->activator->s.number >
			0)
		{
			// NPC's do short shot
...
void FX_EmplacedHitPlayer(gentity_t* hit, vec3_t origin, vec3_t normal, const qboolean eweb)
{
	if (eweb)
	{
	...
	{
		theFxScheduler.PlayEffect("eweb/droid_impact", origin, normal);
	}
}
