...
void CG_DrawJK2ForcePower(const centity_t* cent, const int x, const int y)
{
	...
	else
	{
		...
		{
			if (!cent->gent->client->ps.BlasterAttackChainCount && (cg_com_outcast.integer == 1 || cg_com_outcast.
				integer == 2))
			{
				cgi_R_SetColor(otherHUDBits[OHB_JK2FORCEAMOUNT].color);
...
float cg_draw_radar(const float y)
{
	...
	for (int i = cg.radarEntityCount - 1; i >= 0; i--)
	{
		...
		AngleVectors(cg.predicted_player_state.viewangles, dir_look, nullptr, nullptr);
...
static qboolean CG_RenderingFromMiscCamera()
{
	...
	if (cg.snap->ps.viewEntity > 0 &&
		...
		// cent && cent->gent && cent->gent->client && cent->gent->client->ps.viewEntity)
	{
...
static void CG_Draw2D()
{
	...
	else if (cg.predicted_player_state.PlayerEffectFlags & 1 << PEF_BURNING)
	{
		//draw it as long as it isn't full
		CG_DrawPic(0, 0, 640, 480, cgi_R_RegisterShader("gfx/2d/firesense"));
	}
	else if (cg.predicted_player_state.PlayerEffectFlags & 1 << PEF_FREEZING)
	{
		//draw it as long as it isn't full
