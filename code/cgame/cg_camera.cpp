...
void CGCam_Enable()
{
	...
	if (g_entities[0].inuse && g_entities[0].client)
	{
		...
		if (g_entities[0].client->ps.forcePower <= BLOCK_POINTS_MAX)
		{
...
void CGCam_Update()
{
	...
	if (client_camera.info_state & CAMERA_ACCEL)
	{
		...
		else
		{
			...
			{
				actualFOV_X = sanity_min;
			}
			else if (actualFOV_X > sanity_max)
			{
				actualFOV_X = sanity_max;
			}
	...
	if (client_camera.info_state & CAMERA_MOVING)
	{
		...
		else
		{
			if (client_camera.info_state & CAMERA_CUT)
			{
				// we're doing a cut, so just go to the new origin. none of this fancy pants lerping stuff.
				for (i = 0; i < 3; i++)
				{
...
void CGCam_DrawWideScreen()
{
	...
	if (client_camera.bar_alpha)
	{
		...
		if (cg_drawwidescreenmodesp.integer)
		{
			//
		}
		else
...
void CGCam_UpdateShake(vec3_t origin, vec3_t angles)
{
	...
	const float intensity_scale = 1.0f - static_cast<float>(cg.time - client_camera.shake_start) / static_cast<float>(
		client_camera
		.shake_duration) * ((client_camera.FOV + client_camera.FOV2) / 2.0f / 90.0f);
