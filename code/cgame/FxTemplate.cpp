...
//	none
//------------------------------------------------------
CPrimitiveTemplate::CPrimitiveTemplate() : mRefCount(0), mType()
{
	...
	if (fx_optimizedParticles.integer)
	{
...
bool CPrimitiveTemplate::ParseFloat(const gsl::cstring_view& val, float& min, float& max)
{
	...
	const int v = Q::sscanf(val, min, max);
...
bool CPrimitiveTemplate::ParseVector(const gsl::cstring_view& val, vec3_t min, vec3_t max)
{
	...
	const int v = Q::sscanf(val, min[0], min[1], min[2], max[0], max[1], max[2]);
...
bool CPrimitiveTemplate::ParseShaders(const CGPProperty& grp)
{
	...
	for (auto& value : grp.GetValues())
	{
		...
		{
			any = true;
			const int handle = theFxHelper.RegisterShader(value);
...
bool CPrimitiveTemplate::ParseSounds(const CGPProperty& grp)
{
	...
	for (auto& value : grp.GetValues())
	{
		...
		{
			any = true;
			const int handle = theFxHelper.RegisterSound(value);
			mMediaHandles.AddHandle(handle);
		}
	}
	if (!any)
...
bool CPrimitiveTemplate::ParseModels(const CGPProperty& grp)
{
	...
	for (auto& value : grp.GetValues())
	{
		...
		{
			any = true;
			const int handle = theFxHelper.RegisterModel(value);
			mMediaHandles.AddHandle(handle);
		}
	}
	if (!any)
	...
	int& flags, const int successFlags, const gsl::czstring loadError, const gsl::czstring emptyError)
{
	...
	for (auto& value : grp.GetValues())
	{
		if (!value.empty())
		{
			...
			const int handle = scheduler.RegisterEffect(std::string(value.begin(), value.end()).c_str());
			if (handle)
			{
	...
	const gsl::czstring name)
{
	for (auto& cur : grp.GetProperties())
	{
		...
		{
			const ParseMethod method = pos->second;
...
bool CPrimitiveTemplate::ParsePrimitive(const CGPGroup& grp)
{
	...
	for (auto& prop : grp.GetProperties())
	{
		...
		// Property Parsing
		{
			...
			if (pos != parseMethods.end())
			{
				const PropertyParseMethod method = pos->second;
				(this->*method)(prop);
				continue;
			}
		}
	...
	for (auto& subGrp : grp.GetSubGroups())
	{
		...
		{
			const GroupParseMethod method = pos->second;
			(this->*method)(subGrp);
		}
	}
	return true;
