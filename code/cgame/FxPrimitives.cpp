...
void ClampVec(vec3_t dat, byte* res)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		res[i] = static_cast<unsigned char>(r);
	}
}
...
void CParticle::UpdateSize()
{
	...
	if (mFlags & FX_SIZE_LINEAR)
	{
		...
		perc1 = 1.0f - static_cast<float>(theFxHelper.mTime - mTimeStart)
			/ static_cast<float>(mTimeEnd - mTimeStart);
	}
	...
	if ((mFlags & FX_SIZE_PARM_MASK) == FX_SIZE_NONLINEAR)
	{
		if (theFxHelper.mTime > mSizeParm)
		{
			...
			perc2 = 1.0f - (theFxHelper.mTime - mSizeParm)
				/ (mTimeEnd - mSizeParm);
		}
	...
	else if ((mFlags & FX_SIZE_PARM_MASK) == FX_SIZE_CLAMP)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = (mSizeParm - theFxHelper.mTime)
				/ (mSizeParm - mTimeStart);
		}
		else
		{
...
void CParticle::UpdateRGB()
{
	...
	if ((mFlags & FX_RGB_PARM_MASK) == FX_RGB_NONLINEAR)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = 1.0f - (theFxHelper.mTime - mRGBParm)
				/ (mTimeEnd - mRGBParm);
		}
	...
	else if ((mFlags & FX_RGB_PARM_MASK) == FX_RGB_CLAMP)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = (mRGBParm - theFxHelper.mTime)
				/ (mRGBParm - mTimeStart);
		}
		else
		{
	...
	VectorMA(res, 1.0f - perc1, mRGBEnd, mRefEnt.angles);
	// angles is a temp storage, will get clamped to a byte in the UpdateAlpha section
}
...
void CParticle::UpdateAlpha()
{
	...
	if ((mFlags & FX_ALPHA_PARM_MASK) == FX_ALPHA_NONLINEAR)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = 1.0f - (theFxHelper.mTime - mAlphaParm)
				/ (mTimeEnd - mAlphaParm);
		}
	...
	else if ((mFlags & FX_ALPHA_PARM_MASK) == FX_ALPHA_CLAMP)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = (mAlphaParm - theFxHelper.mTime)
				/ (mAlphaParm - mTimeStart);
		}
		else
		{
			perc2 = 0.0f; // make it full size??
...
//----------------------------
bool COrientedParticle::Cull() const
{
	vec3_t dir;
...
void CTail::UpdateLength()
{
	...
	if ((mFlags & FX_LENGTH_PARM_MASK) == FX_LENGTH_NONLINEAR)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = 1.0f - (theFxHelper.mTime - mLengthParm)
				/ (mTimeEnd - mLengthParm);
		}
	...
	else if ((mFlags & FX_LENGTH_PARM_MASK) == FX_LENGTH_CLAMP)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = (mLengthParm - theFxHelper.mTime)
				/ (mLengthParm - mTimeStart);
		}
		else
		{
			perc2 = 0.0f; // make it full size??
...
void CCylinder::UpdateSize2()
{
	...
	if ((mFlags & FX_SIZE2_PARM_MASK) == FX_SIZE2_NONLINEAR)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = 1.0f - (theFxHelper.mTime - mSize2Parm)
				/ (mTimeEnd - mSize2Parm);
		}
	...
	else if ((mFlags & FX_SIZE2_PARM_MASK) == FX_SIZE2_CLAMP)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = (mSize2Parm - theFxHelper.mTime)
				/ (mSize2Parm - mTimeStart);
		}
		else
		{
			perc2 = 0.0f; // make it full size??
...
void CEmitter::Draw()
{
	...
	if (mFlags & FX_ATTACHED_MODEL)
	{
		...
		for (auto& axi : mRefEnt.axis)
		{
			VectorScale(axi, mRefEnt.radius, axi);
		}
	...
	if (mFlags & FX_EMIT_FX)
	{
		...
		float step = mDensity + Q_flrand(-1.0f, 1.0f) * mVariance;
		step *= step;
...
bool CBezier::Update()
{
	...
	*/
	const float ftime = cg.frametime * 0.001f;
	const float time2 = ftime * ftime * 0.5f;
...
void CBezier::DrawSegment(vec3_t start, vec3_t end, const float texcoord1, const float texcoord2)
{
	...
	const float scale = mRefEnt.radius * 0.5f;
...
void CFlash::Init()
{
	...
	float mod = DotProduct(dif, cg.refdef.viewaxis[0]);
...
void CFlash::Draw()
{
	...
	for (float& i : mRefEnt.lightingOrigin)
	{
		if (i > 1.0f)
		{
			i = 1.0f;
