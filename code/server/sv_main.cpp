...
void SV_CheckTimeouts()
{
	...
	const int droppoint = sv.time - 1000 * sv_timeout->integer;
	const int zombiepoint = sv.time - 1000 * sv_zombietime->integer;
...
void SV_Frame(int msec, const float fractionMsec)
{
	...
	SG_TestSave();
	// returns immediately if not active, used for fake-save-every-cycle to test (mainly) Icarus disk code
