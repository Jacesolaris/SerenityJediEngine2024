...
#if JK2_MODE
#define PRODUCT_NAME			"SerenityJediEngineJO2023-SP"

#define CLIENT_WINDOW_TITLE "SerenityJediEngineJO2023 (SP)"
#define CLIENT_CONSOLE_TITLE "SerenityJediEngineJO2023 Console (SP)"
#define HOMEPATH_NAME_UNIX "SerenityJediEngineJO2023"
#define HOMEPATH_NAME_WIN "SerenityJediEngineJO2023"
#define HOMEPATH_NAME_MACOSX HOMEPATH_NAME_WIN
#else
#define PRODUCT_NAME			"SerenityJediEngine2024-SP"
...
constexpr auto BASEGAME = "base";
constexpr auto SerenityJediEngine2024GAME = "SerenityJediEngine2024";
...
constexpr auto MAX_STRING_CHARS = 1024; // max length of a string passed to Cmd_TokenizeString;
...
constexpr auto BIG_INFO_STRING = 8192; // used for system info key only;
constexpr auto BIG_INFO_KEY = 8192;
constexpr auto BIG_INFO_VALUE = 8192;

#define MAX_QPATH 64		// max length of a quake game pathname;
#ifdef PATH_MAX
#define MAX_OSPATH			PATH_MAX
#else
constexpr auto MAX_OSPATH = 256; // max length of a filesystem pathname;
#endif

constexpr auto MAX_NAME_LENGTH = 64; // max length of a client name;
...
//
constexpr auto MAX_MAP_AREA_BYTES = 64; // bit vector of area visibility;

// Light Style Constants

constexpr auto LS_STYLES_START = 0;
constexpr auto LS_NUM_STYLES = 32;
...
constexpr auto PROP_GAP_WIDTH = 2; // 3;
constexpr auto PROP_SPACE_WIDTH = 4;
constexpr auto PROP_HEIGHT = 16;

constexpr auto PROP_TINY_SIZE_SCALE = 1;
constexpr auto PROP_SMALL_SIZE_SCALE = 1;
constexpr auto PROP_BIG_SIZE_SCALE = 1;
constexpr auto PROP_GIANT_SIZE_SCALE = 2;

constexpr auto PROP_TINY_HEIGHT = 10;
constexpr auto PROP_GAP_TINY_WIDTH = 1;
constexpr auto PROP_SPACE_TINY_WIDTH = 3;

constexpr auto PROP_BIG_HEIGHT = 24;
constexpr auto PROP_GAP_BIG_WIDTH = 3;
constexpr auto PROP_SPACE_BIG_WIDTH = 6;

constexpr auto BLINK_DIVISOR = 600;
constexpr auto PULSE_DIVISOR = 75;

constexpr auto UI_LEFT = 0x00000000; // default;
constexpr auto UI_CENTER = 0x00000001;
constexpr auto UI_RIGHT = 0x00000002;
constexpr auto UI_FORMATMASK = 0x00000007;
constexpr auto UI_SMALLFONT = 0x00000010;
constexpr auto UI_BIGFONT = 0x00000020; // default;

constexpr auto UI_DROPSHADOW = 0x00000800;
constexpr auto UI_BLINK = 0x00001000;
constexpr auto UI_INVERSE = 0x00002000;
constexpr auto UI_PULSE = 0x00004000;
...
//
constexpr auto CIN_system = 1;
constexpr auto CIN_loop = 2;
constexpr auto CIN_hold = 4;
constexpr auto CIN_silent = 8;
constexpr auto CIN_shader = 16;
...
constexpr auto BIGCHAR_WIDTH = 16;
constexpr auto BIGCHAR_HEIGHT = 16;

constexpr auto GIANTCHAR_WIDTH = 32;
...
constexpr auto MAX_BATTERIES = 2500;
...
constexpr auto TRUNCATE_LENGTH = 64;
...
constexpr auto CVAR_TEMP = 0; // can be set even when cheats are disabled, but is not archived;
constexpr auto CVAR_ARCHIVE = 1; // set to cause it to be saved to vars.rc;
// used for system variables, not for player
// specific configurations
constexpr auto CVAR_USERINFO = 2; // sent to server on connect or change;
constexpr auto CVAR_SERVERINFO = 4; // sent in response to front end requests;
constexpr auto CVAR_SYSTEMINFO = 8; // these cvars will be duplicated on all clients;
constexpr auto CVAR_INIT = 16; // don't allow change from console at all,;
// but can be set from the command line
constexpr auto CVAR_LATCH = 32; // will only change when C code next does;
// a Cvar_Get(), so it can't be changed
// without proper initialization.  modified
// will be set, even though the value hasn't
// changed yet
constexpr auto CVAR_ROM = 64; // display only, cannot be set by user at all;
constexpr auto CVAR_USER_CREATED = 128; // created by a set command;
constexpr auto CVAR_SAVEGAME = 256; // store this in the savegame;
constexpr auto CVAR_CHEAT = 512; // can not be changed if cheats are disabled;
constexpr auto CVAR_NORESTART = 1024; // do not clear when a cvar_restart is issued;

constexpr auto CVAR_SERVER_CREATED = 2048; // cvar was created by a server the client connected to.;
constexpr auto CVAR_VM_CREATED = 4096; // cvar was created exclusively in one of the VMs.;
constexpr auto CVAR_PROTECTED = 8192; // prevent modifying this var from VMs or the server;
constexpr auto CVAR_NODEFAULT = 16384; // do not write to config if matching with default value;

#define CVAR_ARCHIVE_ND		(CVAR_ARCHIVE | CVAR_NODEFAULT)
// These flags are only returned by the Cvar_Flags() function
constexpr auto CVAR_MODIFIED = 0x40000000; // Cvar was modified;
constexpr auto CVAR_NONEXISTENT = 0x80000000; // Cvar doesn't exist.;
...
constexpr auto MAX_CVAR_VALUE_STRING = 256;
...
constexpr auto KEYCATCH_CONSOLE = 1;
constexpr auto KEYCATCH_UI = 2;
...
constexpr auto MAX_SUBMODELS = 512; // nine bits;

constexpr auto MAX_FX = 256;
...
Ghoul2 Insert Start
*/
constexpr auto MAX_CHARSKINS = 256; // character skins;
/*
Ghoul2 Insert End
*/
...
constexpr auto CS_SERVERINFO = 0; // an info string with all the serverinfo cvars;
constexpr auto CS_SYSTEMINFO = 1; // an info string for server system to client system configuration (timescale, etc);

constexpr auto RESERVED_CONFIGSTRINGS = 2; // game can't modify below this, only the system can;
//
...
constexpr auto CS_MUSIC = 2;
constexpr auto CS_MESSAGE = 3; // from the map worldspawn's message field;
constexpr auto CS_ITEMS = 4; // string of 0's and 1's that tell which items are present;
constexpr auto CS_AMBIENT_SET = 5; // ambient set information for the player;
...
using forcePowers_t = enum
{
	...
	FP_DRAIN,
	//hold/duration - drain force power for health
	FP_SEE,
	//duration - detect/see hidden enemies
...
using skills_t = enum
{
	...
	//detpack skill
	SK_REPEATER,
	//Repeater/clone Rifle skill
	SK_DISRUPTOR,
	//Disruptor/sniper rifle skill
	SK_BLUESTYLE,
	//Yellow lightsaber style
	SK_REDSTYLE,
	//Red lightsaber style
	SK_PURPLESTYLE,
	//Purple lightsaber style
	SK_GREENSTYLE,
	//Green lightsaber style
	SK_DUALSTYLE,
	//Dual lightsaber style
	SK_STAFFSTYLE,
	//Staff lightsaber style
	SK_REPEATERUPGRADE,
	//[Repeater]
	SK_FLECHETTE,
	//[Flechette]
	SK_BLASTERRATEOFFIREUPGRADE,
	//[BlasterRateOfFire]
	SK_ACROBATICS,
	SK_SHIELD,
	SK_TUSKEN_RIFLE,
...
//
constexpr auto MAX_PERSISTANT = 16;

constexpr auto MAX_POWERUPS = 16;
constexpr auto MAX_WEAPONS = 32;
#define MAX_AMMO				MAX_WEAPONS
constexpr auto MAX_INVENTORY = 15; // See INV_MAX;
constexpr auto MAX_SECURITY_KEYS = 5;
constexpr auto MAX_SECURITY_KEY_MESSSAGE = 24;

constexpr auto MAX_PS_EVENTS = 2; // this must be a power of 2 unless you change some &'s to %'s -ste;
...
constexpr auto SFL_NOT_LOCKABLE = 1 << 0; //can't get into a saberlock;
constexpr auto SFL_NOT_THROWABLE = 1 << 1;
//can't be thrown - FIXME: maybe make this a max level of force saber throw that can be used with this saber?;
constexpr auto SFL_NOT_DISARMABLE = 1 << 2; //can't be dropped;
constexpr auto SFL_NOT_ACTIVE_BLOCKING = 1 << 3; //don't to try to block incoming shots with this saber;
constexpr auto SFL_TWO_HANDED = 1 << 4; //uses both hands;
constexpr auto SFL_SINGLE_BLADE_THROWABLE = 1 << 5; //can throw this saber if only the first blade is on;
constexpr auto SFL_RETURN_DAMAGE = 1 << 6; //when returning from a saber throw, it keeps spinning and doing damage;
//NEW FLAGS
constexpr auto SFL_ON_IN_WATER = 1 << 7; //if set, weapon stays active even in water;
constexpr auto SFL_BOUNCE_ON_WALLS = 1 << 8;
//if set, the saber will bounce back when it hits solid architecture (good for real-sword type mods);
constexpr auto SFL_BOLT_TO_WRIST = 1 << 9;
...
//Move Restrictions
constexpr auto SFL_NO_PULL_ATTACK = 1 << 10; //if set, cannot do pull+attack move (move not available in MP anyway);
constexpr auto SFL_NO_BACK_ATTACK = 1 << 11; //if set, cannot do back-stab moves;
constexpr auto SFL_NO_STABDOWN = 1 << 12; //if set, cannot do stabdown move (when enemy is on ground);
constexpr auto SFL_NO_WALL_RUNS = 1 << 13; //if set, cannot side-run or forward-run on walls;
constexpr auto SFL_NO_WALL_FLIPS = 1 << 14; //if set, cannot do backflip off wall or side-flips off walls;
constexpr auto SFL_NO_WALL_GRAB = 1 << 15; //if set, cannot grab wall & jump off;
constexpr auto SFL_NO_ROLLS = 1 << 16; //if set, cannot roll;
constexpr auto SFL_NO_FLIPS = 1 << 17; //if set, cannot do flips;
constexpr auto SFL_NO_CARTWHEELS = 1 << 18; //if set, cannot do cartwheels;
constexpr auto SFL_NO_KICKS = 1 << 19;
//if set, cannot do kicks (can't do kicks anyway if using a throwable saber/sword);
constexpr auto SFL_NO_MIRROR_ATTACKS = 1 << 20;
//if set, cannot do the simultaneous attack left/right moves (only available in Dual Lightsaber Combat Style);
constexpr auto SFL_NO_ROLL_STAB = 1 << 21; //if set, cannot do roll-stab move at end of roll;
//SABER FLAGS2
//Primary Blade Style
constexpr auto SFL2_NO_WALL_MARKS = 1 << 0;
//if set, stops the saber from drawing marks on the world (good for real-sword type mods);
constexpr auto SFL2_NO_DLIGHT = 1 << 1;
//if set, stops the saber from drawing a dynamic light (good for real-sword type mods);
constexpr auto SFL2_NO_BLADE = 1 << 2; //if set, stops the saber from drawing a blade (good for real-sword type mods);
constexpr auto SFL2_NO_CLASH_FLARE = 1 << 3;
//if set, the saber will not do the big, white clash flare with other sabers;
constexpr auto SFL2_NO_DISMEMBERMENT = 1 << 4;
//if set, the saber never does dismemberment (good for pointed/blunt melee weapons);
constexpr auto SFL2_NO_IDLE_EFFECT = 1 << 5;
//if set, the saber will not do damage or any effects when it is idle (not in an attack anim).  (good for real-sword type mods);
constexpr auto SFL2_ALWAYS_BLOCK = 1 << 6;
//if set, the blades will always be blocking (good for things like shields that should always block);
constexpr auto SFL2_NO_MANUAL_DEACTIVATE = 1 << 7; //if set, the blades cannot manually be toggled on and off;
constexpr auto SFL2_TRANSITION_DAMAGE = 1 << 8;
//if set, the blade does damage in start, transition and return anims (like strong style does);
//Secondary Blade Style
constexpr auto SFL2_NO_WALL_MARKS2 = 1 << 9;
//if set, stops the saber from drawing marks on the world (good for real-sword type mods);
constexpr auto SFL2_NO_DLIGHT2 = 1 << 10;
//if set, stops the saber from drawing a dynamic light (good for real-sword type mods);
constexpr auto SFL2_NO_BLADE2 = 1 << 11; //if set, stops the saber from drawing a blade (good for real-sword type mods);
constexpr auto SFL2_NO_CLASH_FLARE2 = 1 << 12;
//if set, the saber will not do the big, white clash flare with other sabers;
constexpr auto SFL2_NO_DISMEMBERMENT2 = 1 << 13;
//if set, the saber never does dismemberment (good for pointed/blunt melee weapons);
constexpr auto SFL2_NO_IDLE_EFFECT2 = 1 << 14;
//if set, the saber will not do damage or any effects when it is idle (not in an attack anim).  (good for real-sword type mods);
constexpr auto SFL2_ALWAYS_BLOCK2 = 1 << 15;
//if set, the blades will always be blocking (good for things like shields that should always block);
constexpr auto SFL2_NO_MANUAL_DEACTIVATE2 = 1 << 16; //if set, the blades cannot manually be toggled on and off;
constexpr auto SFL2_TRANSITION_DAMAGE2 = 1 << 17;
//if set, the blade does damage in start, transition and return anims (like strong style does);
...
using saberInfo_t = struct
{
	...
	//force powers that cannot be used while this saber is on (bitfield) - FIXME: maybe make this a limit on the max level, per force power, that can be used with this type?
	...
	qhandle_t fallSound[3];
	//none - if set, plays one of these 3 sounds when weapon drops to the ground - NOTE: must provide all 3!!!
	...
	// Custom saber glow, blade & dlight color code
	char glowshader[MAX_QPATH]; //none - if set, overrides the shader used for the saber glow
	...
	int trailStyle2;
	//0 - default (0) is normal, 1 is a motion blur and 2 is no trail at all (good for real-sword type mods)
	char g2MarksShader2[MAX_QPATH];
	//none - if set, the game will use this shader for marks on enemies instead of the default "gfx/damage/saberglowmark"
	char g2WeaponMarkShader2[MAX_QPATH];
	//none - if set, the game will ry to project this shader onto the weapon when it damages a person (good for a blood splatter on the weapon)
	...
	qboolean ActiveManualOnly() const
	{
		for (int i = 0; i < numBlades; i++)
		{
		...
		ojk::SavedGameHelper& saved_game) const
	{
		...
		saved_game.write<int32_t>(surrenderAnim);
		...
		// Custom saber glow, blade & dlight color code
		saved_game.write<int8_t>(glowshader);
		saved_game.write<int8_t>(bladeshader);
		saved_game.write<float>(saberDLightColor[0]);
		saved_game.write<float>(saberDLightColor[1]);
		saved_game.write<float>(saberDLightColor[2]);
	}
		...
		ojk::SavedGameHelper& saved_game)
	{
		...
		saved_game.read<int32_t>(surrenderAnim);
		...
		// Custom saber glow, blade & dlight color code
		saved_game.read<int8_t>(glowshader);
		saved_game.read<int8_t>(bladeshader);
		saved_game.read<float>(saberDLightColor[0]);
		saved_game.read<float>(saberDLightColor[1]);
		saved_game.read<float>(saberDLightColor[2]);
	}
};
...
public:
	...
	qboolean throwable;
	//whether or not this saber can be thrown - FIXME: maybe make this a max level of force saber throw that can be used with this saber?
...
// if this ever changes then update the table "static const save_field_t savefields_gClient[]"!!!!!!!!!!!!;
...
public:
	...
	int BlockPointRegenRate; //default is 100ms
	int forcePowerRegenAmount; //default is 1
	int BlockPointRegenAmount; //default is 1
#endif // !JK2_MODE
	...
	float waterheight;
	//exactly what the z org of the water is (will be +4 above if under water, -4 below if not in water)
	...
	int gesturinglaststartTime;

	int surrendertimeplayer;
		...
		ojk::SavedGameHelper& saved_game) const
	{
		...
		saved_game.write<int32_t>(saberLockHitIncrementTime);
		saved_game.write<int32_t>(saberLockHitCheckTime);
		...
		saved_game.write<int32_t>(BlockPointRegenRate);
		saved_game.write<int32_t>(forcePowerRegenAmount);
		saved_game.write<int32_t>(BlockPointRegenAmount);
#endif // !JK2_MODE
		...
		saved_game.write<int32_t>(stasisJediTime);
		saved_game.write<int32_t>(cloakFuel);
		saved_game.write<int32_t>(BarrierFuel);
		saved_game.write<int32_t>(blockPoints);
		...
		saved_game.write<int32_t>(userInt3);
		saved_game.write<int32_t>(userFloat1);
		saved_game.write<int32_t>(userFloat2);
		saved_game.write<int32_t>(userFloat3);
		...
		saved_game.write<int32_t>(gesturinglaststartTime);

		saved_game.write<int32_t>(surrendertimeplayer);
		...
		saved_game.write<int32_t>(grappletimeplayer);
		saved_game.write<int32_t>(grapplestartTime);
		saved_game.write<int32_t>(grapplelaststartTime);

		saved_game.write<int32_t>(communicatingflags);
		saved_game.write<int32_t>(jetpackFuel);
		saved_game.write<int32_t>(jetPackOn);
		...
		saved_game.write<int32_t>(flamethrowerOn);
		saved_game.write<int32_t>(IsSprinting);
		saved_game.write<int32_t>(sprintFuel);
		saved_game.write<int32_t>(sprintkDebRecharge);
		...
		ojk::SavedGameHelper& saved_game)
	{
		...
		saved_game.read<int32_t>(saberLockHitIncrementTime);
		saved_game.read<int32_t>(saberLockHitCheckTime);
		...
		saved_game.read<int32_t>(BlockPointRegenRate);
		saved_game.read<int32_t>(forcePowerRegenAmount);
		saved_game.read<int32_t>(BlockPointRegenAmount);
#endif // !JK2_MODE
		...
		saved_game.read<int32_t>(BarrierFuel);
		saved_game.read<int32_t>(blockPoints);
		...
		saved_game.read<int32_t>(userInt3);
		saved_game.read<int32_t>(userFloat1);
		saved_game.read<int32_t>(userFloat2);
		saved_game.read<int32_t>(userFloat3);
		...
		saved_game.read<int32_t>(gesturinglaststartTime);

		saved_game.read<int32_t>(surrendertimeplayer);
		...
		saved_game.read<int32_t>(grappletimeplayer);
		saved_game.read<int32_t>(grapplestartTime);
		...
		saved_game.read<int32_t>(jetPackOn);
		...
		saved_game.read<int32_t>(flamethrowerOn);
		saved_game.read<int32_t>(IsSprinting);
		saved_game.read<int32_t>(sprintFuel);
		saved_game.read<int32_t>(sprintkDebRecharge);
...
constexpr auto BUTTON_FORCE_LIGHTNING = 2; // displays talk balloon and disables actions;
constexpr auto BUTTON_USE_FORCE = 4;
constexpr auto BUTTON_FORCE_DRAIN = 8; // draining;
constexpr auto BUTTON_VEH_SPEED = 8; // used for some horrible vehicle hack... :);
constexpr auto BUTTON_WALKING = 16;
// walking can't just be infered from MOVE_RUN because a key pressed late in the frame will;
...
constexpr auto BUTTON_FORCE_FOCUS = 256; // any key whatsoever;
//New  buttons
constexpr auto BUTTON_REPULSE = 512; //+button9;
constexpr auto BUTTON_FORCEGRASP = 1024; //+button10;
constexpr auto BUTTON_PRONEUP = 2048; //+button11;
constexpr auto BUTTON_GRAPPLE = 4096; //+button12;
constexpr auto BUTTON_KICK = 8192; //+button13;
constexpr auto BUTTON_BLOCK = 16384; //+button14;
constexpr auto BUTTON_DASH = 32768; //+button15;
constexpr auto BUTTON_16 = 65536;
constexpr auto BUTTON_17 = 131072;
constexpr auto BUTTON_18 = 262144;
constexpr auto BUTTON_19 = 524288;
constexpr auto BUTTON_20 = 1048576;

constexpr auto MOVE_RUN = 120; // if forwardmove or rightmove are >= MOVE_RUN,;
// then BUTTON_WALKING should be set

//Bitmasks for view locking
constexpr auto LOCK_RIGHT = 1;
constexpr auto LOCK_LEFT = 2;
constexpr auto LOCK_UP = 4;
constexpr auto LOCK_DOWN = 8;

//Bitmasks for move locking
constexpr auto LOCK_MOVERIGHT = 16;
constexpr auto LOCK_MOVELEFT = 32;
constexpr auto LOCK_MOVEFORWARD = 64;
constexpr auto LOCK_MOVEBACK = 128;
constexpr auto LOCK_MOVEUP = 256;
constexpr auto LOCK_MOVEDOWN = 512;

using genCmds_t = enum
{
	GENCMD_FORCE_HEAL = 1,
	GENCMD_FORCE_SPEED,
	GENCMD_FORCE_THROW,
...
using entityState_t = struct entityState_s
{
	// !!!!!!!!!!! LOADSAVE-affecting struct !!!!!!!!!!!!!
...
using SSkinGoreData = struct SSkinGoreData_s
{
	vec3_t angles;
	vec3_t position;
	int currentTime;
	...
	int fadeOutTime;
	//specify the duration of fading, from the lifeTime (e.g. 3000 will start fading 3 seconds before removal and be faded entirely by removal)
...
========================================================================
*/
using stringID_table_t = struct stringID_table_s
{
	const char* name;
	int id;
};
...
//
constexpr auto SG_SCR_WIDTH = 512; //256;
constexpr auto SG_SCR_HEIGHT = 512; //256;
constexpr auto iSG_COMMENT_SIZE = 64;

constexpr auto sCVARNAME_PLAYERSAVE = "playersave"; // used for level-transition, accessed by game and server modules;
