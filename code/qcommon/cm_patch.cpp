...
static qboolean	CM_NeedsSubdivision(vec3_t a, vec3_t b, vec3_t c)
{
	...
	const float dist = VectorLengthSquared(delta);

	return static_cast<qboolean>(dist >= SUBDIVIDE_DISTANCE * SUBDIVIDE_DISTANCE);
}
...
static void CM_TransposeGrid(cGrid_t* grid) {
	...
	const int l = grid->width;
	grid->width = grid->height;
	grid->height = l;

	const qboolean tempWrap = grid->wrapWidth;
...
static void CM_SetGridWrapWidth(cGrid_t* grid) {
	...
	for (i = 0; i < grid->height; i++) {
		for (j = 0; j < 3; j++) {
			const float d = grid->points[0][i][j] - grid->points[grid->width - 1][i][j];
...
constexpr auto POINT_EPSILON = 0.1;
/*
======================
CM_ComparePoints
======================
...
constexpr auto NORMAL_EPSILON = 0.00015;
...
static int CM_PointOnPlaneSide(float* p, int planeNum) {
	...
	const float* plane = planes[planeNum].plane;

	const float d = DotProduct(p, plane) - plane[3];
...
static void CM_PatchCollideFromGrid(cGrid_t* grid, patchCollide_t* pf) {
	...
	for (i = 0; i < grid->width - 1; i++) {
		for (j = 0; j < grid->height - 1; j++) {
			...
			noAdjust[EN_TOP] = static_cast<qboolean>(borders[EN_TOP] == gridPlanes[i][j][0]);
			...
			noAdjust[EN_BOTTOM] = static_cast<qboolean>(borders[EN_BOTTOM] == gridPlanes[i][j][1]);
			...
			noAdjust[EN_LEFT] = static_cast<qboolean>(borders[EN_LEFT] == gridPlanes[i][j][1]);
			...
			noAdjust[EN_RIGHT] = static_cast<qboolean>(borders[EN_RIGHT] == gridPlanes[i][j][0]);
	...
	if (numFacets)
	{
		pf->facets = static_cast<facet_t*>(Z_Malloc(numFacets * sizeof * pf->facets, TAG_BSP, qfalse));
...
cGrid_t* cm_grid = nullptr;
void CM_GridAlloc()
{
	if (cm_grid) return;
	cm_grid = static_cast<cGrid_t*>(Z_Malloc(sizeof(cGrid_t), TAG_TEMP_WORKSPACE, qfalse));
}
...
patchCollide_s* CM_GeneratePatchCollide(int width, int height, vec3_t* points) {
	...
	// collided against
	patchCollide_t* pf = static_cast<patchCollide_t*>(Z_Malloc(sizeof * pf, TAG_BSP, qfalse));
...
int CM_CheckFacetPlane(float* plane, vec3_t start, vec3_t end, float* enterFrac, float* leaveFrac, int* hit) {
	...
	*hit = qfalse;
	...
	const float d1 = DotProduct(start, plane) - plane[3];
	const float d2 = DotProduct(end, plane) - plane[3];
...
constexpr auto BOX_FRONT = 0;
constexpr auto BOX_BACK = 1;
constexpr auto BOX_CROSS = 2;
