...
void UI_SetActiveMenu(const char* menuname, const char* menuID)
{
	...
	if (Q_stricmp(menuname, "ingame1") == 0) //outcast version
	{
		ui.Cvar_Set("cl_paused", "1");
		UI_InGameMenu1(menuID);
		return;
	}

	if (Q_stricmp(menuname, "ingame2") == 0) //mod version
	{
		ui.Cvar_Set("cl_paused", "1");
		UI_InGameMenu2(menuID);
		return;
	}

	if (Q_stricmp(menuname, "ingame3") == 0) //yavIV version
	{
		ui.Cvar_Set("cl_paused", "1");
		UI_InGameMenu3(menuID);
		return;
	}

	if (Q_stricmp(menuname, "ingame4") == 0) //eoc version
	{
		ui.Cvar_Set("cl_paused", "1");
		UI_InGameMenu4(menuID);
		return;
	}
