...
void misc_dlight_use(gentity_t* ent, gentity_t* other, gentity_t* activator)
{
	...
	ent->misc_dlight_active = static_cast<qboolean>(!ent->misc_dlight_active); //toggle
...
{
	vec3_t dir;
	const gentity_t* target = nullptr;
...
void camera_aim(gentity_t* self)
{
	...
	{
		//I am the viewEntity
		if (player->client->usercmd.buttons & BUTTON_BLOCK || player->client->usercmd.forwardmove || player->client->
			usercmd.rightmove || player->client->usercmd.upmove)
		{
	...
	if (self->target)
	{
		const gentity_t* targ = G_Find(nullptr, FOFS(targetname), self->target);
		if (targ)
		{
			vec3_t angles, dir;
...
void maglock_link(gentity_t* self)
{
	...
	gi.trace(&trace, start, vec3_origin, vec3_origin, end, self->s.number, MASK_SHOT, static_cast<EG2_Collision>(0), 0);
...
void shield_power_converter_use(gentity_t* self, const gentity_t* other, const gentity_t* activator)
{
	...
	if (self->setTime < level.time)
	{
		...
		const int dif = 100 - activator->client->ps.stats[STAT_ARMOR]; // FIXME: define for max armor?
...
void ammo_power_converter_use(gentity_t* self, gentity_t* other, const gentity_t* activator)
{
	...
	if (self->setTime < level.time)
	{
		const int difBlaster = ammoData[AMMO_BLASTER].max - ps->ammo[AMMO_BLASTER];
		const int difPowerCell = ammoData[AMMO_POWERCELL].max - ps->ammo[AMMO_POWERCELL];
		const int difMetalBolts = ammoData[AMMO_METAL_BOLTS].max - ps->ammo[AMMO_METAL_BOLTS];
...
void welder_think(gentity_t* self)
{
	...
	const int newBolt = gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*flash");
...
void SP_misc_exploding_crate(gentity_t* ent)
{
	...
	ent->contents = CONTENTS_SOLID | CONTENTS_OPAQUE | CONTENTS_BODY | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP;
	//CONTENTS_SOLID;
	ent->takedamage = qtrue;
...
void misc_atst_use(gentity_t* self, gentity_t* other, gentity_t* activator)
{
	...
	else
	{
		...
		const vec3_t newOrg = {
			activator->currentOrigin[0], activator->currentOrigin[1],
			activator->currentOrigin[2] + (self->maxs[2] - self->mins[2]) + 1
		};
...
void SP_misc_atst_drivable(gentity_t* ent)
{
	...
	NPC_ATST_Precache();
	ent->NPC_type = static_cast<char*>("atst");
...
void radar_icon_use(gentity_t* ent, gentity_t* other, gentity_t* activator)
{
	...
	ent->misc_dlight_active = static_cast<qboolean>(!ent->misc_dlight_active);
	if (ent->misc_dlight_active)
	{
		ent->svFlags |= SVF_BROADCAST;
		ent->s.eFlags2 |= EF2_RADAROBJECT;
