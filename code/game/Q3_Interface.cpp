...
static void Q3_SetWeapon(int ent_id, const char* wp_name);
static void Q3_SetItem(int ent_id, const char* item_name);
...
=============
*/
static void Q3_SetDYaw(int ent_id, float data);

static void Q3_SetAngles(const int ent_id, vec3_t angles)
{
...
=============
*/
static void Q3_SetOrigin(const int ent_id, vec3_t origin)
{
	gentity_t* ent = &g_entities[ent_id];
...
=============
*/
static qboolean Q3_SetTeleportDest(const int ent_id, vec3_t org)
{
	gentity_t* teleEnt = &g_entities[ent_id];
...
=============`
*/
static void Q3_SetCopyOrigin(const int ent_id, const char* name)
{
	gentity_t* found = G_Find(nullptr, FOFS(targetname), name);

	if (found)
	{
		Q3_SetOrigin(ent_id, found->currentOrigin);
		SetClientViewAngle(&g_entities[ent_id], found->s.angles);
	}
	else
	{
...
=============
*/
static void Q3_SetVelocity(const int ent_id, const int axis, const float speed)
{
	const gentity_t* found = &g_entities[ent_id];
	//FIXME: Not supported
	if (!found)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetVelocity invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetAdjustAreaPortals(const int ent_id, const qboolean adjust)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetAdjustAreaPortals: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetDmgByHeavyWeapOnly(const int ent_id, const qboolean dmg)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDmgByHeavyWeapOnly: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetShielded(const int ent_id, const qboolean dmg)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetShielded: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetNoGroups(const int ent_id, const qboolean noGroups)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoGroups: invalid ent_id %d\n", ent_id);
		return;
	}
...
// NEEDED???
*/
/*static void Q3_Lerp2Start( int ent_id, int taskID, float duration )
{
	gentity_t	*ent = &g_entities[ent_id];

	if(!ent)
	{
		Quake3Game()->DebugPrint( WL_WARNING, "Q3_Lerp2Start: invalid ent_id %d\n", ent_id);
		return;
	}
...
// NEEDED???
*/
/*static void Q3_Lerp2End( int ent_id, int taskID, float duration )
{
	gentity_t	*ent = &g_entities[ent_id];

	if(!ent)
	{
		Quake3Game()->DebugPrint( WL_WARNING, "Q3_Lerp2End: invalid ent_id %d\n", ent_id);
		return;
	}
...
=============
*/
/*static void Q3_Lerp2Pos( int taskID, int ent_id, vec3_t origin, vec3_t angles, float duration )
{
	gentity_t	*ent = &g_entities[ent_id];
	vec3_t		ang;
	int			i;
	...
	if(!ent)
	{
		Quake3Game()->DebugPrint( WL_WARNING, "Q3_Lerp2Pos: invalid ent_id %d\n", ent_id);
		return;
	}
	...
	=============
	*/
void Q3_Lerp2Origin(const int taskID, const int ent_id, vec3_t origin, const float duration)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_Lerp2Origin: invalid ent_id %d\n", ent_id);
		return;
	}
...
static void Q3_SetOriginOffset(const int ent_id, const int axis, const float offset)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetOriginOffset: invalid ent_id %d\n", ent_id);
		return;
	}
	...
	Q3_Lerp2Origin(-1, ent_id, origin, duration);
}
...
=============
*/
/*static void Q3_Lerp2Angles( int taskID, int ent_id, vec3_t angles, float duration )
{
	gentity_t	*ent = &g_entities[ent_id];
	vec3_t		ang;
	int			i;
	...
	if(!ent)
	{
		Quake3Game()->DebugPrint( WL_WARNING, "Q3_Lerp2Angles: invalid ent_id %d\n", ent_id);
		return;
	}
...
=============
*/
/*static int	Q3_GetTag( int ent_id, const char *name, int lookup, vec3_t info )
{
	gentity_t	*ent = &g_entities[ ent_id ];
...
=============
*/
static qboolean Q3_SetNavGoal(const int ent_id, const char* name)
{
	gentity_t* ent = &g_entities[ent_id];
	vec3_t goalPos;
...
============
*/
static void SetLowerAnim(const int ent_id, const int animID)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "SetLowerAnim: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void SetUpperAnim(const int ent_id, const int animID)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "SetUpperAnim: invalid ent_id %d\n", ent_id);
		return;
	}
...
=============
*/
qboolean Q3_SetAnimUpper(const int ent_id, const char* anim_name)
{
	...
	if (!PM_HasAnimation(&g_entities[ent_id], anim_id))
	{
		return qfalse;
	}

	SetUpperAnim(ent_id, anim_id);
	return qtrue;
}
...
=============
*/
qboolean Q3_SetAnimLower(const int ent_id, const char* anim_name)
{
...
void Q3_SetAnimHoldTime(const int ent_id, const int int_data, const qboolean lower)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetAnimHoldTime: invalid ent_id %d\n", ent_id);
		return;
	}
...
=============
*/
static void Q3_SetEnemy(const int ent_id, const char* name)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetEnemy: invalid ent_id %d\n", ent_id);
		return;
	}
...
=============
*/
static void Q3_SetLeader(const int ent_id, const char* name)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetLeader: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetPlayerTeam(const int ent_id, const char* teamName)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetPlayerTeam: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetEnemyTeam(const int ent_id, const char* teamName)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetEnemyTeam: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
void Q3_SetHealth(const int ent_id, int data)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetHealth: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
void Q3_SetArmor(const int ent_id, const int data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetArmor: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static qboolean Q3_SetBState(const int ent_id, const char* bs_name)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetBState: invalid ent_id %d\n", ent_id);
		return qtrue;
	}
...
============
*/
static qboolean Q3_SetTempBState(const int ent_id, const char* bs_name)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetTempBState: invalid ent_id %d\n", ent_id);
		return qtrue;
	}
...
============
*/
static void Q3_SetDefaultBState(const int ent_id, const char* bs_name)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDefaultBState: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetDPitch(const int ent_id, float data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDPitch: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetDYaw(const int ent_id, const float data)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDYaw: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetShootDist(const int ent_id, const float data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetShootDist: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetVisrange(const int ent_id, const float data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetVisrange: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetEarshot(const int ent_id, const float data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetEarshot: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetVigilance(const int ent_id, const float data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetVigilance: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetVFOV(const int ent_id, const int data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetVFOV: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetHFOV(const int ent_id, const int data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetHFOV: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetWidth(const int ent_id, const int data)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetWidth: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetTimeScale(int ent_id, const char* data)
{
...
============
*/
static void Q3_SetInvisible(const int ent_id, const qboolean invisible)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetInvisible: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetVampire(const int ent_id, const qboolean vampire)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self || !self->client)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetVampire: ent_id %d not a client\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetGreetAllies(const int ent_id, const qboolean greet)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetGreetAllies: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetViewTarget(const int ent_id, const char* name)
{
	const gentity_t* self = &g_entities[ent_id];
	...
	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetViewTarget: invalid ent_id %d\n", ent_id);
		return;
	}
	...
	Q3_SetDYaw(ent_id, viewangles[YAW]);
	if (!g_skippingcin || !g_skippingcin->integer)
	{
		Q3_SetDPitch(ent_id, viewangles[PITCH]);
	}
}
...
============
*/
static void Q3_SetWatchTarget(const int ent_id, const char* name)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetWatchTarget: invalid ent_id %d\n", ent_id);
		return;
	}
...
void Q3_SetLoopSound(const int ent_id, const char* name)
{
	sfxHandle_t index;
	gentity_t* self = &g_entities[ent_id];
...
void Q3_SetICARUSFreeze(int ent_id, const char* name, const qboolean freeze)
{
...
void Q3_SetViewEntity(const int ent_id, const char* name)
{
	gentity_t* self = &g_entities[ent_id];
	...
	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_ERROR, "Q3_SetViewEntity: invalid ent_id %d\n", ent_id);
		return;
	}
...
static void Q3_SetWeapon(const int ent_id, const char* wp_name)
{
	gentity_t* self = &g_entities[ent_id];
	...
	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetWeapon: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetItem(const int ent_id, const char* item_name)
{
	const gentity_t* self = &g_entities[ent_id];
...
============
*/
static void Q3_SetWalkSpeed(const int ent_id, const int int_data)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetWalkSpeed: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetRunSpeed(const int ent_id, const int int_data)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetRunSpeed: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetYawSpeed(const int ent_id, const float float_data)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetYawSpeed: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetAggression(const int ent_id, const int int_data)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetAggression: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetAim(const int ent_id, const int int_data)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetAim: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetFriction(const int ent_id, const int int_data)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetFriction: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
void Q3_SetGravity(const int ent_id, const float float_data)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetGravity: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetWait(const int ent_id, const float float_data)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetWait: invalid ent_id %d\n", ent_id);
		return;
	}
...
static void Q3_SetShotSpacing(const int ent_id, const int int_data)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetShotSpacing: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetFollowDist(const int ent_id, const float float_data)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetFollowDist: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
void Q3_SetScale(const int ent_id, const float float_data)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetScale: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetRenderCullRadius(const int ent_id, const float float_data)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetRenderCullRadius: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetCount(const int ent_id, const char* data)
{
	gentity_t* self = &g_entities[ent_id];
	float val;
	...
	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetCount: invalid ent_id %d\n", ent_id);
		return;
	}
...
*/
/*
static void Q3_SetSquadName (int ent_id, const char *squadname)
{
	gentity_t	*self  = &g_entities[ent_id];

	if ( !self )
	{
		Quake3Game()->DebugPrint( IGameInterface::WL_WARNING, "Q3_SetSquadName: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetTargetName(const int ent_id, const char* targetname)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetTargetName: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetTarget(const int ent_id, const char* target)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetTarget: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetTarget2(const int ent_id, const char* target2)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetTarget2: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetRemoveTarget(const int ent_id, const char* target)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetRemoveTarget: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetPainTarget(const int ent_id, const char* targetname)
{
	gentity_t* self = &g_entities[ent_id];

	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetPainTarget: invalid ent_id %d\n", ent_id);
		return;
	}
...
static void Q3_SetForcePowerLevel(const int ent_id, const int forcePower, const int forceLevel)
{
	...
	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_ERROR, "Q3_SetForcePowerLevel: invalid ent_id %d\n", ent_id);
		return;
	}
	...
	if (forceLevel > self->client->ps.forcePowerLevel[forcePower] && ent_id == 0 && forceLevel > 0)
	{
	}
...
static void Q3_GiveSecurityKey(const int ent_id, const char* keyname)
{
	const gentity_t* other = &g_entities[ent_id];

	if (!other)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_ERROR, "Q3_GiveSecurityKey: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
void Q3_SetParm(const int ent_id, const int parmNum, const char* parmValue)
{
	gentity_t* ent = &g_entities[ent_id];
	float val;
	...
	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetParm: invalid ent_id %d\n", ent_id);
		return;
	}
...
=============
*/
static void Q3_SetCaptureGoal(const int ent_id, const char* name)
{
	const gentity_t* ent = &g_entities[ent_id];
	...
	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetCaptureGoal: invalid ent_id %d\n", ent_id);
		return;
	}
...
=============
*/
static void Q3_SetEvent(const int ent_id, const char* event_name)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetEvent: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
/*static void Q3_Use( int ent_id, const char *target )
{
	gentity_t	*ent  = &g_entities[ent_id];

	if ( !ent )
	{
		Quake3Game()->DebugPrint( IGameInterface::WL_WARNING, "Q3_Use: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static qboolean Q3_SetBehaviorSet(const int ent_id, const int toSet, const char* scriptname)
{
	gentity_t* ent = &g_entities[ent_id];
	...
	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetBehaviorSet: invalid ent_id %d\n", ent_id);
		return qfalse;
	}
...
============
*/
static void Q3_SetDelayScriptTime(const int ent_id, const int delayTime)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDelayScriptTime: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetIgnorePain(const int ent_id, const qboolean data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetIgnorePain: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetIgnoreEnemies(const int ent_id, const qboolean data)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetIgnoreEnemies: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetIgnoreAlerts(const int ent_id, const qboolean data)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetIgnoreAlerts: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetNoTarget(const int ent_id, const qboolean data)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoTarget: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetDontShoot(const int ent_id, const qboolean add)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDontShoot: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetDontFire(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDontFire: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetFireWeapon(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_FireWeapon: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetFireWeaponNoAnim(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_FireWeaponNoAnim: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetSafeRemove(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetSafeRemove: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetBobaJetPack(const int ent_id, const qboolean add)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetBobaJetPack: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetInactive(const int ent_id, const qboolean add)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetInactive: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetFuncUsableVisible(const int ent_id, const qboolean visible)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetFuncUsableVisible: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetLockedEnemy(const int ent_id, const qboolean locked)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetLockedEnemy: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetNoMindTrick(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoMindTrick: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetCrouched(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetCrouched: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetWalking(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetWalking: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetRunning(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetRunning: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetForcedMarch(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetForcedMarch: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetChaseEnemies(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetChaseEnemies: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetLookForEnemies(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetLookForEnemies: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetFaceMoveDir(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetFaceMoveDir: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetAltFire(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetAltFire: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetDontFlee(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDontFlee: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetNoResponse(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoResponse: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetCombatTalk(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetCombatTalk: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetAlertTalk(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetAlertTalk: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetUseCpNearest(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetUseCpNearest: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetNoForce(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoForce: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetNoAcrobatics(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoAcrobatics: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetUseSubtitles(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetUseSubtitles: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetNoFallToDeath(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoFallToDeath: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetDismemberable(const int ent_id, const qboolean dismemberable)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDismemberable: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetMoreLight(const int ent_id, const qboolean add)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetMoreLight: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetUndying(const int ent_id, const qboolean undying)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetUndying: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetInvincible(const int ent_id, const qboolean invincible)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetInvincible: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetForceInvincible(const int ent_id, const qboolean forceInv)
{
	const gentity_t* self = &g_entities[ent_id];

	if (!self || !self->client)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetForceInvincible: ent_id %d not a client\n", ent_id);
		return;
	}

	Q3_SetInvincible(ent_id, forceInv);
	if (forceInv)
	{
...
============
*/
static void Q3_SetNoAvoid(const int ent_id, const qboolean noAvoid)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoAvoid: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static qboolean Q3_SetSolid(const int ent_id, const qboolean solid)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetSolid: invalid ent_id %d\n", ent_id);
		return qtrue;
	}
...
static void Q3_SetLean(const int ent_id, const int lean)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetLean: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetForwardMove(const int ent_id, const int fmoveVal)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetForwardMove: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetRightMove(const int ent_id, const int rmoveVal)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetRightMove: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetLockAngle(const int ent_id, const char* lockAngle)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetLockAngle: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_CameraGroup(const int ent_id, const char* camG)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_CameraGroup: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_RemoveRHandModel(const int ent_id, char* addModel)
{
	gentity_t* ent = &g_entities[ent_id];
...
============
*/
static void Q3_AddRHandModel(const int ent_id, const char* addModel)
{
	gentity_t* ent = &g_entities[ent_id];
...
============
*/
static void Q3_AddLHandModel(const int ent_id, const char* addModel)
{
	gentity_t* ent = &g_entities[ent_id];
...
============
*/
static void Q3_RemoveLHandModel(const int ent_id, char* addModel)
{
	gentity_t* ent = &g_entities[ent_id];
...
static void Q3_LookTarget(const int ent_id, char* targetName)
{
	...
	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_LookTarget: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_Face(const int ent_id, const int expression, float holdtime)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_Face: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetPlayerUsable(const int ent_id, const qboolean usable)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetPlayerUsable: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetDisableShaderAnims(const int ent_id, const int disabled)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetDisableShaderAnims: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetShaderAnim(const int ent_id, const int disabled)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetShaderAnim: invalid ent_id %d\n", ent_id);
		return;
	}
...
void Q3_SetBroadcast(const int ent_id, const qboolean broadcast)
{
	gentity_t* ent = &g_entities[ent_id];
	if (broadcast)
	{
...
void Q3_SetForcePower(const int ent_id, const int forcePower, const qboolean powerOn)
{
	const gentity_t* ent = &g_entities[ent_id];
	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetForcePower: invalid ent_id %d\n", ent_id);
		return;
	}
	if (!ent->client)
	{
...
static void Q3_SetStartFrame(const int ent_id, const int start_frame)
{
	...
	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetStartFrame: invalid ent_id %d\n", ent_id);
		return;
	}
...
static void Q3_SetEndFrame(const int ent_id, const int end_frame)
{
	...
	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetEndFrame: invalid ent_id %d\n", ent_id);
		return;
	}
...
static void Q3_SetAnimFrame(const int ent_id, const int animFrame)
{
	...
	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetAnimFrame: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetLoopAnim(const int ent_id, const qboolean loopAnim)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetLoopAnim: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetShields(const int ent_id, const qboolean shields)
{
	const gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetShields: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetSaberActive(const int ent_id, const qboolean active)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetSaberActive: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
	...
	[in]	int ent_id		The ID of the Entity to modify.
...
============
*/
static void Q3_SetSaberBladeActive(const int ent_id, const int iSaber, const int iBlade, const qboolean bActive = qtrue)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetSaberBladeActive: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetNoKnockback(const int ent_id, const qboolean noKnockback)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoKnockback: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
static void Q3_SetInterface(int ent_id, const char* data)
{
...
============
*/
static qboolean Q3_SetLocation(const int ent_id, const char* location)
{
	gentity_t* ent = &g_entities[ent_id];
...
static void Q3_SetPlayerLocked(int ent_id, const qboolean locked)
{
...
============
*/
static void Q3_SetLockPlayerWeapons(int ent_id, const qboolean locked)
{
	gentity_t* ent = &g_entities[0];
...
============
*/
static void Q3_SetNoImpactDamage(const int ent_id, const qboolean noImp)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_SetNoImpactDamage: invalid ent_id %d\n", ent_id);
		return;
	}
...
============
*/
/*void SetVar( int taskID, int ent_id, const char *type_name, const char *data )
{
...
============
*/
static void Q3_Remove(const int ent_id, const char* name)
{
	gentity_t* ent = &g_entities[ent_id];
	gentity_t* victim;
...
void Q3_DismemberLimb(const int ent_id, char* hitLocName)
{
	gentity_t* self = &g_entities[ent_id];
	...
	if (!self)
	{
		Quake3Game()->DebugPrint(IGameInterface::WL_WARNING, "Q3_DismemberLimb: invalid ent_id %d\n", ent_id);
		return;
	}
...
void CQuake3GameInterface::SetVar(int taskID, int ent_id, const char* type_name, const char* data)
{
...
int CQuake3GameInterface::PlayIcarusSound(const int taskID, const int ent_id, const char* name, const char* channel)
{
	gentity_t* ent = &g_entities[ent_id];
...
// Lerps the origin and angles of an entity to the destination values.
void CQuake3GameInterface::Lerp2Pos(const int taskID, const int ent_id, vec3_t origin, vec3_t angles, float duration)
{
	gentity_t* ent = &g_entities[ent_id];

	if (!ent)
	{
		DebugPrint(WL_WARNING, "Lerp2Pos: invalid ent_id %d\n", ent_id);
		return;
	}
...
//	 void	CQuake3GameInterface::Lerp2Origin( int taskID, int ent_id, vec3_t origin, float duration ) {}

// Lerps the angles to the destination value.
void CQuake3GameInterface::Lerp2Angles(const int taskID, const int ent_id, vec3_t angles, const float duration)
{
	gentity_t* ent = &g_entities[ent_id];
	vec3_t ang;
	...
	if (!ent)
	{
		DebugPrint(WL_WARNING, "Lerp2Angles: invalid ent_id %d\n", ent_id);
		return;
	}
...
// Gets the value of a tag by the give name.
int CQuake3GameInterface::GetTag(const int ent_id, const char* name, const int lookup, vec3_t info)
{
	const gentity_t* ent = &g_entities[ent_id];
...
//	 void	CQuake3GameInterface::Lerp2Start( int taskID, int ent_id, float duration ) {}
//	 void	CQuake3GameInterface::Lerp2End( int taskID, int ent_id, float duration ) {}

void CQuake3GameInterface::Set(const int task_id, const int ent_id, const char* type_name, const char* data)
{
	gentity_t* ent = &g_entities[ent_id];
	float float_data;
	int int_data;
	...
	case SET_COPY_ORIGIN:
		Q3_SetCopyOrigin(ent_id, data);
		break;

	case SET_ANGLES:
		//Q3_SetAngles( ent_id, *(vec3_t *) data);
		sscanf(data, "%f %f %f", &vector_data[0], &vector_data[1], &vector_data[2]);
	...
	case SET_XVELOCITY:
		float_data = atof(data);
		Q3_SetVelocity(ent_id, 0, float_data);
		break;

	case SET_YVELOCITY:
		float_data = atof(data);
		Q3_SetVelocity(ent_id, 1, float_data);
		break;

	case SET_ZVELOCITY:
		float_data = atof(data);
		Q3_SetVelocity(ent_id, 2, float_data);
		break;

	case SET_Z_OFFSET:
		float_data = atof(data);
		Q3_SetOriginOffset(ent_id, 2, float_data);
		break;

	case SET_ENEMY:
		Q3_SetEnemy(ent_id, data);
		break;

	case SET_LEADER:
		Q3_SetLeader(ent_id, data);
		break;

	case SET_NAVGOAL:
		if (Q3_SetNavGoal(ent_id, data))
		{
			Q3_TaskIDSet(ent, TID_MOVE_NAV, task_id);
	...
	case SET_ANIM_UPPER:
		if (Q3_SetAnimUpper(ent_id, data))
		{
	...
	case SET_ANIM_LOWER:
		if (Q3_SetAnimLower(ent_id, data))
		{
	...
	case SET_ANIM_HOLDTIME_LOWER:
		int_data = atoi(data);
		Q3_SetAnimHoldTime(ent_id, int_data, qtrue);
		Q3_TaskIDClear(&ent->taskID[TID_ANIM_BOTH]); //We only want to wait for the bottom
		Q3_TaskIDSet(ent, TID_ANIM_LOWER, task_id);
		return; //Don't call it back
	...
	case SET_ANIM_HOLDTIME_UPPER:
		int_data = atoi(data);
		Q3_SetAnimHoldTime(ent_id, int_data, qfalse);
		Q3_TaskIDClear(&ent->taskID[TID_ANIM_BOTH]); //We only want to wait for the top
		Q3_TaskIDSet(ent, TID_ANIM_UPPER, task_id);
		return; //Don't call it back
	...
	case SET_PLAYER_TEAM:
		Q3_SetPlayerTeam(ent_id, data);
		break;

	case SET_ENEMY_TEAM:
		Q3_SetEnemyTeam(ent_id, data);
		break;

	case SET_HEALTH:
		int_data = atoi(data);
		Q3_SetHealth(ent_id, int_data);
		break;

	case SET_ARMOR:
		int_data = atoi(data);
		Q3_SetArmor(ent_id, int_data);
		break;

	case SET_BEHAVIOR_STATE:
		if (!Q3_SetBState(ent_id, data))
		{
	...
	case SET_DEFAULT_BSTATE:
		Q3_SetDefaultBState(ent_id, data);
		break;

	case SET_TEMP_BSTATE:
		if (!Q3_SetTempBState(ent_id, data))
		{
			Q3_TaskIDSet(ent, TID_BSTATE, task_id);
			return; //don't complete
		}
	...
	case SET_CAPTURE:
		Q3_SetCaptureGoal(ent_id, data);
		break;

	case SET_DPITCH:
		//FIXME: make these set tempBehavior to BS_FACE and await completion?  Or set lockedDesiredPitch/Yaw and aimTime?
		float_data = atof(data);
		Q3_SetDPitch(ent_id, float_data);
		Q3_TaskIDSet(ent, TID_ANGLE_FACE, task_id);
		return;

	case SET_DYAW:
		float_data = atof(data);
		Q3_SetDYaw(ent_id, float_data);
		Q3_TaskIDSet(ent, TID_ANGLE_FACE, task_id);
		return;

	case SET_EVENT:
		Q3_SetEvent(ent_id, data);
		break;

	case SET_VIEWTARGET:
		Q3_SetViewTarget(ent_id, data);
		Q3_TaskIDSet(ent, TID_ANGLE_FACE, task_id);
		return;

	case SET_WATCHTARGET:
		Q3_SetWatchTarget(ent_id, data);
		break;

	case SET_VIEWENTITY:
		Q3_SetViewEntity(ent_id, data);
		break;

	case SET_LOOPSOUND:
		Q3_SetLoopSound(ent_id, data);
		break;

	case SET_ICARUS_FREEZE:
	case SET_ICARUS_UNFREEZE:
		Q3_SetICARUSFreeze(ent_id, data, static_cast<qboolean>(toSet == SET_ICARUS_FREEZE));
		break;

	case SET_WEAPON:
		Q3_SetWeapon(ent_id, data);
		break;

	case SET_ITEM:
		Q3_SetItem(ent_id, data);
		break;

	case SET_WALKSPEED:
		int_data = atoi(data);
		Q3_SetWalkSpeed(ent_id, int_data);
		break;

	case SET_RUNSPEED:
		int_data = atoi(data);
		Q3_SetRunSpeed(ent_id, int_data);
		break;

	case SET_WIDTH:
		int_data = atoi(data);
		Q3_SetWidth(ent_id, int_data);
		return;

	case SET_YAWSPEED:
		float_data = atof(data);
		Q3_SetYawSpeed(ent_id, float_data);
		break;

	case SET_AGGRESSION:
		int_data = atoi(data);
		Q3_SetAggression(ent_id, int_data);
		break;

	case SET_AIM:
		int_data = atoi(data);
		Q3_SetAim(ent_id, int_data);
		break;

	case SET_FRICTION:
		int_data = atoi(data);
		Q3_SetFriction(ent_id, int_data);
		break;

	case SET_GRAVITY:
		float_data = atof(data);
		Q3_SetGravity(ent_id, float_data);
		break;

	case SET_WAIT:
		float_data = atof(data);
		Q3_SetWait(ent_id, float_data);
		break;

	case SET_FOLLOWDIST:
		float_data = atof(data);
		Q3_SetFollowDist(ent_id, float_data);
		break;

	case SET_SCALE:
		float_data = atof(data);
		Q3_SetScale(ent_id, float_data);
		break;

	case SET_RENDER_CULL_RADIUS:
		float_data = atof(data);
		Q3_SetRenderCullRadius(ent_id, float_data);
		break;

	case SET_COUNT:
		Q3_SetCount(ent_id, data);
		break;

	case SET_SHOT_SPACING:
		int_data = atoi(data);
		Q3_SetShotSpacing(ent_id, int_data);
		break;

	case SET_IGNOREPAIN:
		if (!Q_stricmp("true", data))
			Q3_SetIgnorePain(ent_id, qtrue);
		else if (!Q_stricmp("false", data))
			Q3_SetIgnorePain(ent_id, qfalse);
		break;

	case SET_IGNOREENEMIES:
		if (!Q_stricmp("true", data))
			Q3_SetIgnoreEnemies(ent_id, qtrue);
		else if (!Q_stricmp("false", data))
			Q3_SetIgnoreEnemies(ent_id, qfalse);
		break;

	case SET_IGNOREALERTS:
		if (!Q_stricmp("true", data))
			Q3_SetIgnoreAlerts(ent_id, qtrue);
		else if (!Q_stricmp("false", data))
			Q3_SetIgnoreAlerts(ent_id, qfalse);
		break;

	case SET_DONTSHOOT:
		if (!Q_stricmp("true", data))
			Q3_SetDontShoot(ent_id, qtrue);
		else if (!Q_stricmp("false", data))
			Q3_SetDontShoot(ent_id, qfalse);
		break;

	case SET_DONTFIRE:
		if (!Q_stricmp("true", data))
			Q3_SetDontFire(ent_id, qtrue);
		else if (!Q_stricmp("false", data))
			Q3_SetDontFire(ent_id, qfalse);
		break;

	case SET_LOCKED_ENEMY:
		if (!Q_stricmp("true", data))
			Q3_SetLockedEnemy(ent_id, qtrue);
		else if (!Q_stricmp("false", data))
			Q3_SetLockedEnemy(ent_id, qfalse);
		break;

	case SET_NOTARGET:
		if (!Q_stricmp("true", data))
			Q3_SetNoTarget(ent_id, qtrue);
		else if (!Q_stricmp("false", data))
			Q3_SetNoTarget(ent_id, qfalse);
		break;

	case SET_LEAN:
		if (!Q_stricmp("right", data))
			Q3_SetLean(ent_id, LEAN_RIGHT);
		else if (!Q_stricmp("left", data))
			Q3_SetLean(ent_id, LEAN_LEFT);
		else
			Q3_SetLean(ent_id, LEAN_NONE);
		break;

	case SET_SHOOTDIST:
		float_data = atof(data);
		Q3_SetShootDist(ent_id, float_data);
		break;

	case SET_TIMESCALE:
		Q3_SetTimeScale(ent_id, data);
		break;

	case SET_VISRANGE:
		float_data = atof(data);
		Q3_SetVisrange(ent_id, float_data);
		break;

	case SET_EARSHOT:
		float_data = atof(data);
		Q3_SetEarshot(ent_id, float_data);
		break;

	case SET_VIGILANCE:
		float_data = atof(data);
		Q3_SetVigilance(ent_id, float_data);
		break;

	case SET_VFOV:
		int_data = atoi(data);
		Q3_SetVFOV(ent_id, int_data);
		break;

	case SET_HFOV:
		int_data = atoi(data);
		Q3_SetHFOV(ent_id, int_data);
		break;

	case SET_TARGETNAME:
		Q3_SetTargetName(ent_id, data);
		break;

	case SET_TARGET:
		Q3_SetTarget(ent_id, data);
		break;

	case SET_TARGET2:
		Q3_SetTarget2(ent_id, data);
		break;

	case SET_LOCATION:
		if (!Q3_SetLocation(ent_id, data))
		{
	...
	case SET_PARM15:
	case SET_PARM16:
		Q3_SetParm(ent_id, toSet - SET_PARM1, data);
		break;
	...
	case SET_FFDEATHSCRIPT:
	case SET_MINDTRICKSCRIPT:
		if (!Q3_SetBehaviorSet(ent_id, toSet, data))
			DebugPrint(WL_ERROR, "SetBehaviorSet: Invalid bSet %s\n", type_name);
		break;

	case SET_NO_MINDTRICK:
		if (!Q_stricmp("true", data))
			Q3_SetNoMindTrick(ent_id, qtrue);
		else
			Q3_SetNoMindTrick(ent_id, qfalse);
		break;
	...
	case SET_DELAYSCRIPTTIME:
		int_data = atoi(data);
		Q3_SetDelayScriptTime(ent_id, int_data);
		break;

	case SET_CROUCHED:
		if (!Q_stricmp("true", data))
			Q3_SetCrouched(ent_id, qtrue);
		else
			Q3_SetCrouched(ent_id, qfalse);
		break;

	case SET_WALKING:
		if (!Q_stricmp("true", data))
			Q3_SetWalking(ent_id, qtrue);
		else
			Q3_SetWalking(ent_id, qfalse);
		break;

	case SET_RUNNING:
		if (!Q_stricmp("true", data))
			Q3_SetRunning(ent_id, qtrue);
		else
			Q3_SetRunning(ent_id, qfalse);
		break;

	case SET_CHASE_ENEMIES:
		if (!Q_stricmp("true", data))
			Q3_SetChaseEnemies(ent_id, qtrue);
		else
			Q3_SetChaseEnemies(ent_id, qfalse);
		break;

	case SET_LOOK_FOR_ENEMIES:
		if (!Q_stricmp("true", data))
			Q3_SetLookForEnemies(ent_id, qtrue);
		else
			Q3_SetLookForEnemies(ent_id, qfalse);
		break;

	case SET_FACE_MOVE_DIR:
		if (!Q_stricmp("true", data))
			Q3_SetFaceMoveDir(ent_id, qtrue);
		else
			Q3_SetFaceMoveDir(ent_id, qfalse);
		break;

	case SET_altFire:
		if (!Q_stricmp("true", data))
			Q3_SetAltFire(ent_id, qtrue);
		else
			Q3_SetAltFire(ent_id, qfalse);
		break;

	case SET_DONT_FLEE:
		if (!Q_stricmp("true", data))
			Q3_SetDontFlee(ent_id, qtrue);
		else
			Q3_SetDontFlee(ent_id, qfalse);
		break;

	case SET_FORCED_MARCH:
		if (!Q_stricmp("true", data))
			Q3_SetForcedMarch(ent_id, qtrue);
		else
			Q3_SetForcedMarch(ent_id, qfalse);
		break;

	case SET_NO_RESPONSE:
		if (!Q_stricmp("true", data))
			Q3_SetNoResponse(ent_id, qtrue);
		else
			Q3_SetNoResponse(ent_id, qfalse);
		break;

	case SET_NO_COMBAT_TALK:
		if (!Q_stricmp("true", data))
			Q3_SetCombatTalk(ent_id, qtrue);
		else
			Q3_SetCombatTalk(ent_id, qfalse);
		break;

	case SET_NO_ALERT_TALK:
		if (!Q_stricmp("true", data))
			Q3_SetAlertTalk(ent_id, qtrue);
		else
			Q3_SetAlertTalk(ent_id, qfalse);
		break;

	case SET_USE_CP_NEAREST:
		if (!Q_stricmp("true", data))
			Q3_SetUseCpNearest(ent_id, qtrue);
		else
			Q3_SetUseCpNearest(ent_id, qfalse);
		break;

	case SET_NO_FORCE:
		if (!Q_stricmp("true", data))
			Q3_SetNoForce(ent_id, qtrue);
		else
			Q3_SetNoForce(ent_id, qfalse);
		break;

	case SET_NO_ACROBATICS:
		if (!Q_stricmp("true", data))
			Q3_SetNoAcrobatics(ent_id, qtrue);
		else
			Q3_SetNoAcrobatics(ent_id, qfalse);
		break;

	case SET_USE_SUBTITLES:
		if (!Q_stricmp("true", data))
			Q3_SetUseSubtitles(ent_id, qtrue);
		else
			Q3_SetUseSubtitles(ent_id, qfalse);
		break;

	case SET_NO_FALLTODEATH:
		if (!Q_stricmp("true", data))
			Q3_SetNoFallToDeath(ent_id, qtrue);
		else
			Q3_SetNoFallToDeath(ent_id, qfalse);
		break;

	case SET_DISMEMBERABLE:
		if (!Q_stricmp("true", data))
			Q3_SetDismemberable(ent_id, qtrue);
		else
			Q3_SetDismemberable(ent_id, qfalse);
		break;

	case SET_MORELIGHT:
		if (!Q_stricmp("true", data))
			Q3_SetMoreLight(ent_id, qtrue);
		else
			Q3_SetMoreLight(ent_id, qfalse);
		break;
	...
	case SET_UNDYING:
		if (!Q_stricmp("true", data))
			Q3_SetUndying(ent_id, qtrue);
		else
			Q3_SetUndying(ent_id, qfalse);
		break;

	case SET_INVINCIBLE:
		if (!Q_stricmp("true", data))
			Q3_SetInvincible(ent_id, qtrue);
		else
			Q3_SetInvincible(ent_id, qfalse);
		break;

	case SET_NOAVOID:
		if (!Q_stricmp("true", data))
			Q3_SetNoAvoid(ent_id, qtrue);
		else
			Q3_SetNoAvoid(ent_id, qfalse);
		break;

	case SET_SOLID:
		if (!Q_stricmp("true", data))
		{
			if (!Q3_SetSolid(ent_id, qtrue))
			{
		...
		{
			Q3_SetSolid(ent_id, qfalse);
		}
		break;

	case SET_INVISIBLE:
		if (!Q_stricmp("true", data))
			Q3_SetInvisible(ent_id, qtrue);
		else
			Q3_SetInvisible(ent_id, qfalse);
		break;

	case SET_VAMPIRE:
		if (!Q_stricmp("true", data))
			Q3_SetVampire(ent_id, qtrue);
		else
			Q3_SetVampire(ent_id, qfalse);
		break;

	case SET_FORCE_INVINCIBLE:
		if (!Q_stricmp("true", data))
			Q3_SetForceInvincible(ent_id, qtrue);
		else
			Q3_SetForceInvincible(ent_id, qfalse);
		break;

	case SET_GREET_ALLIES:
		if (!Q_stricmp("true", data))
			Q3_SetGreetAllies(ent_id, qtrue);
		else
			Q3_SetGreetAllies(ent_id, qfalse);
		break;

	case SET_PLAYER_LOCKED:
		if (!Q_stricmp("true", data))
			Q3_SetPlayerLocked(ent_id, qtrue);
		else
			Q3_SetPlayerLocked(ent_id, qfalse);
		break;

	case SET_LOCK_PLAYER_WEAPONS:
		if (!Q_stricmp("true", data))
			Q3_SetLockPlayerWeapons(ent_id, qtrue);
		else
			Q3_SetLockPlayerWeapons(ent_id, qfalse);
		break;

	case SET_NO_IMPACT_DAMAGE:
		if (!Q_stricmp("true", data))
			Q3_SetNoImpactDamage(ent_id, qtrue);
		else
			Q3_SetNoImpactDamage(ent_id, qfalse);
		break;

	case SET_FORWARDMOVE:
		int_data = atoi(data);
		Q3_SetForwardMove(ent_id, int_data);
		break;

	case SET_RIGHTMOVE:
		int_data = atoi(data);
		Q3_SetRightMove(ent_id, int_data);
		break;

	case SET_LOCKYAW:
		Q3_SetLockAngle(ent_id, data);
		break;

	case SET_CAMERA_GROUP:
		Q3_CameraGroup(ent_id, data);
		break;
	...
	case SET_LOOK_TARGET:
		Q3_LookTarget(ent_id, const_cast<char*>(data));
		break;

	case SET_ADDRHANDBOLT_MODEL:
		Q3_AddRHandModel(ent_id, data);
		break;

	case SET_REMOVERHANDBOLT_MODEL:
		Q3_RemoveRHandModel(ent_id, const_cast<char*>(data));
		break;

	case SET_ADDLHANDBOLT_MODEL:
		Q3_AddLHandModel(ent_id, data);
		break;

	case SET_REMOVELHANDBOLT_MODEL:
		Q3_RemoveLHandModel(ent_id, const_cast<char*>(data));
		break;
	...
	case SET_FACENORMAL:
		float_data = atof(data);
		Q3_Face(ent_id, toSet, float_data);
		break;
	...
	case SET_PLAYER_USABLE:
		if (!Q_stricmp("true", data))
		{
			Q3_SetPlayerUsable(ent_id, qtrue);
		}
		else
		{
			Q3_SetPlayerUsable(ent_id, qfalse);
		}
		break;

	case SET_STARTFRAME:
		int_data = atoi(data);
		Q3_SetStartFrame(ent_id, int_data);
		break;

	case SET_ENDFRAME:
		int_data = atoi(data);
		Q3_SetEndFrame(ent_id, int_data);
	...
	case SET_ANIMFRAME:
		int_data = atoi(data);
		Q3_SetAnimFrame(ent_id, int_data);
		return;

	case SET_LOOP_ANIM:
		if (!Q_stricmp("true", data))
		{
			Q3_SetLoopAnim(ent_id, qtrue);
		}
		else
		{
			Q3_SetLoopAnim(ent_id, qfalse);
		}
		break;
	...
	case SET_INTERFACE:
		if (!Q_stricmp("true", data))
		{
			Q3_SetInterface(ent_id, "1");
		}
		else
		{
			Q3_SetInterface(ent_id, "0");
		}
	...
	case SET_SHIELDS:
		if (!Q_stricmp("true", data))
		{
			Q3_SetShields(ent_id, qtrue);
		}
		else
		{
			Q3_SetShields(ent_id, qfalse);
		}
		break;
	...
	case SET_SABERACTIVE:
		if (!Q_stricmp("true", data))
		{
			Q3_SetSaberActive(ent_id, qtrue);
		}
		else
		{
			Q3_SetSaberActive(ent_id, qfalse);
		}
		break;
	...
	case SET_SABER1BLADEON:
		...
		int_data = atoi(data);
		Q3_SetSaberBladeActive(ent_id, 0, int_data, qtrue);
		break;
	...
	case SET_SABER1BLADEOFF:
		...
		int_data = atoi(data);
		Q3_SetSaberBladeActive(ent_id, 0, int_data, qfalse);
		break;
	...
	case SET_SABER2BLADEON:
		// Get which Blade to activate.
		int_data = atoi(data);
		Q3_SetSaberBladeActive(ent_id, 1, int_data, qtrue);
		break;
	...
	case SET_SABER2BLADEOFF:
		// Get which Blade to deactivate.
		int_data = atoi(data);
		Q3_SetSaberBladeActive(ent_id, 1, int_data, qfalse);
		break;
	...
	case SET_ADJUST_AREA_PORTALS:
		if (!Q_stricmp("true", data))
		{
			Q3_SetAdjustAreaPortals(ent_id, qtrue);
		}
		else
		{
			Q3_SetAdjustAreaPortals(ent_id, qfalse);
		}
		break;
	...
	case SET_DMG_BY_HEAVY_WEAP_ONLY:
		if (!Q_stricmp("true", data))
		{
			Q3_SetDmgByHeavyWeapOnly(ent_id, qtrue);
		}
		else
		{
			Q3_SetDmgByHeavyWeapOnly(ent_id, qfalse);
		}
		break;
	...
	case SET_SHIELDED:
		if (!Q_stricmp("true", data))
		{
			Q3_SetShielded(ent_id, qtrue);
		}
		else
		{
			Q3_SetShielded(ent_id, qfalse);
		}
		break;
	...
	case SET_NO_GROUPS:
		if (!Q_stricmp("true", data))
		{
			Q3_SetNoGroups(ent_id, qtrue);
		}
		else
		{
			Q3_SetNoGroups(ent_id, qfalse);
		}
		break;
	...
	case SET_FIRE_WEAPON:
		if (!Q_stricmp("true", data))
		{
			Q3_SetFireWeapon(ent_id, qtrue);
		}
		else if (!Q_stricmp("false", data))
		{
			Q3_SetFireWeapon(ent_id, qfalse);
		}
		break;
	...
	case SET_FIRE_WEAPON_NO_ANIM:
		if (!Q_stricmp("true", data))
		{
			Q3_SetFireWeaponNoAnim(ent_id, qtrue);
		}
		else if (!Q_stricmp("false", data))
		{
			Q3_SetFireWeaponNoAnim(ent_id, qfalse);
		}
		break;
	case SET_SAFE_REMOVE:
		if (!Q_stricmp("true", data))
		{
			Q3_SetSafeRemove(ent_id, qtrue);
		}
		else if (!Q_stricmp("false", data))
		{
			Q3_SetSafeRemove(ent_id, qfalse);
		}
		break;
	...
	case SET_BOBA_JET_PACK:
		if (!Q_stricmp("true", data))
		{
			Q3_SetBobaJetPack(ent_id, qtrue);
		}
		else if (!Q_stricmp("false", data))
		{
			Q3_SetBobaJetPack(ent_id, qfalse);
		}
		break;
	...
	case SET_INACTIVE:
		if (!Q_stricmp("true", data))
		{
			Q3_SetInactive(ent_id, qtrue);
		}
		else if (!Q_stricmp("false", data))
		{
			Q3_SetInactive(ent_id, qfalse);
		}
		else if (!Q_stricmp("unlocked", data))
		{
			un_lock_doors(&g_entities[ent_id]);
		}
		else if (!Q_stricmp("locked", data))
		{
			lock_doors(&g_entities[ent_id]);
		}
		break;
	...
	case SET_FUNC_USABLE_VISIBLE:
		if (!Q_stricmp("true", data))
		{
			Q3_SetFuncUsableVisible(ent_id, qtrue);
		}
		else if (!Q_stricmp("false", data))
		{
			Q3_SetFuncUsableVisible(ent_id, qfalse);
		}
		break;
	...
	case SET_NO_KNOCKBACK:
		if (!Q_stricmp("true", data))
		{
			Q3_SetNoKnockback(ent_id, qtrue);
		}
		else
		{
			Q3_SetNoKnockback(ent_id, qfalse);
		}
		break;
	...
	case SET_REMOVE_TARGET:
		Q3_SetRemoveTarget(ent_id, data);
		break;
	...
	case SET_DISABLE_SHADER_ANIM:
		if (!Q_stricmp("true", data))
		{
			Q3_SetDisableShaderAnims(ent_id, qtrue);
		}
		else
		{
			Q3_SetDisableShaderAnims(ent_id, qfalse);
		}
		break;
	...
	case SET_SHADER_ANIM:
		if (!Q_stricmp("true", data))
		{
			Q3_SetShaderAnim(ent_id, qtrue);
		}
		else
		{
			Q3_SetShaderAnim(ent_id, qfalse);
		}
		break;
	...
	case SET_FORCE_BLINDING_LEVEL:
		int_data = atoi(data);
		Q3_SetForcePowerLevel(ent_id, toSet - SET_FORCE_HEAL_LEVEL, int_data);
		break;

	case SET_SABER1:
	case SET_SABER2:
		WP_SetSaber(&g_entities[ent_id], toSet - SET_SABER1, data);
		break;

	case SET_PLAYERMODEL:
		G_ChangePlayerModel(&g_entities[ent_id], data);
		break;

		// NOTE: Preconditions for entering a vehicle still exist. This is not assured to work. -Areis
	case SET_VEHICLE:
		Use(ent_id, data);
		//		G_DriveVehicle( &g_entities[ent_id], NULL, (char *)data );
		break;

	case SET_SECURITY_KEY:
		Q3_GiveSecurityKey(ent_id, data);
		break;

	case SET_SABER1_COLOR1:
	case SET_SABER1_COLOR2:
		WP_SaberSetColor(&g_entities[ent_id], 0, toSet - SET_SABER1_COLOR1, data);
		break;
	case SET_SABER2_COLOR1:
	case SET_SABER2_COLOR2:
		WP_SaberSetColor(&g_entities[ent_id], 1, toSet - SET_SABER2_COLOR1, data);
		break;
	case SET_DISMEMBER_LIMB:
		Q3_DismemberLimb(ent_id, const_cast<char*>(data));
		break;
	case SET_NO_PVS_CULL:
		Q3_SetBroadcast(ent_id, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	...
	case SET_CLOAK: // Created: 01/08/03 by AReis.
		...
		{
			Saboteur_Cloak(&g_entities[ent_id]);
		}
		else
		{
			Saboteur_Decloak(&g_entities[ent_id]);
		}
		break;
	case SET_FORCE_HEAL:
		Q3_SetForcePower(ent_id, FP_HEAL, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_SPEED:
		Q3_SetForcePower(ent_id, FP_SPEED, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_PUSH:
		Q3_SetForcePower(ent_id, FP_PUSH, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_PUSH_FAKE:
		ForceThrow(&g_entities[ent_id], qfalse, qtrue);
		break;
	case SET_FORCE_PULL:
		Q3_SetForcePower(ent_id, FP_PULL, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_MIND_TRICK:
		Q3_SetForcePower(ent_id, FP_TELEPATHY, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_GRIP:
		Q3_SetForcePower(ent_id, FP_GRIP, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_LIGHTNING:
		Q3_SetForcePower(ent_id, FP_LIGHTNING, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_SABERTHROW:
		Q3_SetForcePower(ent_id, FP_SABERTHROW, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_RAGE:
		Q3_SetForcePower(ent_id, FP_RAGE, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_PROTECT:
		Q3_SetForcePower(ent_id, FP_PROTECT, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_ABSORB:
		Q3_SetForcePower(ent_id, FP_ABSORB, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_DRAIN:
		Q3_SetForcePower(ent_id, FP_DRAIN, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_STASIS:
		Q3_SetForcePower(ent_id, FP_STASIS, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_DESTRUCTION:
		Q3_SetForcePower(ent_id, FP_DESTRUCTION, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_GRASP:
		Q3_SetForcePower(ent_id, FP_GRASP, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_REPULSE:
		Q3_SetForcePower(ent_id, FP_REPULSE, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_LIGHTNING_STRIKE:
		Q3_SetForcePower(ent_id, FP_LIGHTNING_STRIKE, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_FEAR:
		Q3_SetForcePower(ent_id, FP_FEAR, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_DEADLYSIGHT:
		Q3_SetForcePower(ent_id, FP_DEADLYSIGHT, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_BLAST:
		Q3_SetForcePower(ent_id, FP_BLAST, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_INSANITY:
		Q3_SetForcePower(ent_id, FP_INSANITY, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;
	case SET_FORCE_BLINDING:
		Q3_SetForcePower(ent_id, FP_BLINDING, static_cast<qboolean>(Q_stricmp("true", data) == 0));
		break;

	case SET_WINTER_GEAR: // Created: 03/26/03 by AReis.
	{
		// If this is a (fake) Player NPC or this IS the Player...
		if (ent_id == 0 || ent->NPC_type && Q_stricmp(ent->NPC_type, "player") == 0)
		{
	...
	case SET_NO_ANGLES:
		if (ent_id >= 0 && ent_id < ENTITYNUM_WORLD)
		{
			if (Q_stricmp("true", data) == 0)
			{
				g_entities[ent_id].flags |= FL_NO_ANGLES;
			}
			else
			{
				g_entities[ent_id].flags &= ~FL_NO_ANGLES;
	...
	case SET_RADAR_OBJECT:
		if (ent_id >= 0 && ent_id < ENTITYNUM_WORLD)
		{
			if (Q_stricmp("true", data) == 0)
			{
				g_entities[ent_id].s.eFlags2 |= EF2_RADAROBJECT;
			}
			else
			{
				g_entities[ent_id].s.eFlags2 &= ~EF2_RADAROBJECT;
	...
	default:
		SetVar(task_id, ent_id, type_name, data);
...
void CQuake3GameInterface::Use(const int ent_id, const char* name)
{
	...
	if (!ent)
	{
		DebugPrint(WL_WARNING, "Use: invalid ent_id %d\n", ent_id);
		return;
	}
...
void CQuake3GameInterface::Activate(const int ent_id, const char* name)
{
	Q3_SetInactive(ent_id, qtrue);
}

void CQuake3GameInterface::Deactivate(const int ent_id, const char* name)
{
	Q3_SetInactive(ent_id, qfalse);
}

// Kill an entity.
void CQuake3GameInterface::Kill(const int ent_id, const char* name)
{
	gentity_t* ent = &g_entities[ent_id];
	gentity_t* victim;
...
// Remove an entity.
void CQuake3GameInterface::Remove(const int ent_id, const char* name)
{
	gentity_t* ent = &g_entities[ent_id];
	gentity_t* victim;
...
void CQuake3GameInterface::Play(const int taskID, const int ent_id, const char* type, const char* name)
{
	gentity_t* ent = &g_entities[ent_id];
...
int CQuake3GameInterface::GetFloat(const int ent_id, const char* name, float* value)
{
	const gentity_t* ent = &g_entities[ent_id];
	//	gclient_t	*client;
...
int CQuake3GameInterface::GetVector(const int ent_id, const char* name, vec3_t value)
{
	const gentity_t* ent = &g_entities[ent_id];
	if (!ent)
	{
...
int CQuake3GameInterface::GetString(const int ent_id, const char* name, char** value)
{
	const gentity_t* ent = &g_entities[ent_id];
	if (!ent)
	{
...
int CQuake3GameInterface::LinkGame(const int ent_id, const int icarusID)
{
	gentity_t* pEntity = &g_entities[ent_id];
...
// Access functions
int CQuake3GameInterface::CreateIcarus(const int ent_id)
{
	gentity_t* pEntity = &g_entities[ent_id];

	// If the entity doesn't have an Icarus ID, obtain and assign a new one.
	if (pEntity->m_iIcarusID == IIcarusInterface::ICARUS_INVALID)
		pEntity->m_iIcarusID = IIcarusInterface::GetIcarus()->GetIcarusID(ent_id);
...
int CQuake3GameInterface::IsFrozen(const int ent_id)
{
	return g_entities[ent_id].svFlags & SVF_ICARUS_FREEZE;
}
