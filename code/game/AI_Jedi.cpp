...
qboolean npc_is_jedi(const gentity_t* self)
{
...
qboolean npc_force_master(const gentity_t* self)
{
	switch (self->client->NPC_class)
	{
	case CLASS_SITHLORD:
...
qboolean Jedi_CultistForceUser(const gentity_t* self)
{
	if (!self || !self->client)
	{
		return qfalse;
...
qboolean Self_IsGunner(const gentity_t* self)
{
	...
	for (int saber_num = 0; saber_num < MAX_SABERS; saber_num++)
	{
		for (int blade_num = 0; blade_num < NPC->enemy->client->ps.saber[saber_num].numBlades; blade_num++)
		{
				...
				&& NPC->enemy->client->ps.saber[saber_num].blade[blade_num].length > 0)
			{
				...
				VectorMA(NPC->enemy->client->ps.saber[saber_num].blade[blade_num].muzzle_pointOld,
				...
				VectorMA(NPC->enemy->client->ps.saber[saber_num].blade[blade_num].muzzle_point,
					NPC->enemy->client->ps.saber[saber_num].blade[blade_num].length,
				...
				dist = ShortestLineSegBewteen2LineSegs(
					NPC->enemy->client->ps.saber[saber_num].blade[blade_num].muzzle_point, saber_tip, bottom, top,
	...
	if (NPC->client->ps.saberEventFlags)
	{
		...
		if (NPC->client->ps.saberEventFlags & SEF_PARRIED)
		{
			...
			if (NPC->enemy && (!NPC->enemy->client || PM_SaberInKnockaway(NPC->enemy->client->ps.saber_move)))
			{
				...
				{
					//my saber is not in a parrying position
					Jedi_AdjustSaberAnimLevel(NPC, NPC->client->ps.saber_anim_level - 1); //use a faster attack
				}
			}
			else
			{
		...
		if (!NPC->client->ps.weaponTime && NPC->client->ps.saberEventFlags & SEF_HITENEMY) //hit enemy
		{
			...
			if (!Q_irand(0, 2))
			{
				Jedi_AdjustSaberAnimLevel(NPC, NPC->client->ps.saber_anim_level + 1);
			}
		...
		if (NPC->client->ps.saberEventFlags & SEF_BLOCKED)
		{
				...
				BLOCKED_PARRY_BROKEN)
			{
				...
				Jedi_AdjustSaberAnimLevel(NPC, NPC->client->ps.saber_anim_level + 1); //use a stronger attack
				if (d_JediAI->integer || g_DebugSaberCombat->integer)
				{
	...
	if (NPC->client->NPC_class == CLASS_REBORN)
	{
		if (NPCInfo->aiFlags & NPCAI_HEAL_ROSH)
		{
			...
			if (NPC->client->leader)
			{
				...
				if (NPC->client->leader->client->ps.legsAnim == BOTH_FORCEHEAL_START && TIMER_Done(
					NPC, "healRoshDebounce"))
				{
					if (Kothos_HealRosh())
					{
						helping_rosh = qtrue;
