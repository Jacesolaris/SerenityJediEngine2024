...
// HFile Bindings
////////////////////////////////////////////////////////////////////////////////////////
bool HFILEopen_read(int& handle, const char* filepath)
{
	gi.FS_FOpenFile(filepath, &handle, FS_READ);
	return handle != 0;
}

bool HFILEopen_write(int& handle, const char* filepath)
{
	gi.FS_FOpenFile(filepath, &handle, FS_WRITE);
	return handle != 0;
}
...
namespace NAV
{
	enum
	{
		...
		NUM_REGIONS = NUM_NODES / 3,
		// Had to raise this up for bounty
		NUM_CELLS = 32,
		// should be the square root of NUM_NODES
		NUM_NODES_PER_CELL = 60,
		// had to raise this for t3_bounty
		NUM_TARGETS = 5,
		// max number of outgoing edges from a given node
...
using TAlertList = ratl::array_vs<SDangerAlert, NAV::MAX_ALERTS_PER_AGENT>;
using TEntityAlertList = ratl::array_vs<TAlertList, MAX_GENTITIES>;
using TNamedNodeList = ratl::vector_vs<NAV::TNodeHandle, NAV::MAX_NODES_PER_NAME>;
using TNameToNodeMap = ratl::map_vs<hstring, TNamedNodeList, NAV::NUM_NODES>;

using TEdgesPerEnt = ratl::vector_vs<NAV::TEdgeHandle, NAV::MAX_EDGES_PER_ENT>;
using TEntEdgeMap = ratl::map_vs<int, TEdgesPerEnt, NAV::MAX_BLOCKING_ENTS>;
...
using TPathUsers = ratl::pool_vs<SPathUser, NAV::MAX_PATH_USERS>;
using TPathUserIndex = ratl::array_vs<int, MAX_GENTITIES>;

using TNeighbors = ratl::vector_vs<gentity_t*, STEER::MAX_NEIGHBORS>;
...
//
////////////////////////////////////////////////////////////////////////////////////////
class c_graph_user final : public TGraph::user
{
private:
...
public:
	...
	//
	////////////////////////////////////////////////////////////////////////////////////
	bool can_be_invalid(const CWayEdge& Edge) const override
	{
	...
	bool is_valid(CWayEdge& Edge, const int EndPoint = 0) const override
	{
		...
		//-----------------------------------------------------------------
		if (mActor && Edge.mFlags.get_bit(CWayEdge::WE_FLYING) && mActor->NPC && !(mActor->NPC->scriptFlags &
			SCF_NAV_CAN_FLY))
		{
			return false;
		}
		...
		// If The Actor Can't Fly, But This Is A Flying Edge, It's Invalid
		//-----------------------------------------------------------------
		if (mActor && Edge.mFlags.get_bit(CWayEdge::WE_JUMPING) && mActor->NPC && !(mActor->NPC->scriptFlags &
			SCF_NAV_CAN_JUMP))
		{
			return false;
		}
		...
		if (Edge.mEntityNum != ENTITYNUM_NONE)
		{
			...
			if (ent)
			{
				...
				{
					const bool StartOpen = ent->spawnflags & 1;
					const bool Closed = StartOpen ? ent->moverState == MOVER_POS2 : ent->moverState == MOVER_POS1;

					// If It Is Closed, We Want To Check If It Will Auto Open For Us
					//---------------------------------------------------------------
					if (Closed)
					{
						const gentity_t* owner = &g_entities[Edge.mOwnerNum];
						if (owner)
						{
							// Check To See If The Owner Is Inactive Or Locked, Or Unavailable To The NPC
							//----------------------------------------------------------------------------
							if (owner->svFlags & SVF_INACTIVE ||
								owner == ent && owner->spawnflags & (MOVER_PLAYER_USE | MOVER_FORCE_ACTIVATE |
									MOVER_LOCKED) ||
							...
							// Look For A Key
							//----------------
							if (mActor != nullptr && owner->spawnflags & MOVER_GOODIE)
							{
								const int key = INV_GoodieKeyCheck(mActor);
								if (!key)
								{
									return false;
								}
	...
	float cost(const CWayEdge& Edge, const c_way_node& B) const override
	{
		...
		if (mActor)
		{
			const int eHandle = GetGraph().edge_index(Edge);
	...
	const int OverrideContents = 0)
{
	...
	//---------------
	mMoveTraceCount++;
	gi.trace(&mMoveTrace, Start.v, Mins.v, Maxs.v, Stop.v, IgnoreEnt, contents, static_cast<EG2_Collision>(0), 0);
	...
	//----------------------------------------------------------------------------------------
	if (CheckForDoNotEnter && RetryIfStartInDoNotEnter && (mMoveTrace.allsolid == qtrue || mMoveTrace.startsolid ==
		qtrue))
	{
		contents &= ~CONTENTS_BOTCLIP;
...
bool NAV::GoTo(gentity_t* actor, const TNodeHandle target, const float MaxDangerLevel)
{
	...
	if (HasPath)
	{
		HasPath = UpdatePath(actor, target, MaxDangerLevel);
	}
	...
	if (!HasPath)
	{
		HasPath = FindPath(actor, target, MaxDangerLevel);
	}
	...
	if (HasPath)
	{
		...
		if (HasPath)
		{
			...
			{
				STEER::Blocked(actor, NextPosition(actor));
			}
		}
		else
		{
			STEER::Blocked(actor, GetNodePosition(target));
		}
	}
...
bool NAV::GoTo(gentity_t* actor, gentity_t* target, const float MaxDangerLevel)
{
	...
	if (targetNode != 0)
	{
		...
		if (HasPath)
		{
			HasPath = UpdatePath(actor, targetNode, MaxDangerLevel);
		}
		...
		if (!HasPath)
		{
			HasPath = FindPath(actor, targetNode, MaxDangerLevel);
		}
...
bool NAV::LoadFromEntitiesAndSaveToFile(const char* filename, int checksum)
{
	...
	/*	TGraphTriang	*Triang = new TGraphTriang(mGraph);
		...
		delete Triang;
	*/
	mCells.fill_cells_nodes(CELL_RANGE);
	...
	for (nodeIter = mGraph.nodes_begin(); nodeIter != mGraph.nodes_end(); ++nodeIter)
	{
		...
		//----------------------------------
		for (tgtNum = 0; tgtNum < NUM_TARGETS; tgtNum++)
		{
			...
			(at->mType == PT_COMBATNODE || at->mType == PT_GOALNODE))
		{
			...
			for (int cellNode = 0; cellNode < Cell.mNodes.size(); cellNode++)
			{
				...
				//---------------------------------------------
				if (fabsf(node.mPoint[2] - at->mPoint[2]) > MAX_EDGE_FLOOR_DIST)
				{
					continue;
				}
				...
				//------------------------------------------------------------------------------------------
				if (node.mType == PT_COMBATNODE || node.mType == PT_GOALNODE)
				{
					nonWPCount++;
					if (nonWPCount > MAX_NONWP_NEIGHBORS)
					{
						continue;
					}

					if (cost > MAX_EDGE_AUTO_LEN / 2.0f)
					{
						continue;
					}
				}
	...
	//==================================================================
	auto ToBeRemoved = new ratl::vector_vs<int, NUM_EDGES>;
	for (edgeIter = mGraph.edges_begin(); edgeIter != mGraph.edges_end(); ++edgeIter)
	{
		...
		if (way_edge.mFlags.get_bit(CWayEdge::WE_JUMPING))
		{
			way_edge.mFlags.set_bit(CWayEdge::WE_SIZE_LARGE);
			way_edge.mFlags.set_bit(CWayEdge::WE_CANBEINVAL);
			way_edge.mFlags.set_bit(CWayEdge::WE_DESIGNERPLACED);
			continue;
		}
		...
		// Try Medium
		//------------
		CanGo = TestEdge(way_edge.mNodeA, way_edge.mNodeB, static_cast<qboolean>(IsDebugEdge));
		if (!CanGo)
		{
			way_edge.mFlags.clear_bit(CWayEdge::WE_SIZE_LARGE);
			way_edge.mFlags.set_bit(CWayEdge::WE_SIZE_MEDIUM);
			if (IsDebugEdge)
			{
		...
		if (!CanGo)
		{
			ToBeRemoved->push_back(at_handle);
			if (IsDebugEdge)
			{
	...
	//===================================
	mCells.fill_cells_edges(CELL_RANGE);
...
void NAV::RegisterDangerSense(gentity_t* actor, const int alertEventIndex)
{
	...
	{
		const int edgeHandle = cellEdges[cellEdgeIndex];
...
void NAV::SpawnedPoint(gentity_t* ent, const EPointType type)
{
	...
	const CVec3 Start(ent->currentOrigin);
	...
	const int node_handle = mGraph.insert_node(node);

	ent->waypoint = node_handle;

	mCells.expand_bounds(node_handle);

	if (!node.mName.empty())
	{
		const TNameToNodeMap::iterator nameFinder = mNodeNames.find(node.mName);

		if (nameFinder == mNodeNames.end())
		{
			TNamedNodeList list;
			list.clear();
			list.push_back(node_handle);
			mNodeNames.insert(node.mName, list);
		}
		else
		{
			(*nameFinder).push_back(node_handle);
		}
	}
}
	...
	if (mGraph.size_edges() > 0)
	{
		...
		//===========================
		{
			...
			for (int i = 0; i < Cell.mNodes.size() && !mNearestNavSort.full(); i++)
			{
				...
				//--------------------------------
				if (NodeSort.mDistance > VIEW_RANGE * VIEW_RANGE)
				{
					continue;
				}
				...
				if (previous && previous != NodeSort.mHandle && !InSameRegion(previous, NodeSort.mHandle))
				{
				...
				if (goal && goal != NodeSort.mHandle && !InSameRegion(goal, NodeSort.mHandle))
				{
			...
			for (int j = 0; j < mNearestNavSort.size(); j++)
			{
				...
				//-----------------------------------
				if (ViewNavTrace(Pos, mGraph.get_node(mNearestNavSort[j].mHandle).mPoint))
				{
					return mNearestNavSort[j].mHandle;
				}
			}
...
//
////////////////////////////////////////////////////////////////////////////////////////
NAV::TNodeHandle NAV::ChooseClosestNeighbor(const TNodeHandle NodeHandle, const vec3_t& position)
{
	if (NodeHandle != WAYPOINT_NONE && NodeHandle > 0)
	{
		const CVec3 pos(position);
		...
		TNodeHandle Cur = WAYPOINT_NONE;
		TNodeHandle Best = NodeHandle;
		float BestDist = mGraph.get_node(Cur).mPoint.Dist2(pos);

		for (int i = 0; i < neighbors.size(); i++)
		{
			Cur = neighbors[i].mNode;
			const float cur_dist = mGraph.get_node(Cur).mPoint.Dist2(pos);
			if (Best == WAYPOINT_NONE || BestDist < cur_dist)
			{
				Best = Cur;
				BestDist = cur_dist;
			}
...
NAV::TNodeHandle NAV::ChooseFarthestNeighbor(gentity_t* actor, const vec3_t& target, float maxSafeDot)
{
	...
	const CVec3 targetPos(target);
	CVec3 actorToTgt(targetPos - actorPos);
	const float actorToTgtDist = actorToTgt.Norm();

	TNodeHandle cur = GetNearestNode(actor);
	...
	if (cur < 0)
	{
		const CWayEdge& edge = mGraph.get_edge(abs(cur));
	...
	TNodeHandle best = WAYPOINT_NONE;
	float bestDist = 0.0f;
...
bool NAV::FindPath(gentity_t* actor, TNodeHandle target, const float MaxDangerLevel)
{
	...
	TNodeHandle start = GetNearestNode(actor, true, target);
	...
	//----------------------------
	const float minRadius = Min(actor->mins[0], actor->mins[1]);
	const float maxRadius = Max(actor->maxs[0], actor->maxs[1]);
	...
	const int numEdges = puser.mPath.size() - 1;
	...
	for (int edge = 0; edge < numEdges; edge++)
	{
		...
		CVec3 AtOnEdge(At);
		const float AtOnEdgeScale = AtOnEdge.ProjectToLineSeg(PointA, PointB);
		const float AtDistToEdge = AtOnEdge.Dist(At);

		if (AtOnEdgeScale > 0.1f && AtOnEdgeScale < 0.9f)
		{
			if (AtDistToEdge < radius || AtDistToEdge < radius * 20.0f && MoveTrace(
				At, AtOnEdge, actor->mins, actor->maxs, actor->s.number, true, true, false))
			{
	...
	//----------------------------------------
	if (MaxDangerLevel != 1.0f && PathDangerLevel(NPC) > MaxDangerLevel)
	{
		puser.mSuccess = false;
	}
...
bool NAV::InSameRegion(gentity_t* actor, gentity_t* target)
{
	...
	if (mRegion.size() > 0)
	{
		TNodeHandle actNode = GetNearestNode(actor);
		TNodeHandle tgtNode = GetNearestNode(target);
...
bool NAV::InSameRegion(gentity_t* actor, const vec3_t& position)
{
	...
	{
		TNodeHandle actNode = GetNearestNode(actor);
		TNodeHandle tgtNode = GetNearestNode(position);
...
bool NAV::InSameRegion(const TNodeHandle A, const TNodeHandle B)
{
	if (mRegion.size() > 0)
	{
		TNodeHandle actNode = A;
		TNodeHandle tgtNode = B;
		...
		mUser.ClearActor();
		const bool hasEdge = mRegion.has_valid_edge(actNode, tgtNode, mUser);
		if (tempActor)
		{
...
bool NAV::OnNeighboringPoints(const TNodeHandle A, const TNodeHandle B)
{
	...
	const int edgeNum = mGraph.get_edge_across(A, B);
	if (edgeNum &&
...
bool NAV::InSafeRadius(const CVec3 at, const TNodeHandle atNode, const TNodeHandle targetNode)
{
	...
	if (targetNode > 0 && atNode != targetNode)
	{
		...
		// Does The Edge Exist?
		//----------------------
		const int atToTargetEdgeIndex = mGraph.get_edge_across(atNode, targetNode);
		if (atToTargetEdgeIndex != 0)
		{
			...
			{
				const float at_dist_to_edge = at.DistToLine(atToTargetEdge.PointA(), atToTargetEdge.PointB());

				// Now Are We Close Enough To The Edge
				//-------------------------------------
				if (atToTargetEdge.mFlags.get_bit(CWayEdge::WE_SIZE_LARGE))
				{
					return at_dist_to_edge < SC_LARGE_RADIUS;
				}
				return at_dist_to_edge < SC_MEDIUM_RADIUS;
			} // not valid edge
		} // no edge
	} // no valid target
	return false;
...
bool NAV::FindPath(gentity_t* actor, gentity_t* target, const float MaxDangerLevel)
{
	...
	if (target != nullptr && actor != nullptr)
	{
		...
		if (target->lastWaypoint != WAYPOINT_NONE)
		{
...
bool NAV::UpdatePath(gentity_t* actor, const TNodeHandle target, const float MaxDangerLevel)
{
	...
	if (path[path.size() - 1].mETA < level.time || MaxDangerLevel != 1.0f && PathDangerLevel(NPC) > MaxDangerLevel)
	{
		...
		//--------------------------------------------------------------------------------------------
		const TNodeHandle node_handle = mPathUsers[pathUserNum].mEnd; // Remember The End As Our Target
		if (node_handle == WAYPOINT_NONE)
		{
			ClearPath(actor);
			return false;
		}
		mPathUsers[pathUserNum].mEnd = WAYPOINT_NONE; // Clear Out The Old End
		if (!FindPath(actor, node_handle, MaxDangerLevel))
		{
			mPathUsers[pathUserNum].mEnd = node_handle;
			return false;
		}
		return true;
	}
...
float NAV::PathDangerLevel(gentity_t* actor)
{
	...
	for (; curPathAt >= 0; curPathAt--)
	{
		...
		if (prevNode < 0 || mGraph.get_edge_across(prevNode, PPoint.mNode))
		{
			...
			{
				cur_edge = mGraph.get_edge_across(prevNode, PPoint.mNode);
			}
			for (int al_index = 0; al_index < TAlertList::CAPACITY; al_index++)
			{
				if (al[al_index].mHandle == cur_edge && al[al_index].mDanger > DangerLevel)
				{
					DangerLevel = al[al_index].mDanger;
				}
			}
		}
		...
		{
			const float enemy_danger_level = enemyPos.DistToLine(prevPoint, PPoint.mPoint) / enemySafeDist;
			if (enemy_danger_level > DangerLevel)
			{
				DangerLevel = enemy_danger_level;
			}
		}
...
//
////////////////////////////////////////////////////////////////////////////////////////
void NAV::show_debug_info(const vec3_t& player_position, int player_waypoint)
{
	mUser.ClearActor();
	...
	if (NAVDEBUG_showNodes || NAVDEBUG_showCombatPoints || NAVDEBUG_showNavGoals)
	{
		for (TGraph::TNodes::iterator atIter = mGraph.nodes_begin(); atIter != mGraph.nodes_end(); ++atIter)
		{
			...
			if (gi.inPVS(player_position, at.mPoint.v))
			{
				if (at.mType == PT_WAYNODE && NAVDEBUG_showNodes)
				{
					if (NAVDEBUG_showPointLines)
					{
						...
						{
							CG_DrawEdge(at.mPoint.v, at_end.v, EDGE_NODE_NORMAL);
						}
					}
					else
					{
				...
				else if (at.mType == PT_COMBATNODE && NAVDEBUG_showCombatPoints)
				{
					if (NAVDEBUG_showPointLines)
					{
						CG_DrawEdge(at.mPoint.v, at_end.v, EDGE_NODE_COMBAT);
					}
					else
					{
				...
				else if (at.mType == PT_GOALNODE && NAVDEBUG_showNavGoals)
				{
					if (NAVDEBUG_showPointLines)
					{
						CG_DrawEdge(at.mPoint.v, at_end.v, EDGE_NODE_GOAL);
					}
					else
					{
	...
	if (NAVDEBUG_showEdges)
	{
		for (TGraph::TEdges::iterator atIter = mGraph.edges_begin(); atIter != mGraph.edges_end(); ++atIter)
		{
			...
			if (AvePos.Dist2(player_position) < 500.0f * 500.0f && gi.inPVS(player_position, AvePos.v))
			{
	...
	if (NAVDEBUG_showGrid)
	{
		...
		for (int x = 0; x < TGraphCells::SIZEX; x++)
		{
			for (int y = 0; y < TGraphCells::SIZEY; y++)
			{
				...
				if (center.Dist(player_position) < 10000.0f)
				{
	...
	{
		player_waypoint = player->waypoint ? player->waypoint : player->lastWaypoint;
		...
		{
			const CWayEdge& edge = mGraph.get_edge(abs(player_waypoint));
			CVec3 PosOnLine(player_position);
...
void NAV::ShowStats()
{
	...
	mGraph.ProfilePrint("");
	mGraph.ProfilePrint("--------------------------------------------------------");
	mGraph.ProfilePrint("              Star Wars - SerenityJediEngine2024        ");
...
void STEER::DeActivate(gentity_t* actor, usercmd_t* ucmd)
{
	...
	if (!NPC_Jumping())
	{
		...
		if (suser.mSpeed > 0.0f)
		{
			...
			const float fDot = Com_Clamp(-127.0f, 127.0f, DotProduct(forward, MoveDir.v) * 127.0f);
			const float rDot = Com_Clamp(-127.0f, 127.0f, DotProduct(right, MoveDir.v) * 127.0f);
		...
		if (suser.mBlocked)
		{
			...
			//-----------------------------------------------------------------------------------
			actor->NPC->blockedTargetEntity = nullptr;
		...
		//-----------------------------------------------------------------------------------
		if (HasBeenBlockedFor(actor, 2000))
		{
...
float STEER::Persue(gentity_t* actor, gentity_t* target, const float slowingDistance)
{
	...
	if (target->client)
	{
		const float DistToTarget = ProjectedTargetPosition.Dist(suser.mPosition);

		CVec3 TargetVelocity(target->client->ps.velocity);
		const float TargetSpeed = TargetVelocity.SafeNorm();
		if (TargetSpeed > 0.0f)
		{
...
gentity_t* STEER::SelectLeader(gentity_t* actor)
{
	...
	for (int i = 0; i < suser.mNeighbors.size(); i++)
	{
		if (suser.mNeighbors[i]->s.number > actor->s.number && !Q_stricmp(
			suser.mNeighbors[i]->NPC_type, actor->NPC_type))
		{
...
float STEER::FollowLeader(gentity_t* actor, gentity_t* leader, const float dist)
{
	...
	const float LeaderSpeed = leader->resultspeed;
	const int TimeRemaining = leader->followPosRecalcTime - level.time;

	if (TimeRemaining < 0 || LeaderSpeed > 0.0f && TimeRemaining > 1000)
	{
		const CVec3 LeaderPosition(leader->currentOrigin);
		...
		const float SpeedScale = 1.0f - LeaderSpeed / MaxSpeed;
	...
	const float weight = 1.0f)
{
	...
	if (!Safe && Side == Side_None)
	{
		...
		if (ContactNum != ENTITYNUM_WORLD && ContactNum != ENTITYNUM_NONE && g_entities[ContactNum].client != nullptr)
		{
			...
			if (ContactSpeed > 0.01f)
			{
				if (ContactSpeed < ProjectSpeed)
				{
					...
					const float DirectionSimilarity = fabsf(MyDirection.Dot(ContactDirection));
			...
			else
			{
				...
				const CVec3 AbsMin(Contact->absmin);
				const CVec3 AbsMax(Contact->absmax);
		...
		if (!Safe)
		{
			...
			//-----------------------------------------------------------
			if (static_cast<ESide>(actor->NPC->lastAvoidSteerSide) == Side_Right ||
				static_cast<ESide>(actor->NPC->lastAvoidSteerSide) == Side_None && ContactNormal.Dot(ProjectDirection) <
				0.0f)
			{
				ContactNormal *= -1.0f;
...
bool STEER::Reached(gentity_t* actor, gentity_t* target, const float targetRadius, bool flying)
{
	...
	const CVec3 ActorPos(actor->currentOrigin);
	const CVec3 ActorMins(actor->absmin);
	const CVec3 ActorMaxs(actor->absmax);

	const CVec3 TargetPos(target->currentOrigin);
...
bool STEER::Reached(gentity_t* actor, const NAV::TNodeHandle target, const float targetRadius, bool flying)
{
	...
	const CVec3 TargetPos(NAV::GetNodePosition(target));
