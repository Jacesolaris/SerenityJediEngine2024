...
constexpr auto NPCAI_MOVING = 0x00000004;
constexpr auto NPCAI_TOUCHED_GOAL = 0x00000008;
constexpr auto NPCAI_PUSHED = 0x00000010;
constexpr auto NPCAI_NO_COLL_AVOID = 0x00000020;
constexpr auto NPCAI_BLOCKED = 0x00000040;
constexpr auto NPCAI_SUBBOSS_CHARACTER = 0x00000080; //Alora, tough reborn;
constexpr auto NPCAI_OFF_PATH = 0x00000100;
constexpr auto NPCAI_IN_SQUADPOINT = 0x00000200;
constexpr auto NPCAI_STRAIGHT_TO_DESTPOS = 0x00000400;
constexpr auto NPCAI_HEAVY_MELEE = 0x00000800; //4x melee damage, dismemberment;
constexpr auto NPCAI_NO_SLOWDOWN = 0x00001000;
constexpr auto NPCAI_LOST = 0x00002000; //Can't nav to his goal;
constexpr auto NPCAI_SHIELDS = 0x00004000; //Has shields, borg can adapt;
constexpr auto NPCAI_GREET_ALLIES = 0x00008000; //Say hi to nearby allies;
constexpr auto NPCAI_FORM_TELE_NAV = 0x00010000; //Tells formation people to use nav info to get to;
constexpr auto NPCAI_ENROUTE_TO_HOMEWP = 0x00020000; //Lets us know to run our lostenemyscript when we get to homeWp;
constexpr auto NPCAI_MATCHPLAYERWEAPON = 0x00040000;
//Match the player's weapon except when it changes during cinematics;
constexpr auto NPCAI_DIE_ON_IMPACT = 0x00100000; //Next time you crashland, die!;
constexpr auto NPCAI_WALKING = 0x00200000;
constexpr auto NPCAI_STOP_AT_LOS = 0x00400000; //Stop Running When We Hit LOS;
constexpr auto NPCAI_NAV_THROUGH_BREAKABLES = 0x00800000;
//Navigation allows connections through breakable (func_glass, func_breakable or misc_model_breakable);
constexpr auto NPCAI_KNEEL = 0x01000000; //Kneel befor Zod;
constexpr auto NPCAI_FLY = 0x02000000; //Fly, My Pretty!;
constexpr auto NPCAI_FLAMETHROW = 0x04000000;
constexpr auto NPCAI_ROSH = 0x08000000;
//I am Rosh, when I'm hurt, drop to one knee and wait for Vil or Dasariah to heal me;
constexpr auto NPCAI_HEAL_ROSH = 0x10000000;
//Constantly look for NPC with NPC_type of rosh_dark, follow him, heal him if needbe;
...
constexpr auto SCF_WALKING = 0x00000002; //Force BUTTON_WALKING to be pressed;
constexpr auto SCF_MORELIGHT = 0x00000004; //NPC will have a minlight of 96;
constexpr auto SCF_LEAN_RIGHT = 0x00000008; //Force rightmove+BUTTON_USE;
constexpr auto SCF_LEAN_LEFT = 0x00000010; //Force leftmove+BUTTON_USE;
constexpr auto SCF_RUNNING = 0x00000020; //Takes off walking button, overrides SCF_WALKING;
constexpr auto SCF_altFire = 0x00000040; //Force to use alt-fire when firing;
constexpr auto SCF_NO_RESPONSE = 0x00000080; //NPC will not do generic responses to being used;
constexpr auto SCF_FFDEATH = 0x00000100; //Just tells player_die to run the friendly fire deathscript;
constexpr auto SCF_NO_COMBAT_TALK = 0x00000200; //NPC will not use their generic combat chatter stuff;
constexpr auto SCF_CHASE_ENEMIES = 0x00000400;
//NPC chase enemies - FIXME: right now this is synonymous with using combat points... should it be?;
constexpr auto SCF_LOOK_FOR_ENEMIES = 0x00000800; //NPC be on the lookout for enemies;
constexpr auto SCF_FACE_MOVE_DIR = 0x00001000;
//NPC face direction it's moving - FIXME: not really implemented right now;
constexpr auto SCF_IGNORE_ALERTS = 0x00002000; //NPC ignore alert events;
constexpr auto SCF_DONT_FIRE = 0x00004000; //NPC won't shoot;
constexpr auto SCF_DONT_FLEE = 0x00008000; //NPC never flees;
constexpr auto SCF_FORCED_MARCH = 0x00010000; //NPC that the player must aim at to make him walk;
constexpr auto SCF_NO_GROUPS = 0x00020000; //NPC cannot alert groups or be part of a group;
constexpr auto SCF_FIRE_WEAPON = 0x00040000; //NPC will fire his (her) weapon;
constexpr auto SCF_NO_MIND_TRICK = 0x00080000; //Not succeptible to mind tricks;
constexpr auto SCF_USE_CP_NEAREST = 0x00100000;
//Will use combat point close to it, not next to player or try and flank player;
constexpr auto SCF_NO_FORCE = 0x00200000; //Not succeptible to force powers;
constexpr auto SCF_NO_FALLTODEATH = 0x00400000; //NPC will not scream and tumble and fall to hit death over large drops;
constexpr auto SCF_NO_ACROBATICS = 0x00800000; //Jedi won't jump, roll or cartwheel;
constexpr auto SCF_USE_SUBTITLES = 0x01000000;
//Regardless of subtitle setting, this NPC will display subtitles when it speaks lines;
constexpr auto SCF_NO_ALERT_TALK = 0x02000000; //Will not say alert sounds, but still can be woken up by alerts;
constexpr auto SCF_NAV_CAN_FLY = 0x04000000; //Navigation allows connections through air;
constexpr auto SCF_FLY_WITH_JET = 0x08000000; //Must Fly With A Jet;
constexpr auto SCF_PILOT = 0x10000000; //Can pilot a vehicle;
constexpr auto SCF_NAV_CAN_JUMP = 0x20000000; //Can attempt to jump when blocked;
constexpr auto SCF_FIRE_WEAPON_NO_ANIM = 0x40000000; //Fire weapon but don't play weapon firing anim;
constexpr auto SCF_SAFE_REMOVE = 0x80000000; //Remove NPC when it's safe (when player isn't looking);

//Debug flag definitions

constexpr auto AID_IDLE = 0x00000000; //Nothing is happening;
constexpr auto AID_ACQUIRED = 0x00000001; //A target has been found;
constexpr auto AID_LOST = 0x00000002; //Alert, but no target is in sight;
constexpr auto AID_CONFUSED = 0x00000004; //Is unable to come up with a course of action;
constexpr auto AID_LOSTPATH = 0x00000008; //Cannot make a valid movement due to lack of connections;
...
using lookMode_t = enum //# lookMode_e
{
	LM_ENT = 0,
	LM_INTEREST
};

using jumpState_t = enum //# jumpState_e
{
	JS_WAITING = 0,
	JS_FACING,
	JS_CROUCHING,
