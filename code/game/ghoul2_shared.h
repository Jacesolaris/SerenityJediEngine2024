...
constexpr auto BONE_ANGLES_PREMULT = 0x0001;
constexpr auto BONE_ANGLES_POSTMULT = 0x0002;
constexpr auto BONE_ANGLES_REPLACE = 0x0004;
//rww - RAGDOLL_BEGIN
constexpr auto BONE_ANGLES_RAGDOLL = 0x2000; // the rag flags give more details;
constexpr auto BONE_ANGLES_IK = 0x4000; // the rag flags give more details;
//rww - RAGDOLL_END
#define BONE_ANGLES_TOTAL			( BONE_ANGLES_PREMULT | BONE_ANGLES_POSTMULT | BONE_ANGLES_REPLACE )

constexpr auto BONE_ANIM_OVERRIDE = 0x0008;
constexpr auto BONE_ANIM_OVERRIDE_LOOP = 0x0010; // Causes Last Frame To Lerp to First Frame And Start Over;
#define BONE_ANIM_OVERRIDE_FREEZE	(0x0040 + BONE_ANIM_OVERRIDE)	// Causes Last Frame To Freeze And Not Loop To Beginning
constexpr auto BONE_ANIM_BLEND = 0x0080; // Blends to and from previously played frame on same bone for given time;
constexpr auto BONE_ANIM_NO_LERP = 0x1000;
...
constexpr auto BONE_INDEX_INVALID = -1;
...
struct boneInfo_t
{
	...
	boneInfo_t() :
		...
		originalTrueBoneMatrix(),
		parentTrueBoneMatrix(),
		parentOriginalTrueBoneMatrix(),
		originalOrigin{}, originalAngles{},
		lastShotDir{},
		basepose(nullptr), baseposeInv(nullptr),
		baseposeParent(nullptr),
		baseposeInvParent(nullptr),
		parentRawBoneIndex(0),
		ragOverrideMatrix(), extraMatrix(),
		extraVec1{}, extraFloat1(0),
		extraInt1(0), ikPosition{},
		ikSpeed(0),
		epVelocity{},
		epGravFactor(0),
		solidCount(0),
...
constexpr auto MAX_GHOUL_COUNT_BITS = 8;
// bits required to send across the MAX_G2_MODELS inside of the networking - this is the only restriction on ghoul models possible per entity;

using surfaceInfo_v = std::vector<surfaceInfo_t>;
using boneInfo_v = std::vector<boneInfo_t>;
using boltInfo_v = std::vector<boltInfo_t>;
using mdxaBone_v = std::vector<mdxaBone_t>;

// defines for stuff to go into the mflags
constexpr auto GHOUL2_NOCOLLIDE = 0x001;
constexpr auto GHOUL2_NORENDER = 0x002;
...
#define BSAVE_END_FIELD mTransformedVertsArray	// this is the end point for loadsave, keep it up to date it you change anything
	...
	CGhoul2Info() :
		...
		mMeshFrameNum(-1),
		mFlags(0),
		mTransformedVertsArray(nullptr),
		mBoneCache(nullptr),
		mSkin(0),
		mValid(false),
...
{
public:
	virtual ~IGhoul2InfoArray() = default;
	virtual int New() = 0;
...
constexpr auto G2_FRONTFACE = 1;
constexpr auto G2_BACKFACE = 0;
