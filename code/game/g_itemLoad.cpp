...
static void IT_Name(const char** hold_buf)
{
	...
	else if (!Q_stricmp(token_str, "ITM_SBD_PISTOL_PICKUP"))
...
static void IT_WorldModel(const char** hold_buf)
{
	...
	//	Q_strncpyz(bg_itemlist[itemParms.itemNum].world_model[0],token_str,len);
}
...
static void IT_Tag(const char** hold_buf)
{
	...
	else if (!Q_stricmp(token_str, "FP_INSANITY"))
	{
		tag = FP_INSANITY;
	}
	else if (!Q_stricmp(token_str, "FP_BLINDING"))
	{
		tag = FP_BLINDING;
	}
