...
constexpr auto MAX_LOB_DIST_SQUARED = 200704; //448*448;
#define REPEATER_ALT_SIZE				3
constexpr auto GENERATOR_HEALTH = 25;
constexpr auto TURN_ON = 0x00000000;
constexpr auto TURN_OFF = 0x00000100;
constexpr auto GALAK_SHIELD_HEALTH = 250;
...
void NPC_GM_Pain(gentity_t* self, gentity_t* inflictor, gentity_t* attacker, const vec3_t point, const int damage,
...
void NPC_BSGM_Attack()
{
		...
		&& !NPC->enemy->s.number)
	{
		...
		else if (NPC->wait)
		{
			...
			else if (DistanceHorizontalSquared(NPC->client->renderInfo.eyePoint, NPC->enemy->currentOrigin) > 4096 &&
	...
	else if (NPC->lockCount) //already shooting laser
	{
		...
		if (NPC->lockCount == 1)
		{
			...
			if (TIMER_Done(NPC, "beamDelay"))
			{
				...
				if (!NPCInfo->coverTarg)
				{
					...
					if (NPCInfo->coverTarg)
					{
						G_SetOrigin(NPCInfo->coverTarg, NPC->client->renderInfo.muzzle_point);
		...
		else
		{
			...
			else
			{
				...
				VectorMA(NPC->client->renderInfo.muzzle_point, 1024, NPC->client->renderInfo.muzzleDir, end);
				gi.trace(&trace, NPC->client->renderInfo.muzzle_point, mins, maxs, end, NPC->s.number, MASK_SHOT,
					G2_NOCOLLIDE, 0);
	...
	else
	{
		...
		{
			//enemy within 256
			if (NPC->client->ps.weapon == WP_REPEATER && NPCInfo->scriptFlags & SCF_altFire)
			{
		...
		else if ((enemyDist > MAX_LOB_DIST_SQUARED || NPC->enemy->s.weapon == WP_TURRET && !Q_stricmp(
			"PAS", NPC->enemy->classname))
	...
	if (NPC_ClearLOS(NPC->enemy))
	{
		...
		else
		{
			...
			if (NPC->client->ps.weapon == WP_REPEATER && NPCInfo->scriptFlags & SCF_altFire && enemyDist <
	...
	if (NPC->client->ps.weapon == WP_REPEATER && NPCInfo->scriptFlags & SCF_altFire && shoot && TIMER_Done(
		NPC, "attackDelay"))
	{
		vec3_t muzzle;
		vec3_t target;
		...
		target[0] += Q_flrand(-5, 5) + crandoms() * (6 - NPCInfo->currentAim) * 2;
		target[1] += Q_flrand(-5, 5) + crandoms() * (6 - NPCInfo->currentAim) * 2;
		target[2] += Q_flrand(-5, 5) + crandoms() * (6 - NPCInfo->currentAim) * 2;
