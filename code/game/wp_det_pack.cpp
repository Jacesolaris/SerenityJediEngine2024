...
//---------------------------------------------------------
{
	...
	missile->clipmask = CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_SHOTCLIP;
		...
		if (ent->client->ps.eFlags & EF_PLANTED_CHARGE)
		{
			...
			while ((found = G_Find(found, FOFS(classname), "detpack")) != nullptr)
			{
				if (found->activator == ent)
				{
					...
					AddSoundEvent(nullptr, found->currentOrigin, found->splashRadius * 2, AEL_DANGER, qfalse, qtrue);
					//FIXME: are we on ground or not?
