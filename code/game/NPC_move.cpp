...
constexpr auto NPC_JUMP_PREP_BACKUP_DIST = 34.0f;
...
void NPC_JumpAnimation()
{
	...
	if (NPC->client->NPC_class == CLASS_BOBAFETT || NPC->client->NPC_class == CLASS_MANDO
		|| (NPC->client->NPC_class == CLASS_REBORN || NPC->client->NPC_class == CLASS_SITHLORD || NPC->client->NPC_class
			== CLASS_CLONETROOPER && NPC->s.weapon != WP_SABER)
		|| NPC->client->NPC_class == CLASS_ROCKETTROOPER
		|| NPCInfo->rank != RANK_CREWMAN && NPCInfo->rank <= RANK_LT_JG)
	{
		//can't do acrobatics
		if (NPC->s.weapon == WP_SABER) //saber out
		{
			jumpAnim = BOTH_FORCEJUMP2;
		}
		else
		{
...
qboolean NPC_TryJump()
{
	...
	const float targetDistanceZ = NPCInfo->jumpDest[2] - NPC->currentOrigin[2];
	...
	const bool WithinForceJumpRange = fabsf(targetDistanceZ) > 0 || targetDistanceXY > 128;
	...
	{
		const float minSafeRadius = NPC->maxs[0] * 1.5f + NPCInfo->jumpTarget->maxs[0] * 1.5f;
...
inline qboolean NPC_GetMoveInformation(vec3_t dir, float* distance)
{
	...
	if (PM_InLedgeMove(NPC->client->ps.legsAnim))
	{
		ucmd.forwardmove = 64;
	}
...
#endif//	AI_TIMERS
	return static_cast<qboolean>(moveSuccess);
}
