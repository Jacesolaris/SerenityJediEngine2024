...
using material_t = enum //# material_e
{
	...
	MAT_GLASS,
	// not a real chunk type, just plays an effect with glass sprites
	MAT_ELECTRICAL,
	// sparks only
	MAT_ELEC_METAL,
	// sparks/electrical type metal
	MAT_DRK_STONE,
	// brown
	MAT_LT_STONE,
	// tan
	MAT_GLASS_METAL,
	// glass sprites and METAl chunk
	MAT_METAL2,
	// electrical metal type
	MAT_NONE,
	// no chunks
	MAT_GREY_STONE,
	// grey
	MAT_METAL3,
	// METAL and METAL2 chunks
	MAT_CRATE1,
	// yellow multi-colored crate chunks
	MAT_GRATE1,
	// grate chunks
	MAT_ROPE,
	// for yavin trial...no chunks, just wispy bits
	MAT_CRATE2,
	// read multi-colored crate chunks
	MAT_WHITE_METAL,
...
constexpr auto MAX_CUSTOM_BASIC_SOUNDS = 14;
constexpr auto MAX_CUSTOM_COMBAT_SOUNDS = 17;
constexpr auto MAX_CUSTOM_EXTRA_SOUNDS = 36;
constexpr auto MAX_CUSTOM_JEDI_SOUNDS = 23;
constexpr auto MAX_CUSTOM_CALLOUT_SOUNDS = 11;
#define	MAX_CUSTOM_SOUNDS	(MAX_CUSTOM_CALLOUT_SOUNDS + MAX_CUSTOM_JEDI_SOUNDS + MAX_CUSTOM_EXTRA_SOUNDS + MAX_CUSTOM_COMBAT_SOUNDS + MAX_CUSTOM_BASIC_SOUNDS)
...
public:
		...
		ojk::SavedGameHelper& saved_game) const
	{
		...
		saved_game.write<int32_t>(customCalloutSoundDir);
	}
...
//renderFlags
constexpr auto RF_LOCKEDANGLE = 1;
...
using playerTeamStateState_t = enum
{
	TEAM_BEGIN,
...
constexpr auto MAX_MISSION_OBJ = 400;
...
constexpr auto FOLLOW_ACTIVE1 = -1;
constexpr auto FOLLOW_ACTIVE2 = -2;
...
public:
	...
	int BarrierToggleTime;
	int BarrierDebRecharge;
	int BarrierDebReduce;
	...
	qboolean hookhasbeenfired; // grapple hook if out
		...
		ojk::SavedGameHelper& saved_game) const
	{
		...
		saved_game.write<int32_t>(VaderBreathTime);
		...
		saved_game.write<int32_t>(BarrierToggleTime);
		saved_game.write<int32_t>(BarrierDebRecharge);
		saved_game.write<int32_t>(BarrierDebReduce);
		saved_game.write<int32_t>(stunDamage);
		saved_game.write<int32_t>(stunTime);
		saved_game.write<int32_t>(AmputateDamage);
		saved_game.write<int32_t>(AmputateTime);
		saved_game.write<int32_t>(fireHeld);
		saved_game.write<int32_t>(stunHeld);
		saved_game.write<int32_t>(hook);
		saved_game.write<int32_t>(stun);
		saved_game.write<int32_t>(hookhasbeenfired);
		saved_game.write<int32_t>(hookDebounceTime);
		saved_game.write<int32_t>(stunhasbeenfired);
		saved_game.write<int32_t>(stunDebounceTime);
		...
		saved_game.write<int32_t>(StaggerAnimTime);
		saved_game.write<int32_t>(MassiveBounceAnimTime);
		...
		ojk::SavedGameHelper& saved_game)
	{
		...
		saved_game.read<int32_t>(VaderBreathTime);
		...
		saved_game.read<int32_t>(BarrierToggleTime);
		saved_game.read<int32_t>(BarrierDebRecharge);
		saved_game.read<int32_t>(BarrierDebReduce);
		saved_game.read<int32_t>(stunDamage);
		saved_game.read<int32_t>(stunTime);
		saved_game.read<int32_t>(AmputateDamage);
		saved_game.read<int32_t>(AmputateTime);
		saved_game.read<int32_t>(fireHeld);
		saved_game.read<int32_t>(stunHeld);
		saved_game.read<int32_t>(hook);
		saved_game.read<int32_t>(stun);
		saved_game.read<int32_t>(hookhasbeenfired);
		saved_game.read<int32_t>(hookDebounceTime);
		saved_game.read<int32_t>(stunhasbeenfired);
		saved_game.read<int32_t>(stunDebounceTime);
		...
		saved_game.read<int32_t>(StaggerAnimTime);
		saved_game.read<int32_t>(MassiveBounceAnimTime);
...
constexpr auto MAX_INHAND_WEAPONS = 2;
constexpr auto MAX_HOLSTER_WEAPONS = 2;

using NPCBackupInfo_t = struct NPCBackupInfo_s
{
...
using centity_t = struct centity_s;
...
struct gentity_s
{
	...
	*/
	...
	union
	{
		...
		qboolean has_bounced;
		// for thermal Det.  we force at least one bounce to happen before it can do proximity checks
	};
...
using weaponInfo_t = struct weaponInfo_s
{
	...
	void (*alt_missileTrailFunc)(centity_t*, const weaponInfo_s* wi);
