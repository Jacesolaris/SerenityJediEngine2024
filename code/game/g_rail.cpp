...
// Constants
////////////////////////////////////////////////////////////////////////////////////////
constexpr auto MAX_TRACKS = 4;
constexpr auto MAX_LANES = 8;
constexpr auto MAX_MOVERS = 150;
constexpr auto MAX_MOVER_ENTS = 10;
constexpr auto MAX_MOVERS_TRACK = 80;
constexpr auto MAX_COLS = 32;
constexpr auto MAX_ROWS = 96;
constexpr auto MAX_ROW_HISTORY = 10;

constexpr auto WOOSH_DEBUG = 0;
constexpr auto WOOSH_ALL_RANGE = 1500.0f;
constexpr auto WOOSH_SUPPORT_RANGE = 2500.0f;
constexpr auto WOOSH_TUNNEL_RANGE = 3000.0f;
...
public:
	void Setup(const gentity_t* ent)
	{
		...
		AngleVectors(ent->s.angles, mDirection.v, nullptr, nullptr);
		mDirection.SafeNorm();
		mVelocity = mDirection;
		mVelocity *= mSpeedGridCellsPerSecond * mGridCellSize;

		mNextUpdateDelay = 1000.0f / static_cast<float>(mSpeedGridCellsPerSecond);
	...
	void SnapVectorToGrid(CVec3& Vec) const
	{
		SnapFloatToGrid(Vec[0]);
		SnapFloatToGrid(Vec[1]);
	}

	void SnapFloatToGrid(float& f) const
	{
		f = static_cast<int>(f);
		...
		int Offset = static_cast<int>(f) % static_cast<int>(mGridCellSize);
		...
		assert(static_cast<int>(f) % static_cast<int>(mGridCellSize) == 0);
	}
...
public:
	...
	void Initialize()
	{
		...
		const hstring target = mEnt->target;
		...
		if (mTrack)
		{
			...
			mCols = static_cast<int>((mEnt->maxs[mTrack->mWAxis] - mEnt->mins[mTrack->mWAxis]) / mTrack->mGridCellSize)
				+ 1;
			mRows = static_cast<int>((mEnt->maxs[mTrack->mHAxis] - mEnt->mins[mTrack->mHAxis]) / mTrack->mGridCellSize)
				+ 1;
		...
		if (player && gi.WE_IsOutside(player->currentOrigin))
		{
			...
			for (int moverIndex = 0; moverIndex < mRailMovers.size(); moverIndex++)
			{
				...
				if (mover.Active() && !mover.mSoundPlayed)
				{
					...
					float playerToMoverDistance = VectorNormalize(playerToMover);
						...
						)
					{
						...
						else
						{
							...
							//--------------------------------------------------
							float playerToMoverDistanceFraction = playerToMoverDistance / WOOSH_ALL_RANGE;
...
void CRailTrack::Update()
{
	...
	for (int attempt = 0; attempt < mNumMoversPerRow; attempt++)
	{
		...
		while (!mTestCols.empty())
		{
			...
			//--------------------------------------------------------
			const int testColIndex = Q_irand(0, mTestCols.size() - 1);
			const int atCol = mTestCols[testColIndex];
			...
			if (TestMoverInCells(mover, atCol))
			{
				...
				if (mover->mCenter)
				{
					...
					const float deltaOffset = mGridCenter[mWAxis] - mover->mOriginOffset[mWAxis];
	...
	{
		mCells.get(col, EraseRow) = nullptr;
	}
}
...
void CRailTrack::RandomizeTestCols(const int startCol, const int stopCol)
{
	...
	{
		const int swapA = Q_irand(0, numCols - 1);
		const int swapB = Q_irand(0, numCols - 1);
		if (swapA != swapB)
		{
...
bool CRailTrack::TestMoverInCells(const CRailMover* mover, const int atCol)
{
	...
	{
		if (mCells.get(atCol + moverCol, mRow/*+moverRow*/) != nullptr)
		{
			return false;
		}
	}
