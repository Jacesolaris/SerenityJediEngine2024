...
namePrecache_m* as_preCacheMap = nullptr;
...
using fieldtype_t = enum
{
	...
	F_GSTRING,
	// string on disk, pointer in memory, TAG_GAME
	F_VECTOR,
	F_VECTOR4,
	F_ANGLEHACK,
	F_ENTITY,
	// index on disk, pointer in memory
	F_ITEM,
	// index on disk, pointer in memory
	F_CLIENT,
	// index on disk, pointer in memory
...
char* G_NewString(const char* string)
{
	...
	const auto newb = static_cast<char*>(G_Alloc(l));
...
static qboolean SpawnForCurrentDifficultySetting(const gentity_t* ent)
{
	...
	if (ent->spawnflags & (1 << (8 + g_spskill->integer)))
	{
...
char* G_AddSpawnVarToken(const char* string)
{
	...
	char* dest = spawnVarChars + numSpawnVarChars;
...
void SP_worldspawn()
{
	...
	for (i = 0; i < numSpawnVars; i++)
	{
		...
		{
			g_entities[ENTITYNUM_WORLD].max_health = static_cast<int>(static_cast<float>(atoi(spawnVars[i][1])) * 0.7f);
		}
	}
	...
	for (i = 1; i < LS_NUM_STYLES; i++)
	{
		...
		const int lengthGreen = strlen(s);
...
void G_ParsePrecaches()
{
	...
	for (int i = 0; i < globals.num_entities; i++)
	{
		...
		{
			(*as_preCacheMap)[static_cast<char*>(ent->soundSet)] = 1;
		}
	}
}
