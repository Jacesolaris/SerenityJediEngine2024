...
constexpr auto SENTRY_FORWARD_BASE_SPEED = 10;
constexpr auto SENTRY_FORWARD_MULTIPLIER = 5;

constexpr auto SENTRY_VELOCITY_DECAY = 0.85f;
constexpr auto SENTRY_STRAFE_VEL = 256;
constexpr auto SENTRY_STRAFE_DIS = 200;
constexpr auto SENTRY_UPWARD_PUSH = 32;
constexpr auto SENTRY_HOVER_HEIGHT = 24;
...
void Sentry_Fire()
{
	...
	const int which = NPCInfo->burstCount % 3;
	switch (which)
	{
	case 0:
	...
	G_PlayEffect("bryar_old/muzzle_flash", muzzle, forward);
...
void Sentry_Hunt(const qboolean visible, const qboolean advance)
{
	...
	const float speed = SENTRY_FORWARD_BASE_SPEED + SENTRY_FORWARD_MULTIPLIER * g_spskill->integer;
