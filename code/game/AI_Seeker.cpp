...
constexpr auto SEEKER_STRAFE_VEL = 100;
constexpr auto SEEKER_STRAFE_DIS = 200;
constexpr auto SEEKER_UPWARD_PUSH = 32;

constexpr auto SEEKER_FORWARD_BASE_SPEED = 10;
constexpr auto SEEKER_FORWARD_MULTIPLIER = 2;
	...
	const int mod, int hit_loc)
{
	...
	{
		G_Damage(self, nullptr, nullptr, vec3_origin, vec3_origin, 999, 0, MOD_FALLING);
	}
...
void Seeker_Hunt(const qboolean visible, const qboolean advance)
{
	...
	const float speed = SEEKER_FORWARD_BASE_SPEED + SEEKER_FORWARD_MULTIPLIER * g_spskill->integer;
...
void Seeker_Fire()
{
	...
	missile->methodOfDeath = MOD_SEEKER; //MOD_ENERGY
	missile->clipmask = MASK_SHOT;
}

//------------------------------------
void Seeker_Ranged(const qboolean visible, const qboolean advance)
{
	if (NPC->client->NPC_class != CLASS_BOBAFETT && NPC->client->NPC_class != CLASS_MANDO)
	{
		if (NPC->count > 0)
