...
void G_PlayDoorLoopSound(gentity_t* ent)
{
	...
	const sfxHandle_t sfx = CAS_GetBModelSound(ent->soundSet, BMS_MID);
...
void G_PlayDoorSound(gentity_t* ent, const int type)
{
	...
	const sfxHandle_t sfx = CAS_GetBModelSound(ent->soundSet, type);
...
============
*/
gentity_t* G_TestEntityPosition(const gentity_t* ent)
{
	trace_t tr;
	int mask;
...
===============
*/
void SetMoverState(gentity_t* ent, const moverState_t mover_state, const int time)
{
	vec3_t delta;
	float f;

	ent->moverState = mover_state;
...
================
*/
void MatchTeam(gentity_t* team_leader, int mover_state, const int time)
{
	for (gentity_t* slave = team_leader; slave; slave = slave->teamchain)
	{
		SetMoverState(slave, static_cast<moverState_t>(mover_state), time);
	}
}
...
void Use_BinaryMover_Go(gentity_t* ent)
{
	...
	if (ent->moverState == MOVER_2TO1)
	{
		...
		{
			vec3_t cur_delta;
			VectorSubtract(ent->currentOrigin, ent->pos1, cur_delta);
			float fPartial = VectorLength(cur_delta) / VectorLength(ent->s.pos.trDelta);
			VectorScale(ent->s.pos.trDelta, fPartial, cur_delta);
...
void Use_BinaryMover(gentity_t* ent, gentity_t* other, gentity_t* activator)
{
	...
	if (ent->spawnflags & MOVER_GOODIE)
	{
		...
		const int key = INV_GoodieKeyCheck(activator);
		if (key)
		{
		...
		else
		{
			...
			const auto text = "cp @SP_INGAME_NEED_KEY_TO_OPEN";
...
void InitMoverTrData(gentity_t* ent)
{
	...
	const float distance = VectorLength(move);
	if (!ent->speed)
	{
		ent->speed = 100;
	}
...
void SP_func_door(gentity_t* ent)
{
	...
	const float distance = DotProduct(abs_movedir, size) - lip;
	...
	if (!(ent->flags & FL_TEAMSLAVE))
	{
		...
		if (!(ent->spawnflags & MOVER_LOCKED) && (ent->targetname || health || ent->spawnflags & MOVER_PLAYER_USE || ent
			->spawnflags & MOVER_FORCE_ACTIVATE))
		{
...
void SP_func_button(gentity_t* ent)
{
	...
	abs_movedir[2] = fabs(ent->movedir[2]);
	VectorSubtract(ent->maxs, ent->mins, size);
	const float distance = abs_movedir[0] * size[0] + abs_movedir[1] * size[1] + abs_movedir[2] * size[2] - lip;
	VectorMA(ent->pos1, distance, ent->movedir, ent->pos2);
...
constexpr auto TRAIN_START_ON = 1;
constexpr auto TRAIN_TOGGLE = 2;
constexpr auto TRAIN_BLOCK_STOPS = 4;
...
void Reached_Train(gentity_t* ent)
{
	...
	// calculate duration
	VectorSubtract(ent->pos2, ent->pos1, move);
	const float length = VectorLength(move);
	...
	*/
	...
	if (next->spawnflags & 1)
	{
		...
		for (const float ang : angs)
		{
			AngleNormalize360(ang);
		}
...
void func_bobbing_use(gentity_t* self, gentity_t* other, gentity_t* activator)
{
	...
	if (self->s.pos.trType == TR_SINE)
	{
		...
		self->radius = (level.time - self->s.pos.trTime) / static_cast<float>(self->s.pos.trDuration);
	}
	else
	{
...
void SP_func_pendulum(gentity_t* ent)
{
	...
	float length = fabs(ent->mins[2]);
	if (length < 8)
	{
		length = 8;
	}

	const float freq = 1 / (M_PI * 2) * sqrt(g_gravity->value / (3 * length));
...
constexpr auto FUNC_WALL_OFF = 1;
constexpr auto FUNC_WALL_ANIM = 2;
