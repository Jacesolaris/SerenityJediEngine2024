...
void target_laser_think(gentity_t* ent)
{
	...
	{
		vec3_t point;
		VectorMA(ent->enemy->s.origin, 0.5, ent->enemy->mins, point);
		VectorMA(point, 0.5, ent->enemy->maxs, point);
	...
	gi.trace(&tr, ent->s.origin, nullptr, nullptr, end, ent->s.number,
	...
	if (tr.entityNum)
	{
		...
		G_Damage(&g_entities[tr.entityNum], ent, ent->activator, ent->movedir,
			tr.endpos, ent->damage, DAMAGE_NO_KNOCKBACK, MOD_ENERGY);
	}
...
void scriptrunner_run(gentity_t* ent)
{
	if (ent->count != -1)
	{
		...
		{
			ent->e_UseFunc = useF_NULL;
			ent->behaviorSet[BSET_USE] = nullptr;
			return;
		}
		--ent->count;
	}
	...
	if (ent->behaviorSet[BSET_USE])
	{
		if (ent->spawnflags & 1)
		{
			...
			if (ent->activator->m_iIcarusID == IIcarusInterface::ICARUS_INVALID)
			{
				...
				if (Quake3Game()->ValidEntity(ent->activator))
				{
					Quake3Game()->InitEntity(ent->activator);
				}
				else
				{
					Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
			...
			Quake3Game()->DebugPrint(IGameInterface::WL_VERBOSE, "target_scriptrunner running %s on activator %s\n",
				ent->behaviorSet[BSET_USE], ent->activator->targetname);

			Quake3Game()->RunScript(ent->activator, ent->behaviorSet[BSET_USE]);
		}
		else
		{
			if (ent->activator)
...
void target_gravity_change_use(gentity_t* self, gentity_t* other, gentity_t* activator)
{
	...
	{
		const int grav = floor(self->speed);
		/*
...
void target_level_change_use(gentity_t* self, gentity_t* other, gentity_t* activator)
{
	...
	{
		G_ChangeMap(self->message, self->target, static_cast<qboolean>((self->spawnflags & 1) != 0));
	}
	if (self->count >= 0)
	{
