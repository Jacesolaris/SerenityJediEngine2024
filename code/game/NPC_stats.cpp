...
int G_ParseAnimFileSet(const char* skeleton_name, const char* model_name = nullptr)
{
	...
	//------------------------------------------------------------------------------
	if (file_index >= level.numKnownAnimFileSets)
	{
		...
		animation_t* animations = level.knownAnimFileSets[file_index].animations;
		animevent_t* legs_anim_events = level.knownAnimFileSets[file_index].legsAnimEvents;
		animevent_t* torso_anim_events = level.knownAnimFileSets[file_index].torsoAnimEvents;
...
qboolean NPC_ParseParms(const char* npc_name, gentity_t* npc)
{
	...
	if (npc->NPC)
	{
		...
		if (npc->client->NPC_class == CLASS_KYLE || (is_outcast_map() || com_outcast->integer == 1))
		{
...
void NPC_LoadParms1() //jko version
{
	...
	//now load in the .npc definitions
	const int file_cnt = gi.FS_GetFileList("ext_data/jkonpcs", ".npc", npc_extension_list_buf,
		sizeof npc_extension_list_buf);
...
void NPC_LoadParms2() //mod version
{
	...
	//now load in the .npc definitions
	const int file_cnt = gi.FS_GetFileList("ext_data/crnpcs", ".npc", npc_extension_list_buf,
		sizeof npc_extension_list_buf);
...
void NPC_LoadParms3() //yav version
{
	...
	//now load in the .npc definitions
	const int file_cnt = gi.FS_GetFileList("ext_data/yavnpcs", ".npc", npc_extension_list_buf,
		sizeof npc_extension_list_buf);
...
void NPC_LoadParms4() //eoc version
{
	...
	//now load in the .npc definitions
	const int file_cnt = gi.FS_GetFileList("ext_data/kotornpcs", ".npc", npc_extension_list_buf,
		sizeof npc_extension_list_buf);
	...
	for (int i = 0; i < file_cnt; i++, hold_char += npc_ext_fn_len + 1)
	{
		...
		int len = gi.FS_ReadFile(va("ext_data/kotornpcs/%s", hold_char), reinterpret_cast<void**>(&buffer));
