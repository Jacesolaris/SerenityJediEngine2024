...
extern void Q3_SetParm(int ent_id, int parmNum, const char* parmValue);
...
static void NPC_SetMiscDefaultData(gentity_t* ent)
{
	...
	if (!Q_stricmp("ShadowTrooper", ent->NPC_type) ||
		!Q_stricmp("ShadowTrooper2", ent->NPC_type) ||
		!Q_stricmp("ShadowTrooperclass", ent->NPC_type))
	{
		ent->flags |= FL_SHIELDED;
	}
...
int NPC_WeaponsForTeam(const team_t team, const int spawnflags, const char* NPC_type)
{
	...
	case TEAM_ENEMY:
		...
		{
			return 1 << WP_TUSKEN_RIFLE;
		}
		if (Q_stricmpn("stofficer2", NPC_type, 9) == 0)
		{
		...
		if (Q_stricmp("stormpilot2", NPC_type) == 0)
		{
			return 1 << WP_BLASTER_PISTOL;
		}
...
void SP_NPC_Human_Merc(gentity_t* self)
{
	if (!self->NPC_type)
	{
		...
		else if (self->spawnflags & 1)
		{
			...
			{
				const int npc_pick = Q_irand(0, 6);
				switch (npc_pick)
				{
				case 0:
				case 1:
		...
		else
		{
			...
			if (!g_newgameplusJKA->integer)
			{
				...
				case 4:
					...
					break;
				case 5:
					self->NPC_type = "Battle_Trooper";
					break;
				case 6:
					self->NPC_type = "SBD_Trooper";
					break;
				case 7:
				default: //just in case
					self->NPC_type = "StormTrooper";
...
void Svcmd_NPC_f()
{
	...
	if (!*cmd)
	{
		...
		gi.Printf(
			" spawn vehicle [NPC type (from *.npc files, only for NPCs that are CLASS_VEHICLE and have a .veh file)]\n");
