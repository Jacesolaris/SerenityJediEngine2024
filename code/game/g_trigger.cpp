...
constexpr auto ENTDIST_PLAYER = 1;
constexpr auto ENTDIST_NPC = 2;
...
void Touch_Multi(gentity_t* self, gentity_t* other, trace_t* trace)
{
	...
	if (self->spawnflags & 8)
	{
		...
		if (other->s.number == 0 && (other->client->ps.weapon > MAX_PLAYER_WEAPONS || other->client->ps.weapon <=
			WP_NONE || !playerUsableWeapons[other->client->ps.weapon]))
		{
...
void SP_trigger_multiple(gentity_t* ent)
{
	...
	//was 0.5 ... but that means wait can never be zero... we should probably put it back to 0.5, though...
...
==============================================================================
*/
constexpr auto PUSH_CONVEYOR = 32;
...
void AimAtTarget(gentity_t* self)
{
	...
	const gentity_t* ent = G_PickTarget(self->target);
	if (!ent)
	{
		G_FreeEntity(self);
		return;
	...
	float height = ent->s.origin[2] - origin[2];
	if (height < 0)
	{
	...
	const float time = sqrt(height / (.5 * gravity));
	if (!time)
	{
		G_FreeEntity(self);
		return;
	...
	self->s.origin2[2] = 0;
	const float dist = VectorNormalize(self->s.origin2);

	const float forward = dist / time;
...
==============================================================================
*/
constexpr auto SNAP_ANGLES = 1;
constexpr auto NO_MISSILES = 2;
...
void trigger_teleporter_touch(const gentity_t* self, gentity_t* other, trace_t* trace)
{
	...
	else if (!(self->svFlags & SVF_NO_TELEPORT) && !(self->spawnflags & NO_MISSILES) && VectorLengthSquared(
		other->s.pos.trDelta))
	{
	...
	if (actual_dmg)
	{
		...
		if (self->spawnflags & 32)
		{
			...
			if (other->NPC && other->client &&
				(other->client->NPC_class == CLASS_BOBAFETT || other->client->NPC_class == CLASS_MANDO || other->client
...
void space_touch(const gentity_t* self, const gentity_t* other, trace_t* trace)
{
	...
	if (other->s.m_iVehicleNum)
	{
		...
		const gentity_t* veh = &g_entities[other->s.m_iVehicleNum];
...
void shipboundary_touch(gentity_t* self, gentity_t* other, trace_t* trace)
{
	...
	gentity_t* ent = G_Find(nullptr, FOFS(targetname), self->target);
	if (!ent || !ent->inuse)
	{
		//this is bad
		...
		if (other->client->ps.eFlags2 & EF2_HYPERSPACE)
		{
			...
			if (time_frac >= HYPERSPACE_TELEPORT_FRAC)
			{
				...
				if (other->m_pVehicle && other->m_pVehicle->m_pPilot)
				{
					...
					TeleportPlayer(other->m_pVehicle->m_pPilot, newOrg, ent->s.angles);
...
extern void Q3_Lerp2Origin(int taskID, int ent_id, vec3_t origin, float duration);
...
void asteroid_move_to_start2(gentity_t* self, const gentity_t* owner_trigger)
{
	...
	if (owner_trigger)
	{
		...
		self->e_ThinkFunc = thinkF_asteroid_move_to_start;
...
{
	//move asteroid to a new start position
	asteroid_move_to_start2(self, &g_entities[self->ownerNum]);
}
...
void SP_func_timer(gentity_t* self)
{
	...
	{
		self->random = self->wait - 1;
		//NOTE: was - FRAMETIME, but FRAMETIME is in msec (100) and these numbers are in *seconds*!
...
// spawnflag
constexpr auto TRIGGERVISIBLE_FORCESIGHT = 2;
