...
constexpr auto MAX_VEH_WEAPON_DATA_SIZE = 0x400000;
constexpr auto MAX_VEHICLE_DATA_SIZE = 0x1000000;
...
using vehFieldType_t = enum
{
	...
	VF_FLOAT,
	VF_STRING,
	// string on disk, pointer in memory
	VF_VECTOR,
	VF_BOOL,
	VF_VEHTYPE,
	VF_ANIM,
...
{
	for (size_t i = 0; i < numVehWeaponFields; i++)
	{
...
static qboolean BG_ParseVehWeaponParm(vehWeaponInfo_t* vehWeapon, const char* parmName, const char* pValue)
{
	...
	vehField_t* vehWeaponField = FindVehWeaponParm(parmName);
	...
	{
	case VF_INT:
		*reinterpret_cast<int*>(b + vehWeaponField->ofs) = atoi(value);
		break;
	case VF_FLOAT:
		*reinterpret_cast<float*>(b + vehWeaponField->ofs) = atof(value);
		break;
	case VF_STRING: // string on disk, pointer in memory
		if (!*reinterpret_cast<char**>(b + vehWeaponField->ofs))
		{
	...
	case VF_VECTOR:
		...
		{
			Com_Printf(
				S_COLOR_YELLOW"BG_ParseVehWeaponParm: VEC3 sscanf() failed to read 3 floats ('angle' key bug?)\n");
			VectorClear(vec);
		}
		reinterpret_cast<float*>(b + vehWeaponField->ofs)[0] = vec[0];
		reinterpret_cast<float*>(b + vehWeaponField->ofs)[1] = vec[1];
		reinterpret_cast<float*>(b + vehWeaponField->ofs)[2] = vec[2];
		break;
	case VF_BOOL:
		*reinterpret_cast<qboolean*>(b + vehWeaponField->ofs) = static_cast<qboolean>(atof(value) != 0);
		break;
	case VF_VEHTYPE:
		vehType = static_cast<vehicleType_t>(GetIDForString(VehicleTable, value));
...
vehField_t vehicleFields[] =
{
	...
	{"numHands", VFOFS(numHands), VF_INT},
	...
	{"centerOfGravity", VFOFS(centerOfGravity), VF_VECTOR},
	...
	{"throttleSticks", VFOFS(throttleSticks), VF_BOOL},
	//if true, speed stays at whatever you accel/decel to, unless you turbo or brake
	{"strafePerc", VFOFS(strafePerc), VF_FLOAT},
	...
	{"turboStartFX", VFOFS(iTurboStartFX), VF_EFFECT},
	...
	{"explodeFX", VFOFS(iExplodeFX), VF_EFFECT},
	...
	{"weap1SoundNoAmmo", VFOFS(weapon[0].soundNoAmmo), VF_SOUND_CLIENT},
	//sound to play when try to fire weapon 1 with no ammo
	{"weap2SoundNoAmmo", VFOFS(weapon[1].soundNoAmmo), VF_SOUND_CLIENT},
	...
	{"hoverStrength", VFOFS(hoverStrength), VF_FLOAT},
	...
	//new stuff
	{"maxPassengers", VFOFS(maxPassengers), VF_INT},
	...
	//camera stuff
	{"cameraOverride", VFOFS(cameraOverride), VF_BOOL},
	...
	{"cameraAlpha", VFOFS(cameraAlpha), VF_FLOAT},
	//fade out the vehicle to this alpha (0.1-1.0f) if it's in the way of the crosshair
	{"cameraPitchDependantVertOffset", VFOFS(cameraPitchDependantVertOffset), VF_BOOL},
...
static qboolean BG_ParseVehicleParm(vehicleInfo_t* vehicle, const char* parmName, const char* pValue)
{
	...
	// Loop through possible parameters
	vehField_t* vehField = FindVehicleParm(parmName);
	...
	case VF_IGNORE:
		break;
	case VF_INT:
		*reinterpret_cast<int*>(b + vehField->ofs) = atoi(value);
		break;
	case VF_FLOAT:
		*reinterpret_cast<float*>(b + vehField->ofs) = atof(value);
		break;
	case VF_STRING: // string on disk, pointer in memory
		if (!*reinterpret_cast<char**>(b + vehField->ofs))
		{
	...
	case VF_VECTOR:
		...
		reinterpret_cast<float*>(b + vehField->ofs)[0] = vec[0];
		reinterpret_cast<float*>(b + vehField->ofs)[1] = vec[1];
		reinterpret_cast<float*>(b + vehField->ofs)[2] = vec[2];
		break;
	case VF_BOOL:
		*reinterpret_cast<qboolean*>(b + vehField->ofs) = static_cast<qboolean>(atof(value) != 0);
		break;
	case VF_VEHTYPE:
		vehType = static_cast<vehicleType_t>(GetIDForString(VehicleTable, value));
	...
	break;
	case VF_WEAPON: // take string, resolve into index into VehWeaponParms
		*reinterpret_cast<int*>(b + vehField->ofs) = VEH_Vehweapon_indexForName(value);
		break;
	case VF_MODEL: // take the string, get the G_ModelIndex
#ifdef QAGAME
...
int veh_load_vehicle(const char* vehicle_name)
{
	...
	while (p)
	{
		...
		if (!Q_stricmp(token, vehicle_name))
		{
			break;
		}
	...
	while (true)
	{
		...
		Q_strncpyz(parm_name, token, sizeof parm_name);
		value = COM_ParseExt(&p, qtrue);
		if (!value || !value[0])
		{
			Com_Printf(S_COLOR_RED"ERROR: Vehicle token '%s' has no value!\n", parm_name);
		}
		else if (Q_stricmp("weap1", parm_name) == 0)
		{
...
int VEH_VehicleIndexForName(const char* vehicleName)
{
	...
	v = veh_load_vehicle(vehicleName);
	if (v == VEHICLE_NONE)
	{
...
void BG_VehWeaponLoadParms()
{
	...
	int totallen = mainBlockLen = len;
	char* marker = VehWeaponParms + totallen;
	*marker = 0;
...
#else
	tempReadBuffer = static_cast<char*>(gi.Malloc(MAX_VEH_WEAPON_DATA_SIZE, TAG_G_ALLOC, qtrue));
#endif
	...
	for (int i = 0; i < fileCnt; i++, holdChar += vehExtFNLen + 1)
	{
...
void BG_VehicleLoadParms()
{
	...
	//remember where to store the next one
	int totallen = mainBlockLen = len;
	char* marker = VehicleParms + totallen;
	*marker = 0;
...
#else
	tempReadBuffer = static_cast<char*>(gi.Malloc(MAX_VEHICLE_DATA_SIZE, TAG_G_ALLOC, qtrue));
#endif
...
extern qboolean trap_G2API_GetBoltMatrix(void* ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t* matrix,
	const vec3_t angles, const vec3_t position, const int frame_num, qhandle_t* modelList, vec3_t scale);
