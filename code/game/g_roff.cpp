			...
			else if (r2 && strstr(teststr, "sound"))
			{
				...
				if (ent->s.eType == ET_MOVER)
				{
					...
					else
					{
						ent->s.loopSound = 0;
						sprintf(errMsg,
							"cgi_S_RegisterSound(%s) failed to return a valid sfxHandle_t for additional argument. Setting 'loopSound' to 0.",
							addlArg);
						goto functionend;
					}
				}
...
static qboolean G_ValidRoff(roff_hdr2_t* header)
{
	if (strncmp(header->mHeader, "ROFF", 4) == 0)
	{
		...
		if (LittleLong header->mVersion == ROFF_VERSION && LittleFloat reinterpret_cast<roff_hdr_t*>(header)->mCount > 0.0f)
		{
...
{
	const auto header = reinterpret_cast<roff_hdr_t*>(data);
	int count;
	int i;
	...
	{
		count = static_cast<int>(header->mCount);

		// We are Old School(tm)
		roffs[num_roffs].type = 1;

		roffs[num_roffs].data = G_Alloc(count * sizeof(move_rotate_t));
		auto mem = static_cast<move_rotate_t*>(roffs[num_roffs].data);
		...
		if (mem)
		{
			...
			auto roff_data = reinterpret_cast<move_rotate_t*>(&header[1]);
	...
	else if (LittleLong header->mVersion == ROFF_VERSION2)
	{
		...
		const auto hdr = reinterpret_cast<roff_hdr2_t*>(data);
		count = LittleLong hdr->mCount;

		roffs[num_roffs].frames = count;
		roffs[num_roffs].data = G_Alloc(count * sizeof(move_rotate2_t));
		auto mem = static_cast<move_rotate2_t*>(roffs[num_roffs].data);
		...
		if (mem)
		{
			...
			// Step past the header to get to the goods
			auto roff_data = reinterpret_cast<move_rotate2_t*>(&hdr[1]);
...
int G_LoadRoff(const char* fileName)
{
	...
	len = gi.FS_ReadFile(file, reinterpret_cast<void**>(&data));
	...
	const auto header = reinterpret_cast<roff_hdr2_t*>(data);
...
void G_Roff(gentity_t* ent)
{
	...
	if (roff->type == 2)
	{
		const move_rotate2_t* data = &static_cast<move_rotate2_t*>(roff->data)[ent->roff_ctr];
	...
	{
		const move_rotate_t* data = &static_cast<move_rotate_t*>(roff->data)[ent->roff_ctr];
		VectorCopy(data->origin_delta, org);
		VectorCopy(data->rotate_delta, ang);
	}
