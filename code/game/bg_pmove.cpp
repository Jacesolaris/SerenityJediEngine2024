...
{
	const int other_entityNum = trace->entityNum;
	...
	const gentity_t* trace_ent = &g_entities[other_entityNum];

	if (other_entityNum == ENTITYNUM_WORLD || trace_ent->bmodel && trace_ent->s.pos.trType == TR_STATIONARY)
	{
	...
	{
		DoImpact(pm->gent, &g_entities[other_entityNum], damage_self, trace);
	}

	if (other_entityNum >= ENTITYNUM_WORLD)
	{
		return qfalse;
	}
			...
			if (anim != -1 && PM_HasAnimation(pm->gent, anim))
			{
					...
					.solid != SOLID_BMODEL || DotProduct(wallNormal, ideal_normal) > 0.7))
				{
						...
						|| wallNormal[2] >= 0.0f && wallNormal[2] <= MAX_WALL_RUN_Z_NORMAL)
					{
						...
						{
							PM_Setsaber_move(LS_BUTTERFLY_RIGHT);
						}
						else if (anim == BOTH_BUTTERFLY_LEFT)
						{
							PM_Setsaber_move(LS_BUTTERFLY_LEFT);
						}
						else
						{
			...
			PM_ControlledByPlayer()) && BG_AllowThirdPersonSpecialMove(pm->ps) && !cg.zoomMode))
	{
			...
			&& !pm_saber_in_special_attack(pm->ps->torsoAnim))
		{
				...
				&& pm->cmd.buttons & BUTTON_ATTACK) //want to attack
			{
				...
				{
					PM_Setsaber_move(PM_SaberBackflipAttackMove()); //backflip attack
				}
			}
		}
	}
...
void PM_HoverTrace()
{
	...
	else
	{
		...
		pm->trace(trace, pm->ps->origin, pm->mins, pm->maxs, point, pm->ps->client_num, trace_contents,
			static_cast<EG2_Collision>(0), 0);
		if (trace->plane.normal[2] >= min_normal)
		{
	...
	else
	{
		...
		if (p_veh->m_vAngularVelocity == 0.0f)
		{
				...
				level.time > LastMatrixJumpTime + 5000 && VectorLength(pm->ps->velocity) > 30.0f)
			{
				...
				trace_t trace2;
				gi.trace(&trace2, predicted_apx, pm->mins, pm->maxs, predicted_land_position, pm->ps->client_num,
...
qboolean PM_GettingUpFromKnockDown(const float standheight, const float crouchheight)
{
	...
	{
		//in a knockdown
		const int min_time_left = pm_min_get_up_time(pm->gent);
...
void PM_FootSlopeTrace(float* p_diff, float* p_interval)
{
	...
	pm->trace(&trace, foot_l_org, foot_mins, foot_maxs, foot_l_bot, pm->ps->client_num, pm->tracemask,
		static_cast<EG2_Collision>(0), 0);
	VectorCopy(trace.endpos, foot_l_bot);
	VectorCopy(trace.plane.normal, foot_l_slope);

	pm->trace(&trace, foot_r_org, foot_mins, foot_maxs, foot_r_bot, pm->ps->client_num, pm->tracemask,
		static_cast<EG2_Collision>(0), 0);
...
static void PM_Footsteps()
{
	...
	if (pm->ps->forcePowersActive & 1 << FP_DRAIN && pm->ps->forceDrainentityNum < ENTITYNUM_WORLD)
	{
		//draining
		return;
	}
				...
				&& (!pm->ps->dualSabers || !pm->ps->saber[1].Active()))
			{
				if (!PM_AdjustStandAnimForSlope())
				{
					if (pm->ps->legsAnim != BOTH_LOSE_SABER && !PM_SaberInMassiveBounce(pm->ps->torsoAnim) || !pm->
						ps->legsAnimTimer)
					{
						switch (pm->gent->client->ps.saber_anim_level)
						{
						case SS_FAST:
						case SS_TAVION:
						case SS_MEDIUM:
			...
			else
			{
				...
				if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive())
				{
					if (pm->ps->client_num < MAX_CLIENTS || PM_ControlledByPlayer())
					{
						...
						else if (pm->ps->saber_anim_level == SS_TAVION ||
...
void PM_Setsaber_move(saber_moveName_t new_move)
{
	...
	else if (new_move == LS_A_JUMP_T__B_ && pm->ps->saber_anim_level == SS_DESANN)
	{
		anim = Q_irand(BOTH_FJSS_TR_BL, BOTH_FJSS_TL_BR);
	}
	else if (pm->ps->saber_anim_level == SS_STAFF && new_move >= LS_S_TL2BR && new_move < LS_REFLECT_LL)
	{
		...
		else
		{
			...
			anim += (pm->ps->saber_anim_level - FORCE_LEVEL_1) * SABER_ANIM_GROUP_SIZE;
		}
	}
	else if ((pm->ps->saber_anim_level == SS_DUAL || pm->ps->dualSabers && pm->ps->saber[1].Active()) && new_move >=
	...
	else if (pm->ps->saber_anim_level > FORCE_LEVEL_1 &&
	...
	if (pm->ps->torsoAnim == anim)
	{
		...
		if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive())
		{
			...
			if (pm->ps->saber_anim_level == SS_STRONG)
			{
				wp_saber_swing_sound(pm->gent, 0, SWING_FAST);
			}
		}
		...
		if (pm->cmd.buttons & BUTTON_ATTACK)
		{
			...
			if (!(pm->ps->pm_flags & PMF_ATTACK_HELD))
			{
				...
				if (ValidAnimFileIndex(gent->client->clientInfo.animFileIndex))
				{
					...
					if (!Q_irand(0, 2))
					{
						if (pm->ps->client_num < MAX_CLIENTS && g_saberLockCinematicCamera->integer < 1)
						{
							if (!Q_irand(0, 3))
							{
								//PM_AddEvent(EV_JUMP);
				...
				if (ValidAnimFileIndex(genemy->client->clientInfo.animFileIndex))
				{
					...
					{
						if (g_check_increment_lock_anim(genemy->client->ps.torsoAnim, SABER_LOCK_LOSE))
						{
							PM_SetAnimFrame(genemy, anim->firstFrame + anim->numFrames - remaining, qtrue, qtrue);
						}
						else
...
qboolean PM_CheckUpsideDownAttack()
{
	...
	case BOTH_FLIP_HOLD7:
		pm->ps->pm_flags |= PMF_SLOW_MO_FALL;
		PM_Setsaber_move(LS_UPSIDE_DOWN_ATTACK);
		return qtrue;
	default:;
	}
	return qfalse;
		...
		else
		{
			...
			else
			{
				...
				case LS_KICK_B_AIR:
					...
					break;
				case LS_KICK_R_AIR:
					PM_Setsaber_move(LS_KICK_R);
					break;
				case LS_KICK_L_AIR:
					PM_Setsaber_move(LS_KICK_L);
					break;
				default:
					break;
				}
		...
		&& !PM_InKnockDown(pm->ps)) //not already in a kick
	{
		...
		else if (pm->cmd.forwardmove)
		{
			...
			if (pm->cmd.forwardmove > 0)
			{
					...
					SaberActive())
				{
					if (G_CheckEnemyPresence(pm->gent, DIR_FRONT, 64.0f, 12.0f))
					{
						...
						{
							PM_Setsaber_move(LS_HILT_BASH);
						}
						else
						{
							PM_Setsaber_move(LS_SMACK_L);
						}
					}
					else
					{
			...
			else
			{
				...
				{
					PM_Setsaber_move(LS_KICK_B3);
				}
				else
				{
					PM_Setsaber_move(LS_KICK_B2);
				}
			}
			pm->cmd.forwardmove = 0;
		}
...
qboolean PM_SaberBlocking()
{
	...
	if (pm->ps->saberBlocked)
	{
		...
		case BLOCKED_BOUNCE_MOVE:
		{
			...
			pm->ps->torsoAnimTimer = 0;
			PM_Setsaber_move(static_cast<saber_moveName_t>(pm->ps->saber_move));
			pm->ps->weaponTime = pm->ps->torsoAnimTimer;
		...
		case BLOCKED_ATK_BOUNCE:
				...
				|| !PM_SaberInAttack(pm->ps->saber_move) && !PM_SaberInStart(pm->ps->saber_move))
			{
				...
				PM_Setsaber_move(static_cast<saber_moveName_t>(next_move));
				pm->ps->weaponTime = pm->ps->torsoAnimTimer;
			}
			else
			{
		...
		case BLOCKED_UPPER_RIGHT:
			if (pm->ps->saberBounceMove != LS_NONE)
			{
				PM_Setsaber_move(static_cast<saber_moveName_t>(pm->ps->saberBounceMove));
				pm->ps->weaponTime = pm->ps->torsoAnimTimer;
			}
		...
		case BLOCKED_UPPER_RIGHT_PROJ:
			...
			{
				PM_Setsaber_move(LS_K1_TR);
			}
			else
			{
				PM_Setsaber_move(LS_REFLECT_UR);
			}
		...
		case BLOCKED_UPPER_LEFT_PROJ:
			if (pm->ps->saber[0].type == SABER_DOOKU
				...
				|| pm->ps->saber[0].type == SABER_PALP && pm->ps->userInt3 & 1 << FLAG_BLOCKDRAINED)
			{
				PM_Setsaber_move(LS_K1_TL);
			}
			else
			{
				PM_Setsaber_move(LS_REFLECT_UL);
			}
			was_attacked_by_gun = qtrue;
			break;
		case BLOCKED_LOWER_RIGHT:
			...
			{
				PM_Setsaber_move(LS_PARRY_LR);
			}
			break;
		case BLOCKED_LOWER_RIGHT_PROJ:
			PM_Setsaber_move(LS_REFLECT_LR);
			was_attacked_by_gun = qtrue;
			break;
		case BLOCKED_LOWER_LEFT:
			if (pm->ps->saberBounceMove != LS_NONE)
			...
			{
				PM_Setsaber_move(LS_PARRY_LL);
			}
			break;
		case BLOCKED_LOWER_LEFT_PROJ:
			PM_Setsaber_move(LS_REFLECT_LL);
			was_attacked_by_gun = qtrue;
			break;
		case BLOCKED_TOP:
			if (pm->ps->saberBounceMove != LS_NONE)
			...
			{
				PM_Setsaber_move(LS_PARRY_UP);
			}
			break;
		case BLOCKED_TOP_PROJ:
			PM_Setsaber_move(LS_REFLECT_UP);
			was_attacked_by_gun = qtrue;
			break;
		case BLOCKED_FRONT:
			if (pm->ps->saberBounceMove != LS_NONE)
				...
				|| pm->ps->saber[0].type == SABER_ASBACKHAND)
			{
				...
				|| pm->ps->saber[0].type == SABER_PALP && pm->ps->userInt3 & 1 << FLAG_BLOCKDRAINED)
			{
				PM_Setsaber_move(LS_REFLECT_ATTACK_FRONT);
			}
			else
			{
				if (pm->ps->saber_anim_level == SS_DUAL)
				...
				{
					PM_Setsaber_move(LS_PARRY_FRONT);
				}
			}
			break;
		case BLOCKED_FRONT_PROJ:
			...
			{
				PM_Setsaber_move(LS_REFLECT_FRONT);
			}
			was_attacked_by_gun = qtrue;
			break;
		case BLOCKED_BLOCKATTACK_LEFT:
			PM_Setsaber_move(LS_REFLECT_ATTACK_LEFT);
			break;
		case BLOCKED_BLOCKATTACK_RIGHT:
			PM_Setsaber_move(LS_REFLECT_ATTACK_RIGHT);
			break;
		case BLOCKED_BLOCKATTACK_FRONT:
			PM_Setsaber_move(LS_REFLECT_ATTACK_FRONT);
			break;
...
void PM_WeaponLightsaber()
{
		...
		&& pm->gent->weaponModel[1])
	{
		...
		pm->ps->saber_anim_level = SS_DUAL;
		...
		&& !pm->ps->saber[0].blade[1].active)
	{
		...
		pm->ps->saber_anim_level = pm->ps->saber[0].singleBladeStyle; //SS_STRONG;
		anim_level_overridden = qtrue;
	}
	else if (pm->gent
		...
		&& WP_SaberStyleValidForSaber(pm->gent, cg.saber_anim_levelPending))
	{
		...
		&& (WP_SaberStyleValidForSaber(pm->gent, pm->ps->saber_anim_level)
			|| WP_UseFirstValidSaberStyle(pm->gent, &pm->ps->saber_anim_level)))
	{
		...
		&& (pm->ps->saber_move == LS_READY || pm->ps->saber_move == LS_NONE)) //ready for another one
	{
		...
		PM_Setsaber_move(static_cast<saber_moveName_t>(pm->ps->saberMoveNext));
	...
	if (PM_CanDoKata())
	{
		...
		if (override_move == LS_INVALID)
		{
			...
			case SS_FAST:
				...
				{
					PM_Setsaber_move(LS_YODA_SPECIAL);
				}
				else
				{
					PM_Setsaber_move(LS_A1_SPECIAL);
				}
				break;
			case SS_MEDIUM:
				PM_Setsaber_move(LS_A2_SPECIAL);
				break;
			case SS_STRONG:
				if (pm->ps->saber[0].type == SABER_PALP)
				{
					PM_Setsaber_move(LS_A_JUMP_PALP_);
				}
				else
				{
					PM_Setsaber_move(LS_A3_SPECIAL);
				}
				break;
			case SS_DESANN:
				if (pm->ps->saber[0].type == SABER_PALP)
				...
				{
					PM_Setsaber_move(LS_A5_SPECIAL);
				}
				break;
			case SS_TAVION:
				if (pm->ps->saber[0].type == SABER_YODA)
				...
				{
					PM_Setsaber_move(LS_A4_SPECIAL);
				}
				break;
			case SS_DUAL:
				if (pm->ps->saber[0].type == SABER_GRIE)
				{
					PM_Setsaber_move(LS_DUAL_SPIN_PROTECT_GRIE);
				}
				...
				{
					PM_Setsaber_move(LS_STAFF_SOULCAL);
				}
				else if (pm->ps->saber[0].type == SABER_BACKHAND
					|| pm->ps->saber[0].type == SABER_ASBACKHAND)
				{
				...
				{
					PM_Setsaber_move(LS_DUAL_SPIN_PROTECT);
				}
				break;
			case SS_STAFF:
				PM_Setsaber_move(LS_STAFF_SOULCAL);
		...
		pm->ps->communicatingflags & 1 << KICKING)
	{
		...
		{
			//NPCs spin the staff
			PM_Setsaber_move(LS_SPINATTACK);
			return;
		}
		PM_CheckKick();
		return;
		...
		else
		{
			...
			if (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_START)
			{
				...
				if (pm->ps->torsoAnimTimer >= 200)
				{
					...
					PM_Setsaber_move(LS_LEAP_ATTACK);
				}
				return;
			}
				...
				else
				{
					...
					if (pm->ps->client_num >= MAX_CLIENTS && !PM_ControlledByPlayer()
						&& (Q_irand(0, pm->ps->forcePowerLevel[FP_SABER_OFFENSE] - 1)
							...
							|| pm->ps->saber_anim_level == SS_FAST && pm->gent
			...
			PM_Setsaber_move(static_cast<saber_moveName_t>(newmove));
...
void Pmove(pmove_t* pmove)
{
	...
	PM_UpdateViewAngles(pm->ps->saber_anim_level, pm->ps, &pm->cmd, pm->gent);
...
void PM_AdjustAngleForWallGrab(playerState_t* ps, usercmd_t* ucmd)
{
	if (ps->pm_flags & PMF_STUCK_TO_WALL && PM_InLedgeMove(ps->legsAnim))
	{
		...
		if (ps->legsAnim != BOTH_LEDGE_MERCPULL)
		{
			...
			gi.trace(&trace, trace_from, nullptr, nullptr, traceTo, ps->client_num, pm->tracemask,
				static_cast<EG2_Collision>(0), 0);
...
qboolean LedgeTrace(trace_t* trace, vec3_t dir, float* lerpup, float* lerpfwd, float* lerpyaw)
{
	...
	gi.trace(trace, trace_from, nullptr, nullptr, traceTo, pm->ps->client_num, pm->tracemask,
		static_cast<EG2_Collision>(0), 0);
...
void PM_CheckGrab()
{
	...
	gi.trace(&trace, pm->ps->origin, pm->mins, pm->maxs, trace_to, pm->ps->client_num, MASK_PLAYERSOLID,
		static_cast<EG2_Collision>(0), 0);
