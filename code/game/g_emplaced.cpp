...
void EWebPositionUser(gentity_t* owner, gentity_t* eweb)
{
	...
	{
		//extra checks
		gi.trace(&tr, owner->currentOrigin, owner->mins, owner->maxs, owner->currentOrigin, owner->s.number,
			owner->clipmask, static_cast<EG2_Collision>(0), 0);
	...
	if (!tr.startsolid && !tr.allsolid)
	{
		...
		VectorCopy(p, p2);
		p2[2] -= STEPSIZE;
		gi.trace(&tr, p, owner->mins, owner->maxs, p2, owner->s.number, owner->clipmask, static_cast<EG2_Collision>(0),
			0);
...
// spawnflag
constexpr auto EMPLACED_INACTIVE = 1;
constexpr auto EMPLACED_FACING = 2;
constexpr auto EMPLACED_VULNERABLE = 4;
constexpr auto EWEB_INVULNERABLE = 4;
constexpr auto EMPLACED_PLAYERUSE = 8;
...
void eweb_use(gentity_t* self, const gentity_t* other, gentity_t* activator)
{
	...
	VectorCopy(activator->currentOrigin, self->pos4);
...
void SP_emplaced_eweb(gentity_t* ent)
{
	...
	ent->headBolt = gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "cannon_Xrot");
	...
	int hit_loc)
{
	...
	gi.G2API_SetBoneAnglesIndex(&self->ghoul2[self->playerModel], self->lowerLumbarBone, ugly, BONE_ANGLES_POSTMULT,
...
{
	if (((*ucmd)->buttons & BUTTON_USE || (*ucmd)->forwardmove < 0 || (*ucmd)->upmove > 0) && ent->owner && ent->owner->
		delay + 500 < level.time)
	{
