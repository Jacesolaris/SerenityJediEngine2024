...
constexpr auto LSTATE_FLEE = 2;
constexpr auto LSTATE_BERZERK = 3;
...
static void Howler_TryDamage(const int damage, const qboolean tongue, const qboolean knockdown)
{
	...
	gi.trace(&tr, start, vec3_origin, vec3_origin, end, NPC->s.number, MASK_SHOT, static_cast<EG2_Collision>(0), 0);
...
static void Howler_Attack(const float enemy_dist, const qboolean howl)
{
	...
	case BOTH_MELEE1:
		if (NPC->client->ps.legsAnimTimer > 650 //more than 13 frames left
			&& PM_AnimLength(NPC->client->clientInfo.animFileIndex, static_cast<animNumber_t>(NPC->client->ps.legsAnim))
			- NPC->client->ps.legsAnimTimer >= 800) //at least 16 frames into anim
		{
	...
	case BOTH_MELEE2:
		if (NPC->client->ps.legsAnimTimer > 350 //more than 7 frames left
			&& PM_AnimLength(NPC->client->clientInfo.animFileIndex, static_cast<animNumber_t>(NPC->client->ps.legsAnim))
			- NPC->client->ps.legsAnimTimer >= 550) //at least 11 frames into anim
		{
...
static void Howler_Combat()
{
	...
	else
	{
		...
		const float distance = DistanceHorizontal(NPC->currentOrigin, NPC->enemy->currentOrigin);

		if (NPC->enemy && NPC->enemy->client && PM_InKnockDown(&NPC->enemy->client->ps))
		{
			//get really close to knocked down enemies
			advance = distance > MIN_DISTANCE ? qtrue : qfalse;
		}
		else
		{
			advance = distance > MAX_DISTANCE ? qtrue : qfalse; //MIN_DISTANCE
		}
	...
	if (!faced)
	{
		if (TIMER_Done(NPC, "attacking")) // not attacking
		{
			//not standing around
		...
		else if (TIMER_Done(NPC, "standing"))
		{
			...
			else if (NPCInfo->last_ucmd.buttons & BUTTON_WALKING)
			{
				...
				if (TIMER_Done(NPC, "walking"))
				{
					...
					if (Q_irand(0, 5) || DistanceSquared(NPC->enemy->currentOrigin, NPC->currentOrigin) <
						MAX_DISTANCE_SQR)
					{
						//run for a while
			...
			else
			{
				...
				if (TIMER_Done(NPC, "running"))
				{
					...
					if (Q_irand(0, 8) || DistanceSquared(NPC->enemy->currentOrigin, NPC->currentOrigin) <
						MAX_DISTANCE_SQR)
					{
						//walk for a while
