...
qboolean INV_GoodieKeyTake(const gentity_t* target)
{
	if (!target || !target->client)
	{
		return qfalse;
...
int INV_GoodieKeyCheck(const gentity_t* target)
{
	if (!target || !target->client)
	{
		return qfalse;
...
================
*/
qboolean INV_SecurityKeyGive(const gentity_t* target, const char* keyname)
{
...
void INV_SecurityKeyTake(const gentity_t* target, const char* keyname)
{
...
qboolean INV_SecurityKeyCheck(const gentity_t* target, const char* keyname)
{
	if (!target || !keyname || !target->client)
	{
		return qfalse;
