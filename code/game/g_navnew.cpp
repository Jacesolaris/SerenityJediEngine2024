...
qboolean NAV_HitNavGoal(vec3_t point, vec3_t mins, vec3_t maxs, vec3_t dest, const int radius, const qboolean flying)
{
	...
	if (radius)
	{
		...
		if (!flying)
		{
			...
			return static_cast<qboolean>(VectorLengthSquared(diff) <= radius * radius);
		}
		//must hit exactly
		return static_cast<qboolean>(DistanceSquared(dest, point) <= radius * radius);
...
qboolean NAV_CheckAhead(const gentity_t* self, vec3_t end, trace_t& trace, int clipmask)
{
	...
	const float radius = self->maxs[0] > self->maxs[1] ? self->maxs[0] : self->maxs[1];
	const float dist = Distance(self->currentOrigin, end);
...
qboolean NPC_ClearPathToGoal(gentity_t* goal)
{
	...
	const float dist = Distance(NPC->currentOrigin, goal->currentOrigin);
	const float tFrac = 1.0f - radius / dist;
