...
================
*/
void ResampleSfx(sfx_t* sfx, const int iInRate, const int iInWidth, byte* p_data)
{
	int		i_sample;

	const float f_step_scale = static_cast<float>(iInRate) / dma.speed;	// this is usually 0.5, 1, or 2

	// When step scale is > 1 (we're down sampling), we really ought to run a low pass filter on the samples

	const int iOutCount = static_cast<int>(sfx->iSoundLengthInSamples / f_step_scale);
	...
	{
		const int iSrcSample = ui_sample_frac >> 8;
		ui_sample_frac += ui_frac_step;
		if (iInWidth == 2) {
			i_sample = LittleShort reinterpret_cast<short*>(p_data)[iSrcSample];
		}
		else {
			i_sample = (int)((unsigned char)p_data[iSrcSample] - 128) << 8;
		}
						...
						// should always be true, unless file is fucked, in which case, stop this conversion process
					{
						...
						if (!qbForceStereo) // no point for stereo files, which are for music and therefore no lip-sync
						{
							const auto pbUnpackBuffer = static_cast<byte*>(Z_Malloc(
								iRawPCMDataSize + 10, TAG_TEMP_WORKSPACE, qfalse)); // won't return if fails
...
#define INT_MIN     (-2147483647 - 1) /* minimum (signed) int value */
								...
								// force this to be oldest sound file, therefore disposable...
								pSFX->bInMemory = true;
						...
						if (f)
						{
							...
							if (iWritten)
							{
								...
								strncpy(pTAG->title, Filename_WithoutPath(Filename_WithoutExt(sFilename)),
									sizeof pTAG->title);
...
void S_MP3_CalcVols_f()
{
	...
	constexpr char sUsage[] = "Usage: mp3_calcvols [-rescan] <startdir>\ne.g. mp3_calcvols sound/chars";
	...
	for (int i = 1; i < Cmd_Argc(); i++)
	{
		if (Cmd_Argv(i)[0] == '-')
		{
			...
			else if (!Q_stricmp(Cmd_Argv(i), "-stereo"))
			{
				qbForceStereo = qtrue;
			}
			else
...
static qboolean S_LoadSound_Actual(sfx_t* sfx)
{
	...
	if (Q_stricmpn(psExt, ".mp3", 4) == 0)
	{
		...
		if (MP3_IsValid(sLoadName, data, size, qfalse))
		{
			...
			if (S_LoadSound_DirIsAllowedToKeepMP3s(sfx->sSoundName)
				// NOT sLoadName, this uses original un-languaged name
				&&
				MP3Stream_InitFromFile(sfx, data, size, sLoadName, iRawPCMDataSize + 2304 /* + 1 MP3 frame size, jic */,
					qfalse)
				)
			{
			...
			else
			{
				...
				//
				{
					...
					{
						const auto pbUnpackBuffer = static_cast<byte*>(Z_Malloc(iRawPCMDataSize + 10 + 2304 /* <g> */,
						...
						{
							Com_Printf(
								S_COLOR_YELLOW"**** MP3 %s final unpack size %d different to previous value %d\n",
						...
						if (s_UseOpenAL)
						{
							if (strstr(sfx->sSoundName, "chars"))
							{
								sfx->lipSyncData = static_cast<char*>(Z_Malloc(
							...
							if (alGetError() == AL_NO_ERROR)
							{
								...
								alBufferData(Buffer, AL_FORMAT_MONO16, sfx->pSoundData, sfx->iSoundLengthInSamples * 2,
									22050);
								if (alGetError() == AL_NO_ERROR)
								{
									sfx->Buffer = Buffer;
	...
	else
	{
		...
		*/
		...
		if (s_UseOpenAL)
		{
			...
			{
				sfx->lipSyncData = static_cast<char*>(Z_Malloc(sfx->iSoundLengthInSamples / 1000 + 1, TAG_SND_RAWDATA,
					qfalse));
				S_PreProcessLipSync(sfx);
			}
			else
...
qboolean S_LoadSound(sfx_t* sfx)
{
	...
	const qboolean bReturn = S_LoadSound_Actual(sfx);
