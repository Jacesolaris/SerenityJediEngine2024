...
extern int S_AddLocalSet(const char* name, vec3_t listener_origin, vec3_t origin, int ent_id, int time);
...
qboolean CL_InitCGameVM(void* gameLibrary)
{
	...
	const auto dllEntry = static_cast<DllEntryProc*>(Sys_LoadFunction(gameLibrary, "dllEntry"));
	cgvm.entryPoint = static_cast<intptr_t(*)(int, ...)>(Sys_LoadFunction(gameLibrary, "vmMain"));

	if (!cgvm.entryPoint || !dllEntry)
	{
		const auto gamename = "SerenityJediEngine2024-game";
...
qboolean CL_GetSnapshot(const int snapshotNumber, snapshot_t* snapshot)
{
	...
	const clSnapshot_t* clSnap = &cl.frames[snapshotNumber & PACKET_MASK];
	if (!clSnap->valid)
	{
		return qfalse;
	}
...
intptr_t CL_CgameSystemCalls(intptr_t* args)
{
	...
	case CG_CM_LOADMAP:
		CL_CM_LoadMap(static_cast<const char*>(VMA(1)), static_cast<qboolean>(args[2] != 0));
		return 0;
	...
	case CG_R_SETREFRACTIONPROP:
		...
		*(re.tr_distortionPrePost()) = static_cast<qboolean>(args[3]);
		*(re.tr_distortionNegate()) = static_cast<qboolean>(args[4]);
		return 0;
