	...
	const char* psAudioFile /* = NULL */)
{
	...
	Q_strncpyz(cinTable[currentHandle].file_name, name, MAX_OSPATH);
...
static void PlayCinematic(const char* arg, const char* s, const qboolean qbInGame)
{
	...
	if (S_FileExists(arg))
	{
		...
		if (!Q_stricmp(arg, "video/jk0101_sw.roq"))
		{
			...
			{
				hCrawl = re.RegisterShaderNoMip("menu/video/tc_english"); //yav version of text crawl
			}
			else if (cl_com_outcast->integer == 4)
			{
				hCrawl = re.RegisterShaderNoMip("menu/video/tc_gen"); //eoc version of text crawl
			}
			else if (cl_com_outcast->integer == 5)
			{
				hCrawl = re.RegisterShaderNoMip("menu/video/tc_priv"); //eoc version of text crawl
			}
			else if (cl_com_outcast->integer == 6)
			{
				hCrawl = re.RegisterShaderNoMip("menu/video/tc_und"); //eoc version of text crawl
			}
			else if (cl_com_outcast->integer == 7)
			{
				hCrawl = re.RegisterShaderNoMip("menu/video/tc_noc"); //eoc version of text crawl
			}
			else if (cl_com_outcast->integer == 8)
			{
				hCrawl = re.RegisterShaderNoMip("menu/video/tc_pib"); //eoc version of text crawl
			}
			else if (cl_com_outcast->integer == 9)
			{
				hCrawl = re.RegisterShaderNoMip("menu/video/tc_red"); //eoc version of text crawl
			}
			else if (cl_com_outcast->integer == 10)
			{
				hCrawl = re.RegisterShaderNoMip("menu/video/tc_english"); //academy version of text crawl
			}
			else
