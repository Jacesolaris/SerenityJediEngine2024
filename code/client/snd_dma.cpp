...
constexpr auto MAX_LOOP_SOUNDS = 64;
int numLoopSounds;
...
{
	const float flMagnitude = sqrt(sqr(v->x) + sqr(v->y) + sqr(v->z));
	...
	const soundChannel_t channel)
{
	...
	const vec_t dot = -DotProduct(listener_axis[1], source_vec);
...
====================
*/
void S_StartSound(const vec3_t origin, const int entity_num, soundChannel_t entchannel, const sfxHandle_t sfxHandle)
{
	channel_t* ch;
	...
	if (s_show->integer == 1)
	{
		Com_Printf("%i : %s for ent %d, chan=%d\n", s_paintedtime, sfx->sSoundName, entity_num, entchannel);
	}
#ifdef USE_OPENAL
	if (s_UseOpenAL)
	{
		...
		if (entchannel == CHAN_VOICE)
		{
			...
			// by the Voice Volume)
			if (strstr(sfx->sSoundName, "sand_creature") != nullptr || strstr(sfx->sSoundName, "howler") != nullptr)
			{
...
void S_StartLocalSound(const sfxHandle_t sfxHandle, int channelNum)
{
	...
	S_StartSound(nullptr, listener_number, static_cast<soundChannel_t>(channelNum), sfxHandle);
}
...
void S_Respatialize(const int entity_num, const vec3_t head, vec3_t axis[3], const qboolean inwater)
{
	...
	else
	{
		...
		for (i = 0; i < MAX_CHANNELS; i++, ch++)
		{
			...
			if (ch->entnum == listener_number || ch->entchannel == CHAN_VOICE_GLOBAL || ch->entchannel ==
				CHAN_ANNOUNCER)
			{
...
void S_SoundList_f()
{
	...
	for (sfx = s_knownSfx, i = 0; i < s_numSfx; i++, sfx++)
	{
		...
		const auto bMP3DumpOverride = static_cast<qboolean>(bShouldBeMP3 &&
			cv_MP3overhead &&
			!sfx->bDefaultSound &&
	...
	if (s_lLastEnvUpdate + ENV_UPDATE_RATE < lCurTime)
	{
		...
		for (i = 0; i < Q_min(s_NumFXSlots, s_lNumEnvironments); i++)
		{
			...
			if (s_FXSlotInfo[i].lEnvID == s_EnvironmentID)
			{
				...
				float flMagnitude = 1.0f - 2.0f * sin(flGamma / 2.0f) / flGamma;
			...
			else
			{
				...
				float flGamma = acos(LP1.x * LP2.x + LP1.y * LP2.y + LP1.z * LP2.z);
				float flMagnitude = 2.0f * sin(flGamma / 2.0f) / flGamma;
