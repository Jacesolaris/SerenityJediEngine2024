...
//
using mdxmHeader_t = struct
{
	//
	// ( first 3 fields are same format as MD3/MDR so we can apply easy model-format-type checks )
	//
	int ident;
	// "IDP3" = MD3, "RDM5" = MDR, "2LGM"(GL2 Mesh) = MDX   (cruddy char order I know, but I'm following what was there in other versions)
...
// {
using mdxmHierarchyOffsets_t = struct
{
...
using mdxmSurfHierarchy_t = struct
{
	char name[MAX_QPATH];
	unsigned int flags;
...
using mdxmLODSurfOffset_t = struct
{
	// added in GLM version 3 for ingame use at Jake's request
...
// {
using mdxaSkelOffsets_t = struct
{
