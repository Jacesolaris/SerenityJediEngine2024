...
//
static qboolean Taiwanese_IsTrailingPunctuation(const unsigned int uiCode)
{
...
//
static qboolean Japanese_IsTrailingPunctuation(const unsigned int uiCode)
{
...
//
static qboolean Chinese_IsTrailingPunctuation(const unsigned int uiCode)
{
...
{
#ifdef JK2_MODE
	// JK2 does this func a little differently
...
{
#ifdef JK2_MODE
	// Yes..even this func is a little different, to the point where it doesn't work.
	...
	{
		int iAdvanceCount;
		unsigned int uiLetter = AnyLanguage_ReadCharFromString(const_cast<char*>(psText), &iAdvanceCount, nullptr);
		...
		if (uiLetter == '^')
		{
			if (*psText >= '0' &&
				*psText <= '9')
			{
				uiLetter = AnyLanguage_ReadCharFromString(const_cast<char*>(psText), &iAdvanceCount, nullptr);
				psText += iAdvanceCount;
				continue;
			}
		}
	...
	// using ceil because we need to make sure that all the text is contained within the integer pixel width we're returning
	return static_cast<int>(ceilf(fMaxWidth));
#endif
}
...
int RE_Font_StrLenChars(const char* psText)
{
	...
	while (*psText)
	{
		...
		//
		int iAdvanceCount;
		const unsigned int uiLetter = AnyLanguage_ReadCharFromString(const_cast<char*>(psText), &iAdvanceCount, nullptr);
		psText += iAdvanceCount;
...
{
	// HAAAAAAAAAAAAAAAX..fix me please
#ifdef JK2_MODE
	...
	*/
	...
	while (*psText && !bNextTextWouldOverflow)
	{
		...
		default:
			...
			float fThisScale = uiLetter > static_cast<unsigned>(g_iNonScaledCharRange) ? fScaleAsian : fScale;
