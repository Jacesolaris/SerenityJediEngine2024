		...
		int flip()
		{
			...
			for (TEdgesIter CurEdge = mEdges.begin(); CurEdge != mEdges.end(); ++CurEdge)
			{
				...
				if (!(*CurEdge).mOnHull)
				{
					...
					int EdgeHandle = mEdges.index_to_handle(CurEdge.index());
					int PtA = EdgeAt.mA;
					int PtB = EdgeAt.mB;
					int PtR = FaceR.opposing_node(PtA, PtB);
					int PtL = FaceL.opposing_node(PtA, PtB);
						...
						)
					{
						...
						//--------------------------------------------------------------------------------------
						int EdgeRL = FaceR.relative_left(EdgeHandle);
						int EdgeRR = FaceR.relative_right(EdgeHandle);

						int EdgeLL = FaceL.relative_left(EdgeHandle);
						int EdgeLR = FaceL.relative_right(EdgeHandle);
