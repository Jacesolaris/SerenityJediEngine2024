...
namespace ragl
{
	...
	public:
		...
		{
			const int numNeighbors = mLinks[nodeA].size();
		...
		void		remove_edge(int nodeA, int nodeB, bool reflexive = true)
		{
			...
			{
				assert("Unable To Remove Edge" == nullptr);
				return;
			}

			int	link_num;

			for (link_num = 0; link_num < mLinks[nodeA].size(); link_num++)
			{
				if (mLinks[nodeA][link_num].mNode == nodeB)
				{
					if (mLinks[nodeA][link_num].mEdge && mEdges.is_used(mLinks[nodeA][link_num].mEdge))
					{
						mEdges.free(mLinks[nodeA][link_num].mEdge);
					}
					mLinks[nodeA].erase_swap(link_num);
					break;
				}
			}

			for (link_num = 0; link_num < mLinks[nodeB].size(); link_num++)
			{
				if (mLinks[nodeB][link_num].mNode == nodeA)
				{
					if (mLinks[nodeB][link_num].mEdge && mEdges.is_used(mLinks[nodeB][link_num].mEdge))
					{
						mEdges.free(mLinks[nodeB][link_num].mEdge);
					}
					mLinks[nodeB].erase_swap(link_num);
					break;
				}
			}
		}
	...
	private:
		...
		public:
			...
			void			reheapify(int handle)
			{
				...
				const int Pos = mHandleToPos[handle];
	...
	public:
		using TVisited = ratl::vector_vs<search_node, MAXNODES>;
		using TVisitedHandles = ratl::array_vs<int, MAXNODES>;
		using TNodeState = ratl::bits_vs<MAXNODES>;
