...
namespace ratl
{
	...
	public:
		...
		T& get(float x, float y)
		{
			...
			const int xint = static_cast<int>((x - mMins[0]) / mScale[0]);
			const int yint = static_cast<int>((y - mMins[1]) / mScale[1]);
		...
		void get_cell_coords(float x, float y, int& xint, int& yint) const
		{
			...
			xint = static_cast<int>((x - mMins[0]) / mScale[0]);
			yint = static_cast<int>((y - mMins[1]) / mScale[1]);
	...
	public:
		...
		iterator begin(float xReal, float yReal)
		{
			...
			const int x = static_cast<int>((xReal - mMins[0]) / mScale[0]);
			const int y = static_cast<int>((yReal - mMins[1]) / mScale[1]);
