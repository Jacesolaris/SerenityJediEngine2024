	...
	for (axis = 0; axis < 3; axis++)
	{
		for (dir = -1; dir <= 1; dir += 2, order++)
		{
			...
			if (i == facet->numBorders) {
				...
				facet->borderNoAdjust[facet->numBorders] = static_cast<qboolean>(0);
	...
	for (j = 0; j < w->numpoints; j++)
	{
		...
		for (axis = 0; axis < 3; axis++)
		{
			for (dir = -1; dir <= 1; dir += 2)
			{
				...
				{
					const float d = DotProduct(w->p[l], plane) - plane[3];
					if (d > 0.1)
...
static void CM_PatchCollideFromGrid(cGrid_t* grid, patchCollide_t* pf) {
	...
	for (i = 0; i < grid->width - 1; i++) {
		for (j = 0; j < grid->height - 1; j++) {
			...
			if (gridPlanes[i][j][0] == gridPlanes[i][j][1]) {
				...
				facet->borderNoAdjust[0] = static_cast<qboolean>(noAdjust[EN_TOP]);
				facet->borderPlanes[1] = borders[EN_RIGHT];
				facet->borderNoAdjust[1] = static_cast<qboolean>(noAdjust[EN_RIGHT]);
				facet->borderPlanes[2] = borders[EN_BOTTOM];
				facet->borderNoAdjust[2] = static_cast<qboolean>(noAdjust[EN_BOTTOM]);
				facet->borderPlanes[3] = borders[EN_LEFT];
				facet->borderNoAdjust[3] = static_cast<qboolean>(noAdjust[EN_LEFT]);
				...
				facet->borderNoAdjust[0] = static_cast<qboolean>(noAdjust[EN_BOTTOM]);
				facet->borderPlanes[1] = borders[EN_LEFT];
				facet->borderNoAdjust[1] = static_cast<qboolean>(noAdjust[EN_LEFT]);
	...
	if (numFacets)
	{
		pf->facets = static_cast<facet_t*>(Hunk_Alloc(numFacets * sizeof * pf->facets, h_high));
...
patchCollide_s* CM_GeneratePatchCollide(int width, int height, vec3_t* points) {
	...
	// collided against
	patchCollide_t* pf = static_cast<patchCollide_s*>(Hunk_Alloc(sizeof * pf, h_high));
...
void CM_DrawDebugSurface(void (*drawPoly)(int color, int num_points, const float* points)) {
	...
	for (i = 0, facet = pc->facets; i < pc->numFacets; i++, facet++) {
		for (int k = 0; k < facet->numBorders + 1; k++) {
			...
			w = BaseWindingForPlane(plane, plane[3]);
			for (int j = 0; j < facet->numBorders + 1 && w; j++) {
				//
				if (j < facet->numBorders) {
