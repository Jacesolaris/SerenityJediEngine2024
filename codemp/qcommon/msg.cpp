...
void MSG_InitOOB(msg_t* buf, byte* data, const int length)
{
	if (!g_nOverrideChecked)
	{
		//Check for netf overrides
...
void MSG_WriteAngle16(msg_t* sb, const float f)
{
...
{
	int c = static_cast<signed char>(MSG_ReadBits(msg, 8));
...
{
	int c = static_cast<unsigned char>(MSG_ReadBits(msg, 8));
	if (msg->readcount > msg->cursize)
	{
		c = -1;
	}
...
{
	int c = static_cast<short>(MSG_ReadBits(msg, 16));
	if (msg->readcount > msg->cursize)
	{
		c = -1;
	}
...
using netField_t = struct netField_s
{
	const char* name;
	size_t offset;
	int bits; // 0 = float
...
netField_t entityStateFields[] =
{
	...
	{NETF(iModelScale), 10},
	//0-1024 (guess it's gotta be increased if we want larger allowable scale.. but 1024% is pretty big)
	// full bits used
	...
	{NETF(forceFrame), 16},
	//if you have over 65536 frames, then this will explode. Of course if you have that many things then lots of things will probably explode.
	{NETF(generic1), 8},
	...
	{NETF(forceSpeedRecoveryTime), 32},
	{NETF(PlayerEffectFlags), 16},

	{NETF(ManualBlockingFlags), 32},
	{NETF(ManualBlockingTime), 32},
	...
	{NETF(cloakCharcgestartTime), 32},
	{NETF(cloakchargelaststartTime), 32},
...
==================
*/
void MSG_WriteDeltaEntity(msg_t* msg, entityState_s* from, entityState_s* to, const qboolean force)
{
	int i;
	netField_t* field;
	int* fromF, * toF;
	...
	{
		fromF = reinterpret_cast<int*>(reinterpret_cast<byte*>(from) + field->offset);
		toF = reinterpret_cast<int*>(reinterpret_cast<byte*>(to) + field->offset);
		if (*fromF != *toF)
		{
			lc = i + 1;
#ifndef FINAL_BUILD
	...
	for (i = 0, field = entityStateFields; i < lc; i++, field++)
	{
		...
		{
			// float
			const float fullFloat = *reinterpret_cast<float*>(toF);
			...
			if (field->bits == 0)
			{
				...
				else
				{
					if (MSG_ReadBits(msg, 1) == 0)
					{
						...
						*reinterpret_cast<float*>(toF) = trunc;
						if (print)
						{
					...
					else
					{
						...
						*toF = MSG_ReadBits(msg, 32);
						if (print)
						{
							Com_Printf("%s:%f ", field->name, *reinterpret_cast<float*>(toF));
						}
					}
				}
			}
...
netField_t playerStateFields[] =
{
	...
	//unfortunately we need this for fall damage calculation (client needs to know the distance for the fall noise)
	...
	{PSF(damageCount), 8},
	{PSF(inAirAnim), 1},
	//just transmit it for the sake of knowing right when on the client to play a land anim, it's only 1 bit
	{PSF(eventParms[1]), 8},
	...
	//NOTE: Not just an index, used as a (1 << val) bitflag for up to 16 clients
	...
	{PSF(lastHitLoc[1]), 0},
	//currently only used so client knows to orient disruptor disintegration.. seems a bit much for just that though.
	...
	{PSF(cloakCharcgestartTime), 32},
	{PSF(cloakchargelaststartTime), 32},
...
using bitStorage_t = struct bitStorage_s;
...
{
#else
void MSG_WriteDeltaPlayerstate(msg_t * msg, playerState_s * from, playerState_s * to, const qboolean isVehiclePS)
{
#endif
	int i;
	playerState_t dummy{};
	...
	for (i = 0, field = PSFields; i < lc; i++, field++)
	{
		...
		{
			// float
			fullFloat = *reinterpret_cast<float*>(toF);
			trunc = static_cast<int>(fullFloat);
...
===================
*/
void MSG_ReadDeltaPlayerstate(msg_t * msg, playerState_t * from, playerState_t * to, const qboolean isVehiclePS)
{
	int i, lc;
	netField_t* field;
	netField_t* PSFields = playerStateFields;
...
void MSG_initHuffman()
{
	...
	for (int i = 0; i < 256; i++)
	{
		...
		{
			Huff_addRef(&msgHuff.compressor, static_cast<byte>(i)); // Do update
			Huff_addRef(&msgHuff.decompressor, static_cast<byte>(i)); // Do update
		}
	}
}
