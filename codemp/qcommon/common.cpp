...
void QDECL Com_Printf(const char* fmt, ...)
{
	...
	if (rd_buffer)
	{
		if (strlen(msg) + strlen(rd_buffer) > static_cast<size_t>(rd_buffersize - 1))
		{
			rd_flush(rd_buffer);
			*rd_buffer = 0;
		}
	...
	if (com_logfile && com_logfile->integer)
	{
		...
		if (!logfile && FS_Initialized() && !opening_qconsole)
		{
			...
			time(&aclock);
			const tm* newtime = localtime(&aclock);
...
int Com_Filter(char* filter, char* name, const int casesensitive)
{
	...
	while (*filter)
	{
		if (*filter == '*')
		{
			...
			if (strlen(buf))
			{
				char* ptr = Com_StringContains(name, buf, casesensitive);
...
int Com_EventLoop(void)
{
	...
	while (true)
	{
		...
		case SE_JOYSTICK_AXIS:
			...
			break;
		case SE_CONSOLE:
			if (static_cast<char*>(ev.evPtr)[0] == '\\' || static_cast<char*>(ev.evPtr)[0] == '/')
			{
				Cbuf_AddText(static_cast<char*>(ev.evPtr) + 1);
			}
			else
			{
...
{
	Cbuf_ExecuteText(EXEC_NOW, "exec SerenityJediEngine2024-MP-default.cfg\n");
	Cbuf_Execute(); // Always execute after exec to prevent text buffer overflowing

	if (!Com_SafeMode())
	{
		// skip the q3config.cfg and autoexec_mp.cfg if "safe" is on the command line
...
void Com_Init(char* commandLine)
{
	...
	try
	{
		...
		Com_RandomBytes(reinterpret_cast<byte*>(&qport), sizeof(int));
...
static void Com_WriteConfigToFile(const char* filename)
{
	...
	FS_Printf(f, "// generated by SerenityJediEngine2024 MP, do not modify\n");
	Key_WriteBindings(f);
...
void Com_WriteConfig_f(void)
{
	...
	if (!FS_FilenameCompare(filename, "SerenityJediEngine2024-MP-default.cfg") || !FS_FilenameCompare(filename, "SerenityJediEngine2024-SP-default.cfg"))
	{
...
void Com_RandomBytes(byte* string, const int len)
{
	...
	for (int i = 0; i < len; i++)
		string[i] = static_cast<unsigned char>(rand() % 256);
}
...
uint32_t ConvertUTF8ToUTF32(char* utf8CurrentChar, char** utf8NextChar)
{
	...
	{
		Com_DPrintf("Unrecognized UTF-8 lead byte: 0x%x\n", static_cast<unsigned>(*c));
		c++;
	}
