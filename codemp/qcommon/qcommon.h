...
using cmdList_t = struct cmdList_s
{
	const char* name;
...
cvar_t* Cvar_Get(const char* var_name, const char* var_value, uint32_t flags, const char* var_desc = nullptr);
...
using huffman_t = struct huffman_s
{
	huff_t compressor;
	huff_t decompressor;
};
...
void Huff_offsetTransmit(const huff_t* huff, int ch, byte* fout, int* offset, int maxoffset);
