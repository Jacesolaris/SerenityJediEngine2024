...
int CROFFSystem::Cache(const char* file, const qboolean isClient)
{
	...
	else
	{
		...
		if (len <= 0)
		{
			...
			len = FS_ReadFile(va("mpscripts/%s.rof", otherPath), reinterpret_cast<void**>(&data));
			if (len <= 0)
			{
...
//---------------------------------------------------------------------------
qboolean CROFFSystem::Play(const int ent_id, const int id, const qboolean do_translation, const qboolean isClient)
{
	sharedEntity_t* ent = nullptr;

	if (!isClient)
	{
		ent = SV_Gentity_num(ent_id);
...
//---------------------------------------------------------------------------
qboolean CROFFSystem::PurgeEnt(const int ent_id, const qboolean isClient)
{
	for (auto itr = mROFFEntList.begin(); itr != mROFFEntList.end(); ++itr)
	{
		if ((*itr)->mIsClient == isClient && (*itr)->mEntID == ent_id)
		{
