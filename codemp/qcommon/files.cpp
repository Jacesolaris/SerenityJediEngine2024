...
static void FS_CopyFile(char* fromOSPath, char* toOSPath) {
	...
	if (fwrite(buf, 1, len, f) != static_cast<unsigned>(len))
	{
		fclose(f);
		free(buf);
...
long FS_FOpenFileRead(const char* filename, fileHandle_t* file, qboolean uniqueFILE) {
	...
	const bool isUserConfig = !Q_stricmp(filename, "autoexec_mp.cfg") || !Q_stricmp(filename, Q3CONFIG_CFG);
...
=================
*/
int FS_Write(const void* buffer, int len, const fileHandle_t h) {
	FS_AssertInitialised();
...
long FS_ReadFile(const char* qpath, void** buffer) {
	...
	byte* buf;	// quiet compiler warning
	...
	if (strstr(qpath, ".cfg")) {
		...
		if (com_journal && com_journal->integer == 2) {
			...
			buf = static_cast<unsigned char*>(Hunk_AllocateTempMemory(len + 1));
			*buffer = buf;
	...
	buf = static_cast<byte*>(Z_Malloc(len + 1, TAG_FILESYS, qfalse));
...
static pack_t* FS_LoadZipFile(const char* zipfile, const char* basename)
{
	...
	fileInPack_t* buildBuffer = static_cast<fileInPack_s*>(Z_Malloc(gi.number_entry * sizeof(fileInPack_t) + len,
		TAG_FILESYS, qtrue));
	char* namePtr = reinterpret_cast<char*>(buildBuffer) + gi.number_entry * sizeof(fileInPack_t);
	int* fs_headerLongs = static_cast<int*>(Z_Malloc((gi.number_entry + 1) * sizeof(int), TAG_FILESYS, qtrue));
	...
	const auto pack = static_cast<pack_t*>(Z_Malloc(sizeof(pack_t) + i * sizeof(fileInPack_t*), TAG_FILESYS, qtrue));
	pack->hashSize = i;
...
qboolean FS_CompareZipChecksum(const char* zipfile)
{
	...
	const int checksum = thepak->checksum;
	FS_FreePak(thepak);
...
char** FS_ListFilteredFiles(const char* path, const char* extension, char* filter, int* numfiles) {
	...
	*numfiles = nfiles;
	...
	char** listCopy = static_cast<char**>(Z_Malloc((nfiles + 1) * sizeof * listCopy, TAG_FILESYS));
	for (i = 0; i < nfiles; i++) {
		listCopy[i] = list[i];
	}
	listCopy[i] = nullptr;
	...
	for (int i = 0; i < fs_numServerReferencedPaks; i++) {
		...
		if (!havepak && fs_serverReferencedPakNames[i] && *fs_serverReferencedPakNames[i]) {
			...
			if (dlstring)
			{
				...
				if (strlen(origpos) + (origpos - neededpaks) >= static_cast<unsigned>(len - 1))
				{
					*origpos = '\0';
					break;
				}
...
static void FS_Startup(const char* gameName)
{
	...
	fs_gamedirvar = Cvar_Get("fs_game", "SerenityJediEngine2024", CVAR_INIT | CVAR_SYSTEMINFO, "Mod directory");
...
void FS_PureServerSetReferencedPaks(const char* pakSums, const char* pakNames) {
	...
	for (i = 0; i < static_cast<int>(ARRAY_LEN(fs_serverReferencedPakNames)); i++)
	{
...
qboolean FS_WriteToTemporaryFile(const void* data, size_t dataLength, char** temp_file_path)
{
	...
	const DWORD tempPathResult = GetTempPath(MAX_PATH, tempPath);
	if (tempPathResult)
	{
		TCHAR tempFileName[MAX_PATH];
		const UINT tempFileNameResult = GetTempFileName(tempPath, "OJK", 0, tempFileName);
		if (tempFileNameResult)
		{
			const HANDLE file = CreateFile(
				tempFileName, GENERIC_WRITE, 0, nullptr,
				CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
			if (file != INVALID_HANDLE_VALUE)
			{
				DWORD bytesWritten = 0;
				if (WriteFile(file, data, dataLength, &bytesWritten, nullptr))
				{
					...
					if (temp_file_path)
					{
						const size_t fileNameLen = strlen(tempFileName);
						*temp_file_path = static_cast<char*>(Z_Malloc(fileNameLen + 1, TAG_FILESYS));
						Q_strncpyz(*temp_file_path, tempFileName, fileNameLen + 1);
					}
