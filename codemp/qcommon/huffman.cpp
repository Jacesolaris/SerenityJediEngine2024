...
void Huff_putBit(const int bit, byte* fout, int* offset)
{
	bloc = *offset;
	if ((bloc & 7) == 0)
	{
...
int Huff_getBit(const byte* fin, int* offset)
{
	bloc = *offset;
	const int t = fin[(bloc >> 3)] >> (bloc & 7) & 0x1;
	bloc++;
	*offset = bloc;
	return t;
}
...
void Huff_addRef(huff_t* huff, const byte ch)
{
...
void Huff_Init(huffman_t* huff)
{
	...
	// Initialize the tree & list with the NYT node
	huff->decompressor.tree = huff->decompressor.lhead = huff->decompressor.ltail = huff->decompressor.loc[NYT] = &huff
		->decompressor.nodeList[huff->decompressor.blocNode++];
	...
	// Add the NYT (not yet transmitted) node into the tree/list */
	huff->compressor.tree = huff->compressor.lhead = huff->compressor.loc[NYT] = &huff->compressor.nodeList[huff->
		compressor.blocNode++];
