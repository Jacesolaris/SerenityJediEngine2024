...
#pragma pack(push)
#pragma pack(1)
using StaticZeroMem_t = struct StaticZeroMem_s
{
	zoneHeader_t Header;
	//	byte mem[0];
	zoneTail_t Tail;
...
using StaticMem_t = struct StaticMem_s
{
	zoneHeader_t Header;
	byte mem[2];
	zoneTail_t Tail;
...
void* Z_Malloc(const int iSize, const memtag_t eTag, const qboolean bZeroit, const int iUnusedAlign)
{
	...
	if (iSize == 0)
	{
		auto pMemory = reinterpret_cast<zoneHeader_t*>(&gZeroMalloc);
		return &pMemory[1];
	}
...
extern "C" Q_EXPORT void SerenityJediEngine2024_minizip_free(void* to_free);
...
void SerenityJediEngine2024_minizip_free(void* to_free)
{
	Z_Free(to_free);
}
...
char* CopyString(const char* in)
{
	if (!in[0])
	{
		return reinterpret_cast<char*>(&gEmptyString) + sizeof(zoneHeader_t);
	}
	if (!in[1])
	{
		if (in[0] >= '0' && in[0] <= '9')
		{
			return reinterpret_cast<char*>(&gNumberString[in[0] - '0']) + sizeof(zoneHeader_t);
		}
	}
...
{
	return 64 * 1024 * 1024 - (Z_MemSize(TAG_HUNK_MARK1) + Z_MemSize(TAG_HUNK_MARK2));
