...
void RemoveBoneCache(const CBoneCache* bone_cache)
{
	...
	g_Ghoul2Allocations -= sizeof(*bone_cache);
#endif
...
int G2_GetBoneDependents(CGhoul2Info& ghoul2, const int boneNum, int* temp_dependents, int max_dep)
{
	...
	const mdxaSkel_t* skel = reinterpret_cast<mdxaSkel_t*>((byte*)bone_cache.header + sizeof(mdxaHeader_t) + offsets->offsets[boneNum]);
	int i;
	int ret = 0;
...
void G2_RagGetBoneBasePoseMatrixLow(const CGhoul2Info& ghoul2, const int boneNum, const mdxaBone_t& bone_matrix,
	mdxaBone_t& ret_matrix, vec3_t scale)
{
	...
	const mdxaSkelOffsets_t* offsets = reinterpret_cast<mdxaSkelOffsets_t*>((byte*)bone_cache.header + sizeof(
		mdxaHeader_t));
	const auto skel = reinterpret_cast<mdxaSkel_t*>((byte*)bone_cache.header + sizeof(mdxaHeader_t) + offsets->offsets[
		boneNum]);
...
void G2_GetBoneMatrixLow(const CGhoul2Info& ghoul2, const int boneNum, const vec3_t scale, mdxaBone_t& retMatrix,
	mdxaBone_t*& ret_basepose, mdxaBone_t*& ret_basepose_inv)
{
	...
	const mdxaSkelOffsets_t* offsets = reinterpret_cast<mdxaSkelOffsets_t*>((byte*)bone_cache.header + sizeof(mdxaHeader_t));
	const auto skel = reinterpret_cast<mdxaSkel_t*>((byte*)bone_cache.header + sizeof(mdxaHeader_t) + offsets->offsets[boneNum]);
	Multiply_3x4Matrix(&bolt, const_cast<mdxaBone_t*>(&bone_cache.Eval(boneNum)), &skel->BasePoseMat); // DEST FIRST ARG
	ret_basepose = &skel->BasePoseMat;
	ret_basepose_inv = &skel->BasePoseMatInv;
...
int G2_GetParentBoneMatrixLow(const CGhoul2Info& ghoul2, const int boneNum, const vec3_t scale, mdxaBone_t& retMatrix,
	mdxaBone_t*& ret_basepose, mdxaBone_t*& ret_basepose_inv)
{
	int parent = -1;
	if (ghoul2.mBoneCache)
	{
		...
		{
			G2_GetBoneMatrixLow(ghoul2, parent, scale, retMatrix, ret_basepose, ret_basepose_inv);
		}
	}
	return parent;
}
...
void G2_TimingModel(boneInfo_t& bone, const int currentTime, const int numFrames_in_file, int& currentFrame,
	int& new_frame,
	float& lerp)
{
	assert(bone.startFrame >= 0);
	assert(bone.startFrame <= numFrames_in_file);
	assert(bone.endFrame >= 0);
	assert(bone.endFrame <= numFrames_in_file);
	...
	if (animSize)
	{
			...
			if (bone.flags & BONE_ANIM_OVERRIDE_LOOP)
			{
				...
				if (animSpeed < 0.0f)
				{
					...
					if (newFrame_g < endFrame + 1 && newFrame_g >= endFrame)
					{
						...
						assert(currentFrame >= 0 && currentFrame < numFrames_in_file);
						new_frame = bone.startFrame;
						assert(new_frame >= 0 && new_frame < numFrames_in_file);
					}
					else
					{
				...
				// sanity check
				assert(new_frame < endFrame && new_frame >= bone.startFrame || animSize < 10);
			}
			else
			{
		...
		else
		{
			if (animSpeed > 0.0)
			{
				...
				assert(static_cast<int>(endFrame) <= numFrames_in_file);
				if (new_frame >= static_cast<int>(endFrame))
				{
...
void G2_SetUpBolts(mdxaHeader_t* header, CGhoul2Info& ghoul2, const mdxaBone_v& bone_ptr, boltInfo_v& bolt_list)
{
	const mdxaSkelOffsets_t* offsets = reinterpret_cast<mdxaSkelOffsets_t*>(reinterpret_cast<byte*>(header) + sizeof(mdxaHeader_t));

	for (auto& i : bolt_list)
	{
		if (i.boneNumber != -1)
		{
			// figure out where the bone hirearchy info is
			const auto skel = reinterpret_cast<mdxaSkel_t*>(reinterpret_cast<byte*>(header) + sizeof(mdxaHeader_t) + offsets->offsets[i.boneNumber]);
			Multiply_3x4Matrix(&i.position, &bone_ptr[i.boneNumber].second, &skel->BasePoseMat);
		}
	}
}
...
void G2_TransformGhoulBones(boneInfo_v& rootBoneList, const mdxaBone_t& rootMatrix, CGhoul2Info& ghoul2, const int time,
	const bool smooth = true)
{
	...
	const model_t* currentModel = const_cast<model_t*>(ghoul2.currentModel);
	...
	const surfaceInfo_t* surfInfo, model_t* mod)
{
	...
	if (surfInfo && surfInfo->offFlags == G2SURFACEFLAG_GENERATED)
	{
		...
		const int* piBoneRefs = reinterpret_cast<int*>(reinterpret_cast<byte*>(originalSurf) + originalSurf->ofsBoneReferences);
		...
		for (k = 0; k < iNumWeights; k++)
		{
			...
			pTri[0][0] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[0], vert0->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[0][3]);
			pTri[0][1] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[1], vert0->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[1][3]);
			pTri[0][2] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[2], vert0->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[2][3]);
		}
		//		w = vert1->weights;
		fTotalWeight = 0.0f;
		...
		for (k = 0; k < iNumWeights; k++)
		{
			...
			pTri[1][0] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[0], vert1->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[0][3]);
			pTri[1][1] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[1], vert1->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[1][3]);
			pTri[1][2] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[2], vert1->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[2][3]);
		}
		//		w = vert2->weights;
		fTotalWeight = 0.0f;
		...
		for (k = 0; k < iNumWeights; k++)
		{
			...
			pTri[2][0] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[0], vert2->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[0][3]);
			pTri[2][1] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[1], vert2->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[1][3]);
			pTri[2][2] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[2], vert2->vertCoords)
				+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[2][3]);
		}
	...
	else
	{
		...
		matrix3_t sides;
		matrix3_t axes;
		const int* piBoneRefs = reinterpret_cast<int*>(reinterpret_cast<byte*>(surface) + surface->ofsBoneReferences);

		// whip through and actually transform each vertex
		auto v = reinterpret_cast<mdxmVertex_t*>(reinterpret_cast<byte*>(surface) + surface->ofsVerts);
		for (j = 0; j < 3; j++)
		{
			// 			mdxmWeight_t	*w;
			...
			for (k = 0; k < iNumWeights; k++)
			{
				...
				pTri[j][1] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[1], v->vertCoords)
					+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[1][3]);
				pTri[j][2] += fBoneWeight * (DotProduct(bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[2], v->vertCoords)
					+ bone_ptr[piBoneRefs[iBoneIndex]].second.matrix[2][3]);
			}
...
void G2_ProcessGeneratedSurfaceBolts(CGhoul2Info& ghoul2, const mdxaBone_v& bone_ptr, model_t* mod_t)
{
#ifdef G2_PERFORMANCE_ANALYSIS
...
void ProcessModelBoltSurfaces(const int surface_num, surfaceInfo_v& root_s_list,
	mdxaBone_v& bone_ptr, model_t* currentModel, const int lod, boltInfo_v& bolt_list)
{
#ifdef G2_PERFORMANCE_ANALYSIS
	...
	const auto surface = static_cast<mdxmSurface_t*>(G2_FindSurface(currentModel, surface_num, 0));
	const auto surfIndexes = reinterpret_cast<mdxmHierarchyOffsets_t*>(reinterpret_cast<byte*>(currentModel->mdxm) + sizeof(mdxmHeader_t));
	...
	{
		// well alrighty then. Lets see if there is a bolt that is attempting to use it
		const int boltNum = G2_Find_Bolt_Surface_Num(bolt_list, surface_num, 0);
		// yes - ok, processing time.
		if (boltNum != -1)
		{
			G2_ProcessSurfaceBolt(bone_ptr, surface, boltNum, bolt_list, surf_override, currentModel);
		}
	}
	...
	{
		ProcessModelBoltSurfaces(surfInfo->childIndexes[i], root_s_list, bone_ptr, currentModel, lod, bolt_list);
	}
...
void G2_ConstructUsedBoneList(CConstructBoneList& CBL)
{
	...
	const mdxmSurface_t* surface = static_cast<mdxmSurface_t*>(G2_FindSurface(
		CBL.currentModel, CBL.surface_num, 0));
	const mdxmHierarchyOffsets_t* surfIndexes = reinterpret_cast<mdxmHierarchyOffsets_t*>((byte*)CBL.currentModel->mdxm + sizeof(
		mdxmHeader_t));
	const mdxmSurfHierarchy_t* surfInfo = reinterpret_cast<mdxmSurfHierarchy_t*>((byte*)surfIndexes + surfIndexes->offsets[surface->
		thisSurfaceIndex]);
	...
	{
		const int* bonesReferenced = reinterpret_cast<int*>((byte*)surface + surface->ofsBoneReferences);
		...
		for (i = 0; i < surface->numBoneReferences; i++)
		{
			...
			const mdxaSkel_t* skel = reinterpret_cast<mdxaSkel_t*>((byte*)mod_a->mdxa + sizeof(mdxaHeader_t) + offsets->offsets[
				iBoneIndex]);
			...
			while (iParentBone != -1)
			{
				...
				skel = reinterpret_cast<mdxaSkel_t*>(reinterpret_cast<byte*>(mod_a->mdxa) + sizeof(mdxaHeader_t) + offsets->offsets[iParentBone]);
...
void* G2_FindSurface_BC(const model_s* mod, const int index, const int lod)
{
	...
	for (int i = 0; i < lod; i++)
	{
		const mdxmLOD_t* lodData = reinterpret_cast<mdxmLOD_t*>(current);
	...
	const surfaceInfo_t* surfInfo, const model_t* mod, mdxaBone_t& retMatrix)
{
	...
	if (surfInfo && surfInfo->offFlags == G2SURFACEFLAG_GENERATED)
	{
		...
		VectorClear(pTri[1]);
		VectorClear(pTri[2]);
		auto piBoneReferences = reinterpret_cast<int*>(reinterpret_cast<byte*>(originalSurf) + originalSurf->ofsBoneReferences);
...
#ifdef G2EVALRENDER
			const mdxaBone_t& bone = bone_cache.EvalRender(piBoneReferences[iBoneIndex]);
#else
	...
	else
	{
		...
		auto piBoneReferences = reinterpret_cast<int*>((byte*)surface + surface->ofsBoneReferences);
		for (j = 0; j < 3; j++)
		{
			// 			mdxmWeight_t	*w;
...
void G2_GetBoltMatrixLow(CGhoul2Info& ghoul2, int boltNum, const vec3_t scale, mdxaBone_t& retMatrix)
{
	...
	else if (boltList[boltNum].surfaceNumber >= 0)
	{
		...
		{
			surface = static_cast<mdxmSurface_t*>(G2_FindSurface_BC(bone_cache.mod, surfInfo->surface, 0));
		}
	...
	for (int i = 0; i < ghoul2.size(); i++)
	{
		if (ghoul2[i].mModelindex != -1 && ghoul2[i].mValid)
		{
			if (ghoul2[i].mFlags & GHOUL2_NEWORIGIN)
			{
				...
				Multiply_3x4Matrix(&retMatrix, &tempMatrix, const_cast<mdxaBone_t*>(&identity_matrix));
				return;
			}
		}
	}
...
==============
*/
void G2_ConstructGhoulSkeleton(CGhoul2Info_v& ghoul2, const int frame_num, const bool checkForNewOrigin,
	const vec3_t scale)
{
#ifdef G2_PERFORMANCE_ANALYSIS
...
#ifdef _G2_LISTEN_SERVER_OPT
			else if (ghoul2[i].entityNum == ENTITYNUM_NONE || ghoul2[i].mSkelFrameNum != frame_num)
#else
			else
#endif
			{
...
qboolean R_LoadMDXM(model_t* mod, void* buffer, const char* mod_name, qboolean& bAlreadyCached)
{
	...
	for (int l = 0; l < mdxm->numLODs; l++)
	{
		...
		for (i = 0; i < mdxm->numSurfaces; i++)
		{
			...
			{
				Com_Error(ERR_DROP, "R_LoadMDXM: %s has more than %i verts on a surface (%i)", mod_name,
					SHADER_MAX_VERTEXES, surf->numVerts);
			}
			if (surf->numTriangles * 3 > SHADER_MAX_INDEXES)
			{
				Com_Error(ERR_DROP, "R_LoadMDXM: %s has more than %i triangles on a surface (%i)", mod_name,
