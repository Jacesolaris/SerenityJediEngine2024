...
// Given a model handle, and a bone name, we want to set angles specifically for overriding - using a matrix directly
qboolean G2_Set_Bone_Angles_Matrix(const char* file_name, boneInfo_v& blist, const char* boneName, const mdxaBone_t& matrix, const int flags, const qhandle_t* modelList, const int modelIndex, const int blendTime, const int currentTime)
{
	model_t* mod_m;
	if (!file_name[0])
	{
...
void G2_TimingModel(boneInfo_t& bone, int currentTime, int num_frames_in_file, int& currentFrame, int& new_frame,
	float& lerp);
...
qboolean G2_Get_Bone_Anim_Index(boneInfo_v& blist, const int index, const int currentTime, float* currentFrame, int* startFrame, int* endFrame, int* flags, float* retAnimSpeed, qhandle_t* modelList, const int numFrames)
{
	...
	if (index >= 0 && !(index >= static_cast<int>(blist.size()) || blist[index].boneNumber == -1))
	{
		...
		if (blist[index].flags & (BONE_ANIM_OVERRIDE_LOOP | BONE_ANIM_OVERRIDE))
		{
			int lcurrent_frame, new_frame;
			float lerp;
			G2_TimingModel(blist[index], currentTime, numFrames, lcurrent_frame, new_frame, lerp);

			*currentFrame = static_cast<float>(lcurrent_frame) + lerp;
			*startFrame = blist[index].startFrame;
			*endFrame = blist[index].endFrame;
...
void G2_RagGetBoneBasePoseMatrixLow(const CGhoul2Info& ghoul2, int boneNum, const mdxaBone_t& bone_matrix,
...
qboolean G2_SetBoneIKState(CGhoul2Info_v& ghoul2, const int time, const char* boneName, const int ikState,
