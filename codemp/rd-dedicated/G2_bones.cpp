...
qboolean G2_Get_Bone_Anim_Index(boneInfo_v& blist, const int index, const int currentTime, float* currentFrame, int* startFrame, int* endFrame, int* flags, float* retAnimSpeed, qhandle_t* modelList, const int numFrames)
{
	...
	if (index >= 0 && !(index >= static_cast<int>(blist.size()) || blist[index].boneNumber == -1))
	{
		...
		if (blist[index].flags & (BONE_ANIM_OVERRIDE_LOOP | BONE_ANIM_OVERRIDE))
		{
			int lcurrent_frame, newFrame;
			float lerp;
			G2_TimingModel(blist[index], currentTime, numFrames, lcurrent_frame, newFrame, lerp);
...
static qboolean G2_Set_Bone_Anim_No_BS(const CGhoul2Info& ghoul2, boneInfo_v& blist, const char* boneName, const int arg_startFrame, const int arg_endFrame, const int flags, const float animSpeed)
{
	int index = G2_Find_Bone_Rag(&ghoul2, blist, boneName);
	int modFlags = flags;
...
void Rag_Trace(trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, const int contentmask)
{
#ifdef _DEBUG
...
static bool G2_ApplyRealBonePhysics(boneInfo_t& bone, const SRagEffector& e, const CRagDollUpdateParams* params, vec3_t goal_spot, const vec3_t test_mins, const vec3_t test_maxs, const float gravity, const float mass, const float bounce)
{
	trace_t tr;
...
#if 1 //constraint breaks?
				if (limitAngles && (all_solid_count < 32 || bone.RagFlags & RAG_UNSNAPPABLE))
