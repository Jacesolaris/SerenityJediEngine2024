...
qboolean G2_Set_Bone_Angles(const CGhoul2Info* ghl_info, boneInfo_v& blist, const char* bone_name, const float* angles,
	const int flags, const Eorientations up, const Eorientations left,
...
// Given a model handle, and a bone name, we want to set angles specifically for overriding - using a matrix directly
qboolean G2_Set_Bone_Angles_Matrix(const char* file_name, boneInfo_v& blist, const char* bone_name,
	...
	const int num_frames)
{
	...
	if (flags & BONE_ANIM_BLEND)
	{
		...
		if (G2_Get_Bone_Anim_Index(blist, index, current_time, &current_frame, &frame, &end_frame1, &flags1,
			&speed, num_frames))
		{
	...
	if (set_frame != -1)
	{
		blist[index].lastTime = blist[index].startTime = current_time - (set_frame - static_cast<float>(start_frame)) *
			50.0 / anim_speed;
	}
	else
	{
	...
	const int blend_time)
{
	...
	if (index != -1)
	{
		return G2_Set_Bone_Anim_Index(blist, index, start_frame, end_frame, flags, anim_speed, current_time, set_frame,
			blend_time, ghl_info->aHeader->numFrames);
	}
	return qfalse;
}

qboolean G2_Get_Bone_Anim_Range(const CGhoul2Info* ghl_info, const boneInfo_v& blist, const char* bone_name,
...
void G2_TimingModel(boneInfo_t& bone, int current_time, int num_frames_in_file, int& current_frame, int& new_frame,
	float& lerp);
	...
	const int num_frames)
{
	...
	if (index >= 0 && !(index >= static_cast<int>(blist.size()) || blist[index].boneNumber == -1))
	{
		...
		if (blist[index].flags & (BONE_ANIM_OVERRIDE_LOOP | BONE_ANIM_OVERRIDE))
		{
			...
			float lerp;
			G2_TimingModel(blist[index], current_time, num_frames, lcurrent_frame, new_frame, lerp);

			*current_frame = static_cast<float>(lcurrent_frame) + lerp;
			*start_frame = blist[index].start_frame;
			*end_frame = blist[index].end_frame;
	...
	float* current_frame, int* start_frame, int* end_frame, int* flags, float* ret_anim_speed)
{
	...
	if (G2_Get_Bone_Anim_Index(blist, index, current_time, current_frame, start_frame, end_frame, flags, ret_anim_speed,
...
qboolean G2_Pause_Bone_Anim(const CGhoul2Info* ghl_info, boneInfo_v& blist, const char* bone_name, const int current_time)
{
	...
	if (index != -1)
	{
		...
		if (blist[index].pauseTime)
		{
			...
			float current_frame = 0.0f, anim_speed = 1.0f;

			// figure out what frame we are on now
			G2_Get_Bone_Anim(ghl_info, blist, bone_name, blist[index].pauseTime, &current_frame, &start_frame, &end_frame,
				&flags, &anim_speed);
			// reset start time so we are actually on this frame right now
			G2_Set_Bone_Anim(ghl_info, blist, bone_name, start_frame, end_frame, flags, anim_speed, current_time,
				current_frame, 0);
			// no pausing anymore
			blist[index].pauseTime = 0;
		}
...
{
	const mdxaSkelOffsets_t* offsets = reinterpret_cast<mdxaSkelOffsets_t*>((byte*)ghl_info->aHeader + sizeof(
		mdxaHeader_t));
	...
	*/
	...
	for (size_t i = 0; i < blist.size(); i++)
	{
		...
		// figure out what skeletal info structure this bone entry is looking at
		const mdxaSkel_t* skel = reinterpret_cast<mdxaSkel_t*>((byte*)ghl_info->aHeader + sizeof(mdxaHeader_t) + offsets
			->offsets[blist[i].
			boneNumber]);
...
qboolean G2_Set_Bone_Anim_No_BS(const CGhoul2Info& ghoul2, boneInfo_v& blist,
	const char* bone_name, const int arg_start_frame,
...
static char* G2_Get_Bone_Name(const CGhoul2Info* ghl_info, const boneInfo_v& blist, const int bone_num)
{
	...
	for (size_t i = 0; i < blist.size(); i++)
	{
		...
		// figure out what skeletal info structure this bone entry is looking at
		const auto skel = reinterpret_cast<mdxaSkel_t*>((byte*)ghl_info->aHeader + sizeof(mdxaHeader_t) + offsets->offsets[blist[i].
			boneNumber]);
