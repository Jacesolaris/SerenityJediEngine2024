...
void G2_FindRecursiveSurface(model_t* currentModel, int surface_num, surfaceInfo_v& root_list, int* active_surfaces)
{
	...
	const auto surf_indexes = reinterpret_cast<mdxmHierarchyOffsets_t*>(reinterpret_cast<byte*>(currentModel->mdxm) + sizeof(mdxmHeader_t));
...
int G2_AddSurface(CGhoul2Info* ghoul2, const int surface_number, const int poly_number, const float barycentric_i,
	const float barycentric_j, int lod)
{
	surfaceInfo_t temp_slist_entry;
...
int G2_IsSurfaceRendered(const CGhoul2Info* ghlInfo, const char* surfaceName, const surfaceInfo_v& slist)
{
	...
	{
		//must be legal
		const mdxmHierarchyOffsets_t* surf_indexes = reinterpret_cast<mdxmHierarchyOffsets_t*>(reinterpret_cast<byte*>(ghlInfo->currentModel->mdxm) +
			sizeof(mdxmHeader_t));
