...
void UI_InitForceShaders(void)
{
	...
	uiSaberColorShaders[SABER_LIME] = trap->R_RegisterShaderNoMip("menu/art/saber_lime");
}

// Draw the stars spent on the current force power
void UI_DrawForceStars(const rectDef_t* rect, float scale, vec4_t color, int textStyle, const int forceindex, int val, int min, int max)
{
	if (val < min || val > max)
	{
		val = min;
	...
	if (1)	// if (val)
	{
		...
		for (int i = FORCE_LEVEL_1; i <= max; i++)
		{
			...
			{
				const vec4_t grColor = { 0.2f, 0.2f, 0.2f, 1.0f };
...
void UI_SaveForceTemplate()
{
	...
	{ //write it into the light side folder
		trap->FS_Open(va("Account/Rebels/%s.fcf", selectedName), &f, FS_WRITE);
	}
	else
	{ //if it isn't light it must be dark
		trap->FS_Open(va("Account/Empire/%s.fcf", selectedName), &f, FS_WRITE);
	}
...
void UI_ReadLegalForce(void)
{
	...
	for (c = 0; fcfString[i] && c < NUM_FORCE_POWERS; c++, i++)
	{
		...
		for (int currank = FORCE_LEVEL_1; currank <= forcePowerRank; currank++)
		{
...
qboolean UI_ForcePowerRank_HandleKey(int flags, float* special, int key, int num, int min, int max, int type)
{
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER || key == A_BACKSPACE)
	{
		...
		const int forcepower = type - UI_FORCE_RANK - 1;
...
void UI_ForceConfigHandle(int oldindex, int newindex)
{
	...
	{ //we should only be displaying lightside configs, so.. look in the light folder
		...
		if (newindex >= uiInfo.forceConfigCount)
			return;
		len = trap->FS_Open(va("Account/Rebels/%s.fcf", uiInfo.forceConfigNames[newindex]), &f, FS_READ);
	}
	else
	{ //else dark
		...
		len = trap->FS_Open(va("Account/Empire/%s.fcf", uiInfo.forceConfigNames[newindex]), &f, FS_READ);
	}
