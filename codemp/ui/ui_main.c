...
// Stores the skin color so that in non-team games, the player screen remembers the team you chose, in case you're coming back from the force powers screen.
...
void AssetCache(void)
{
	...
	uiInfo.uiDC.Assets.fxPic[1] = trap->R_RegisterShaderNoMip(ART_FX_ORANGE);
	//trap->R_RegisterShaderNoMip( ART_FX_YELLOW );
	uiInfo.uiDC.Assets.fxPic[2] = trap->R_RegisterShaderNoMip(ART_FX_YELLOW);
	//trap->R_RegisterShaderNoMip( ART_FX_GREEN );
	uiInfo.uiDC.Assets.fxPic[3] = trap->R_RegisterShaderNoMip(ART_FX_GREEN);
	//trap->R_RegisterShaderNoMip( ART_FX_TEAL );
	uiInfo.uiDC.Assets.fxPic[4] = trap->R_RegisterShaderNoMip(ART_FX_BLUE);
	uiInfo.uiDC.Assets.fxPic[5] = trap->R_RegisterShaderNoMip(ART_FX_PURPLE);
	//trap->R_RegisterShaderNoMip( ART_FX_CYAN );
	...
	int i_menu_font)
{
	...
	switch (style)
	{
	case ITEM_TEXTSTYLE_NORMAL: iStyleOR = 0;
		break; // JK2 normal text
	case ITEM_TEXTSTYLE_BLINK: iStyleOR = STYLE_BLINK;
		break; // JK2 fast blinking
	case ITEM_TEXTSTYLE_PULSE: iStyleOR = STYLE_BLINK;
		break; // JK2 slow pulsing
	case ITEM_TEXTSTYLE_SHADOWED: iStyleOR = (int)STYLE_DROPSHADOW;
		break; // JK2 drop shadow
	case ITEM_TEXTSTYLE_OUTLINED: iStyleOR = (int)STYLE_DROPSHADOW;
		break; // JK2 drop shadow
	case ITEM_TEXTSTYLE_OUTLINESHADOWED: iStyleOR = (int)STYLE_DROPSHADOW;
		break; // JK2 drop shadow
	case ITEM_TEXTSTYLE_SHADOWEDMORE: iStyleOR = (int)STYLE_DROPSHADOW;
		break; // JK2 drop shadow
	default:;
	}
	...
	int limit, int style, int i_menu_font)
{
	...
	//
	{
		...
		{
			const int iFontIndex = MenuFontToHandle(i_menu_font);
			const int iNextXpos = trap->R_Font_StrLenPixels(sTemp, iFontIndex, scale);

			Text_Paint(x + iNextXpos, y, scale, color, va("%c", cursor), 0, limit, style | ITEM_TEXTSTYLE_BLINK,
				i_menu_font);
		}
	}
}
	...
	int limit, int i_menu_font)
{
	...
	if (x + iPixelLen > *maxX)
	{
			...
			&& psOut < &sTemp[sizeof sTemp - 1])
		{
			...
			const unsigned int uiLetter = trap->R_AnyLanguage_ReadCharFromString(psText, &iAdvanceCount, NULL);
		...
		*maxX = 0; // feedback
		Text_Paint(x, y, scale, color, sTemp, adjust, limit, ITEM_TEXTSTYLE_NORMAL, i_menu_font);
	}
	else
	{
		...
		*maxX = x + iPixelLen; // feedback the next position, as the caller expects
		Text_Paint(x, y, scale, color, text, adjust, limit, ITEM_TEXTSTYLE_NORMAL, i_menu_font);
	}
}
...
static void UI_BuildPlayerList()
{
	...
	const int count = atoi(Info_ValueForKey(info, "sv_maxclients"));
	...
	for (n = 0; n < count; n++)
	{
		...
		if (info[0])
		{
			...
			uiInfo.playerCount++;
			const int team2 = atoi(Info_ValueForKey(info, "t"));
...
static void UI_SetActiveMenu(uiMenuCommand_t menu)
{
	...
	if (Menu_Count() > 0)
	{
		...
		case UIMENU_BRIEFING:
			trap->Cvar_VariableStringBuffer("ui_menubrief", buf, sizeof buf);
			trap->Key_SetCatcher(KEYCATCH_UI);
			Menus_CloseAll();
...
void UI_ParseMenu(const char* menuFile)
{
	...
	const int handle = trap->PC_LoadSource(menuFile);
	if (!handle)
	{
		return;
	}
...
static const char* handicapValues[] = {
	"None", "95", "90", "85", "80", "75", "70", "65", "60", "55", "50", "45", "40", "35", "30", "25", "20", "15", "10",
	"5",NULL
};

static void UI_DrawHandicap(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	const int h = Com_Clamp(5, 100, trap->Cvar_VariableValue("handicap"));
	const int i = 20 - h / 5;

	Text_Paint(rect->x, rect->y, scale, color, handicapValues[i], 0, 0, textStyle, i_menu_font);
}

static void UI_DrawClanName(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	Text_Paint(rect->x, rect->y, scale, color, UI_Cvar_VariableString("ui_teamName"), 0, 0, textStyle, i_menu_font);
}
...
static void UI_DrawGameType(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	Text_Paint(rect->x, rect->y, scale, color, UI_GetGameTypeName(uiInfo.gameTypes[ui_gametype.integer].gtEnum), 0, 0,
...
static void UI_DrawAutoSwitch(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	...
	if (stripString)
	{
		Text_Paint(rect->x, rect->y, scale, color, stripString, 0, 0, textStyle, i_menu_font);
	}
}
...
static int UI_TeamIndexFromName(const char* name)
{
	if (name && *name)
	{
		for (int i = 0; i < uiInfo.teamCount; i++)
		{
...
{
	const int i = UI_TeamIndexFromName(UI_Cvar_VariableString("ui_teamName"));
	if (i >= 0 && i < uiInfo.teamCount)
	{
		...
		if (uiInfo.teamList[i].teamIcon == -1)
		{
			...
			uiInfo.teamList[i].teamIcon_Metal = trap->R_RegisterShaderNoMip(
...
static void UI_DrawClanCinematic(rectDef_t* rect, float scale, vec4_t color)
{
	...
	if (i >= 0 && i < uiInfo.teamCount)
	{
		if (uiInfo.teamList[i].cinematic >= -2)
		{
			...
			{
				uiInfo.teamList[i].cinematic = trap->CIN_PlayCinematic(va("%s.roq", uiInfo.teamList[i].imageName), 0, 0,
...
static void UI_DrawSkill(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
...
static void UI_DrawGenericNum(rectDef_t* rect, float scale, vec4_t color, int textStyle, int val, int min, int max,
	int type, int i_menu_font)
{
	char s[256];
...
static void UI_DrawForceMastery(rectDef_t* rect, float scale, vec4_t color, int textStyle, int val, int min, int max,
	int i_menu_font)
{
	int i = val;
	if (i < min)
...
static void UI_DrawSkinColor(rectDef_t* rect, float scale, vec4_t color, int textStyle, int val, int min, int max,
	int i_menu_font)
{
	char s[256];
...
static void UI_DrawForceSide(rectDef_t* rect, float scale, vec4_t color, int textStyle, int val, int min, int max,
	int i_menu_font)
{
	char s[256];
	menuDef_t* menu;
...
static void UI_DrawJediNonJedi(rectDef_t* rect, float scale, vec4_t color, int textStyle, int val, int min, int max,
	int i_menu_font)
{
	char s[256];
	//menuDef_t *menu;
...
static void UI_DrawTeamName(rectDef_t* rect, float scale, vec4_t color, qboolean blue, int textStyle, int i_menu_font)
{
	const int i = UI_TeamIndexFromName(UI_Cvar_VariableString(blue ? "ui_blueTeam" : "ui_redTeam"));
	if (i >= 0 && i < uiInfo.teamCount)
	{
		Text_Paint(rect->x, rect->y, scale, color, va("%s: %s", blue ? "Blue" : "Red", uiInfo.teamList[i].teamName),
			0, 0, textStyle, i_menu_font);
	}
}
...
static void UI_DrawMapCinematic(rectDef_t* rect, float scale, vec4_t color, qboolean net)
{
	...
	if (uiInfo.mapList[map].cinematic >= -1)
	{
		if (uiInfo.mapList[map].cinematic == -1)
		{
			uiInfo.mapList[map].cinematic = trap->CIN_PlayCinematic(va("%s.roq", uiInfo.mapList[map].mapLoadName), 0, 0,
				0, 0, CIN_loop | CIN_silent);
		}
...
static void UI_DrawNetSource(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	...
	Text_Paint(rect->x, rect->y, scale, color, va("%s %s", holdSPString,
		GetNetSourceString(ui_netSource.integer)), 0, 0, textStyle,
		i_menu_font);
}
...
{
	int i = trap->Cvar_VariableValue("ui_currentTier");
	if (i < 0 || i >= uiInfo.tierCount)
	{
		i = 0;
	}
	Text_Paint(rect->x, rect->y, scale, color, va("Tier: %s", uiInfo.tierList[i].tierName), 0, 0, textStyle, i_menu_font);
}
...
static void UI_DrawTierMapName(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	int i = trap->Cvar_VariableValue("ui_currentTier");
	if (i < 0 || i >= uiInfo.tierCount)
	{
	...
	int j = trap->Cvar_VariableValue("ui_currentMap");
...
static void UI_DrawTierGameType(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	int i = trap->Cvar_VariableValue("ui_currentTier");
	if (i < 0 || i >= uiInfo.tierCount)
	{
	...
	Text_Paint(rect->x, rect->y, scale, color, uiInfo.gameTypes[uiInfo.tierList[i].gameTypes[j]].gameType, 0, 0,
		textStyle, i_menu_font);
}

static const char* UI_AIFromName(const char* name)
{
	for (int j = 0; j < uiInfo.aliasCount; j++)
	{
...
static void UI_NextOpponent()
{
	...
	const int j = UI_TeamIndexFromName(UI_Cvar_VariableString("ui_teamName"));
	i++;
...
{
	const int i = UI_TeamIndexFromName(UI_Cvar_VariableString("ui_opponentName"));
	if (uiInfo.teamList[i].teamIcon == -1)
	{
		uiInfo.teamList[i].teamIcon = trap->R_RegisterShaderNoMip(uiInfo.teamList[i].imageName);
		uiInfo.teamList[i].teamIcon_Metal = trap->R_RegisterShaderNoMip(va("%s_metal", uiInfo.teamList[i].imageName));
...
static void UI_DrawAllMapsSelection(rectDef_t* rect, float scale, vec4_t color, int textStyle, qboolean net,
	int i_menu_font)
{
	const int map = net ? ui_currentNetMap.integer : ui_currentMap.integer;
	if (map >= 0 && map < uiInfo.mapCount)
	{
		Text_Paint(rect->x, rect->y, scale, color, uiInfo.mapList[map].mapName, 0, 0, textStyle, i_menu_font);
	}
}

static void UI_DrawOpponentName(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	Text_Paint(rect->x, rect->y, scale, color, UI_Cvar_VariableString("ui_opponentName"), 0, 0, textStyle, i_menu_font);
}
...
static void UI_DrawBotName(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	...
	const char* text = UI_GetBotNameByNumber(value);
	Text_Paint(rect->x, rect->y, scale, color, text, 0, 0, textStyle, i_menu_font);
}

static void UI_DrawBotSkill(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
...
static void UI_DrawCrosshair(rectDef_t* rect, float scale, vec4_t color)
{
	...
	const float size = Q_min(rect->w, rect->h);
	UI_DrawHandlePic(rect->x, rect->y, size, size, uiInfo.uiDC.Assets.crosshairShader[uiInfo.currentCrosshair]);
	trap->R_SetColor(NULL);
}

static void UI_DrawSelectedPlayer(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
...
static void UI_DrawServerRefreshDate(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
...
static void UI_DrawServerMOTD(rectDef_t* rect, float scale, vec4_t color, int i_menu_font)
{
	if (uiInfo.serverStatus.motdLen)
	{
			...
			if (uiInfo.serverStatus.motdPaintX <= rect->x + 2)
			{
				if (uiInfo.serverStatus.motdOffset < uiInfo.serverStatus.motdLen)
				{
					uiInfo.serverStatus.motdPaintX += Text_Width(
						&uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset], scale, 1) - 1;
		...
		Text_Paint_Limit(&maxX, uiInfo.serverStatus.motdPaintX, rect->y + rect->h - 3, scale, color,
			&uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset], 0, 0, i_menu_font);
		if (uiInfo.serverStatus.motdPaintX2 >= 0)
		{
			float maxX2 = rect->x + rect->w - 2;
			Text_Paint_Limit(&maxX2, uiInfo.serverStatus.motdPaintX2, rect->y + rect->h - 3, scale, color,
				uiInfo.serverStatus.motd, 0, uiInfo.serverStatus.motdOffset, i_menu_font);
		}
...
static void UI_DrawKeyBindStatus(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	if (Display_KeyBindPending())
	{
	...
	{
		//		Text_Paint(rect->x, rect->y, scale, color, "Press ENTER or CLICK to change, Press BACKSPACE to clear", 0, 0, textStyle,i_menu_font);
	}
}

static void UI_DrawGLInfo(rectDef_t* rect, float scale, vec4_t color, int textStyle, int i_menu_font)
{
	char buff[4096] = { 0 };
	int i = 0;

	Text_Paint(rect->x + 2, rect->y, scale, color, va("GL_VENDOR: %s", uiInfo.uiDC.glconfig.vendor_string), 0, rect->w,
		textStyle, i_menu_font);
	Text_Paint(rect->x + 2, rect->y + 15, scale, color,
		va("GL_VERSION: %s: %s", uiInfo.uiDC.glconfig.version_string, uiInfo.uiDC.glconfig.renderer_string), 0,
		rect->w, textStyle, i_menu_font);
	Text_Paint(rect->x + 2, rect->y + 30, scale, color,
		va("GL_PIXELFORMAT: color(%d-bits) Z(%d-bits) stencil(%d-bits)", uiInfo.uiDC.glconfig.colorBits,
			uiInfo.uiDC.glconfig.depthBits, uiInfo.uiDC.glconfig.stencilBits), 0, rect->w, textStyle, i_menu_font);

	// build null terminated extension strings
	Q_strncpyz(buff, uiInfo.uiDC.glconfig.extensions_string, sizeof buff);
	int y = rect->y + 45;

	const char* extensionName = strtok(buff, " ");
	while (y < rect->y + rect->h && extensionName != NULL)
	{
		if (i % 2 == 0)
		{
			Text_Paint(rect->x + 2, y, scale, color, extensionName, 0, rect->w / 2, textStyle, i_menu_font);
		}
		else
		{
			Text_Paint(rect->x + rect->w / 2, y, scale, color, extensionName, 0, rect->w / 2, textStyle, i_menu_font);
			y += 11;
		}
...
=================
*/
static void UI_Version(rectDef_t* rect, float scale, vec4_t color, int i_menu_font)
{
	const int width = uiInfo.uiDC.textWidth(JK_VERSION, scale, i_menu_font);

	uiInfo.uiDC.drawText(rect->x - width, rect->y, scale, color, JK_VERSION, 0, 0, 0, i_menu_font);
}
	...
	int textStyle, int i_menu_font)
{
	...
	{
	case UI_HANDICAP:
		UI_DrawHandicap(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_SKIN_COLOR:
		UI_DrawSkinColor(&rect, scale, color, textStyle, uiSkinColor, TEAM_FREE, TEAM_BLUE, i_menu_font);
		break;
	case UI_FORCE_SIDE:
		UI_DrawForceSide(&rect, scale, color, textStyle, uiForceSide, 1, 2, i_menu_font);
		break;
	case UI_JEDI_NONJEDI:
		UI_DrawJediNonJedi(&rect, scale, color, textStyle, uiJediNonJedi, 0, 1, i_menu_font);
		break;
	case UI_FORCE_POINTS:
		UI_DrawGenericNum(&rect, scale, color, textStyle, uiForceAvailable, 1, forceMasteryPoints[MAX_FORCE_RANK],
			ownerDraw, i_menu_font);
		break;
	case UI_FORCE_MASTERY_SET:
		UI_DrawForceMastery(&rect, scale, color, textStyle, uiForceRank, 0, MAX_FORCE_RANK, i_menu_font);
		break;
	case UI_FORCE_RANK:
		UI_DrawForceMastery(&rect, scale, color, textStyle, uiForceRank, 0, MAX_FORCE_RANK, i_menu_font);
		break;
	...
	case UI_PLAYERMODEL:
		...
		break;
	case UI_CLANNAME:
		UI_DrawClanName(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_CLANLOGO:
	...
	case UI_PREVIEWCINEMATIC:
		...
		break;
	case UI_GAMETYPE:
		UI_DrawGameType(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_NETGAMETYPE:
		UI_DrawNetGameType(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_AUTOSWITCHLIST:
		UI_DrawAutoSwitch(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_JOINGAMETYPE:
		UI_DrawJoinGameType(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_MAPPREVIEW:
	...
	case UI_BLUETEAMNAME:
		...
		break;
	case UI_REDTEAMNAME:
		UI_DrawTeamName(&rect, scale, color, qfalse, textStyle, i_menu_font);
		break;
	case UI_BLUETEAM1:
	case UI_BLUETEAM2:
	case UI_BLUETEAM3:
	...
	case UI_BLUETEAM8:
		...
		UI_DrawTeamMember(&rect, scale, color, qtrue, iUse, textStyle, i_menu_font);
		break;
	case UI_REDTEAM1:
	case UI_REDTEAM2:
	case UI_REDTEAM3:
	...
	case UI_REDTEAM8:
		...
		UI_DrawTeamMember(&rect, scale, color, qfalse, iUse, textStyle, i_menu_font);
		break;
	case UI_NETSOURCE:
		UI_DrawNetSource(&rect, scale, color, textStyle, i_menu_font);
		break;
	...
	case UI_NETMAPCINEMATIC:
		...
		break;
	case UI_NETFILTER:
		UI_DrawNetFilter(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_TIER:
		UI_DrawTier(&rect, scale, color, textStyle, i_menu_font);
		break;
	...
	case UI_OPPONENTLOGO_NAME:
		...
		break;
	case UI_TIER_MAPNAME:
		UI_DrawTierMapName(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_TIER_GAMETYPE:
		UI_DrawTierGameType(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_ALLMAPS_SELECTION:
		UI_DrawAllMapsSelection(&rect, scale, color, textStyle, qtrue, i_menu_font);
		break;
	case UI_MAPS_SELECTION:
		UI_DrawAllMapsSelection(&rect, scale, color, textStyle, qfalse, i_menu_font);
		break;
	case UI_OPPONENT_NAME:
		UI_DrawOpponentName(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_BOTNAME:
		UI_DrawBotName(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_BOTSKILL:
		UI_DrawBotSkill(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_REDBLUE:
		UI_DrawRedBlue(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_CROSSHAIR:
		UI_DrawCrosshair(&rect, scale, color);
		break;
	case UI_SELECTEDPLAYER:
		UI_DrawSelectedPlayer(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_SERVERREFRESHDATE:
		UI_DrawServerRefreshDate(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_SERVERMOTD:
		UI_DrawServerMOTD(&rect, scale, color, i_menu_font);
		break;
	case UI_GLINFO:
		UI_DrawGLInfo(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_KEYBINDSTATUS:
		UI_DrawKeyBindStatus(&rect, scale, color, textStyle, i_menu_font);
		break;
	case UI_VERSION:
		UI_Version(&rect, scale, color, i_menu_font);
		break;
	default:
		break;
	}
...
static qboolean UI_Handicap_HandleKey(int flags, float* special, int key)
{
	...
	{
		int h = Com_Clamp(5, 100, trap->Cvar_VariableValue("handicap"));
		if (key == A_MOUSE2)
		{
			h -= 5;
		}
...
static qboolean UI_GameType_HandleKey(int flags, float* special, int key, qboolean resetMap)
{
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER)
	{
		const int oldCount = UI_MapCountByGameType(qtrue);
...
static qboolean UI_InSoloMenu(void)
{
	...
	const itemDef_t* item = Menu_FindItemByName(menu, name);
	if (item)
	{
		return qtrue;
	}
...
static qboolean UI_NetSource_HandleKey(int flags, float* special, int key)
{
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER)
	{
		...
		if (value >= UIAS_GLOBAL1 && value <= UIAS_GLOBAL5)
		{
			...
			{
				char cvarname[sizeof"sv_master1"];
				char masterstr[2];
...
static qboolean UI_SelectedPlayer_HandleKey(int flags, float* special, int key)
{
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER)
	{
		...
		int selected = trap->Cvar_VariableValue("cg_selectedPlayer");
...
static qboolean UI_OwnerDrawHandleKey(int ownerDraw, int flags, float* special, int key)
{
	...
	case UI_FORCE_RANK_SABERTHROW:
		findex = ownerDraw - UI_FORCE_RANK - 1;
		//this will give us the index as long as UI_FORCE_RANK is always one below the first force rank index
		return UI_ForcePowerRank_HandleKey(flags, special, key, uiForcePowersRank[findex], 0,
			NUM_FORCE_POWER_LEVELS - 1, ownerDraw);
	case UI_EFFECTS:
		break;
	case UI_GAMETYPE:
...
static int QDECL UI_ServersQsortCompare(const void* arg1, const void* arg2)
{
	return trap->LAN_CompareServers(UI_SourceForLAN(), uiInfo.serverStatus.sortKey, uiInfo.serverStatus.sortDir,
		*(int*)arg1, *(int*)arg2);
}
...
static void UI_LoadMods()
{
	...
	const int numdirs = trap->FS_GetFileList("$modlist", "", dirlist, sizeof dirlist);
	char* dirptr = dirlist;
...
static void UI_LoadMovies()
{
	...
	if (uiInfo.movieCount)
	{
		...
		{
			const int len = strlen(moviename);
...
static void UI_LoadDemosInDirectory(loadDemoContext_t* ctx, const char* directory)
{
	...
	for (int j = 0; j < 2; j++)
	{
		...
		for (; uiInfo.loadedDemos < uiInfo.demoCount; uiInfo.loadedDemos++)
		{
			...
			const size_t len = strlen(demoname);
			Com_sprintf(uiInfo.demoList[uiInfo.loadedDemos], sizeof uiInfo.demoList[0], "%s%s", dirPath + 1,
				demoname);
			demoname += len + 1;
		}
	...
	const char* dirListEnd = ctx->directoryList + sizeof ctx->directoryList;
	if (ctx->dirListHead < dirListEnd)
	{
		int i;
		const int dirListSpaceRemaining = dirListEnd - ctx->dirListHead;
		const int numFiles = trap->FS_GetFileList(directory, "/", ctx->dirListHead, dirListSpaceRemaining);

		// Find end of this list so we have a base pointer for the child folders to use
		char* dirList = ctx->dirListHead;
...
static void UI_StartSkirmish(qboolean next)
{
	...
	int temp = trap->Cvar_VariableValue("g_gametype");
	trap->Cvar_Set("ui_gameType", va("%i", temp));

	if (next)
	{
		int actual;
		const int index = trap->Cvar_VariableValue("ui_mapIndex");
	...
	const int g = uiInfo.gameTypes[ui_gametype.integer].gtEnum;
	trap->Cvar_SetValue("g_gametype", g);
	trap->Cmd_ExecuteText(EXEC_APPEND,
		va("wait ; wait ; map %s\n", uiInfo.mapList[ui_currentMap.integer].mapLoadName));
	const float skill = trap->Cvar_VariableValue("g_spSkill");
	trap->Cvar_Set("ui_scoreMap", uiInfo.mapList[ui_currentMap.integer].mapName);

	int k = UI_TeamIndexFromName(UI_Cvar_VariableString("ui_opponentName"));
	...
	temp = trap->Cvar_VariableValue("g_doWarmup");
	...
	temp = trap->Cvar_VariableValue("g_warmup");
...
{
	const int val = trap->Cvar_VariableValue(name);
...
static qboolean UI_DeferMenuScript(char** args)
{
	...
	if (!Q_stricmp(name, "VideoSetup"))
	{
		...
		const qboolean deferred = trap->Cvar_VariableValue("ui_r_modified") ? qtrue : qfalse;
	...
	if (!Q_stricmp(name, "RulesBackout"))
	{
		const qboolean deferred = trap->Cvar_VariableValue("ui_rules_backout") ? qtrue : qfalse;
...
void UI_GetVideoSetup(void)
{
	...
	trap->Cvar_Register(NULL, "ui_r_detailtextures", "1", CVAR_ROM | CVAR_INTERNAL);
	trap->Cvar_Register(NULL, "ui_r_ext_compress_textures", "1", CVAR_ROM | CVAR_INTERNAL);
...
{
	const int gameType = trap->Cvar_VariableValue("g_gametype");
...
static void UI_GetCharacterCvars(void)
{
	...
	const char* model = UI_Cvar_VariableString("model");
...
static void UI_FindCurrentSiegeTeamClass(void)
{
	...
	const int baseClass = (int)trap->Cvar_VariableValue("ui_siege_class");
...
static void UI_UpdateSiegeObjectiveGraphics(void)
{
	...
	for (teamI = 1; teamI < 3; teamI++)
	{
		...
		{
			Menu_SetItemBackground(menu, va("tm%i_icon%i", teamI, objI),
				va("*team%i_objective%i_mapicon", teamI, objI));
			Menu_SetItemBackground(menu, va("tm%i_l_icon%i", teamI, objI),
				va("*team%i_objective%i_mapicon", teamI, objI));
		}
	}
	...
	for (teamI = 1; teamI < 3; teamI++)
	{
		for (objI = 1; objI < 8; objI++)
		{
			UI_SetSiegeObjectiveGraphicPos(menu, va("tm%i_icon%i", teamI, objI),
				va("team%i_objective%i_mappos", teamI, objI));
		}
	}
}
...
static void UI_SetSaberBoxesandHilts(void)
{
	...
	itemDef_t* item = Menu_FindItemByName(menu, "box2middle");
	...
	item = Menu_FindItemByName(menu, "box2bottom");
	...
	item = Menu_FindItemByName(menu, "box3middle");
	...
	item = Menu_FindItemByName(menu, "box3bottom");
...
static qboolean UI_CheckPassword(void)
{
	...
	const int index = uiInfo.serverStatus.currentServer;
...
static void UI_JoinServer(void)
{
	...
	if (uiInfo.serverStatus.currentServer >= 0 && uiInfo.serverStatus.currentServer < uiInfo.serverStatus.
		numDisplayServers)
	{
		trap->LAN_GetServerAddressString(UI_SourceForLAN()/*ui_netSource.integer*/,
		...
		team == SIEGETEAM_TEAM2)
	{
		...
		if (baseClass >= SPC_INFANTRY && baseClass < SPC_MAX)
		{
			...
			if (index >= 0 && index < BG_SiegeCountBaseClass(team, baseClass))
			{
				...
				{
					siegeClass_t* holdClass = BG_GetClassOnBaseClass(team, baseClass, index);
...
static void UI_RunMenuScript(char** args)
{
	...
	if (String_Parse(args, &name))
	{
		...
		else if (Q_stricmp(name, "resetDefaults") == 0)
		{
			...
			trap->Cmd_ExecuteText(EXEC_APPEND, "exec SerenityJediEngine2024-MP-default.cfg\n");
			trap->Cmd_ExecuteText(EXEC_APPEND, "vid_restart\n");
		}
		...
		{
			trap->LAN_GetServerAddressString(UI_SourceForLAN(),
				uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer],
		...
		{
			trap->Cvar_Set("ui_singlePlayerActive", "0");
			if (uiInfo.currentFoundPlayerServer >= 0 && uiInfo.currentFoundPlayerServer < uiInfo.
		...
		else if (Q_stricmp(name, "addFavorite") == 0)
		{
			if (ui_netSource.integer != UIAS_FAVORITES)
			{
				char string[MAX_HOSTNAMELENGTH] = { 0 };
				...
				if (strlen(string) > 0 && strlen(addr) > 0)
				{
					int res;
					res = trap->LAN_AddServer(AS_FAVORITES, string, addr);
					if (res == 0)
					{
		...
		else if (Q_stricmp(name, "deleteFavorite") == 0)
		{
			...
			{
				char addr[MAX_ADDRESSLENGTH] = { 0 };
				trap->LAN_GetServerInfo(
					AS_FAVORITES, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], buff,
					MAX_STRING_CHARS);
				addr[0] = '\0';
				Q_strncpyz(addr, Info_ValueForKey(buff, "addr"), sizeof addr);
				if (strlen(addr) > 0)
		...
		else if (Q_stricmp(name, "createFavorite") == 0)
		{
			...
			//rww - don't know why this check was here.. why would you want to only add new favorites when the filter was favorites?
			{
				...
				string[0] = addr[0] = '\0';
				Q_strncpyz(string, UI_Cvar_VariableString("ui_favoriteName"), sizeof string);
		...
		else if (Q_stricmp(name, "setsiegeclassandteam") == 0)
		{
			...
			else if (oldteam == team)
			{
				...
				{
					if (strcmp(startclassString, bgSiegeClasses[g_UIGloballySelectedSiegeClass].name) == 0)
					{
						goTeam = qfalse;
					}
				}
		...
		else if (Q_stricmp(name, "setscreensaberhilt") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "hiltbut");
				if (item)
				{
		...
		else if (Q_stricmp(name, "setscreensaberhilt1") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "hiltbut1");
				if (item)
				{
					if (saberSingleHiltInfo[item->cursorPos])
					{
		...
		else if (Q_stricmp(name, "setscreensaberhilt2") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "hiltbut2");
				if (item)
				{
					if (saberSingleHiltInfo[item->cursorPos])
					{
		...
		else if (Q_stricmp(name, "setscreensaberstaff") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "hiltbut_staves");
				if (item)
				{
					if (saberSingleHiltInfo[item->cursorPos])
					{
		...
		else if (Q_stricmp(name, "setsiegeobjbuttons") == 0)
		{
			...
			if (menu)
			{
				...
				if (String_Parse(args, &itemArg))
				{
					...
					item = Menu_FindItemByName(menu, string);
					if (item)
					{
						char string2[512];
					...
					item = Menu_FindItemByName(menu, itemArg);
					if (item)
					{
						// store item name
		...
		else if (Q_stricmp(name, "updatesiegeweapondesc") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "base_class_weapons_feed");
				if (item)
				{
					char info[MAX_INFO_VALUE];
					trap->Cvar_VariableStringBuffer(va("ui_class_weapondesc%i", item->cursorPos), info,
						sizeof info);
		...
		else if (Q_stricmp(name, "updatesiegeinventorydesc") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "base_class_inventory_feed");
				if (item)
				{
					char info[MAX_INFO_VALUE];
		...
		else if (Q_stricmp(name, "updatesiegeforcedesc") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "base_class_force_feed");
				if (item)
				{
					int i;
					...
					for (i = 0; i < NUM_FORCE_POWERS; i++)
					{
						if (strcmp(HolocronIcons[i], info) == 0)
						{
		...
		else if (Q_stricmp(name, "resetitemdescription") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "itemdescription");
				if (item)
				{
		...
		else if (Q_stricmp(name, "resetsiegelistboxes") == 0)
		{
			...
			if (menu)
			{
				item = Menu_FindItemByName(menu, "description");
				if (item)
				{
					listBoxDef_t* listPtr = item->typeData.listbox;
					if (listPtr)
		...
		else if (Q_stricmp(name, "setcurrentNetMap") == 0)
		{
			...
			{
				itemDef_t* item;
				item = Menu_FindItemByName(menu, "maplist");
				if (item)
				{
					listBoxDef_t* listPtr = item->typeData.listbox;
					if (listPtr)
...
static int UI_HeadCountByColor(void)
{
	...
	for (int i = 0; i < uiInfo.q3HeadCount; i++)
	{
...
static void UI_InsertServerIntoDisplayList(int num, int position)
{
	...
	for (int i = uiInfo.serverStatus.numDisplayServers; i > position; i--)
	{
...
{
	for (int i = 0; i < uiInfo.serverStatus.numDisplayServers; i++)
	{
		if (uiInfo.serverStatus.displayServers[i] == num)
		{
			uiInfo.serverStatus.numDisplayServers--;
			for (int j = i; j < uiInfo.serverStatus.numDisplayServers; j++)
			{
...
static void UI_BuildServerDisplayList(int force)
{
	...
	if (len == 0)
	{
		Q_strncpyz(uiInfo.serverStatus.motd, "Welcome to SerenityJediEngine2024 MP!", sizeof uiInfo.serverStatus.motd);
...
static void UI_SortServerStatusInfo(serverStatusInfo_t* info)
{
	...
	//
	int index = 0;
	const int numLines = Com_Clampi(0, MAX_SERVERSTATUS_LINES, info->numLines);
	for (int i = 0; serverStatusCvars[i].name; i++)
	{
		for (int j = 0; j < numLines; j++)
		{
			...
			{
				// swap lines
				char* tmp1 = info->lines[index][0];
				char* tmp2 = info->lines[index][3];
...
static void UI_BuildFindPlayerList(qboolean force)
{
	...
	else
	{
		...
		// set resend time
		int resend = ui_serverStatusTimeOut.integer / 2 - 10;
		if (resend < 50)
		{
			resend = 50;
		}
	...
	for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++)
	{
			...
			uiInfo.pendingServerStatus.server[i].startTime < uiInfo.uiDC.realTime - ui_serverStatusTimeOut.integer)
		{
			...
			if (uiInfo.pendingServerStatus.num < uiInfo.serverStatus.numDisplayServers)
			{
				...
				const int lanSource = UI_SourceForLAN();
				trap->LAN_GetServerAddressString(
					lanSource, uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num],
					uiInfo.pendingServerStatus.server[i].adrstr,
					sizeof uiInfo.pendingServerStatus.server[i].adrstr);
				trap->LAN_GetServerInfo(
					lanSource, uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num], infoString,
					sizeof infoString);
...
static void UI_BuildServerStatus(qboolean force)
{
	...
	if (uiInfo.serverStatus.currentServer < 0 || uiInfo.serverStatus.currentServer > uiInfo.serverStatus.
		numDisplayServers || uiInfo.serverStatus.numDisplayServers == 0)
	{
		return;
	}
...
{
	static char info[MAX_STRING_CHARS];
	// don't change this size without changing the sizes inside the SaberProperName calls
	...
	*handle1 = *handle2 = *handle3 = -1;
	...
	if (feederID == FEEDER_SIEGE_TEAM1)
	{
	...
	if (feederID == FEEDER_SIEGE_TEAM2)
	{
		return ""; //nothing I guess, the description part can cover this
		/*
		if (!siegeTeam1)
	...
	else if (feederID == FEEDER_SERVERS)
	{
		if (index >= 0 && index < uiInfo.serverStatus.numDisplayServers)
		{
			...
			{
				trap->LAN_GetServerInfo(UI_SourceForLAN(), uiInfo.serverStatus.displayServers[index], info,
					MAX_STRING_CHARS);
				lastColumn = column;
				lastTime = uiInfo.uiDC.realTime;
			}
			const int ping = atoi(Info_ValueForKey(info, "ping"));
			if (ping == -1)
			{
			...
			case SORT_HOST:
			{
				...
				const int gametype = atoi(Info_ValueForKey(info, "gametype"));
				//check for password
				if (atoi(Info_ValueForKey(info, "needpass")))
				{
				...
				if (gametype != GT_JEDIMASTER)
				{
					...
					//check force
					const qboolean restrictedForce = atoi(Info_ValueForKey(info, "fdisable"));
					...
					//check weaps
					const int wDisable = atoi(Info_ValueForKey(info, "wdisable"));
	...
	else if (feederID == FEEDER_COLORCHOICES)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].ColorCount)
		{
			*handle1 = trap->R_RegisterShaderNoMip(
				uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Color[index].shader);
	...
	else if (feederID == FEEDER_PLAYER_SKIN_HEAD)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount)
		{
			*handle1 = trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s",
				...
				uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHead[
					index].name));
	...
	else if (feederID == FEEDER_PLAYER_SKIN_TORSO)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorsoCount)
		{
			*handle1 = trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s",
				uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
				uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorso[
					index].name));
	...
	else if (feederID == FEEDER_PLAYER_SKIN_LEGS)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLegCount)
		{
			*handle1 = trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s",
				uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
				uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLeg[index]
				.name));
...
static qhandle_t UI_FeederItemImage(float feederID, int index)
{
	...
	if (feederID == FEEDER_Q3HEADS)
	{
		...
		if (index >= 0 && index < uiInfo.q3HeadCount)
		{
			...
			if (!uiInfo.q3HeadIcons[index])
			{
				...
				int i1 = strlen(uiInfo.q3HeadNames[index]);

				while (uiInfo.q3HeadNames[index][i1] != '/')
				{
					i1--;
				}

				i1++;
				int skinPlace = i1; //remember that this is where the skin name begins

				//now, build a full path out of what's in q3HeadNames, into iconNameFromSkinName
				Com_sprintf(iconNameFromSkinName, sizeof iconNameFromSkinName, "models/players/%s",
					uiInfo.q3HeadNames[index]);

				i1 = strlen(iconNameFromSkinName);

				while (iconNameFromSkinName[i1] != '/')
				{
					i1--;
				}

				i1++;
				iconNameFromSkinName[i1] = 0; //terminate, and append..
				...
				{
					iconNameFromSkinName[i1] = uiInfo.q3HeadNames[index][skinPlace];
					i1++;
					skinPlace++;
				}
				iconNameFromSkinName[i1] = 0;
	...
	{
		const int team = (int)trap->Cvar_VariableValue("ui_team");
		const int baseClass = (int)trap->Cvar_VariableValue("ui_siege_class");
	...
	else if (feederID == FEEDER_SIEGE_CLASS_FORCE)
	{
		...
		if (menu)
		{
			...
			if (item)
			{
				const listBoxDef_t* listPtr = item->typeData.listbox;
				if (listPtr)
				{
...
qboolean UI_FeederSelection(float feederFloat, int index, itemDef_t* item)
{
	...
	else if (feederID == FEEDER_MOVES)
	{
		...
		if (menu)
		{
			...
			if (item_def_s)
			{
				const modelDef_t* modelPtr = item_def_s->typeData.model;
				if (modelPtr)
				{
					char modelPath[MAX_QPATH];
					int animRunLength;
					...
					Com_sprintf(modelPath, sizeof modelPath, "models/players/%s/model.glm",
						UI_Cvar_VariableString("ui_char_model"));
					ItemParse_asset_model_go(item_def_s, modelPath, &animRunLength);
					UI_UpdateCharacterSkin();
					...
					UI_SaberAttachToChar(item_def_s);
				}
			}
		}
	}
	...
	else if (feederID == FEEDER_MAPS || feederID == FEEDER_ALLMAPS)
	{
		...
		const int map = feederID == FEEDER_ALLMAPS ? ui_currentNetMap.integer : ui_currentMap.integer;
		if (uiInfo.mapList[map].cinematic >= 0)
		{
			trap->CIN_StopCinematic(uiInfo.mapList[map].cinematic);
			uiInfo.mapList[map].cinematic = -1;
		}
		const char* checkValid = UI_SelectedMap(index, &actual);
		...
		if (feederID == FEEDER_MAPS)
		{
			...
			trap->Cvar_Update(&ui_currentMap);
			uiInfo.mapList[ui_currentMap.integer].cinematic = trap->CIN_PlayCinematic(
				va("%s.roq", uiInfo.mapList[ui_currentMap.integer].mapLoadName), 0, 0, 0, 0,
				CIN_loop | CIN_silent);
		}
		else
		{
			...
			trap->Cvar_Update(&ui_currentNetMap);
			uiInfo.mapList[ui_currentNetMap.integer].cinematic = trap->CIN_PlayCinematic(
				va("%s.roq", uiInfo.mapList[ui_currentNetMap.integer].mapLoadName), 0, 0, 0, 0,
				CIN_loop | CIN_silent);
		}
	}
	else if (feederID == FEEDER_SERVERS)
	{
		...
		trap->LAN_GetServerInfo(UI_SourceForLAN(), uiInfo.serverStatus.displayServers[index], info,
			MAX_STRING_CHARS);
		uiInfo.serverStatus.currentServerPreview = trap->R_RegisterShaderNoMip(
		...
		if (mapName && *mapName)
		{
			uiInfo.serverStatus.currentServerCinematic = trap->CIN_PlayCinematic(
	...
	else if (feederID == FEEDER_PLAYER_SKIN_HEAD)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount)
		{
			trap->Cvar_Set("ui_char_skin_head",
	...
	else if (feederID == FEEDER_PLAYER_SKIN_TORSO)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorsoCount)
		{
			trap->Cvar_Set("ui_char_skin_torso",
	...
	else if (feederID == FEEDER_PLAYER_SKIN_LEGS)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLegCount)
		{
			trap->Cvar_Set("ui_char_skin_legs",
...
static qboolean GameType_Parse(char** p, qboolean join)
{
	...
	while (1)
	{
		...
		if (token[0] == '{')
		{
			...
			if (join)
			{
				if (!String_Parse(p, &uiInfo.joinGameTypes[uiInfo.numJoinGameTypes].gameType) || !Int_Parse(
					p, &uiInfo.joinGameTypes[uiInfo.numJoinGameTypes].gtEnum))
				{
					return qfalse;
				}
			}
			...
			{
				if (!String_Parse(p, &uiInfo.gameTypes[uiInfo.numGameTypes].gameType) || !Int_Parse(
					p, &uiInfo.gameTypes[uiInfo.numGameTypes].gtEnum))
				{
					return qfalse;
				}
			}
...
static qboolean MapList_Parse(char** p)
{
	...
	while (1)
	{
		...
		if (token[0] == '{')
		{
			if (!String_Parse(p, &uiInfo.mapList[uiInfo.mapCount].mapName) || !String_Parse(
				p, &uiInfo.mapList[uiInfo.mapCount].mapLoadName)
			...
			uiInfo.mapList[uiInfo.mapCount].levelShot = trap->R_RegisterShaderNoMip(
				va("levelshots/%s_small", uiInfo.mapList[uiInfo.mapCount].mapLoadName));
...
void UI_LoadForceConfig_List(void)
{
	...
	Com_sprintf(uiInfo.forceConfigNames[uiInfo.forceConfigCount],
		sizeof uiInfo.forceConfigNames[uiInfo.forceConfigCount], "Custom");
...
nextSearch:
	if (lightSearch)
	{
		...
		numfiles = trap->FS_GetFileList("Account/Rebels", "fcf", filelist, 2048);
		uiInfo.forceConfigLightIndexBegin = uiInfo.forceConfigCount - 1;
	}
	else
	{
		//search dark side folder
		numfiles = trap->FS_GetFileList("Account/Empire", "fcf", filelist, 2048);
		uiInfo.forceConfigDarkIndexBegin = uiInfo.forceConfigCount - 1;
	}

	char* fileptr = filelist;

	for (int j = 0; j < numfiles && uiInfo.forceConfigCount < MAX_FORCE_CONFIGS; j++, fileptr += filelen + 1)
	{
		char configname[128];
		...
		Com_sprintf(uiInfo.forceConfigNames[uiInfo.forceConfigCount],
			sizeof uiInfo.forceConfigNames[uiInfo.forceConfigCount], configname);
		uiInfo.forceConfigCount++;
	}
...
static void UI_Init(qboolean inGameLoad)
{
	...
	{
		// wide screen
		uiInfo.uiDC.bias = 0.5 * (uiInfo.uiDC.glconfig.vidWidth - uiInfo.uiDC.glconfig.vidHeight * (640.0 /
			480.0));
	}
	else
	{
	...
	int i_menu_font)
{
	...
	const vec4_t colorLtGreyAlpha = { 0, 0, 0, .5 };

	UI_FillRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, colorLtGreyAlpha);

	const char* s = GetCRDelineatedString("MENUS", "DOWNLOAD_STUFF", 0); // "Downloading:"
	...
	const int downloadSize = trap->Cvar_VariableValue("cl_downloadSize");
	const int downloadCount = trap->Cvar_VariableValue("cl_downloadCount");
	const int downloadTime = trap->Cvar_VariableValue("cl_downloadTime");

	const int leftWidth = 320;

	UI_SetColor(colorWhite);

	Text_PaintCenter(centerPoint, yStart + 112, scale, colorWhite, sDownLoading, 0, i_menu_font);
	Text_PaintCenter(centerPoint, yStart + 192, scale, colorWhite, sEstimatedTimeLeft, 0, i_menu_font);
	Text_PaintCenter(centerPoint, yStart + 248, scale, colorWhite, sTransferRate, 0, i_menu_font);
	...
	Text_PaintCenter(centerPoint, yStart + 136, scale, colorWhite, s, 0, i_menu_font);
	...
	{
		Text_PaintCenter(leftWidth, yStart + 216, scale, colorWhite, "estimating", 0, i_menu_font);
		Text_PaintCenter(leftWidth, yStart + 160, scale, colorWhite,
			va("(%s %s %s %s)", dlSizeBuf, sOf, totalSizeBuf, sCopied), 0, i_menu_font);
	}
	else
	{
		int xferRate;
		...
		{
			char dlTimeBuf[64];
			const int n = downloadSize / xferRate; // estimated time for entire d/l in secs

			// We do it in K (/1024) because we'd overflow around 4MB
			UI_PrintTime(dlTimeBuf, sizeof dlTimeBuf,
				(n - downloadCount / 1024 * n / (downloadSize / 1024)) * 1000);

			Text_PaintCenter(leftWidth, yStart + 216, scale, colorWhite, dlTimeBuf, 0, i_menu_font);
			Text_PaintCenter(leftWidth, yStart + 160, scale, colorWhite,
				va("(%s %s %s %s)", dlSizeBuf, sOf, totalSizeBuf, sCopied), 0, i_menu_font);
		}
		else
		{
			...
			{
				Text_PaintCenter(leftWidth, yStart + 160, scale, colorWhite, va("(%s %s)", dlSizeBuf, sCopied), 0,
					i_menu_font);
			}
		}
		...
		if (xferRate)
		{
			Text_PaintCenter(leftWidth, yStart + 272, scale, colorWhite, va("%s/%s", xferRateBuf, sSec), 0,
				i_menu_font);
		}
	}
}
...
static void UI_DrawConnectScreen(qboolean overlay)
{
	...
	if (trap->GetConfigString(CS_SERVERINFO, info, sizeof info))
	{
		...
		Text_PaintCenter(centerPoint, yStart, scale, colorWhite,
			va(/*"Loading %s"*/sStringEdTemp, Info_ValueForKey(info, "mapname")), 0, FONT_SMALL2);
	}
...
static void UI_StopServerRefresh(void)
{
	...
	const int count = trap->LAN_GetServerCount(UI_SourceForLAN());
...
static void UI_StartServerRefresh(qboolean full)
{
	qtime_t q;
	trap->RealTime(&q);
	trap->Cvar_Set(va("ui_lastServerRefresh_%i", ui_netSource.integer),
		va("%s-%i, %i @ %i:%02i", GetMonthAbbrevString(q.tm_mon), q.tm_mday, 1900 + q.tm_year, q.tm_hour,
			q.tm_min));
	...
	if (ui_netSource.integer >= UIAS_GLOBAL1 && ui_netSource.integer <= UIAS_GLOBAL5)
	{
		char* ptr = UI_Cvar_VariableString("debug_protocol");
		if (strlen(ptr))
		{
		...
		else
		{
			trap->Cmd_ExecuteText(EXEC_NOW, va("globalservers %d %d full empty\n", ui_netSource.integer - 1,
				(int)trap->Cvar_VariableValue("protocol")));
		}
	}
}
