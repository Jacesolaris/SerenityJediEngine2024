...
typedef struct uiImport_s {
	...
	int				(*R_LerpTag)							(orientation_t* tag, clip_handle_t mod, int start_frame, int end_frame, float frac, const char* tagName);
	...
	qboolean(*G2API_AttachG2Model)					(void* ghoul2_from, int modelIndexFrom, void* ghoul2_to, int toBoltIndex, int toModel);
