/*
===========================================================================
Copyright (C) 2013 - 2016, SerenityJediEngine2024 contributors
...
static int SV_CreateChallenge(int timestamp, netadr_t from)
{
	...
	HMAC_MD5_Update(&challenger, reinterpret_cast<byte*>(&timestamp), sizeof timestamp);
	...
	challenge |= static_cast<unsigned>(timestamp & 0x1) << 31;
...
int SV_CreateChallenge(const netadr_t from)
{
	...
	const int currentTimestamp = svs.time >> 14;
...
qboolean SV_VerifyChallenge(int receivedChallenge, netadr_t from)
{
	...
	const int currentTimestamp = svs.time >> 14;
	const int currentPeriod = currentTimestamp & 0x1;
	...
	const int challengePeriod = static_cast<unsigned>(receivedChallenge) >> 31 & 0x1;
	...
	const int expectedChallenge = SV_CreateChallenge(challengeTimestamp, from);
	return static_cast<qboolean>(receivedChallenge == expectedChallenge);
}