...
void SV_Startup(void)
{
	...
	svs.clients = static_cast<client_s*>(Z_Malloc(sizeof(client_t) * sv_maxclients->integer, TAG_CLIENTS, qtrue));
...
void SV_ChangeMaxClients(void)
{
	...
	const int oldMaxClients = sv_maxclients->integer;
	...
	const auto oldClients = static_cast<client_s*>(Hunk_AllocateTempMemory(count * sizeof(client_t)));
	...
	svs.clients = static_cast<client_s*>(Z_Malloc(sv_maxclients->integer * sizeof(client_t), TAG_CLIENTS, qtrue));
	...
	*/
	...
	for (i = 0; i < sv_maxclients->integer; i++)
	{
		...
		if (svs.clients[i].state >= CS_CONNECTED)
		{
			...
			const char* denied = GVM_ClientConnect(i, qfalse, isBot); // firstTime = qfalse
			if (denied)
			{
...
#define G2_VERT_SPACE_SERVER_SIZE 2048
...
void SV_Shutdown(char* finalmsg)
{
	...
	*/
	...
	SV_ClearServer();
	CM_ClearMap();
	//jfm: add a clear here since it's commented out in clearServer.  This prevents crashing cmShaderTable on exit.
