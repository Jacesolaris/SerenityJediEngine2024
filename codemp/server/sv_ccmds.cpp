...
static client_t* SV_GetPlayerByHandle()
{
	...
	for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++)
	{
		...
		Q_strncpyz(clean_name, cl->name, sizeof clean_name);
		Q_StripColor(clean_name);
		//Q_CleanStr( cleanName );
		if (!Q_stricmp(clean_name, s))
		{
			return cl;
		}
	}
...
static void SV_Map_f(void)
{
	...
	// start up the map
	SV_SpawnServer(mapname, kill_bots, eForceReload);
...
static void SV_MapRestart_f(void)
{
	...
	for (i = 0; i < sv_maxclients->integer; i++)
	{
		...
		const char* denied = GVM_ClientConnect(i, qfalse, isBot);
		if (denied)
		{
...
static void SV_RehashBans_f(void)
{
	...
	if ((filelen = FS_SV_FOpenFileRead(filepath, &readfrom)) >= 0)
	{
		...
		char* curpos = textbuf = static_cast<char*>(Z_Malloc(filelen, TAG_TEMP_WORKSPACE));
		...
		for (index = 0; index < SERVER_MAXBANS && curpos + 2 < endpos; index++)
		{
			...
			*newlinepos = '\0';
			...
			{
				serverBans[index].isexception = static_cast<qboolean>(curpos[0] != '0');
...
static void SV_AddBanToList(const qboolean isexception)
{
	...
	else
	{
		...
		const client_t* cl = SV_GetPlayerByNum();
	...
	while (index < serverBansCount)
	{
		...
		if (curban->subnet > mask && (!curban->isexception || isexception) && NET_CompareBaseAdrMask(
			curban->ip, ip, mask))
...
static void SV_DumpUser_f(void)
{
	...
	const client_t* cl = SV_GetPlayerByHandle();
	if (!cl)
	{
		return;
	}
...
void SV_StopRecord_f(void)
{
	...
	if (Cmd_Argc() == 2)
	{
		const int clIndex = atoi(Cmd_Argv(1));
...
static time_t SV_ExtractTimeFromDemoFolder(char* folder)
{
	...
	const size_t timeLen = strlen("0000-00-00_00-00-00");
...
{
	const auto left = (char*)arg1, right = (char*)arg2;
	const time_t leftTime = SV_ExtractTimeFromDemoFolder(left);
	const time_t rightTime = SV_ExtractTimeFromDemoFolder(right);
...
static int SV_FindLeafFolders(const char* baseFolder, char* result, const int maxResults, const int maxFolderLength)
{
	...
	for (int i = 0; i < numFiles; i++)
	{
		if (Q_stricmp(fileName, ".") && Q_stricmp(fileName, ".."))
		{
			...
			const int newResults =
				SV_FindLeafFolders(fullFolder, nextResult, maxResults - resultCount, maxFolderLength);
...
static void SV_Record_f(void)
{
	...
	if (Cmd_Argc() == 3)
	{
		const int clIndex = atoi(Cmd_Argv(2));
		if (clIndex < 0 || clIndex >= sv_maxclients->integer)
		{
			Com_Printf("Unknown client number %d.\n", clIndex);
			return;
