...
char* GVM_ClientConnect(const int client_num, const qboolean firstTime, const qboolean isBot)
{
	if (gvm->isLegacy)
		return reinterpret_cast<char*>(VM_Call(gvm, GAME_CLIENT_CONNECT, client_num, firstTime, isBot));
	VMSwap v(gvm);

	return ge->ClientConnect(client_num, firstTime, isBot);
}

void GVM_ClientBegin(const int client_num)
{
	if (gvm->isLegacy)
	{
...
void GVM_ClientDisconnect(const int client_num)
{
	if (gvm->isLegacy)
	{
...
void GVM_ClientCommand(const int client_num)
{
	if (gvm->isLegacy)
	{
...
void GVM_ClientThink(const int client_num, usercmd_t* ucmd)
{
	if (gvm->isLegacy)
	{
...
void GVM_ROFF_NotetrackCallback(const int ent_id, const char* notetrack)
{
	if (gvm->isLegacy)
	{
		VM_Call(gvm, GAME_ROFF_NOTETRACK_CALLBACK, ent_id, reinterpret_cast<intptr_t>(notetrack));
		return;
	}
	VMSwap v(gvm);

	ge->ROFF_NotetrackCallback(ent_id, notetrack);
}
...
qboolean GVM_NAV_ClearPathToPoint(const int ent_id, vec3_t pmins, vec3_t pmaxs, vec3_t point, const int clipmask,
	const int okToHitEnt)
{
	if (gvm->isLegacy)
		return static_cast<qboolean>(VM_Call(gvm, GAME_NAV_CLEARPATHTOPOINT, ent_id, reinterpret_cast<intptr_t>(pmins),
			reinterpret_cast<intptr_t>(pmaxs), reinterpret_cast<intptr_t>(point),
			clipmask,
			okToHitEnt));
	VMSwap v(gvm);

	return ge->NAV_ClearPathToPoint(ent_id, pmins, pmaxs, point, clipmask, okToHitEnt);
}

qboolean GVM_NPC_ClearLOS2(const int ent_id, const vec3_t end)
{
	if (gvm->isLegacy)
		return static_cast<qboolean>(VM_Call(gvm, GAME_NAV_CLEARLOS, ent_id, reinterpret_cast<intptr_t>(end)));
	VMSwap v(gvm);

	return ge->NPC_ClearLOS2(ent_id, end);
}
...
qboolean GVM_NAV_CheckNodeFailedForEnt(const int ent_id, const int nodeNum)
{
	if (gvm->isLegacy)
		return static_cast<qboolean>(VM_Call(gvm, GAME_NAV_CHECKNODEFAILEDFORENT, ent_id, nodeNum));
	VMSwap v(gvm);
...
void Q3_SetVar(int taskID, int ent_id, const char* type_name, const char* data);
...
static void SV_GameDropClient(const int client_num, const char* reason)
{
...
static void SV_GameSendServerCommand(const int client_num, const char* text)
{
	if (client_num == -1)
	{
...
static void SV_SetBrushModel(sharedEntity_t* ent, const char* name)
{
	...
	if (name[0] == '*')
	{
		ent->s.model_index = atoi(name + 1);

		if (sv.mLocalSubBSPIndex != -1)
		{
			ent->s.model_index += sv.mLocalSubBSPModelOffset;
		}

		h = CM_InlineModel(ent->s.model_index);
	...
	{
		ent->s.model_index = CM_LoadSubBSP(va("maps/%s.bsp", name + 1), qfalse);
		CM_ModelBounds(ent->s.model_index, mins, maxs);
		...
		h = CM_InlineModel(ent->s.model_index);
		ent->r.contents = CM_ModelContents(h, CM_FindSubBSP(ent->s.model_index));
	}
	else
	{
...
static void SV_GetUsercmd(const int client_num, usercmd_t* cmd)
{
	if (client_num < 0 || client_num >= sv_maxclients->integer)
	{
...
qboolean SV_ROFF_Play(const int ent_id, const int roff_id, const qboolean do_translation)
{
	return theROFFSystem.Play(ent_id, roff_id, do_translation, qfalse);
}

qboolean SV_ROFF_Purge_Ent(const int ent_id)
{
	return theROFFSystem.PurgeEnt(ent_id, qfalse);
}
...
static qboolean ICARUS_IsInitialized(const int ent_id)
{
	if (!gSequencers[ent_id] || !gTaskManagers[ent_id])
		return qfalse;

	return qtrue;
}

static qboolean ICARUS_MaintainTaskManager(const int ent_id)
{
	if (gTaskManagers[ent_id])
	{
		gTaskManagers[ent_id]->Update();
		return qtrue;
	}
	return qfalse;
}

static qboolean ICARUS_IsRunning(const int ent_id)
{
	if (!gTaskManagers[ent_id] || !gTaskManagers[ent_id]->IsRunning())
		return qfalse;
	return qtrue;
}
...
static void SV_Nav_AddFailedEdge(const int ent_id, const int startID, const int endID)
{
	navigator.AddFailedEdge(ent_id, startID, endID);
}
...
static int SV_BotGetServerCommand(const int client_num, char* message, const int size)
{
	return SV_BotGetConsoleMessage(client_num, message, size);
}

static void SV_BotUserCommand(const int client_num, usercmd_t* ucmd)
{
...
static void SV_G2API_GetGLAName(void* ghoul2, const int model_index, char* fillBuf)
{
...
static int SV_G2API_AddBolt(void* ghoul2, const int model_index, const char* bone_name)
{
...
static qboolean SV_G2API_SetSurfaceOnOff(void* ghoul2, const char* surface_name, const int flags)
{
...
static qboolean SV_G2API_DoesBoneExist(void* ghoul2, const int model_index, const char* bone_name)
{
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	return re->G2API_DoesBoneExist(g2, model_index, bone_name);
}

static int SV_G2API_GetSurfaceRenderStatus(void* ghoul2, const int model_index, const char* surface_name)
{
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
...
static qboolean SV_G2API_RagPCJGradientSpeed(void* ghoul2, const char* bone_name, const float speed)
{
	return re->G2API_RagPCJGradientSpeed(*static_cast<CGhoul2Info_v*>(ghoul2), bone_name, speed);
}

static qboolean SV_G2API_RagEffectorGoal(void* ghoul2, const char* bone_name, vec3_t pos)
{
	return re->G2API_RagEffectorGoal(*static_cast<CGhoul2Info_v*>(ghoul2), bone_name, pos);
}

static qboolean SV_G2API_GetRagBonePos(void* ghoul2, const char* bone_name, vec3_t pos, vec3_t entAngles, vec3_t entPos,
	vec3_t entScale)
{
	return re->G2API_GetRagBonePos(*static_cast<CGhoul2Info_v*>(ghoul2), bone_name, pos, entAngles, entPos, entScale);
}

static qboolean SV_G2API_RagEffectorKick(void* ghoul2, const char* bone_name, vec3_t velocity)
{
...
static qboolean SV_G2API_SetBoneIKState(void* ghoul2, const int time, const char* bone_name, const int ik_state,
...
static qboolean SV_G2API_RemoveBone(void* ghoul2, const char* bone_name, const int model_index)
{
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
...
static void SV_G2API_GetSurfaceName(void* ghoul2, const int surfNumber, const int model_index, char* fillBuf)
{
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
...
intptr_t SV_GameSystemCalls(intptr_t* args)
{
	...
	case G_G2_SETSKIN:
	{
		...
		const int model_index = args[2];

		return re->G2API_SetSkin(g2, model_index, args[3], args[4]);
	}
	...
	{
		CGhoul2Info_v& g2 = *reinterpret_cast<CGhoul2Info_v*>(args[1]);
		const int model_index = args[10];

		return re->G2API_GetBoneAnim(g2, model_index, static_cast<const char*>(VMA(2)), args[3],
	...
	case G_G2_GETSURFACENAME:
	{
		...
		const int model_index = args[3];

		CGhoul2Info_v& g2 = *reinterpret_cast<CGhoul2Info_v*>(args[1]);

		const char* local = re->G2API_GetSurfaceName(g2, model_index, args[2]);
		if (local)
		{
			strcpy(point, local);
		}
