...
typedef enum //# setType_e
{
	...
	SET_RENDER_CULL_RADIUS, //## %f="40.0" # Used to ensure rendering for entities with
	//geographically sprawling animations (world units)
//# #sep ints
...
SET_NO_PVS_CULL,//## %t="BOOL_TYPES" # Turn on/off SVF_broadcast to make sure that the
//affected entity will or will not always be seen by clients.
...
SET_BEHAVIOR_STATE,//## %t="BSTATE_STRINGS" # Change current b_state
SET_DEFAULT_BSTATE,//## %t="BSTATE_STRINGS" # Change fallback b_state
SET_TEMP_BSTATE,//## %t="BSTATE_STRINGS" # Set/Chang a temp b_state
...
SET_FORCE_TEAM_HEAL,	//not in SP, but needed to get the set_force code to work
SET_FORCE_TEAM_FORCE,	//not in SP, but needed to get the set_force code to work
...
SET_OBJECTIVE_CLEARALL,	//## # Force all objectives to be hidde
...
void ParseTags(int ent_id, const char* data);
...
void Q3_SetBroadcast(int ent_id, qboolean broadcast);
void Q3_SetRenderCullRadius(int ent_id, float float_data);
void Q3_SetForcePower(int ent_id, int forcePower, qboolean powerOn);
...
extern	qboolean	Q3_TaskIDPending(const sharedEntity_t* ent, taskID_t taskType);
