...
=============
*/
static int Q3_PlaySound(const int taskID, const int ent_id, const char* name, const char* channel)
{
	const auto sharedMem = reinterpret_cast<T_G_ICARUS_PLAYSOUND*>(sv.mSharedMemory);

	sharedMem->taskID = taskID;
	sharedMem->ent_id = ent_id;
...
============
*/
static void Q3_Set(const int taskID, const int ent_id, const char* type_name, const char* data)
{
	...
	if (GVM_ICARUS_Set())
	{
		gTaskManagers[ent_id]->Completed(taskID);
	}
}
...
void Q3_DebugPrint(const int level, const char* format, ...)
{
	...
	case WL_DEBUG:
	{
		...
		Com_Printf(S_COLOR_BLUE"DEBUG: %s(%d): %s\n", SV_Gentity_num(ent_num)->script_targetname, ent_num, buffer);
		break;
	}
	default:
	case WL_VERBOSE:
...
int ICARUS_LinkEntity(int ent_id, CSequencer* sequencer, CTaskManager* task_manager);
...
static void Q3_Lerp2Pos(const int taskID, const int ent_id, vec3_t origin, vec3_t angles, const float duration)
{
...
static void Q3_Lerp2Origin(const int taskID, const int ent_id, vec3_t origin, const float duration)
{
...
static void Q3_Lerp2Angles(const int taskID, const int ent_id, vec3_t angles, const float duration)
{
...
static int Q3_GetTag(const int ent_id, const char* name, const int lookup, vec3_t info)
{
	...
	const int r = GVM_ICARUS_GetTag();
	VectorCopy(sharedMem->info, info);
	return r;
}

static void Q3_Lerp2Start(const int ent_id, const int taskID, const float duration)
{
...
static void Q3_Lerp2End(const int ent_id, const int taskID, const float duration)
{
...
static void Q3_Use(const int ent_id, const char* target)
{
...
static void Q3_Kill(const int ent_id, const char* name)
{
...
static void Q3_Play(const int taskID, const int ent_id, const char* type, const char* name)
{
...
static int Q3_GetFloat(const int ent_id, const int type, const char* name, float* value)
{
...
static int Q3_GetVector(const int ent_id, const int type, const char* name, vec3_t value)
{
	...
	const int r = GVM_ICARUS_GetVector();
	VectorCopy(sharedMem->value, value);
	return r;
}

static int Q3_GetString(const int ent_id, const int type, const char* name, char** value)
{
	...
	const int r = GVM_ICARUS_GetString();
