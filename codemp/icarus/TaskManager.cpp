...
qboolean CTaskManager::IsRunning(void) const
{
...
int CTaskManager::GetFloat(const int ent_id, const CBlock* block, int& memberNum, float& value) const
{
	...
	if (Check(ID_GET, block, memberNum))
	{
		...
		return m_owner->GetInterface()->I_GetFloat(ent_id, type, name, &value);
	}
	...
	if (bm->GetID() == TK_INT)
	{
		value = static_cast<float>(*static_cast<int*>(block->GetMemberData(memberNum++)));
	}
	else if (bm->GetID() == TK_FLOAT)
	{
		value = *static_cast<float*>(block->GetMemberData(memberNum++));
	}
	else
	{
		assert(0);
...
int CTaskManager::GetVector(const int ent_id, CBlock* block, int& memberNum, vector_t& value)
{
	int type, i;
	...
	if (Check(ID_GET, block, memberNum))
	{
		...
		return m_owner->GetInterface()->I_GetVector(ent_id, type, name, value);
	}
	...
	if (Check(ID_TAG, block, memberNum))
	{
		...
		memberNum++;
		ICARUS_VALIDATE(Get(ent_id, block, memberNum, &tagName));
		ICARUS_VALIDATE(GetFloat(ent_id, block, memberNum, tagLookup));

		if (m_owner->GetInterface()->I_GetTag(ent_id, tagName, static_cast<int>(tagLookup), value) == false)
		{
	...
	for (i = 0; i < 3; i++)
	{
		if (GetFloat(ent_id, block, memberNum, value[i]) == false)
			return false;
	}
...
int CTaskManager::Get(const int ent_id, CBlock* block, int& memberNum, char** value)
{
	...
	if (Check(ID_GET, block, memberNum))
	{
		...
		{
		case TK_STRING:
			if (m_owner->GetInterface()->I_GetString(ent_id, type, name, value) == false)
			{
		...
		case TK_FLOAT:
		{
			...
			if (m_owner->GetInterface()->I_GetFloat(ent_id, type, name, &temp) == false)
			{
				m_owner->GetInterface()->I_DPrintf(WL_ERROR, "Get() parameter \"%s\" could not be found!\n", name);
				return false;
			}
		...
		case TK_VECTOR:
		{
			...
			if (m_owner->GetInterface()->I_GetVector(ent_id, type, name, vval) == false)
			{
				m_owner->GetInterface()->I_DPrintf(WL_ERROR, "Get() parameter \"%s\" could not be found!\n", name);
				return false;
			}
	...
	if (Check(ID_RANDOM, block, memberNum))
	{
		...
		const float ret = m_owner->GetInterface()->I_Random(min, max);
	...
	if (Check(ID_TAG, block, memberNum))
	{
		...
		if (m_owner->GetInterface()->I_GetTag(ent_id, tagName, static_cast<int>(tagLookup), vector) == false)
		{
	...
	if (bm->GetID() == TK_INT)
	{
		const float fval = static_cast<float>(*static_cast<int*>(block->GetMemberData(memberNum++)));
		Com_sprintf(tempBuffer, sizeof tempBuffer, "%f", fval);
		*value = static_cast<char*>(tempBuffer);

		return true;
	}
	if (bm->GetID() == TK_FLOAT)
	{
		const float fval = *static_cast<float*>(block->GetMemberData(memberNum++));
		Com_sprintf(tempBuffer, sizeof tempBuffer, "%f", fval);
		*value = static_cast<char*>(tempBuffer);
	...
	if (bm->GetID() == TK_VECTOR)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			if (GetFloat(ent_id, block, memberNum, vval[i]) == false)
				return false;
		}
	...
	if (bm->GetID() == TK_STRING || bm->GetID() == TK_IDENTIFIER)
	{
		*value = static_cast<char*>(block->GetMemberData(memberNum++));
...
{
	const CTask* task = PopTask(POP_BACK);
...
int CTaskManager::Rotate(CTask* task)
{
	...
	if (Check(ID_TAG, block, memberNum))
	{
		...
		if (m_owner->GetInterface()->I_GetTag(m_ownerID, tagName, static_cast<int>(tagLookup), vector) == false)
		{
			//TODO: Emit warning
			m_owner->GetInterface()->I_DPrintf(WL_ERROR, "Unable to find tag \"%s\"!\n", tagName);
			assert(0);
