...
using bstream_t = struct bstream_s
{
	CBlockStream* stream;
	bstream_s* last;
};
...
public:
	...
	int Init(int ownerID, interface_export_t* ie, CTaskManager* task_manager, ICARUS_Instance* i_icarus);
...
protected:
	...
	int AddAffect(const bstream_t* bstream, int retain, int* id);
	...
	int ParseElse(const CBlock* block, bstream_t* bstream);
	int ParseTask(const CBlock* block, bstream_t* bstream);
