...
using interface_export_t = struct interface_export_s
{
	...
	int (*I_PlaySound)(int taskID, int ent_id, const char* name, const char* channel);
	void (*I_Lerp2Pos)(int taskID, int ent_id, vec3_t origin, vec3_t angles, float duration);
	void (*I_Lerp2Origin)(int taskID, int ent_id, vec3_t origin, float duration);
	void (*I_Lerp2Angles)(int taskID, int ent_id, vec3_t angles, float duration);
	int (*I_GetTag)(int ent_id, const char* name, int lookup, vec3_t info);
	void (*I_Lerp2Start)(int taskID, int ent_id, float duration);
	void (*I_Lerp2End)(int taskID, int ent_id, float duration);
	void (*I_Set)(int taskID, int ent_id, const char* type_name, const char* data);
	void (*I_Use)(int ent_id, const char* name);
	void (*I_Kill)(int ent_id, const char* name);
	void (*I_Remove)(int ent_id, const char* name);
	float (*I_Random)(float min, float max);
	void (*I_Play)(int taskID, int ent_id, const char* type, const char* name);
	...
	void (*I_CameraFade)(float sr, float sg, float sb, float sa, float dr, float dg, float db, float da,
		float duration);
	...
	int (*I_GetFloat)(int ent_id, int type, const char* name, float* value);
	int (*I_GetVector)(int ent_id, int type, const char* name, vec3_t value);
	int (*I_GetString)(int ent_id, int type, const char* name, char** value);
	...
	int (*I_LinkEntity)(int ent_id, CSequencer* sequencer, CTaskManager* task_manager);
};
