...
bool CParticle::Cull(void)
{
	...
	// Can't be too close
	const float len = VectorLengthSquared(dir);
	...
	if (mFlags & FX_APPLY_PHYSICS && !(mFlags & FX_PLAYER_VIEW))
	{
		if (mFlags & FX_EXPENSIVE_PHYSICS)
		{
			...
			if (trace.fraction < 1.0f) //&& !trace.startsolid && !trace.allsolid )
			{
				...
				theFxScheduler.MaterialImpact(&trace, this);
				...
				const float dot = DotProduct(mVel, trace.plane.normal);
...
void CParticle::UpdateSize(void)
{
	...
	if (mFlags & FX_SIZE_LINEAR)
	{
		...
		perc1 = 1.0f - static_cast<float>(theFxHelper.mTime - mTimeStart) / static_cast<float>(mTimeEnd - mTimeStart);
	}
	...
	if ((mFlags & FX_SIZE_PARM_MASK) == FX_SIZE_NONLINEAR)
	{
		if (theFxHelper.mTime > mSizeParm)
		{
			...
			perc2 = 1.0f - (theFxHelper.mTime - mSizeParm) / (mTimeEnd - mSizeParm);
		}
...
void CParticle::UpdateRGB(void)
{
	...
	if ((mFlags & FX_RGB_PARM_MASK) == FX_RGB_NONLINEAR)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = 1.0f - (theFxHelper.mTime - mRGBParm) / (mTimeEnd - mRGBParm);
		}
...
void CParticle::UpdateAlpha(void)
{
	...
	if ((mFlags & FX_ALPHA_PARM_MASK) == FX_ALPHA_NONLINEAR)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = 1.0f - (theFxHelper.mTime - mAlphaParm) / (mTimeEnd - mAlphaParm);
		}
	...
	else if ((mFlags & FX_ALPHA_PARM_MASK) == FX_RGB_LINEAR)
	{
		if (theFxHelper.mTime < mAlphaParm)
		{
			// get percent done, using parm as the start of the non-linear fade
	...
	const int alpha = Com_Clamp(0, 255, perc1 * 255.0f);
	if (mFlags & FX_USE_ALPHA)
	{
		// should use this when using art that has an alpha channel
		mRefEnt.shaderRGBA[3] = static_cast<byte>(alpha);
	}
	else
	{
		// Modulate the rgb fields by the alpha value to do the fade, works fine for additive blending
		mRefEnt.shaderRGBA[0] = static_cast<int>(mRefEnt.shaderRGBA[0]) * alpha >> 8;
		mRefEnt.shaderRGBA[1] = static_cast<int>(mRefEnt.shaderRGBA[1]) * alpha >> 8;
		mRefEnt.shaderRGBA[2] = static_cast<int>(mRefEnt.shaderRGBA[2]) * alpha >> 8;
	}
}
...
//
//--------------------------------
COrientedParticle::COrientedParticle(void) : mNormal{}
{
...
//
//----------------------------
CElectricity::CElectricity(void) : mChaos(0)
{
...
//
//----------------------------
CTail::CTail(void) : mOldOrigin{}, mLengthStart(0), mLengthEnd(0), mLengthParm(0), mLength(0)
{
	mRefEnt.reType = RT_LINE;
}
...
void CTail::UpdateLength(void)
{
	...
	if ((mFlags & FX_LENGTH_PARM_MASK) == FX_LENGTH_NONLINEAR)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = 1.0f - (theFxHelper.mTime - mLengthParm) / (mTimeEnd - mLengthParm);
		}
...
//
//----------------------------
CCylinder::CCylinder(void) : mSize2Start(0), mSize2End(0), mSize2Parm(0)
{
...
void CCylinder::UpdateSize2(void)
{
	...
	if ((mFlags & FX_SIZE2_PARM_MASK) == FX_SIZE2_NONLINEAR)
	{
		...
		{
			// get percent done, using parm as the start of the non-linear fade
			perc2 = 1.0f - (theFxHelper.mTime - mSize2Parm) / (mTimeEnd - mSize2Parm);
		}
	...
	else if ((mFlags & FX_SIZE2_PARM_MASK) == FX_PAPER_PHYSICS)
	{
		if (theFxHelper.mTime < mSize2Parm)
		{
			// get percent done, using parm as the start of the non-linear fade
...
void CEmitter::Draw(void)
{
	...
	if (mFlags & FX_EMIT_FX)
	{
		...
		float step = mDensity + flrand(-mVariance, mVariance);
		step *= step;
...
//
//--------------------------
constexpr auto NEW_MUZZLE = 0;
constexpr auto NEW_TIP = 1;
constexpr auto OLD_TIP = 2;
constexpr auto OLD_MUZZLE = 3;
...
bool CTrail::Update()
{
	...
	const float perc = static_cast<float>(mTimeEnd - theFxHelper.mTime) / static_cast<float>(mTimeEnd - mTimeStart);

	for (auto& mVert : mVerts)
	{
		...
		mVert.curST[0] = mVert.ST[0] * perc + mVert.destST[0] * (1.0f - perc);
		if (mVert.curST[0] > 1.0f)
		{
			mVert.curST[0] = 1.0f;
		}
		mVert.curST[1] = mVert.ST[1] * perc + mVert.destST[1] * (1.0f - perc);
	}
...
{
	// rotate around Z
	float rad = DEG2RAD(mRotDelta[YAW] * theFxHelper.mFrameTime * 0.01f);
	const float cosZ = cosf(rad);
	const float sinZ = sinf(rad);
	// rotate around X
...
void CPoly::PolyInit(void)
{
	...
	VectorScale(org, 1.0f / mCount, org);
...
{
	const float ftime = theFxHelper.mFrameTime * 0.001f;
	const float time2 = ftime * ftime * 0.5f;
	...
	const float segPercent, const float lastSegPercent)
{
	...
	const float scaleBottom = (mSizeStart + (mSizeEnd - mSizeStart) * lastSegPercent) * 0.5f;
	const float scaleTop = (mSizeStart + (mSizeEnd - mSizeStart) * segPercent) * 0.5f;
...
bool FX_WorldToScreen(vec3_t worldCoord, float* x, float* y)
{
	...
	const float xzi = xcenter / transformed[2] * (90.0 / theFxHelper.refdef->fov_x);
	const float yzi = ycenter / transformed[2] * (90.0 / theFxHelper.refdef->fov_y);
...
void CFlash::Init(void)
{
	...
	float mod = DotProduct(dif, theFxHelper.refdef->viewaxis[0]);
...
void FX_FeedTrail(const effectTrailArgStruct_t* a)
{
	...
	FX_AddPrimitive(reinterpret_cast<CEffect**>(&fx), a->mKillTime);
}
