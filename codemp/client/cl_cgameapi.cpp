...
// cgame vmMain calls
//
void CGVM_Init(const int serverMessageNum, const int serverCommandSequence, const int client_num)
{
	if (cgvm->isLegacy)
	{
...
qboolean CGVM_ConsoleCommand(void)
{
	if (cgvm->isLegacy)
	{
		return static_cast<qboolean>(VM_Call(cgvm, CG_CONSOLE_COMMAND));
	}
	VMSwap v(cgvm);
...
void CGVM_DrawActiveFrame(const int serverTime, const stereoFrame_t stereoView, const qboolean demoPlayback)
{
	if (cgvm->isLegacy)
	{
...
void CGVM_KeyEvent(const int key, const qboolean down)
{
	if (cgvm->isLegacy)
	{
...
void CGVM_MouseEvent(const int x, const int y)
{
	if (cgvm->isLegacy)
	{
...
void CGVM_EventHandling(const int type)
{
	if (cgvm->isLegacy)
	{
...
int CGVM_RagCallback(const int callType)
{
	if (cgvm->isLegacy)
	{
...
qboolean CGVM_IncomingConsoleCommand(void)
{
	if (cgvm->isLegacy)
	{
		return static_cast<qboolean>(VM_Call(cgvm, CG_INCOMING_CONSOLE_COMMAND));
	}
	VMSwap v(cgvm);
...
qboolean CGVM_NoUseableForce(void)
{
	if (cgvm->isLegacy)
	{
		return static_cast<qboolean>(VM_Call(cgvm, CG_GET_USEABLE_FORCE));
	}
	VMSwap v(cgvm);
...
void CGVM_GetOrigin(const int ent_id, vec3_t out)
{
	if (cgvm->isLegacy)
	{
		VM_Call(cgvm, CG_GET_ORIGIN, ent_id, reinterpret_cast<intptr_t>(out));
		return;
	}
	VMSwap v(cgvm);

	cge->GetOrigin(ent_id, out);
}

void CGVM_GetAngles(const int ent_id, vec3_t out)
{
	if (cgvm->isLegacy)
	{
		VM_Call(cgvm, CG_GET_ANGLES, ent_id, reinterpret_cast<intptr_t>(out));
		return;
	}
	VMSwap v(cgvm);

	cge->GetAngles(ent_id, out);
}

trajectory_t* CGVM_GetOriginTrajectory(const int ent_id)
{
	if (cgvm->isLegacy)
	{
		return reinterpret_cast<trajectory_t*>(VM_Call(cgvm, CG_GET_ORIGIN_TRAJECTORY, ent_id));
	}
	VMSwap v(cgvm);

	return cge->GetOriginTrajectory(ent_id);
}

trajectory_t* CGVM_GetAngleTrajectory(const int ent_id)
{
	if (cgvm->isLegacy)
	{
		return reinterpret_cast<trajectory_t*>(VM_Call(cgvm, CG_GET_ANGLE_TRAJECTORY, ent_id));
	}
	VMSwap v(cgvm);

	return cge->GetAngleTrajectory(ent_id);
}

void CGVM_ROFF_NotetrackCallback(const int ent_id, const char* notetrack)
{
	if (cgvm->isLegacy)
	{
		VM_Call(cgvm, CG_ROFF_NOTETRACK_CALLBACK, ent_id, reinterpret_cast<intptr_t>(notetrack));
		return;
	}
	VMSwap v(cgvm);

	cge->ROFF_NotetrackCallback(ent_id, notetrack);
}
...
static int CL_S_GetVoiceVolume(const int ent_id)
{
	return s_entityWavVol[ent_id];
}

static void CL_S_Shutup(const qboolean shutup)
{
	s_shutUp = shutup;
}
...
static void _CL_SetUserCmdValue(const int stateValue, const float sensitivityScale, const float mPitchOverride,
	...
	const int invenSel, const qboolean fighterControls)
{
	...
	CL_SetUserCmdValue(stateValue, sensitivityScale, mPitchOverride, mYawOverride, mSensitivityOverride, fpSel,
		invenSel);
}

static void CL_OpenUIMenu(int menuID)
{
	UIVM_SetActiveMenu(static_cast<uiMenuCommand_t>(menuID));
}

static void CGFX_AddLine(vec3_t start, vec3_t end, const float size1, const float size2, const float sizeParm,
...
{
	FX_AddLine(start, end, size1, size2, sizeParm, alpha1, alpha2, alphaParm, sRGB, eRGB, rgbParm, killTime, shader,
		flags);
}
...
{
	FX_AddBezier(b->start, b->end, b->control1, b->control1Vel, b->control2, b->control2Vel, b->size1, b->size2,
		b->sizeParm, b->alpha1, b->alpha2, b->alphaParm, b->sRGB, b->eRGB, b->rgbParm, b->killTime, b->shader,
		b->flags);
}

static void CGFX_AddPrimitive(const effectTrailArgStruct_t* e)
{
	FX_FeedTrail(e);
}
...
static void CGFX_AddSprite(addspriteArgStruct_t* s)
{
	...
	FX_AddParticle(s->origin, s->vel, s->accel, s->scale, s->dscale, 0, s->sAlpha, s->eAlpha, 0, rgb, rgb, 0,
		s->rotation, 0, vec3_origin, vec3_origin, s->bounce, 0, 0, s->life, s->shader, s->flags);
}

static void CGFX_AddElectricity(addElectricityArgStruct_t* p)
{
	FX_AddElectricity(p->start, p->end, p->size1, p->size2, p->sizeParm, p->alpha1, p->alpha2, p->alphaParm, p->sRGB,
		p->eRGB, p->rgbParm, p->chaos, p->killTime, p->shader, p->flags);
}
...
static int CL_ROFF_Cache(const char* file)
{
	return theROFFSystem.Cache(file, qtrue);
}

static qboolean CL_ROFF_Play(const int ent_id, const int roff_id, const qboolean do_translation)
{
	return theROFFSystem.Play(ent_id, roff_id, do_translation, qtrue);
}

static qboolean CL_ROFF_Purge_Ent(const int ent_id)
{
	return theROFFSystem.PurgeEnt(ent_id, qtrue);
}
...
static void CL_SetClientForceAngle(const int time, vec3_t angle)
{
...
static qboolean CL_G2API_GetBoltMatrix(void* ghoul2, const int model_index, const int bolt_index, mdxaBone_t* matrix,
...
static qboolean CL_G2API_GetBoltMatrix_NoReconstruct(void* ghoul2, const int model_index, const int bolt_index,
...
static qboolean CL_G2API_GetBoltMatrix_NoRecNoRot(void* ghoul2, const int model_index, const int bolt_index,
	mdxaBone_t* matrix, const vec3_t angles, const vec3_t position,
	const int frameNum, qhandle_t* model_list, vec3_t scale)
{
	if (!ghoul2) return qfalse;
...
static qboolean CL_G2API_SetBoneAngles(void* ghoul2, const int model_index, const char* bone_name, const vec3_t angles,
...
static qboolean CL_G2API_SetBoneAnim(void* ghoul2, const int model_index, const char* bone_name, const int start_frame,
...
static qboolean CL_G2API_GetBoneAnim(void* ghoul2, const char* bone_name, const int current_time, float* current_frame,
...
static qboolean CL_G2API_GetBoneFrame(void* ghoul2, const char* bone_name, const int current_time, float* current_frame,
	int* model_list, const int model_index)
{
	...
	return re->G2API_GetBoneAnim(g2, model_index, bone_name, current_time, current_frame, &iDontCare1, &iDontCare2,
		&iDontCare3, &fDontCare1, model_list);
}

static void CL_G2API_GetGLAName(void* ghoul2, const int model_index, char* fillBuf)
{
	if (!ghoul2)
	{
		fillBuf[0] = '\0';
	...
	const char* tmp = re->G2API_GetGLAName(*static_cast<CGhoul2Info_v*>(ghoul2), model_index);
	if (tmp)
		strcpy(fillBuf, tmp);
	else
		fillBuf[0] = '\0';
...
static int CL_G2API_CopyGhoul2Instance(void* g2_from, void* g2_to, const int model_index)
{
	if (!g2_from || !g2_to) return 0;
...
static int CL_G2API_AddBolt(void* ghoul2, const int model_index, const char* bone_name)
{
	if (!ghoul2) return 0;
...
{
	if (!ghoul2) return qfalse;
	return re->G2API_SetRootSurface(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, surface_name);
}

static qboolean CL_G2API_SetSurfaceOnOff(void* ghoul2, const char* surface_name, const int flags)
{
	if (!ghoul2) return qfalse;
	return re->G2API_SetSurfaceOnOff(*static_cast<CGhoul2Info_v*>(ghoul2), surface_name, flags);
}
...
static qboolean CL_G2API_DoesBoneExist(void* ghoul2, const int model_index, const char* bone_name)
{
	if (!ghoul2) return qfalse;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	return re->G2API_DoesBoneExist(g2, model_index, bone_name);
}

static int CL_G2API_GetSurfaceRenderStatus(void* ghoul2, const int model_index, const char* surface_name)
{
	if (!ghoul2) return -1;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	return re->G2API_GetSurfaceRenderStatus(g2, model_index, surface_name);
}
...
static void CL_G2API_AbsurdSmoothing(void* ghoul2, const qboolean status)
{
	if (!ghoul2) return;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
...
static qboolean CL_G2API_RagPCJConstraint(void* ghoul2, const char* bone_name, vec3_t min, vec3_t max)
{
	if (!ghoul2) return qfalse;
	return re->G2API_RagPCJConstraint(*static_cast<CGhoul2Info_v*>(ghoul2), bone_name, min, max);
}

static qboolean CL_G2API_RagPCJGradientSpeed(void* ghoul2, const char* bone_name, const float speed)
{
	if (!ghoul2) return qfalse;
	return re->G2API_RagPCJGradientSpeed(*static_cast<CGhoul2Info_v*>(ghoul2), bone_name, speed);
}

static qboolean CL_G2API_RagEffectorGoal(void* ghoul2, const char* bone_name, vec3_t pos)
{
	if (!ghoul2) return qfalse;
	return re->G2API_RagEffectorGoal(*static_cast<CGhoul2Info_v*>(ghoul2), bone_name, pos);
}

static qboolean CL_G2API_GetRagBonePos(void* ghoul2, const char* bone_name, vec3_t pos, vec3_t entAngles, vec3_t entPos,
	vec3_t entScale)
{
	if (!ghoul2) return qfalse;
	return re->G2API_GetRagBonePos(*static_cast<CGhoul2Info_v*>(ghoul2), bone_name, pos, entAngles, entPos, entScale);
}

static qboolean CL_G2API_RagEffectorKick(void* ghoul2, const char* bone_name, vec3_t velocity)
{
	if (!ghoul2) return qfalse;
	return re->G2API_RagEffectorKick(*static_cast<CGhoul2Info_v*>(ghoul2), bone_name, velocity);
}

static qboolean CL_G2API_RagForceSolve(void* ghoul2, const qboolean force)
{
	if (!ghoul2) return qfalse;
...
static qboolean CL_G2API_RemoveBone(void* ghoul2, const char* bone_name, const int model_index)
{
	if (!ghoul2) return qfalse;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	return re->G2API_RemoveBone(g2, model_index, bone_name);
}
...
static void CL_G2API_GetSurfaceName(void* ghoul2, const int surfNumber, const int model_index, char* fillBuf)
{
	if (!ghoul2) return;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	const char* tmp = re->G2API_GetSurfaceName(g2, model_index, surfNumber);
	strcpy(fillBuf, tmp);
}
...
intptr_t CL_CgameSystemCalls(intptr_t* args)
{
	...
	case CG_PRECISIONTIMER_START:
		CL_PrecisionTimerStart(static_cast<void**>(VMA(1)));
		return 0;

	case CG_PRECISIONTIMER_END:
		return CL_PrecisionTimerEnd(reinterpret_cast<void*>(args[1]));
	...
	case CG_CVAR_SET:
		Cvar_VM_Set(static_cast<const char*>(VMA(1)), static_cast<const char*>(VMA(2)), VM_CGAME);
		return 0;
	...
	case CG_CVAR_GETHIDDENVALUE:
		return CL_GetValueForHidden(static_cast<const char*>(VMA(1)));
	...
	case CG_REMOVECOMMAND:
		Cmd_VM_RemoveCommand(static_cast<const char*>(VMA(1)), VM_CGAME);
		return 0;

	case CG_SENDCLIENTCOMMAND:
		CL_AddReliableCommand2(static_cast<const char*>(VMA(1)));
		return 0;
	...
	case CG_CM_LOADMAP:
		CL_CM_LoadMap(static_cast<const char*>(VMA(1)), static_cast<qboolean>(args[2]));
		return 0;
	...
	case CG_CM_TEMPBOXMODEL:
		return CM_TempBoxModel(static_cast<const float*>(VMA(1)), static_cast<const float*>(VMA(2)), /*int capsule*/
			qfalse);
	...
	case CG_CM_POINTCONTENTS:
		return CM_PointContents(static_cast<const float*>(VMA(1)), args[2]);
	...
	case CG_CM_TRANSFORMEDBOXTRACE:
		CM_TransformedBoxTrace(static_cast<trace_t*>(VMA(1)), static_cast<const float*>(VMA(2)),
			...
			static_cast<const float*>(VMA(9)), /*int capsule*/ qfalse);
		return 0;
	...
	case CG_CM_TRANSFORMEDCAPSULETRACE:
		CM_TransformedBoxTrace(static_cast<trace_t*>(VMA(1)), static_cast<const float*>(VMA(2)),
			static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)),
			static_cast<const float*>(VMA(5)), args[6], args[7], static_cast<const float*>(VMA(8)),
			static_cast<const float*>(VMA(9)), /*int capsule*/ qtrue);
		return 0;

	case CG_CM_MARKFRAGMENTS:
		return re->MarkFragments(args[1], static_cast<const vec3_t*>(VMA(2)), static_cast<const float*>(VMA(3)),
			args[4], static_cast<float*>(VMA(5)), args[6], static_cast<markFragment_t*>(VMA(7)));
	...
	case CG_S_STARTSOUND:
		S_StartSound(static_cast<float*>(VMA(1)), args[2], args[3], args[4]);
		return 0;
	...
	case CG_S_ADDREALLOOPINGSOUND:
		/*S_AddRealLoopingSound*/S_AddLoopingSound(args[1], static_cast<const float*>(VMA(2)),
			static_cast<const float*>(VMA(3)), args[4]);
		return 0;
	...
	case CG_S_RESPATIALIZE:
		S_Respatialize(args[1], static_cast<const float*>(VMA(2)), static_cast<vec3_t*>(VMA(3)), args[4]);
		return 0;

	case CG_S_SHUTUP:
		CL_S_Shutup(static_cast<qboolean>(args[1]));
		return 0;
	...
	case CG_AS_ADDPRECACHEENTRY:
		AS_AddPrecacheEntry(static_cast<const char*>(VMA(1)));
		return 0;
	...
	case CG_AS_GETBMODELSOUND:
		return AS_GetBModelSound(static_cast<const char*>(VMA(1)), args[2]);
	...
	case CG_R_ADDPOLYSTOSCENE:
		re->AddPolyToScene(args[1], args[2], static_cast<const polyVert_t*>(VMA(3)), args[4]);
		return 0;

	case CG_R_ADDDECALTOSCENE:
		re->AddDecalToScene(args[1], static_cast<const float*>(VMA(2)), static_cast<const float*>(VMA(3)), VMF(4),
			VMF(5), VMF(6), VMF(7), VMF(8), static_cast<qboolean>(args[9]), VMF(10),
			static_cast<qboolean>(args[11]));
		return 0;

	case CG_R_LIGHTFORPOINT:
		return re->LightForPoint(static_cast<float*>(VMA(1)), static_cast<float*>(VMA(2)), static_cast<float*>(VMA(3)),
			static_cast<float*>(VMA(4)));
	...
	case CG_R_ADDADDITIVELIGHTTOSCENE:
		re->AddAdditiveLightToScene(static_cast<const float*>(VMA(1)), VMF(2), VMF(3), VMF(4), VMF(5));
		return 0;
	...
	case CG_R_LERPTAG:
		return re->LerpTag(static_cast<orientation_t*>(VMA(1)), args[2], args[3], args[4], VMF(5),
	...
	case CG_GETDEFAULTSTATE:
		return CL_GetDefaultState(args[1], static_cast<entityState_t*>(VMA(2)));
	...
	case CG_SETUSERCMDVALUE:
		_CL_SetUserCmdValue(args[1], VMF(2), VMF(3), VMF(4), VMF(5), args[6], args[7], static_cast<qboolean>(args[8]));
		return 0;

	case CG_SETCLIENTFORCEANGLE:
		CL_SetClientForceAngle(args[1], static_cast<float*>(VMA(2)));
		return 0;
	...
	case CG_R_GET_LIGHT_STYLE:
		re->GetLightStyle(args[1], static_cast<unsigned char*>(VMA(2)));
		return 0;
	...
	case CG_R_INPVS:
		return re->inPVS(static_cast<const float*>(VMA(1)), static_cast<const float*>(VMA(2)),
			static_cast<byte*>(VMA(3)));

#ifndef DEBUG_DISABLEFXCALLS
	case CG_FX_ADDLINE:
		CGFX_AddLine(static_cast<float*>(VMA(1)), static_cast<float*>(VMA(2)), VMF(3), VMF(4), VMF(5),
			VMF(6), VMF(7), VMF(8),
			static_cast<float*>(VMA(9)), static_cast<float*>(VMA(10)), VMF(11),
			args[12], args[13], args[14]);
		return 0;
	case CG_FX_REGISTER_EFFECT:
		return FX_RegisterEffect(static_cast<const char*>(VMA(1)));

	case CG_FX_PLAY_EFFECT:
		FX_PlayEffect(static_cast<const char*>(VMA(1)), static_cast<float*>(VMA(2)), static_cast<float*>(VMA(3)),
			args[4], args[5]);
		return 0;
	...
	case CG_FX_PLAY_EFFECT_ID:
		FX_PlayEffectID(args[1], static_cast<float*>(VMA(2)), static_cast<float*>(VMA(3)), args[4], args[5]);
		return 0;

	case CG_FX_PLAY_PORTAL_EFFECT_ID:
		FX_PlayEffectID(args[1], static_cast<float*>(VMA(2)), static_cast<float*>(VMA(3)), args[4], args[5], qtrue);
		return 0;

	case CG_FX_PLAY_ENTITY_EFFECT_ID:
		FX_PlayEntityEffectID(args[1], static_cast<float*>(VMA(2)), static_cast<vec3_t*>(VMA(3)), args[4], args[5],
			args[6], args[7]);
		return 0;

	case CG_FX_PLAY_BOLTED_EFFECT_ID:
		return CGFX_PlayBoltedEffectID(args[1], static_cast<float*>(VMA(2)), reinterpret_cast<void*>(args[3]), args[4], args[5], args[6],
			args[7], static_cast<qboolean>(args[8]));

	case CG_FX_ADD_SCHEDULED_EFFECTS:
		FX_AddScheduledEffects(static_cast<qboolean>(args[1]));
		return 0;
	...
	case CG_FX_INIT_SYSTEM:
		return FX_InitSystem(static_cast<refdef_t*>(VMA(1)));

	case CG_FX_SET_REFDEF:
		FX_SetRefDefFromCGame(static_cast<refdef_t*>(VMA(1)));
		return 0;
	...
	case CG_FX_ADDPOLY:
		CGFX_AddPoly(static_cast<addpolyArgStruct_t*>(VMA(1)));
		return 0;
	case CG_FX_ADDBEZIER:
		CGFX_AddBezier(static_cast<addbezierArgStruct_t*>(VMA(1)));
		return 0;
	case CG_FX_ADDPRIMITIVE:
		CGFX_AddPrimitive(static_cast<effectTrailArgStruct_t*>(VMA(1)));
		return 0;
	case CG_FX_ADDSPRITE:
		CGFX_AddSprite(static_cast<addspriteArgStruct_t*>(VMA(1)));
		return 0;
	case CG_FX_ADDELECTRICITY:
		CGFX_AddElectricity(static_cast<addElectricityArgStruct_t*>(VMA(1)));
		return 0;
#else
	case CG_FX_REGISTER_EFFECT:
	case CG_FX_PLAY_EFFECT:
	...
	case CG_ROFF_CACHE:
		return CL_ROFF_Cache(static_cast<char*>(VMA(1)));

	case CG_ROFF_PLAY:
		return CL_ROFF_Play(args[1], args[2], static_cast<qboolean>(args[3]));
	...
	case CG_G2_LISTSURFACES:
		CL_G2API_ListModelSurfaces(reinterpret_cast<void*>(args[1]));
		return 0;

	case CG_G2_LISTBONES:
		CL_G2API_ListModelBones(reinterpret_cast<void*>(args[1]), args[2]);
		return 0;
	...
	case CG_G2_GETBOLT:
		return CL_G2API_GetBoltMatrix(reinterpret_cast<void*>(args[1]), args[2], args[3], static_cast<mdxaBone_t*>(VMA(4)),
	...
	case CG_G2_GETBOLT_NOREC:
		return CL_G2API_GetBoltMatrix_NoReconstruct(reinterpret_cast<void*>(args[1]), args[2], args[3], static_cast<mdxaBone_t*>(VMA(4)),
			static_cast<const float*>(VMA(5)),
			static_cast<const float*>(VMA(6)), args[7],
			static_cast<qhandle_t*>(VMA(8)), static_cast<float*>(VMA(9)));

	case CG_G2_GETBOLT_NOREC_NOROT:
		return CL_G2API_GetBoltMatrix_NoRecNoRot(reinterpret_cast<void*>(args[1]), args[2], args[3], static_cast<mdxaBone_t*>(VMA(4)),
			static_cast<const float*>(VMA(5)), static_cast<const float*>(VMA(6)),
			args[7], static_cast<qhandle_t*>(VMA(8)), static_cast<float*>(VMA(9)));

	case CG_G2_INITGHOUL2MODEL:
		return CL_G2API_InitGhoul2Model(static_cast<void**>(VMA(1)), static_cast<const char*>(VMA(2)), args[3], args[4],
			args[5], args[6], args[7]);

	case CG_G2_SETSKIN:
		return CL_G2API_SetSkin(reinterpret_cast<void*>(args[1]), args[2], args[3], args[4]);

	case CG_G2_COLLISIONDETECT:
		CL_G2API_CollisionDetect(static_cast<CollisionRecord_t*>(VMA(1)), reinterpret_cast<void*>(args[2]),
			static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)), args[5], args[6],
			static_cast<float*>(VMA(7)), static_cast<float*>(VMA(8)), static_cast<float*>(VMA(9)),
			args[10], args[11], VMF(12));
		return 0;

	case CG_G2_COLLISIONDETECTCACHE:
		CL_G2API_CollisionDetectCache(static_cast<CollisionRecord_t*>(VMA(1)), reinterpret_cast<void*>(args[2]),
			static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)), args[5],
			args[6], static_cast<float*>(VMA(7)), static_cast<float*>(VMA(8)),
			static_cast<float*>(VMA(9)), args[10], args[11], VMF(12));
		return 0;

	case CG_G2_ANGLEOVERRIDE:
		return CL_G2API_SetBoneAngles(reinterpret_cast<void*>(args[1]), args[2], static_cast<const char*>(VMA(3)),
			static_cast<float*>(VMA(4)), args[5], args[6], args[7], args[8],
			static_cast<qhandle_t*>(VMA(9)), args[10], args[11]);

	case CG_G2_CLEANMODELS:
		CL_G2API_CleanGhoul2Models(static_cast<void**>(VMA(1)));
		return 0;

	case CG_G2_PLAYANIM:
		return CL_G2API_SetBoneAnim(reinterpret_cast<void*>(args[1]), args[2], static_cast<const char*>(VMA(3)), args[4], args[5],
			args[6], VMF(7), args[8], VMF(9), args[10]);

	case CG_G2_GETBONEANIM:
		return CL_G2API_GetBoneAnim(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), args[3],
			static_cast<float*>(VMA(4)), static_cast<int*>(VMA(5)), static_cast<int*>(VMA(6)),
			static_cast<int*>(VMA(7)), static_cast<float*>(VMA(8)), static_cast<int*>(VMA(9)),
			args[10]);

	case CG_G2_GETBONEFRAME:
		return CL_G2API_GetBoneFrame(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), args[3],
			static_cast<float*>(VMA(4)), static_cast<int*>(VMA(5)), args[6]);

	case CG_G2_GETGLANAME:
		CL_G2API_GetGLAName(reinterpret_cast<void*>(args[1]), args[2], static_cast<char*>(VMA(3)));
		return 0;

	case CG_G2_COPYGHOUL2INSTANCE:
		return CL_G2API_CopyGhoul2Instance(reinterpret_cast<void*>(args[1]), reinterpret_cast<void*>(args[2]), args[3]);

	case CG_G2_COPYSPECIFICGHOUL2MODEL:
		CL_G2API_CopySpecificGhoul2Model(reinterpret_cast<void*>(args[1]), args[2], reinterpret_cast<void*>(args[3]), args[4]);
		return 0;

	case CG_G2_DUPLICATEGHOUL2INSTANCE:
		CL_G2API_DuplicateGhoul2Instance(reinterpret_cast<void*>(args[1]), static_cast<void**>(VMA(2)));
		return 0;
	...
	case CG_G2_ADDSKINGORE:
		CL_G2API_AddSkinGore(reinterpret_cast<void*>(args[1]), static_cast<SSkinGoreData*>(VMA(2)));
		return 0;

	case CG_G2_CLEARSKINGORE:
		CL_G2API_ClearSkinGore(reinterpret_cast<void*>(args[1]));
		return 0;

	case CG_G2_SIZE:
		return CL_G2API_Ghoul2Size(reinterpret_cast<void*>(args[1]));

	case CG_G2_ADDBOLT:
		return CL_G2API_AddBolt(reinterpret_cast<void*>(args[1]), args[2], static_cast<const char*>(VMA(3)));

	case CG_G2_ATTACHENT:
		return CL_G2API_AttachEnt(static_cast<int*>(VMA(1)), reinterpret_cast<void*>(args[2]), args[3], args[4], args[5]);

	case CG_G2_SETBOLTON:
		CL_G2API_SetBoltInfo(reinterpret_cast<void*>(args[1]), args[2], args[3]);
		return 0;

	case CG_G2_SETROOTSURFACE:
		return CL_G2API_SetRootSurface(reinterpret_cast<void*>(args[1]), args[2], static_cast<const char*>(VMA(3)));

	case CG_G2_SETSURFACEONOFF:
		return CL_G2API_SetSurfaceOnOff(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), args[3]);

	case CG_G2_SETNEWORIGIN:
		return CL_G2API_SetNewOrigin(reinterpret_cast<void*>(args[1]), args[2]);

	case CG_G2_DOESBONEEXIST:
		return CL_G2API_DoesBoneExist(reinterpret_cast<void*>(args[1]), args[2], static_cast<const char*>(VMA(3)));

	case CG_G2_GETSURFACERENDERSTATUS:
		return CL_G2API_GetSurfaceRenderStatus(reinterpret_cast<void*>(args[1]), args[2], static_cast<const char*>(VMA(3)));
	...
	case CG_G2_ABSURDSMOOTHING:
		CL_G2API_AbsurdSmoothing(reinterpret_cast<void*>(args[1]), static_cast<qboolean>(args[2]));
		return 0;

	case CG_G2_SETRAGDOLL:
		CL_G2API_SetRagDoll(reinterpret_cast<void*>(args[1]), static_cast<sharedRagDollParams_t*>(VMA(2)));
		return 0;

	case CG_G2_ANIMATEG2MODELS:
		CL_G2API_AnimateG2Models(reinterpret_cast<void*>(args[1]), args[2], static_cast<sharedRagDollUpdateParams_t*>(VMA(3)));
		return 0;

		//additional ragdoll options -rww
	case CG_G2_RAGPCJCONSTRAINT:
		return CL_G2API_RagPCJConstraint(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), static_cast<float*>(VMA(3)),
			static_cast<float*>(VMA(4)));

	case CG_G2_RAGPCJGRADIENTSPEED:
		return CL_G2API_RagPCJGradientSpeed(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), VMF(3));

	case CG_G2_RAGEFFECTORGOAL:
		return CL_G2API_RagEffectorGoal(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), static_cast<float*>(VMA(3)));

	case CG_G2_GETRAGBONEPOS:
		return CL_G2API_GetRagBonePos(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), static_cast<float*>(VMA(3)),
			static_cast<float*>(VMA(4)), static_cast<float*>(VMA(5)),
			static_cast<float*>(VMA(6)));

	case CG_G2_RAGEFFECTORKICK:
		return CL_G2API_RagEffectorKick(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), static_cast<float*>(VMA(3)));

	case CG_G2_RAGFORCESOLVE:
		return CL_G2API_RagForceSolve(reinterpret_cast<void*>(args[1]), static_cast<qboolean>(args[2]));

	case CG_G2_SETBONEIKSTATE:
		return CL_G2API_SetBoneIKState(reinterpret_cast<void*>(args[1]), args[2], static_cast<const char*>(VMA(3)), args[4],
			static_cast<sharedSetBoneIKStateParams_t*>(VMA(5)));

	case CG_G2_IKMOVE:
		return CL_G2API_IKMove(reinterpret_cast<void*>(args[1]), args[2], static_cast<sharedIKMoveParams_t*>(VMA(3)));

	case CG_G2_REMOVEBONE:
		return CL_G2API_RemoveBone(reinterpret_cast<void*>(args[1]), static_cast<const char*>(VMA(2)), args[3]);

	case CG_G2_ATTACHINSTANCETOENTNUM:
		CL_G2API_AttachInstanceToEntNum(reinterpret_cast<void*>(args[1]), args[2], static_cast<qboolean>(args[3]));
		return 0;
	...
	case CG_G2_OVERRIDESERVER:
		return CL_G2API_OverrideServer(reinterpret_cast<void*>(args[1]));

	case CG_G2_GETSURFACENAME:
		CL_G2API_GetSurfaceName(reinterpret_cast<void*>(args[1]), args[2], args[3], static_cast<char*>(VMA(4)));
		return 0;
	...
	case CG_SET_SHARED_BUFFER:
		RegisterSharedMemory(static_cast<char*>(VMA(1)));
		return 0;
	...
	case CG_RE_WorldEffectCommand:
		re->WorldEffectCommand(static_cast<const char*>(VMA(1)));
		return 0;

	case CG_WE_ADDWEATHERZONE:
		re->AddWeatherZone(static_cast<float*>(VMA(1)), static_cast<float*>(VMA(2)));
		return 0;
...
void CL_BindCGame(void)
{
	...
	char dllName[MAX_OSPATH] = "SerenityJediEngine2024-cgame" ARCH_STRING DLL_EXT;
	...
	if (cgvm && !cgvm->isLegacy)
	{
		...
		cgi.G2_SetGhoul2model_indexes = CL_G2API_SetGhoul2model_indexes;
		...
		const auto GetCGameAPI = reinterpret_cast<GetCGameAPI_t>(cgvm->GetModuleAPI);
		cgameExport_t* ret = GetCGameAPI(CGAME_API_VERSION, &cgi);
		if (!ret)
		{
			//free VM?
