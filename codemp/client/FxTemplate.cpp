...
bool CPrimitiveTemplate::ParseVector(const char* val, vec3_t min, vec3_t max)
{
	...
	const int v = sscanf(val, "%f %f %f   %f %f %f", &min[0], &min[1], &min[2], &max[0], &max[1], &max[2]);
...
bool CPrimitiveTemplate::ParseShaders(const CGPValue* grp)
{
	...
	if (grp->IsList())
	{
		...
		const CGPObject* list = grp->GetList();
		...
		while (list)
		{
			...
			list = static_cast<CGPValue*>(list->GetNext());
		}
	}
	else
	{
...
bool CPrimitiveTemplate::ParseRGB(const CGPGroup* grp)
{
	...
	while (pairs)
	{
		...
		const char* key = pairs->GetName();
		const char* val = pairs->GetTopValue();
