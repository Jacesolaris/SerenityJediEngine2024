...
//
using SoundCompressionMethod_t = enum
{
	ct_16 = 0,
...
using sfx_t = struct sfx_s
{
	short* pSoundData;
	qboolean bDefaultSound; // couldn't be loaded, so use buzz
	qboolean bInMemory;
	// not in Memory, set qtrue when loaded, and qfalse when its buffers are freed up because of being old, so can be reloaded
	SoundCompressionMethod_t eSoundCompressionMethod;
	MP3STREAM* pMP3StreamHeader; // NULL ptr unless thisis an MP3. Use Z_Malloc and Z_Free
	int iSoundLengthInSamples;
	// length in samples, always kept as 16bit now so this is #shorts (watch for stereo later for music?)
...
constexpr auto START_SAMPLE_IMMEDIATE = 0x7fffffff;
...
constexpr auto NUM_STREAMING_BUFFERS = 4;
constexpr auto STREAMING_BUFFER_SIZE = 4608; // 4 decoded MP3 frames;
...
using channel_t = struct channel_s
{
	...
	byte MP3SlidingDecodeBuffer[50000/*12000*/];
	// typical back-request = -3072, so roughly double is 6000 (safety), then doubled again so the 6K pos is in the middle of the buffer)
...
constexpr auto WAV_FORMAT_ADPCM = 2; // not actually implemented, but is the value that you get in a header;
constexpr auto WAV_FORMAT_MP3 = 3;
// not actually used this way, but just ensures we don't match one of the legit formats;

using wavinfo_t = struct wavinfo_s
{
	int format;
	int rate;
	int width;
...
constexpr auto MAX_CHANNELS = 32;
...
constexpr auto MAX_RAW_SAMPLES = 16384;
...
byte* SND_malloc(int i_size, const sfx_t* sfx);
void SND_setup();
