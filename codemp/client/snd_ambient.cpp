...
static void AS_PlayLocalSet(vec3_t listener_origin, vec3_t origin, const ambientSet_t* set, const int ent_id,
	int* lastTime)
{
	vec3_t dir;
	...
	*lastTime = time;
	...
	if (set->numSubWaves)
		S_StartAmbientSound(origin, ent_id, volume, set->subWaves[Q_irand(0, set->numSubWaves - 1)]);
}
