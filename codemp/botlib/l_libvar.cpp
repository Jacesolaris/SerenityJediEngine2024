...
float LibVarStringValue(char* string)
{
	...
	while (*string)
	{
		...
		if (dotfound)
		{
			value = value + static_cast<float>(*string - '0') / static_cast<float>(dotfound);
			dotfound *= 10;
		} //end if
		else
		{
			value = value * 10.0 + static_cast<float>(*string - '0');
		} //end else
		string++;
	} //end while
	return value;
...
{
	const auto v = static_cast<libvar_t*>(GetMemory(sizeof(libvar_t)));
	Com_Memset(v, 0, sizeof(libvar_t));
	v->name = static_cast<char*>(GetMemory(strlen(var_name) + 1));
...
libvar_t* LibVar(char* var_name, char* value)
{
	...
	//variable string
	v->string = static_cast<char*>(GetMemory(strlen(value) + 1));
...
{
	const libvar_t* v = LibVar(var_name, value);
	return v->string;
