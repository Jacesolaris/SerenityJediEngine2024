...
constexpr auto AAS_MAX_REACHABILITYSIZE = 65536;
//number of areas reachability is calculated for each frame
constexpr auto REACHABILITYAREASPERCYCLE = 15;
//number of units reachability points are placed inside the areas
constexpr auto INSIDEUNITS = 2;
constexpr auto INSIDEUNITS_WALKEND = 5;
constexpr auto INSIDEUNITS_WALKSTART = 0.1;
constexpr auto INSIDEUNITS_WATERJUMP = 15;
//area flag used for weapon jumping
constexpr auto AREA_WEAPONJUMP = 8192;	//valid area to weapon jump to;
...
//linked reachability
using aas_lreachability_t = struct aas_lreachability_s
{
...
static float AAS_FaceArea(aas_face_t* face)
{
	...
	int side = edgenum < 0;
	const aas_edge_t* edge = &aasworld.edges[abs(edgenum)];
	const float* v = aasworld.vertexes[edge->v[side]];
...
float AAS_AreaVolume(const int areanum)
{
	...
	const int edgenum = aasworld.edgeindex[face->firstedge];
	const aas_edge_t* edge = &aasworld.edges[abs(edgenum)];
	//
	...
	for (int i = 0; i < area->numfaces; i++)
	{
		...
		const int side = face->backarea != areanum;
		const aas_plane_t* plane = &aasworld.planes[face->planenum ^ side];
		const float d = -(DotProduct(corner, plane->normal) - plane->dist);
		const float a = AAS_FaceArea(face);
		volume += d * a;
	} //end for
...
int AAS_GetJumpPadInfo(const int ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity)
{
	...
	teststart[2] += 64;
	const aas_trace_t trace = AAS_TraceClientBBox(teststart, origin, PRESENCE_CROUCH, -1);
	if (trace.startsolid)
	{
	...
	for (ent2 = AAS_NextBSPEntity(0); ent2; ent2 = AAS_NextBSPEntity(ent2))
	{
		...
		if (strcmp(targetname, target) == 0) break;
	} //end for
	if (!ent2)
	{
	...
	//
	const float height = ent2origin[2] - origin[2];
	const float gravity = aassettings.phys_gravity;
	const float time = sqrt(height / (0.5 * gravity));
	if (!time)
	{
	...
	const float dist = VectorNormalize(velocity);
...
{
	int bot_visualizejumppads, bestareanum;
	...
	for (int i = 0; i < 5 && !areanum; i++)
	{
		for (int j = 0; j < 5 && !areanum; j++)
		{
			for (int k = -1; k <= 1 && !areanum; k++)
			{
				...
				{
					VectorCopy(origin, start);
					start[0] += static_cast<float>(j) * 4 * k;
					start[1] += static_cast<float>(j) * 4 * l;
					start[2] += static_cast<float>(i) * 4;
...
{
	reachabilityheap = static_cast<aas_lreachability_t*>(GetClearedMemory(
...
void AAS_FaceCenter(const int facenum, vec3_t center)
{
	...
	{
		const aas_edge_t* edge = &aasworld.edges[abs(aasworld.edgeindex[face->firstedge + i])];
		VectorAdd(center, aasworld.vertexes[edge->v[0]], center);
		VectorAdd(center, aasworld.vertexes[edge->v[1]], center);
	} //end for
	const float scale = 0.5 / face->numedges;
...
{
	const float maxzvelocity = sqrt(static_cast<float>(30 * 10000));
	const float gravity = aassettings.phys_gravity;
...
float AAS_MaxJumpDistance(const float phys_jumpvel)
{
	...
	const float t = sqrt(aassettings.rs_maxjumpfallheight / (0.5 * phys_gravity));
	//maximum distance
...
int AAS_Reachability_Swim(const int area1num, const int area2num)
{
	...
	for (i = 0; i < area1->numfaces; i++)
	{
		...
		const int side1 = face1num < 0;
		face1num = abs(face1num);
		//
		for (int j = 0; j < area2->numfaces; j++)
		{
			const int face2num = abs(aasworld.faceindex[area2->firstface + j]);
			//
			if (face1num == face2num)
			{
				...
				if (AAS_PointContents(start) & (CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER))
				{
					...
					const aas_plane_t* plane = &aasworld.planes[face1->planenum ^ side1];
...
{
	int i;
	vec3_t end, invgravity;
	constexpr vec3_t gravitydirection = { 0, 0, -1 };
	...
	for (i = 0; i < area1->numfaces; i++)
	{
		const aas_face_t* face1 = &aasworld.faces[abs(aasworld.faceindex[area1->firstface + i])];
		if (!(face1->faceflags & FACE_GROUND)) continue;
		//
		for (int j = 0; j < area2->numfaces; j++)
		{
			const aas_face_t* face2 = &aasworld.faces[abs(aasworld.faceindex[area2->firstface + j])];
			...
			for (int edgenum1 = 0; edgenum1 < face1->numedges; edgenum1++)
			{
				for (int edgenum2 = 0; edgenum2 < face2->numedges; edgenum2++)
				{
					...
					if (abs(aasworld.edgeindex[face1->firstedge + edgenum1]) !=
						...
						continue;
					const int edgenum = aasworld.edgeindex[face1->firstedge + edgenum1];
					const int side = edgenum < 0;
					const aas_edge_t* edge = &aasworld.edges[abs(edgenum)];
					...
					const aas_plane_t* plane2 = &aasworld.planes[face2->planenum];
					...
					//
					const float height = DotProduct(invgravity, start);
...
int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num, int area2num)
{
	...
	vec3_t p1area1, p1area2, p2area1, p2area2;
	vec3_t start, end;
	...
	vec3_t beststart2, vec3_t bestend2, float bestdist)
{
	...
	if (dir2[0])
	{
		const float a2 = dir2[1] / dir2[0];
		const float b2 = v3[1] - a2 * v3[0];
		//point on the edge vector of area2 closest to v1
		p1[0] = (DotProduct(v1, dir2) - (a2 * dir2[0] + b2 * dir2[1])) / dir2[0];
		p1[1] = a2 * p1[0] + b2;
		//point on the edge vector of area2 closest to v2
	...
	{
		//
		const float a1 = dir1[1] / dir1[0];
		const float b1 = v1[1] - a1 * v1[0];
		//point on the edge vector of area1 closest to v3
		p3[0] = (DotProduct(v3, dir1) - (a1 * dir1[0] + b1 * dir1[1])) / dir1[0];
		p3[1] = a1 * p3[0] + b1;
		//point on the edge vector of area1 closest to v4
...
{
	int i, j, k, l, face1num, face2num, edge1num, edge2num;
	float phys_jumpvel, maxjumpdistance, maxjumpheight, bestdist, speed;
	float* v1, * v2, * v3, * v4;
...
{
	int i, j, k, l, edge1num, edge2num, sharededgenum = 0;
	...
	{
		aas_lreachability_t* lreach;
		aas_edge_t* sharededge;
		aas_plane_t* plane2;
		aas_plane_t* plane1;
		vec3_t dir;
		vec3_t sharededgevec;
		vec3_t v2;
		vec3_t v1;
		vec3_t area2point;
		vec3_t area1point;
		int firstv;
		int ladderface2vertical;
		int ladderface1vertical;
...
void AAS_Reachability_Teleport(void)
{
	...
	for (int ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		//
		int area2num = AAS_PointAreaNum(destorigin);
		...
		if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num))
		{
			...
			const aas_trace_t trace = AAS_TraceClientBBox(destorigin, end, PRESENCE_CROUCH, -1);
			if (trace.startsolid)
			{
		...
		VectorScale(mid, 0.5, mid);
		//link an invalid (-1) entity
		aas_link_t* areas = AAS_LinkEntityClientBBox(mins, maxs, -1, PRESENCE_CROUCH);
		if (!areas) botimport.Print(PRT_MESSAGE, "trigger_multiple not in any area\n");
		//
		for (const aas_link_t* link = areas; link; link = link->next_area)
		{
			//if (!AAS_AreaGrounded(link->areanum)) continue;
			if (!AAS_AreaTeleporter(link->areanum)) continue;
			//
			const int area1num = link->areanum;
			//create a new reachability link
			aas_lreachability_t* lreach = AAS_AllocReachability();
			if (!lreach) break;
			lreach->areanum = area2num;
...
{
	int area1num, area2num, modelnum, i, k, l, p;
	float lip, height, speed;
	vec3_t mins, maxs, angles = { 0, 0, 0 };
	vec3_t pos2, platbottom, plattop;
			...
			for (i = 0; i < 9; i++)
			{
				...
				for (int n = 0; n < 3; n++)
				{
					...
					for (int j = 0; j < 8; j++)
					{
						...
						lreach->edgenum = static_cast<int>(height);
						//
...
{
	float speed;
	vec3_t beststart, bestend, hordir, testpoint;

	//
	aas_lreachability_t* lreachabilities = nullptr;
	int bestfacenum = 0;
	const aas_plane_t* bestfaceplane = nullptr;
	//
	for (int i = 1; i < aasworld.numareas; i++)
	{
		...
		{
			const int facenum = aasworld.faceindex[area->firstface + j];
			const aas_face_t* face = &aasworld.faces[abs(facenum)];
			//if not a ground face
			if (!(face->faceflags & FACE_GROUND)) continue;
			//get the ground planes
			...
			{
				const int edgenum = abs(aasworld.edgeindex[face->firstedge + k]);
				const aas_edge_t* edge = &aasworld.edges[edgenum];
				//calculate the minimum distance between the two edges
				float* v1 = aasworld.vertexes[edge->v[0]];
				float* v2 = aasworld.vertexes[edge->v[1]];
				//
				for (int l = 0; l < numpoints; l++)
				{
					float* v3 = facepoints[l];
					float* v4 = facepoints[(l + 1) % numpoints];
					const float dist = AAS_ClosestEdgePoints(v1, v2, v3, v4, faceplane, plane,
						beststart, bestend,
						beststart2, bestend2, bestdist);
					if (dist < bestdist)
					{
		...
		hordir[2] = 0;
		const float hordist = VectorLength(hordir);
		//
...
void AAS_Reachability_FuncBobbing(void)
{
	int spawnflags, axis;
	int i, numareas;
	vec3_t move_end, move_start, move_start_top, move_end_top;
	vec3_t angles = { 0, 0, 0 };
	vec3_t start_edgeverts[4], end_edgeverts[4];
	...
	for (int ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		//get the model number, and skip the leading *
		const int modelnum = atoi(model + 1);
		if (modelnum <= 0)
		{
		...
		for (i = 0; i < 2; i++)
		{
			...
			for (startreach = firststartreach; startreach; startreach = nextstartreach)
			{
				...
				for (endreach = firstendreach; endreach; endreach = nextendreach)
				{
					...
					if (i == 0) lreach->edgenum = static_cast<int>(move_start[axis]) << 16 | static_cast<int>(move_end[axis]) & 0x0000ffff;
					else lreach->edgenum = static_cast<int>(move_end[axis]) << 16 | static_cast<int>(move_start[axis]) & 0x0000ffff;
...
{
	int face2num, i, j, areanum, numareas;
	...
	aas_area_t* area1, * area2;
	aas_lreachability_t* lreach;
	vec3_t areastart, start, end, dir, down = { 0, 0, -1 };
	float* v;
...
void AAS_SetWeaponJumpAreaFlags(void)
{
	...
	for (int ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (!AAS_ValueForBSPEpairKey(ent, "classname", classname, MAX_EPAIRKEY)) continue;
		if (
			strcmp(classname, "item_armor_body") == 0 ||
			strcmp(classname, "item_health") == 0 ||
			strcmp(classname, "weapon_disruptor") == 0 ||
			strcmp(classname, "weapon_repeater") == 0 ||
			strcmp(classname, "weapon_demp2") == 0 ||
			strcmp(classname, "weapon_flechette") == 0 ||
			strcmp(classname, "weapon_rocket_launcher") == 0)
		{
			vec3_t origin;
			if (AAS_VectorForBSPEpairKey(ent, "origin", origin))
			{
				...
				const int areanum = AAS_BestReachableArea(origin, mins, maxs, origin);
...
{
	float speed, zvel;
	vec3_t areastart, start, end, dir;// teststart;
	aas_clientmove_t move;

	constexpr int visualize = qfalse;
	...
	const aas_trace_t trace = AAS_TraceClientBBox(start, end, PRESENCE_CROUCH, -1);
	if (trace.startsolid) return qfalse;
	VectorCopy(trace.endpos, areastart);
	//
	//areastart is now the start point
	//
	for (int i = 0; i < area2->numfaces; i++)
	{
		vec3_t facecenter;
		...
		for (int n = 0; n < 1/*2*/; n++)
		{
			...
			//get the horizontal speed for the jump, if it isn't possible to calculate this
			//speed (the jump is not possible) then there's no jump reachability created
			const int ret = AAS_HorizontalVelocityForJump(zvel, areastart, facecenter, &speed);
			if (ret && speed < 300)
			{
				//direction towards the face center
				VectorSubtract(facecenter, areastart, dir);
...
void AAS_Reachability_WalkOffLedge(const int areanum)
{
	...
	for (int i = 0; i < area->numfaces; i++)
	{
		const int face1num = aasworld.faceindex[area->firstface + i];
		...
		for (int k = 0; k < face1->numedges; k++)
		{
			...
			for (int j = 0; j < area->numfaces; j++)
			{
				const int face2num = aasworld.faceindex[area->firstface + j];
				const aas_face_t* face2 = &aasworld.faces[abs(face2num)];
				...
				for (int l = 0; l < face2->numedges; l++)
				{
					...
					if (abs(edge1num) == abs(edge2num))
					{
						...
						const aas_edge_t* edge = &aasworld.edges[abs(edge1num)];
						const int side = edge1num < 0;
						//
						const float* v1 = aasworld.vertexes[edge->v[side]];
						const float* v2 = aasworld.vertexes[edge->v[!side]];
						//
						const aas_plane_t* plane = &aasworld.planes[face1->planenum];
						//get the points really into the areas
						VectorSubtract(v2, v1, sharededgevec);
						...
						testend[2] -= 1000;
						aas_trace_t trace = AAS_TraceClientBBox(mid, testend, PRESENCE_CROUCH, -1);
						//
						if (trace.startsolid)
						{
						...
						const int reachareanum = AAS_PointAreaNum(trace.endpos);
						...
						//if not going through a cluster portal
						const int numareas = AAS_TraceAreas(mid, testend, areas, nullptr, ARRAY_LEN(areas));
...
void AAS_StoreReachability(void)
{
	...
	aasworld.reachability = static_cast<aas_reachability_t*>(GetClearedMemory((numlreachabilities + 10) * sizeof(aas_reachability_t)));
	...
	for (int i = 0; i < aasworld.numareas; i++)
	{
		...
		for (const aas_lreachability_t* lreach = areareachability[i]; lreach; lreach = lreach->next)
		{
			aas_reachability_t* reach = &aasworld.reachability[areasettings->firstreachablearea +
...
int AAS_ContinueInitReachability(float time)
{
	...
	const int todo = aasworld.numreachabilityareas + static_cast<int>(framereachability);
	...
	for (i = aasworld.numreachabilityareas; i < aasworld.numareas && i < todo; i++)
	{
		...
		if (Sys_MilliSeconds() - start_time > static_cast<int>(reachability_delay)) break;
		//
	...
	{
		botimport.Print(PRT_MESSAGE, "\r%6.1f%%", static_cast<float>(100.0));
	...
	else
	{
		...
		botimport.Print(PRT_MESSAGE, "\r%6.1f%%", static_cast<float>(lastpercentage) / 10);
	} //end else
	//not yet finished
	return qtrue;
...
void AAS_InitReachability(void)
{
	...
	{
#ifndef BSPC
		if (!static_cast<int>(LibVarGetValue("forcereachability")))
		{
	...
	areareachability = static_cast<aas_lreachability_t**>(GetClearedMemory(
