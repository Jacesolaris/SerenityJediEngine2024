...
int AAS_DropToFloor(vec3_t origin, vec3_t mins, vec3_t maxs)
{
	...
	const bsp_trace_t trace = AAS_Trace(origin, mins, maxs, end, 0, CONTENTS_SOLID);
...
int AAS_OnGround(vec3_t origin, const int presencetype, const int passent)
{
	...
	const aas_trace_t trace = AAS_TraceClientBBox(origin, end, presencetype, passent);
...
float AAS_WeaponJumpZVelocity(vec3_t origin, const float radiusdamage)
{
	...
	constexpr vec3_t rocketoffset = { 8, 8, -8 };
	constexpr vec3_t botmins = { -16, -16, -24 };
	constexpr vec3_t botmaxs = { 16, 16, 32 };
	...
	const bsp_trace_t bsptrace = AAS_Trace(start, nullptr, nullptr, end, 1, CONTENTS_SOLID);
...
int AAS_ClipToBBox(aas_trace_t* trace, vec3_t start, vec3_t end, const int presencetype, vec3_t mins, vec3_t maxs)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		const float front = start[i] - planedist;
		const float back = end[i] - planedist;
	...
	if (i != 3)
	{
		...
		trace->lastarea = 0;
		//trace endpos
		for (int j = 0; j < 3; j++) trace->endpos[j] = start[j] + dir[j] * frac;
		return qtrue;
	} //end if
	return qfalse;
...
// Changes Globals:		-
//===========================================================================
int AAS_ClientMovementPrediction(aas_clientmove_s* move,
	...
	vec3_t mins, vec3_t maxs, int visualize)
{
	...
	int numareas;
	vec3_t org, end, feet, start, wishdir;
	vec3_t frame_test_vel, left_test_vel;
	vec3_t up = { 0, 0, 1 };
...
int AAS_HorizontalVelocityForJump(const float zvel, vec3_t start, vec3_t end, float* velocity)
{
	...
	const float maxjump = 0.5 * phys_gravity * (zvel / phys_gravity) * (zvel / phys_gravity);
	//top of the parabolic jump
	const float top = start[2] + maxjump;
	//height the bot will fall from the top
	const float height2fall = top - end[2];
	...
	const float t = sqrt(height2fall / (0.5 * phys_gravity));
