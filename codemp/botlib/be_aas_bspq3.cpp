...
constexpr auto MAX_BSPENTITIES = 2048;

using rgb_t = struct rgb_s
{
	int red;
	int green;
	int blue;
...
//bsp entity epair
using bsp_epair_t = struct bsp_epair_s
{
	char* key;
	char* value;
	bsp_epair_s* next;
...
//bsp data entity
using bsp_entity_t = struct bsp_entity_s
{
	bsp_epair_t* epairs;
};

//id Sofware BSP data
using bsp_t = struct bsp_s
{
...
{
	return static_cast<qboolean>(botimport.inPVS(p1, p2));
...
// Changes Globals:		-
//===========================================================================
int AAS_ValueForBSPEpairKey(const int ent, const char* key, char* value, const int size)
{
	value[0] = '\0';
...
// Changes Globals:		-
//===========================================================================
int AAS_FloatForBSPEpairKey(const int ent, const char* key, float* value)
{
	char buf[MAX_EPAIRKEY];
...
// Changes Globals:		-
//===========================================================================
int AAS_IntForBSPEpairKey(const int ent, const char* key, int* value)
{
	char buf[MAX_EPAIRKEY];
...
{
	bsp_epair_t* nextepair;

	for (int i = 1; i < bspworld.numentities; i++)
	{
		const bsp_entity_t* ent = &bspworld.entities[i];
		for (bsp_epair_t* epair = ent->epairs; epair; epair = nextepair)
		{
...
void AAS_ParseBSPEntities()
{
	...
	while (PS_ReadToken(script, &token))
	{
		...
		while (PS_ReadToken(script, &token))
		{
			...
			const auto epair = static_cast<bsp_epair_t*>(GetClearedHunkMemory(sizeof(bsp_epair_t)));
			...
			epair->key = static_cast<char*>(GetHunkMemory(strlen(token.string) + 1));
			...
			StripDoubleQuotes(token.string);
			epair->value = static_cast<char*>(GetHunkMemory(strlen(token.string) + 1));
...
int AAS_LoadBSPFile()
{
	...
	bspworld.dentdata = static_cast<char*>(GetClearedHunkMemory(bspworld.entdatasize));
