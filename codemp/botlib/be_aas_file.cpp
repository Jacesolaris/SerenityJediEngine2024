...
void AAS_DumpAASData()
{
	...
	aasworld.edges = nullptr;
	aasworld.edgeindexsize = 0;
	if (aasworld.edgeindex) FreeMemory(aasworld.edgeindex);
	aasworld.edgeindex = nullptr;
	aasworld.numfaces = 0;
	if (aasworld.faces) FreeMemory(aasworld.faces);
	aasworld.faces = nullptr;
	aasworld.faceindexsize = 0;
	if (aasworld.faceindex) FreeMemory(aasworld.faceindex);
	aasworld.faceindex = nullptr;
	aasworld.numareas = 0;
	if (aasworld.areas) FreeMemory(aasworld.areas);
	aasworld.areas = nullptr;
	aasworld.numareasettings = 0;
	if (aasworld.areasettings) FreeMemory(aasworld.areasettings);
	aasworld.areasettings = nullptr;
	aasworld.reachabilitysize = 0;
	if (aasworld.reachability) FreeMemory(aasworld.reachability);
	aasworld.reachability = nullptr;
	aasworld.numnodes = 0;
	if (aasworld.nodes) FreeMemory(aasworld.nodes);
	aasworld.nodes = nullptr;
	aasworld.numportals = 0;
	if (aasworld.portals) FreeMemory(aasworld.portals);
	aasworld.portals = nullptr;
	aasworld.numportals = 0;
	if (aasworld.portalindex) FreeMemory(aasworld.portalindex);
	aasworld.portalindex = nullptr;
	aasworld.portalindexsize = 0;
	if (aasworld.clusters) FreeMemory(aasworld.clusters);
	aasworld.clusters = nullptr;
...
char* AAS_LoadAASLump(const fileHandle_t fp, const int offset, const int length, int* lastoffset, const int size)
{
	...
	{
		//just alloc a dummy
		return static_cast<char*>(GetClearedHunkMemory(size + 1));
	} //end if
	//seek to the data
	...
	//allocate memory
	const auto buf = static_cast<char*>(GetClearedHunkMemory(length + 1));
	//read the data
	if (length)
	{
...
int AAS_LoadAASFile(char* filename)
{
	...
	{
		AAS_DData(reinterpret_cast<unsigned char*>(&header) + 8, sizeof(aas_header_t) - 8);
	} //end if
	//
	...
	//load the lumps:
	//bounding boxes
	int offset = header.lumps[AASLUMP_BBOXES].fileofs;
	int length = header.lumps[AASLUMP_BBOXES].filelen;
	aasworld.bboxes = reinterpret_cast<aas_bbox_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_bbox_t)));
	...
	aasworld.edges = reinterpret_cast<aas_edge_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_edge_t)));
	...
	aasworld.edgeindex = reinterpret_cast<aas_edgeindex_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_edgeindex_t)));
	...
	aasworld.faces = reinterpret_cast<aas_face_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_face_t)));
	...
	aasworld.faceindex = reinterpret_cast<aas_faceindex_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_faceindex_t)));
	...
	aasworld.areas = reinterpret_cast<aas_area_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_area_t)));
	...
	aasworld.areasettings = reinterpret_cast<aas_areasettings_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_areasettings_t)));
	...
	aasworld.reachability = reinterpret_cast<aas_reachability_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_reachability_t)));
	...
	aasworld.nodes = reinterpret_cast<aas_node_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_node_t)));
	...
	aasworld.portals = reinterpret_cast<aas_portal_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_portal_t)));
	...
	aasworld.portalindex = reinterpret_cast<aas_portalindex_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_portalindex_t)));
	...
	aasworld.clusters = reinterpret_cast<aas_cluster_t*>(AAS_LoadAASLump(fp, offset, length, &lastoffset, sizeof(aas_cluster_t)));
...
int AAS_WriteAASLump(const fileHandle_t fp, aas_header_t* h, const int lumpnum, const void* data, const int length)
{
