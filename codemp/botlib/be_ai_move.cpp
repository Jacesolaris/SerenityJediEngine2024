...
using bot_movestate_t = struct bot_movestate_s
{
	//input vars (all set outside the movement code)
	vec3_t origin;								//origin of the bot
	vec3_t velocity;							//velocity of the bot
	vec3_t viewoffset;							//view offset
	int entity_num;								//entity number of the bot
...
#define AVOIDREACH
constexpr auto AVOIDREACH_TIME = 6;		//avoid links for 6 seconds after use;
constexpr auto AVOIDREACH_TRIES = 4;
//prediction times
...
constexpr auto MODELTYPE_FUNC_DOOR = 3;
constexpr auto MODELTYPE_FUNC_STATIC = 4;
...
int BotAllocMoveState(void)
{
	for (int i = 1; i <= MAX_CLIENTS; i++)
	{
		if (!botmovestates[i])
		{
			botmovestates[i] = static_cast<bot_movestate_s*>(GetClearedMemory(sizeof(bot_movestate_t)));
			return i;
		} //end if
	} //end for
	return 0;
...
// Changes Globals:		-
//========================================================================
void BotFreeMoveState(const int handle)
{
...
// Changes Globals:		-
//========================================================================
bot_movestate_t* BotMoveStateFromHandle(const int handle)
{
	if (handle <= 0 || handle > MAX_CLIENTS)
	{
		botimport.Print(PRT_FATAL, "move state handle %d out of range\n", handle);
...
// Changes Globals:		-
//========================================================================
void BotInitMoveState(const int handle, bot_initmove_t* initmove)
{
	...
	ms->entity_num = initmove->entity_num;
...
// Changes Globals:		-
//========================================================================
float AngleDiff(const float ang1, const float ang2)
{
...
int BotFuzzyPointReachabilityArea(vec3_t origin)
{
	int j;
	int areas[10];
	vec3_t points[10], end;

	int firstareanum = 0;
	const int areanum = AAS_PointAreaNum(origin);
	if (areanum)
	{
...
// Changes Globals:		-
//===========================================================================
int BotReachabilityArea(vec3_t origin, const int client)
{
	aas_reachability_t reach;
	vec3_t end, mins, maxs, up = { 0, 0, 1 };

	//check if the bot is standing on something
	AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH, mins, maxs);
	VectorMA(origin, -3, up, end);
	const bsp_trace_t bsptrace = AAS_Trace(origin, mins, maxs, end, client, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
	if (!bsptrace.startsolid && bsptrace.fraction < 1 && bsptrace.ent != ENTITYNUM_NONE)
	{
		...
		const int modelnum = AAS_Entitymodel_index(bsptrace.ent);
		const int modeltype = modeltypes[modelnum];
		...
		{
			const int reachnum = AAS_NextModelReachability(0, modelnum);
			if (reachnum)
			{
		...
		//
		const int areanum = BotFuzzyPointReachabilityArea(origin);
		...
		const aas_trace_t trace = AAS_TraceClientBBox(org, end, PRESENCE_CROUCH, -1);
		if (!trace.startsolid)
		{
...
// Changes Globals:		-
//===========================================================================
int BotOnMover(vec3_t origin, const int entnum, aas_reachability_t* reach)
{
	vec3_t mins, maxs, modelorigin, org, end;
	vec3_t angles = { 0, 0, 0 };
	vec3_t boxmins = { -16, -16, -8 }, boxmaxs = { 16, 16, 8 };

	const int modelnum = reach->facenum & 0x0000FFFF;
	...
	//
	const bsp_trace_t trace = AAS_Trace(org, boxmins, boxmaxs, end, entnum, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
...
int BotOnTopOfEntity(bot_movestate_t* ms)
{
	...
	const bsp_trace_t trace = AAS_Trace(ms->origin, mins, maxs, end, ms->entity_num, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
...
// Changes Globals:		-
//===========================================================================
int BotValidTravel(vec3_t origin, aas_reachability_t* reach, const int travelflags)
{
...
// Changes Globals:		-
//===========================================================================
void BotAddToAvoidReach(bot_movestate_t* ms, const int number, const float avoidtime)
{
	int i;
...
// Changes Globals:		-
//===========================================================================
int BotAvoidSpots(vec3_t origin, aas_reachability_t* reach, bot_avoidspot_t* avoidspots, const int numavoidspots)
{
	int checkbetween;
	...
	{
		const float squaredradius = Square(avoidspots[i].radius);
...
// Changes Globals:		-
//===========================================================================
void BotAddAvoidSpot(const int movestate, vec3_t origin, const float radius, const int type)
{
...
// Changes Globals:		-
//===========================================================================
int BotGetReachabilityToGoal(vec3_t origin, const int areanum,
	const int lastgoalareanum, const int lastareanum,
	int* avoidreach, float* avoidreachtimes, int* avoidreachtries,
	bot_goal_t* goal, int travelflags,
	bot_avoidspot_s* avoidspots, const int numavoidspots, int* flags)
{
	int i, t;
	aas_reachability_t reach;
...
// Changes Globals:		-
//===========================================================================
int BotAddToTarget(vec3_t start, vec3_t end, const float maxdist, float* dist, vec3_t target)
{
	vec3_t dir;

	VectorSubtract(end, start, dir);
	const float curdist = VectorNormalize(dir);
...
int BotMovementViewTarget(const int movestate, bot_goal_t* goal, const int travelflags, const float lookahead, vec3_t target)
{
	aas_reachability_t reach;
	vec3_t end;
	float dist;
	...
	while (reachnum && dist < lookahead)
	{
		...
		reachnum = BotGetReachabilityToGoal(reach.end, reach.areanum,
			...
			goal, travelflags, nullptr, 0, nullptr);
...
// Changes Globals:		-
//===========================================================================
int BotVisible(const int ent, vec3_t eye, vec3_t target)
{
	const bsp_trace_t trace = AAS_Trace(eye, nullptr, nullptr, target, ent, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
...
// Changes Globals:		-
//===========================================================================
int BotPredictVisiblePosition(vec3_t origin, int areanum, bot_goal_t* goal, const int travelflags, vec3_t target)
{
	aas_reachability_t reach;
	...
	const int lastgoalareanum = goal->areanum;
	...
	for (int i = 0; i < 20 && areanum != goal->areanum; i++)
	{
		...
		//
		const int reachnum = BotGetReachabilityToGoal(end, areanum,
		...
		AAS_ReachabilityFromNum(reachnum, &reach);
		//
		if (BotVisible(goal->entity_num, goal->origin, reach.start))
		{
			VectorCopy(reach.start, target);
			return qtrue;
		} //end if
		//
		if (BotVisible(goal->entity_num, goal->origin, reach.end))
		{
...
// Changes Globals:		-
//===========================================================================
float BotGapDistance(vec3_t origin, vec3_t hordir, const int entnum)
{
	float startz;
	vec3_t start, end;
	aas_trace_t trace;
	...
	//
	for (int dist = 8; dist <= 100; dist += 8)
	{
...
// Changes Globals:		-
//===========================================================================
int BotCheckBarrierJump(bot_movestate_t* ms, vec3_t dir, const float speed)
{
	...
	//trace right up
	aas_trace_t trace = AAS_TraceClientBBox(ms->origin, end, PRESENCE_NORMAL, ms->entity_num);
	...
	trace = AAS_TraceClientBBox(start, end, PRESENCE_NORMAL, ms->entity_num);
...
// Changes Globals:		-
//===========================================================================
int BotSwimInDirection(bot_movestate_t* ms, vec3_t dir, const float speed, int type)
{
	vec3_t normdir;
...
// Changes Globals:		-
//===========================================================================
int BotWalkInDirection(bot_movestate_t* ms, vec3_t dir, const float speed, int type)
{
	aas_clientmove_t move;

	if (AAS_OnGround(ms->origin, ms->presencetype, ms->entity_num)) ms->moveflags |= MFL_ONGROUND;
	...
	if (ms->moveflags & MFL_ONGROUND)
	{
		...
		if (!(type & MOVE_JUMP))
		{
			...
			if (BotGapDistance(ms->origin, hordir, ms->entity_num) > 0) type |= MOVE_JUMP;
		} //end if
		...
		origin[2] += 0.5;
		AAS_PredictClientMovement(&move, ms->entity_num, origin, presencetype, qtrue,
		...
		if (move.stopevent & SE_HITGROUND)
		{
			...
			float dist = BotGapDistance(move.endpos, tmpdir, ms->entity_num);
			if (dist > 0) return qfalse;
			//
			dist = BotGapDistance(move.endpos, hordir, ms->entity_num);
			if (dist > 0) return qfalse;
		} //end if
...
// Changes Globals:		-
//===========================================================================
int BotMoveInDirection(const int movestate, vec3_t dir, const float speed, const int type)
{
	bot_movestate_t* ms = BotMoveStateFromHandle(movestate);
	if (!ms) return qfalse;
	//if swimming
...
int Intersection(vec2_t p1, vec2_t p2, vec2_t p3, vec2_t p4, vec2_t out)
{
	...
	const float dy1 = p2[1] - p1[1];
	const float dx2 = p4[0] - p3[0];
	const float dy2 = p4[1] - p3[1];

	const float d = dy1 * dx2 - dx1 * dy2;
	if (d != 0)
	{
		const float x1 = p1[1] * dx1 - p1[0] * dy1;
		const float x2 = p3[1] * dx2 - p3[0] * dy2;
		out[0] = static_cast<int>((dx1 * x2 - dx2 * x1) / d);
		out[1] = static_cast<int>((dy1 * x2 - dy2 * x1) / d);
		return qtrue;
	} //end if
	return qfalse;
	//end else
...
// Changes Globals:		-
//===========================================================================
void BotCheckBlocked(bot_movestate_t* ms, vec3_t dir, const int checkbottom, bot_moveresult_t* result)
{
	vec3_t mins, maxs, end, up = { 0, 0, 1 };
	...
	bsp_trace_t trace = AAS_Trace(ms->origin, mins, maxs, end, ms->entity_num,
	...
	else if (checkbottom && !AAS_AreaReachability(ms->areanum))
	{
		...
		AAS_PresenceTypeBoundingBox(ms->presencetype, mins, maxs);
		VectorMA(ms->origin, -3, up, end);
		trace = AAS_Trace(ms->origin, mins, maxs, end, ms->entity_num, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
		if (!trace.startsolid && (trace.ent != ENTITYNUM_WORLD && trace.ent != ENTITYNUM_NONE))
		{
			result->blocked = qtrue;
			result->blockentity = trace.ent;
...
bot_moveresult_t BotTravel_Walk(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	//
	dist = BotGapDistance(ms->origin, hordir, ms->entity_num);
	//
...
bot_moveresult_t BotFinishTravel_Walk(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	const float speed = 400 - (400 - 3 * dist);
	//
	EA_Move(ms->client, hordir, speed);
	VectorCopy(hordir, result.movedir);
	//
...
bot_moveresult_t BotTravel_BarrierJump(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	{
		if (dist > 60) dist = 60;
		const float speed = 360 - (360 - 6 * dist);
		EA_Move(ms->client, hordir, speed);
	} //end else
	VectorCopy(hordir, result.movedir);
	//
...
bot_moveresult_t BotTravel_WaterJump(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	VectorNormalize(dir);
	const float dist = VectorNormalize(hordir);
	//elemantary actions
	//EA_Move(ms->client, dir, 400);
...
bot_moveresult_t BotTravel_WalkOffLedge(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	dir[2] = 0;
	const float reachhordist = VectorLength(dir);
...
bot_moveresult_t BotTravel_Jump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	speed = 350;
	//
	gapdist = BotGapDistance(ms, hordir, ms->entity_num);
	//if pretty close to the start focus on the reachability end
...
bot_moveresult_t BotTravel_Jump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	//check for solids
	trace = AAS_Trace(start, mins, maxs, end, ms->entity_num, MASK_PLAYERSOLID);
...
//	dist1 = BotGapDistance(start, hordir, ms->entity_num);
...
bot_moveresult_t BotTravel_Jump(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	VectorSubtract(ms->origin, reach->start, dir1);
	dir1[2] = 0;
	const float dist1 = VectorNormalize(dir1);
	...
	else
	{
		...
		//
		if (dist2 > 80) dist2 = 80;
		const float speed = 400 - (400 - 5 * dist2);
		EA_Move(ms->client, hordir, speed);
	} //end else
	VectorCopy(hordir, result.movedir);
	//
...
bot_moveresult_t BotTravel_Elevator(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	if (BotOnMover(ms->origin, ms->entity_num, reach))
	{
		vec3_t hordir;
...
void BotFuncBobStartEnd(aas_reachability_t* reach, vec3_t start, vec3_t end, vec3_t origin)
{
	...
	VectorCopy(mid, end);
	const int spawnflags = reach->facenum >> 16;
...
{
	vec3_t dir, bottomcenter, bob_start, bob_end, bob_origin;
	float dist, speed;
	bot_moveresult_t_cleared(result);
...
{
	vec3_t bob_origin, bob_start, bob_end, dir, hordir;
	bot_moveresult_t_cleared(result);
	float speed;
...
int GrappleState(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	{
		if (AAS_EntityType(i) == static_cast<int>(entitytypemissile->value))
		{
			AAS_EntityInfo(i, &entinfo);
			if (entinfo.weapon == static_cast<int>(weapindex_grapple->value))
			{
				return 1;
			} //end if
		} //end if
...
bot_moveresult_t BotTravel_Grapple(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	//
	if (!static_cast<int>(offhandgrapple->value))
	{
	...
	else
	{
			...
			fabs(AngleDiff(result.ideal_viewangles[1], ms->viewangles[1])) < 2)
		{
			...
			VectorAdd(ms->origin, ms->viewoffset, org);
			trace = AAS_Trace(org, nullptr, nullptr, reach->end, ms->entity_num, CONTENTS_SOLID);
...
bot_moveresult_t BotTravel_RocketJump(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	{
		if (dist > 80) dist = 80;
		const float speed = 400 - (400 - 5 * dist);
		EA_Move(ms->client, hordir, speed);
	} //end else
	//look in the movement direction
	Vector2Angles(hordir, result.ideal_viewangles);
	...
	EA_SelectWeapon(ms->client, static_cast<int>(weapindex_rocketlauncher->value));
	//weapon is used for movement
	result.weapon = static_cast<int>(weapindex_rocketlauncher->value);
	result.flags |= MOVERESULT_MOVEMENTWEAPON;
	//
	VectorCopy(hordir, result.movedir);
	//
...
bot_moveresult_t BotTravel_BFGJump(bot_movestate_t* ms, aas_reachability_t* reach)
{
	...
	result.flags |= MOVERESULT_MOVEMENTVIEWSET;
	//select the rocket launcher
	EA_SelectWeapon(ms->client, static_cast<int>(weapindex_bfg10k->value));
	//weapon is used for movement
	result.weapon = static_cast<int>(weapindex_bfg10k->value);
	result.flags |= MOVERESULT_MOVEMENTWEAPON;
	//
	VectorCopy(hordir, result.movedir);
	//
...
// Changes Globals:		-
//===========================================================================
void BotMoveToGoal(bot_moveresult_t* result, const int movestate, bot_goal_t* goal, const int travelflags)
{
	...
	if (ms->moveflags & MFL_ONGROUND)
	{
		const int ent = BotOnTopOfEntity(ms);

		if (ent != -1)
		{
			const int modelnum = AAS_Entitymodel_index(ent);
			if (modelnum >= 0 && modelnum < MAX_MODELS)
			{
				const int modeltype = modeltypes[modelnum];
	...
	else
	{
		...
		int foundjumppad = qfalse;
		VectorMA(ms->origin, -2 * ms->thinktime, ms->velocity, end);
		const int numareas = AAS_TraceAreas(ms->origin, end, areas, nullptr, 16);
		for (int i = numareas - 1; i >= 0; i--)
		{
			if (AAS_AreaJumpPad(areas[i]))
			{
				...
				int lastreachnum = BotGetReachabilityToGoal(end, areas[i],
					ms->lastgoalareanum, ms->lastareanum,
					ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries,
					goal, TFL_JUMPPAD, ms->avoidspots, ms->numavoidspots, nullptr);
				if (lastreachnum)
				{
...
// Changes Globals:		-
//===========================================================================
void BotResetAvoidReach(const int movestate)
{
	bot_movestate_t* ms = BotMoveStateFromHandle(movestate);
	if (!ms) return;
...
// Changes Globals:		-
//===========================================================================
void BotResetMoveState(const int movestate)
{
	bot_movestate_t* ms = BotMoveStateFromHandle(movestate);
	if (!ms) return;
