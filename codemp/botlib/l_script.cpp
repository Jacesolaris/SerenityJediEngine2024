...
void PS_CreatePunctuationTable(script_t* script, punctuation_t* punctuations)
{
	...
	if (!script->punctuationtable) script->punctuationtable = static_cast<punctuation_t**>(GetMemory(256 * sizeof(punctuation_t*)));
	...
	for (int i = 0; punctuations[i].p; i++)
	{
		...
		punctuation_t* lastp = nullptr;
		//sort the punctuations in this table entry on length (longer punctuations first)
		for (p = script->punctuationtable[static_cast<unsigned>(newp->p[0])]; p; p = p->next)
		{
			if (strlen(p->p) < strlen(newp->p))
			{
				newp->next = p;
				if (lastp) lastp->next = newp;
				else script->punctuationtable[static_cast<unsigned>(newp->p[0])] = newp;
				break;
			} //end if
			lastp = p;
		} //end for
	...
	*floatvalue = 0;
	...
	if (subtype & TT_FLOAT)
	{
		...
		while (*string)
		{
			...
			if (dotfound)
			{
				*floatvalue = *floatvalue + static_cast<long double>(*string - '0') /
					static_cast<long double>(dotfound);
				dotfound *= 10;
			} //end if
			else
			{
				*floatvalue = *floatvalue * 10.0 + static_cast<long double>(*string - '0');
			} //end else
			string++;
		} //end while
		*intvalue = static_cast<unsigned long>(*floatvalue);
	} //end if
...
#ifdef PUNCTABLE
	for (punctuation_t* punc = script->punctuationtable[static_cast<unsigned>(*script->script_p)]; punc; punc = punc->next)
	{
#else
	int i;
...
int PS_CheckTokenString(script_t * script, char* string)
{
	...
	if (strcmp(tok.string, string) == 0) return 1;
	//token not available
...
int PS_SkipUntilString(script_t * script, char* string)
{
	...
	{
		if (strcmp(token.string, string) == 0) return 1;
	} //end while
	return 0;
...
script_t* LoadScriptFile(const char* filename)
{
	...
	script = static_cast<script_t*>(buffer);
	Com_Memset(script, 0, sizeof(script_t));
	Q_strncpyz(script->filename, filename, sizeof script->filename);
	script->buffer = static_cast<char*>(buffer) + sizeof(script_t);
...
#endif
	//
	script->length = COM_CompressShader(script->buffer);
...
script_t* LoadScriptMemory(char* ptr, const int length, char* name)
{
	...
	const auto script = static_cast<script_t*>(buffer);
	Com_Memset(script, 0, sizeof(script_t));
