...
constexpr auto AVOID_MINIMUM_TIME = 10;
//default avoid goal time
constexpr auto AVOID_DEFAULT_TIME = 30;
//avoid dropped goal time
constexpr auto AVOID_DROPPED_TIME = 10;
//
constexpr auto TRAVELTIME_SCALE = 0.01;
//item flags
constexpr auto IFL_NOTFREE = 1;		//not in free for all;
constexpr auto IFL_NOTTEAM = 2;	//not in team play;
constexpr auto IFL_NOTSINGLE = 4;		//not in single player;
constexpr auto IFL_NOTBOT = 8;		//bot should never go for this;
constexpr auto IFL_ROAM = 16;		//bot roam goal;

//location in the map "target_location"
using maplocation_t = struct maplocation_s
{
	vec3_t origin;
	int areanum;
...
using campspot_t = struct campspot_s
{
	vec3_t origin;
	int areanum;
	char name[MAX_EPAIRKEY];
...
using levelitem_t = struct levelitem_s
{
...
using iteminfo_t = struct iteminfo_s
{
...
using itemconfig_t = struct itemconfig_s
{
	int numiteminfo;
	iteminfo_t* iteminfo;
};
...
//goal state
using bot_goalstate_t = struct bot_goalstate_s
{
	weightconfig_s* itemweightconfig;	//weight config
...
// Changes Globals:		-
//========================================================================
bot_goalstate_t* BotGoalStateFromHandle(const int handle)
{
...
// Changes Globals:		-
//===========================================================================
void BotInterbreedGoalFuzzyLogic(const int parent1, const int parent2, const int child)
{
	const bot_goalstate_t* p1 = BotGoalStateFromHandle(parent1);
	const bot_goalstate_t* p2 = BotGoalStateFromHandle(parent2);
	const bot_goalstate_t* c = BotGoalStateFromHandle(child);
...
// Changes Globals:		-
//===========================================================================
void BotMutateGoalFuzzyLogic(const int goalstate, float range)
{
...
itemconfig_t* LoadItemConfig(char* filename)
{
	...
	int max_iteminfo = static_cast<int>(LibVarValue("max_iteminfo", "256"));
	if (max_iteminfo < 0)
	{
	...
	const auto ic = static_cast<itemconfig_t*>(GetClearedHunkMemory(sizeof(itemconfig_t) +
		max_iteminfo * sizeof(iteminfo_t)));
	ic->iteminfo = reinterpret_cast<iteminfo_t*>(reinterpret_cast<char*>(ic) + sizeof(itemconfig_t));
	ic->numiteminfo = 0;
	...
	while (PC_ReadToken(source, &token))
	{
		if (strcmp(token.string, "iteminfo") == 0)
		{
			...
			if (!ReadStructure(source, &iteminfo_struct, reinterpret_cast<char*>(ii)))
			{
				FreeMemory(ic);
				FreeSource(source);
				return nullptr;
...
int* ItemWeightIndex(weightconfig_t* iwc, itemconfig_t* ic)
{
	...
	const auto index = static_cast<int*>(GetClearedMemory(sizeof(int) * ic->numiteminfo));
...
void InitLevelItemHeap(void)
{
	...
	const int max_levelitems = static_cast<int>(LibVarValue("max_levelitems", "256"));
	levelitemheap = static_cast<levelitem_t*>(GetClearedMemory(max_levelitems * sizeof(levelitem_t)));
...
void BotInitInfoEntities(void)
{
	...
	for (int ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		//map locations
		if (strcmp(classname, "target_location") == 0)
		{
			const auto ml = static_cast<maplocation_t*>(GetClearedMemory(sizeof(maplocation_t)));
		...
		//camp spots
		else if (strcmp(classname, "info_camp") == 0)
		{
			const auto cs = static_cast<campspot_t*>(GetClearedMemory(sizeof(campspot_t)));
...
void BotInitLevelItems(void)
{
	...
	numlevelitems = 0;
	//
	const itemconfig_t* ic = itemconfig;
	if (!ic) return;
	...
	for (int ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		for (i = 0; i < ic->numiteminfo; i++)
		{
			if (strcmp(classname, ic->iteminfo[i].classname) == 0) break;
		} //end for
		if (i >= ic->numiteminfo)
		{
		...
		if (spawnflags & 1)
		{
			...
			if (!(AAS_PointContents(origin) & CONTENTS_WATER))
			{
				...
				const bsp_trace_t trace = AAS_Trace(origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs, end, -1,
		...
		if (strcmp(classname, "item_botroam") == 0)
		{
			li->flags |= IFL_ROAM;
...
// Changes Globals:		-
//===========================================================================
void BotGoalName(const int number, char* name, const int size)
{
...
// Changes Globals:		-
//===========================================================================
void BotResetAvoidGoals(const int goalstate)
{
...
// Changes Globals:		-
//===========================================================================
void BotDumpAvoidGoals(const int goalstate)
{
	const bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
...
// Changes Globals:		-
//===========================================================================
void BotAddToAvoidGoals(bot_goalstate_t* gs, const int number, const float avoidtime)
{
	int i;
...
// Changes Globals:		-
//===========================================================================
void BotRemoveFromAvoidGoals(const int goalstate, const int number)
{
	bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
...
// Changes Globals:		-
//===========================================================================
float BotAvoidGoalTime(const int goalstate, const int number)
{
	const bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return 0;
	//don't use the goals the bot wants to avoid
...
// Changes Globals:		-
//===========================================================================
void BotSetAvoidGoalTime(const int goalstate, const int number, float avoidtime)
{
	bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs)
		return;
...
// Changes Globals:		-
//===========================================================================
int BotGetLevelItemGoal(const int index, const char* name, bot_goal_t* goal)
{
	if (!itemconfig) return -1;
	const levelitem_t* li = levelitems;
	if (index >= 0)
	{
		for (; li; li = li->next)
		{
...
{
	constexpr vec3_t mins = { -8, -8, -8 }, maxs = { 8, 8, 8 };

	for (const maplocation_t* ml = maplocations; ml; ml = ml->next)
	{
...
int BotGetNextCampSpotGoal(int num, bot_goal_t* goal)
{
	...
	if (num < 0) num = 0;
	int i = num;
	for (const campspot_t* cs = campspots; cs; cs = cs->next)
	{
		if (--i < 0)
		{
...
void BotFindEntityForLevelItem(levelitem_t* li)
{
	...
	for (int ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent))
	{
		...
		const int modelindex = AAS_EntityModelindex(ent);
		//
...
// Changes Globals:		-
//===========================================================================
void BotDumpGoalStack(const int goalstate)
{
	const bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
...
// Changes Globals:		-
//===========================================================================
void BotPushGoal(const int goalstate, bot_goal_t* goal)
{
	bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
...
// Changes Globals:		-
//===========================================================================
void BotPopGoal(const int goalstate)
{
	bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
...
// Changes Globals:		-
//===========================================================================
void BotEmptyGoalStack(const int goalstate)
{
	bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
	gs->goalstacktop = 0;
...
// Changes Globals:		-
//===========================================================================
int BotGetTopGoal(const int goalstate, bot_goal_t* goal)
{
	const bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return qfalse;
...
// Changes Globals:		-
//===========================================================================
int BotGetSecondGoal(const int goalstate, bot_goal_t* goal)
{
	const bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return qfalse;
...
// Changes Globals:		-
//===========================================================================
int BotChooseLTGItem(const int goalstate, vec3_t origin, int* inventory, const int travelflags)
{
	int t;
	float weight, avoidtime;
	iteminfo_t* iteminfo;
	bot_goal_t goal;
	...
	float bestweight = 0;
	const levelitem_t* bestitem = nullptr;
		...
		if (weight > 0)
		{
			...
			if (t > 0)
			{
				...
				if (avoidtime - t * 0.009 > 0)
					continue;
				//
				weight /= static_cast<float>(t) * TRAVELTIME_SCALE;
				//
...
// Changes Globals:		-
//===========================================================================
int BotChooseNBGItem(const int goalstate, vec3_t origin, int* inventory, const int travelflags,
	bot_goal_t* ltg, const float maxtime)
{
	int t, ltg_time;
	float weight, avoidtime;
	iteminfo_t* iteminfo;
...
int BotTouchingGoal(vec3_t origin, bot_goal_t* goal)
{
	...
	constexpr vec3_t safety_maxs = { 0, 0, 0 }; //{4, 4, 10};
	constexpr vec3_t safety_mins = { 0, 0, 0 }; //{-4, -4, 0};
...
// Changes Globals:		-
//===========================================================================
int BotItemGoalInVisButNotVisible(const int viewer, vec3_t eye, vec3_t viewangles, bot_goal_t* goal)
{
	aas_entityinfo_t entinfo;
	vec3_t middle;
	...
	//
	const bsp_trace_t trace = AAS_Trace(eye, nullptr, nullptr, middle, viewer, CONTENTS_SOLID);
...
// Changes Globals:		-
//===========================================================================
void BotResetGoalState(const int goalstate)
{
	bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
...
// Changes Globals:		-
//===========================================================================
int BotLoadItemWeights(const int goalstate, char* filename)
{
	bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
...
// Changes Globals:		-
//===========================================================================
void BotFreeItemWeights(const int goalstate)
{
	const bot_goalstate_t* gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
...
// Changes Globals:		-
//===========================================================================
int BotAllocGoalState(const int client)
{
	for (int i = 1; i <= MAX_CLIENTS; i++)
	{
		if (!botgoalstates[i])
		{
			botgoalstates[i] = static_cast<bot_goalstate_s*>(GetClearedMemory(sizeof(bot_goalstate_t)));
...
// Changes Globals:		-
//========================================================================
void BotFreeGoalState(const int handle)
{
	if (handle <= 0 || handle > MAX_CLIENTS)
	{
		botimport.Print(PRT_FATAL, "goal state handle %d out of range\n", handle);
