...
int AAS_OptimizeFace(optimized_t* optimized, const int facenum)
{
	...
	int optfacenum = optimized->faceoptimizeindex[abs(facenum)];
	if (optfacenum)
	{
	...
	aas_face_t* optface = &optimized->faces[optimized->numfaces];
	...
	for (int i = 0; i < face->numedges; i++)
	{
		...
		const int optedgenum = AAS_OptimizeEdge(optimized, edgenum);
		if (optedgenum)
		{
...
void AAS_OptimizeArea(optimized_t* optimized, const int areanum)
{
	...
	for (int i = 0; i < area->numfaces; i++)
	{
		...
		const int optfacenum = AAS_OptimizeFace(optimized, facenum);
		if (optfacenum)
		{
...
void AAS_OptimizeAlloc(optimized_t* optimized)
{
	...
	optimized->faces = static_cast<aas_face_t*>(GetClearedMemory(aasworld.numfaces * sizeof(aas_face_t)));
	optimized->numfaces = 1; //face zero is a dummy
	optimized->faceindex = static_cast<aas_faceindex_t*>(GetClearedMemory(aasworld.faceindexsize * sizeof(aas_faceindex_t)));
