...
constexpr auto AAS_MAX_PORTALS = 65536;
constexpr auto AAS_MAX_PORTALINDEXSIZE = 65536;
constexpr auto AAS_MAX_CLUSTERS = 65536;
//
constexpr auto MAX_PORTALAREAS = 1024;
...
int AAS_FloodClusterAreas_r(const int areanum, const int clusternum)
{
	...
	if (!nofaceflood)
	{
		...
		{
			const int facenum = abs(aasworld.faceindex[area->firstface + i]);
...
int AAS_FloodClusterAreasUsingReachabilities(const int clusternum)
{
	for (int i = 1; i < aasworld.numareas; i++)
	{
		...
		{
			//the reachable area
			const int areanum = aasworld.reachability[aasworld.areasettings[i].firstreachablearea + j].areanum;
			//if this area is a cluster portal
			if (aasworld.areasettings[areanum].contents & AREACONTENTS_CLUSTERPORTAL)
				continue;
...
void AAS_ConnectedAreas_r(int* areanums, const int numareas, int* connectedareas, const int curarea)
{
	...
	const aas_area_t* area = &aasworld.areas[areanums[curarea]];
...
int AAS_GetAdjacentAreasWithLessPresenceTypes_r(int* areanums, int numareas, const int curareanum)
{
	...
	const aas_area_t* area = &aasworld.areas[curareanum];
	const int presencetype = aasworld.areasettings[curareanum].presencetype;
	for (int i = 0; i < area->numfaces; i++)
	{
		const int facenum = abs(aasworld.faceindex[area->firstface + i]);
		const aas_face_t* face = &aasworld.faces[facenum];
		...
		//
		const int otherpresencetype = aasworld.areasettings[otherareanum].presencetype;
...
int AAS_TestPortals()
{
	...
	{
		const aas_portal_t* portal = &aasworld.portals[i];
		if (!portal->frontcluster)
		{
...
void AAS_InitClustering()
{
	...
	if (aasworld.numclusters >= 1)
	{
		...
		if (!static_cast<int>(LibVarGetValue("forceclustering")) &&
			!static_cast<int>(LibVarGetValue("forcereachability"))) return;
#endif
	} //end if
	...
	aasworld.portals = static_cast<aas_portal_t*>(GetClearedMemory(AAS_MAX_PORTALS * sizeof(aas_portal_t)));
	//initialize portal index memory
	if (aasworld.portalindex) FreeMemory(aasworld.portalindex);
	aasworld.portalindex = static_cast<aas_portalindex_t*>(GetClearedMemory(AAS_MAX_PORTALINDEXSIZE * sizeof(aas_portalindex_t)));
	//initialize cluster memory
	if (aasworld.clusters) FreeMemory(aasworld.clusters);
	aasworld.clusters = static_cast<aas_cluster_t*>(GetClearedMemory(AAS_MAX_CLUSTERS * sizeof(aas_cluster_t)));
	//
	...
	for (i = 0; i < aasworld.numclusters; i++) {
		const int n = aasworld.clusters[i].numreachabilityareas;
