...
//	Swap_Init();
	...
	if (botDeveloper)
	{
		...
		char* homedir = LibVarGetString("homedir");
		char* gamedir = LibVarGetString("gamedir");
		char* basedir = LibVarGetString("com_basegame");
...
//	botimport.Print(PRT_MESSAGE, "------- BotLib Initialization -------\n");
	//
	botlibglobals.maxclients = static_cast<int>(LibVarValue("maxclients", "128"));
	botlibglobals.maxentities = static_cast<int>(LibVarValue("maxentities", "1024"));
...
int Export_BotLibLoadMap(const char* mapname)
{
	...
	const int errnum = AAS_LoadMap(mapname);
	if (errnum != BLERR_NOERROR) return errnum;
...
int BotGetReachabilityToGoal(vec3_t origin, int areanum,
	...
	bot_avoidspot_s* avoidspots, int numavoidspots, int* flags);
