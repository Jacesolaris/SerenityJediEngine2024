...
using directive_t = struct directive_s
{
	char* name;
...
constexpr auto TOKEN_HEAP_SIZE = 4096;
...
#if DEFINEHASHING
define_t** globaldefines = nullptr;
#else
define_t* globaldefines = nullptr;
#endif
...
{
	const auto indent = static_cast<indent_t*>(GetMemory(sizeof(indent_t)));
	indent->type = type;
...
{
	for (const script_t* s = source->scriptstack; s; s = s->next)
	{
...
token_t* PC_CopyToken(token_t* token)
{
	...
	const auto t = static_cast<token_t*>(GetMemory(sizeof(token_t)));
	//	t = freetokens;
	if (!t)
	{
#ifdef BSPC
...
int PC_ReadDefineParms(source_t* source, define_t* define, token_t** parms, const int maxparms)
{
	...
	//
	for (int i = 0; i < define->numparms; i++) parms[i] = nullptr;
	//if no leading "("
...
void PC_PrintDefineHashTable(define_t** definehash)
{
	...
	{
		Log_Write("%4d:", i);
		for (const define_t* d = definehash[i]; d; d = d->hashnext)
		{
			Log_Write(" %s", d->name);
		} //end for
		Log_Write("\n");
...
{
	int i = 0;
	for (const token_t* p = define->parms; p; p = p->next)
	{
		if (strcmp(p->string, name) == 0) return i;
		i++;
	} //end for
	return -1;
...
void PC_AddBuiltinDefines(source_t* source)
{
	...
	{
		auto define = static_cast<define_t*>(GetMemory(sizeof(define_t)));
		Com_Memset(define, 0, sizeof(define_t));
		define->name = static_cast<char*>(GetMemory(strlen(builtin[i].string) + 1));
...
#else //DEFINEHASHING
	for (lastdefine = nullptr, define = source->defines; define; define = define->next)
	{
...
int PC_Directive_define(source_t* source)
{
	...
	//allocate define
	define = static_cast<define_t*>(GetMemory(sizeof(define_t)));
	Com_Memset(define, 0, sizeof(define_t));
	define->name = static_cast<char*>(GetMemory(strlen(token.string) + 1));
	...
	if (!PC_WhiteSpaceBeforeToken(&token) && strcmp(token.string, "(") == 0)
	{
		//read the define parameters
		last = nullptr;
	...
	{
		t = PC_CopyToken(&token);
		if (t->type == TT_NAME && strcmp(t->string, define->name) == 0)
		{
	...
	if (last)
	{
		...
		if (strcmp(define->tokens->string, "##") == 0 ||
			strcmp(last->string, "##") == 0)
		{
...
#if DEFINEHASHING
	src.definehash = static_cast<define_s**>(GetClearedMemory(DEFINEHASHSIZE * sizeof(define_t*)));
#endif //DEFINEHASHING
...
void PC_RemoveAllGlobalDefines(void)
{
	...
	if (globaldefines)
	{
		for (int i = 0; i < DEFINEHASHSIZE; i++)
		{
			...
			{
				define_t* define = globaldefines[i];
...
define_t* PC_CopyDefine(source_t* source, define_t* define)
{
	...
	const auto newdefine = static_cast<define_t*>(GetMemory(sizeof(define_t)));
	//copy the define name
	newdefine->name = static_cast<char*>(GetMemory(strlen(define->name) + 1));
...
// Changes Globals:		-
//============================================================================
using operator_t = struct operator_s
{
	int mOperator;
	int priority;
	int parentheses;
...
constexpr auto MAX_VALUES = 64;
constexpr auto MAX_OPERATORS = 64;
	...
	double* floatvalue, int integer)
{
	...
	for (t = tokens; t; t = t->next)
	{
		...
		case TT_NUMBER:
		{
			...
			if (negativevalue)
			{
				v->intvalue = -static_cast<int>(t->intvalue);
	...
	double* floatvalue, int integer)
{
	...
	token_t* firsttoken = nullptr;
	token_t* lasttoken = nullptr;
	do
	{
...
int PC_Directive_elif(source_t* source)
{
	...
	if (!PC_Evaluate(source, &value, nullptr, qtrue)) return qfalse;
	skip = value == 0;
...
int PC_Directive_evalfloat(source_t* source)
{
	...
	if (!PC_Evaluate(source, nullptr, &value, qfalse)) return qfalse;
	token.line = source->scriptstack->line;
	token.whitespace_p = source->scriptstack->script_p;
	token.endwhitespace_p = source->scriptstack->script_p;
	token.linescrossed = 0;
...
int PC_ReadDirective(source_t* source)
{
	...
	if (token.type == TT_NAME)
	{
		...
		{
			if (strcmp(directives[i].name, token.string) == 0)
			{
...
int PC_DollarDirective_evalint(source_t* source)
{
	...
	if (!PC_DollarEvaluate(source, &value, nullptr, qtrue)) return qfalse;
	//
	token.line = source->scriptstack->line;
	token.whitespace_p = source->scriptstack->script_p;
	token.endwhitespace_p = source->scriptstack->script_p;
...
int PC_DollarDirective_evalfloat(source_t* source)
{
	...
	if (!PC_DollarEvaluate(source, nullptr, &value, qfalse)) return qfalse;
	token.line = source->scriptstack->line;
	token.whitespace_p = source->scriptstack->script_p;
	token.endwhitespace_p = source->scriptstack->script_p;
	token.linescrossed = 0;
	...
	token.intvalue = static_cast<unsigned long>(token.floatvalue);
#endif //NUMBERVALUE
...
int PC_ReadDollarDirective(source_t* source)
{
	...
	if (token.type == TT_NAME)
	{
		...
		{
			if (strcmp(dollardirectives[i].name, token.string) == 0)
			{
...
int PC_ReadToken(source_t* source, token_t* token)
{
	...
	while (true)
	{
		...
		if (token->type == TT_PUNCTUATION && *token->string == '@')	// It is a StringEd key
		{
			...
			PC_ReadSourceToken(source, token);
			char* holdString = &token->string[1];
...
int PC_CheckTokenString(source_t* source, char* string)
{
	...
	if (strcmp(tok.string, string) == 0) return qtrue;
	//
...
int PC_SkipUntilString(source_t* source, char* string)
{
	...
	{
		if (strcmp(token.string, string) == 0) return qtrue;
	} //end while
	return qfalse;
...
void PC_SetIncludePath(source_t* source, char* path)
{
	...
	const size_t len = strlen(source->includepath);
...
source_t* LoadSourceFile(const char* filename)
{
	...
	if (!globaldefines)
	{
		globaldefines = static_cast<define_s**>(GetClearedMemory(DEFINEHASHSIZE * sizeof(define_t*)));
	}
#endif
	...
	source = static_cast<source_t*>(GetMemory(sizeof(source_t)));
...
#if DEFINEHASHING
	source->definehash = static_cast<define_s**>(GetClearedMemory(DEFINEHASHSIZE * sizeof(define_t*)));
#endif //DEFINEHASHING
...
source_t* LoadSourceMemory(char* ptr, const int length, char* name)
{
	...
	const auto source = static_cast<source_t*>(GetMemory(sizeof(source_t)));
	Com_Memset(source, 0, sizeof(source_t));
...
int PC_LoadGlobalDefines(const char* filename)
{
	...
	const int handle = PC_LoadSourceHandle(filename);
	if (handle < 1)
		return qfalse;
...
int PC_ReadTokenHandle(const int handle, pc_token_t* pc_token)
{
	...
	const int ret = PC_ReadToken(sourceFiles[handle], &token);
