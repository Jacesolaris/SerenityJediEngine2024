...
constexpr auto MAX_INVENTORYVALUE = 999999;
#define EVALUATERECURSIVELY

constexpr auto MAX_WEIGHT_FILES = 128;
...
weightconfig_t* ReadWeightConfig(char* filename)
{
	...
	//
	config = static_cast<weightconfig_t*>(GetClearedMemory(sizeof(weightconfig_t)));
	config->numweights = 0;
	...
	while (PC_ReadToken(source, &token))
	{
		if (strcmp(token.string, "weight") == 0)
		{
			...
			config->weights[config->numweights].name = static_cast<char*>(GetClearedMemory(strlen(token.string) + 1));
...
int FindFuzzyWeight(weightconfig_t* wc, char* name)
{
	...
	{
		if (strcmp(wc->weights[i].name, name) == 0)
		{
			return i;
		} //end if
	} //end if
...
float FuzzyWeight_r(int* inventory, fuzzyseperator_t* fs)
{
	...
	if (fs->next)
	{
		if (inventory[fs->index] < fs->next->value)
		{
			...
			const float scale = static_cast<float>(inventory[fs->index] - fs->value) / (fs->next->value - fs->value);
...
float FuzzyWeightUndecided_r(int* inventory, fuzzyseperator_t* fs)
{
	if (inventory[fs->index] < fs->value)
	{
		...
		return fs->minweight + Q_flrand(0.0f, 1.0f) * (fs->maxweight - fs->minweight);
	} //end if
	if (fs->next)
	{
		if (inventory[fs->index] < fs->next->value)
...
void ScaleFuzzySeperatorBalanceRange_r(fuzzyseperator_t* fs, const float scale)
{
	...
	{
		const float mid = (fs->minweight + fs->maxweight) * 0.5;
		//get the weight between bounds
