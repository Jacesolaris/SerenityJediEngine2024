...
void AAS_ContinueInit(const float time)
{
	...
	if (aasworld.savefile || static_cast<int>(LibVarGetValue("forcewrite")))
	{
		//optimize the AAS data
		if (static_cast<int>(LibVarValue("aasoptimize", "0"))) AAS_Optimize();
		//save the AAS file
...
int AAS_LoadFiles(const char* mapname)
{
	...
	const int errnum = AAS_LoadAASFile(aasfile);
...
int AAS_LoadMap(const char* mapname)
{
	...
	//load the map
	const int errnum = AAS_LoadFiles(mapname);
	if (errnum != BLERR_NOERROR)
	{
...
int AAS_Setup(void)
{
	aasworld.maxclients = static_cast<int>(LibVarValue("maxclients", "128"));
	aasworld.maxentities = static_cast<int>(LibVarValue("maxentities", "1024"));
	// as soon as it's set to 1 the routing cache will be saved
	saveroutingcache = LibVar("saveroutingcache", "0");
	//allocate memory for the entities
	if (aasworld.entities) FreeMemory(aasworld.entities);
	aasworld.entities = static_cast<aas_entity_t*>(GetClearedHunkMemory(aasworld.maxentities * sizeof(aas_entity_t)));
