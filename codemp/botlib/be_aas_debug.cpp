...
constexpr auto MAX_DEBUGLINES = 1024;
constexpr auto MAX_DEBUGPOLYGONS = 8192;
...
void AAS_DrawPermanentCross(vec3_t origin, const float size, const int color)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		const int debugline = botimport.DebugLineCreate();
...
void AAS_ShowArea(const int areanum, const int groundfacesonly)
{
	...
	for (int i = 0; i < area->numfaces; i++)
	{
		...
		{
			//edge number
			const int edgenum = abs(aasworld.edgeindex[face->firstedge + j]);
			//check if edge number is in range
			if (edgenum >= aasworld.numedges)
			{
				botimport.Print(PRT_ERROR, "edgenum %d out of range\n", edgenum);
...
void AAS_ShowReachability(aas_reachability_t* reach)
{
	...
	{
		const float zvel = AAS_RocketJumpZVelocity(reach->start);
...
void AAS_FloodAreas_r(const int areanum, const int cluster, int* done)
{
	...
	{
		const aas_reachability_t* reach = &aasworld.reachability[settings->firstreachablearea + i];
...
{
	const auto done = static_cast<int*>(GetClearedMemory(aasworld.numareas * sizeof(int)));
	const int areanum = AAS_PointAreaNum(origin);
	const int cluster = AAS_AreaCluster(areanum);
