...
constexpr auto BBOX_NORMAL_EPSILON = 0.001;

#define ON_EPSILON					0 //0.0005

constexpr auto TRACEPLANE_EPSILON = 0.125;

using aas_tracestack_t = struct aas_tracestack_s
{
...
// Changes Globals:		-
//===========================================================================
void AAS_PresenceTypeBoundingBox(const int presencetype, vec3_t mins, vec3_t maxs)
{
	int index;
	//bounding box size for each presence type
	constexpr vec3_t boxmins[3] = { {0, 0, 0}, {-15, -15, -24}, {-15, -15, -24} };
	constexpr vec3_t boxmaxs[3] = { {0, 0, 0}, { 15,  15,  32}, { 15,  15,   8} };
...
void AAS_InitAASLinkHeap(void)
{
	...
	if (!aasworld.linkheap)
	{
		...
		max_aaslinks = 6144;
#else
		max_aaslinks = static_cast<int>(LibVarValue("max_aaslinks", "6144"));
#endif
		if (max_aaslinks < 0) max_aaslinks = 0;
		aasworld.linkheapsize = max_aaslinks;
		aasworld.linkheap = static_cast<aas_link_t*>(GetHunkMemory(max_aaslinks * sizeof(aas_link_t)));
	} //end if
...
aas_link_t* AAS_AllocAASLink(void)
{
	...
	if (aasworld.freelinks) aasworld.freelinks->prev_ent = nullptr;
	numaaslinks--;
	return link;
...
void AAS_InitAASLinkedEntities(void)
{
	...
	aasworld.arealinkedentities = static_cast<aas_link_t**>(GetClearedHunkMemory(
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaCluster(const int areanum)
{
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaPresenceType(const int areanum)
{
...
int AAS_PointPresenceType(vec3_t point)
{
	...
	const int areanum = AAS_PointAreaNum(point);
...
// Changes Globals:		-
//===========================================================================
float AAS_BoxOriginDistanceFromPlane(vec3_t normal, vec3_t mins, vec3_t maxs, const int side)
{
	vec3_t v1, v2;
	int i;
...
// Changes Globals:		-
//===========================================================================
qboolean AAS_AreaEntityCollision(const int areanum, vec3_t start, vec3_t end,
	const int presencetype, const int passent, aas_trace_t* trace)
{
	...
	//assume no collision
	bsptrace.fraction = 1;
	int collision = qfalse;
	for (const aas_link_t* link = aasworld.arealinkedentities[areanum]; link; link = link->next_ent)
	{
...
// Changes Globals:		-
//===========================================================================
aas_trace_t AAS_TraceClientBBox(vec3_t start, vec3_t end, const int presencetype,
	const int passent)
{
	int side, tmpplanenum;
...
// Changes Globals:		-
//===========================================================================
int AAS_TraceAreas(vec3_t start, vec3_t end, int* areas, vec3_t* points, const int maxareas)
{
	int side, tmpplanenum;
	float front, back, frac;
	vec3_t cur_mid;
...
// Changes Globals:		-
//===========================================================================
qboolean AAS_InsideFace(aas_face_t* face, vec3_t pnormal, vec3_t point, const float epsilon)
{
	int firstvertex, edgenum;
	aas_edge_t* edge;
#ifdef AAS_SAMPLE_DEBUG
	int lastvertex = 0;
#endif //AAS_SAMPLE_DEBUG
...
// Changes Globals:		-
//===========================================================================
qboolean AAS_PointInsideFace(const int facenum, vec3_t point, const float epsilon)
{
	if (!aasworld.loaded) return qfalse;
	...
	for (int i = 0; i < face->numedges; i++)
	{
		...
		vec3_t pointvec;
		vec3_t edgevec;
		const int edgenum = aasworld.edgeindex[face->firstedge + i];
		const aas_edge_t* edge = &aasworld.edges[abs(edgenum)];
		//get the first vertex of the edge
		const int firstvertex = edgenum < 0;
		const float* v1 = aasworld.vertexes[edge->v[firstvertex]];
		const float* v2 = aasworld.vertexes[edge->v[!firstvertex]];
		//edge vector
...
// Changes Globals:		-
//===========================================================================
aas_face_t* AAS_AreaGroundFace(const int areanum, vec3_t point)
{
	constexpr vec3_t up = { 0, 0, 1 };

	if (!aasworld.loaded) return nullptr;

	const aas_area_t* area = &aasworld.areas[areanum];
	for (int i = 0; i < area->numfaces; i++)
	{
		const int facenum = aasworld.faceindex[area->firstface + i];
...
// Changes Globals:		-
//===========================================================================
void AAS_FacePlane(const int facenum, vec3_t normal, float* dist)
{
	const aas_plane_t* plane = &aasworld.planes[aasworld.faces[facenum].planenum];
...
{
	aas_face_t* firstface = nullptr;

	if (!aasworld.loaded) return nullptr;

	//if started in solid no face was hit
	if (trace->startsolid) return nullptr;
...
using aas_linkstack_t = struct aas_linkstack_s {
	int nodenum;		//node found after splitting
};

aas_link_t* AAS_AASLinkEntity(vec3_t absmins, vec3_t absmaxs, const int entnum)
{
	...
	while (true)
	{
		...
		//number of the current node to test the line against
		const int nodenum = lstack_p->nodenum;
		//if it is an area
		if (nodenum < 0)
		{
...
// Changes Globals:		-
//===========================================================================
aas_link_t* AAS_LinkEntityClientBBox(vec3_t absmins, vec3_t absmaxs, const int entnum, const int presencetype)
{
	vec3_t mins, maxs;
...
// Changes Globals:		-
//===========================================================================
int AAS_BBoxAreas(vec3_t absmins, vec3_t absmaxs, int* areas, const int maxareas)
{
	aas_link_t* linkedareas = AAS_AASLinkEntity(absmins, absmaxs, -1);
	int num = 0;
	for (const aas_link_t* link = linkedareas; link; link = link->next_area)
	{
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaInfo(const int areanum, aas_areainfo_t* info)
{
	if (!info)
		return 0;
	if (areanum <= 0 || areanum >= aasworld.numareas)
...
// Changes Globals:		-
//===========================================================================
aas_plane_t* AAS_PlaneFromNum(const int planenum)
{
	if (!aasworld.loaded) return nullptr;
