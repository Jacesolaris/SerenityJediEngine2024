...
void TurretClientRun(centity_t* ent)
{
	if (!ent->ghoul2)
	{
		...
		trap->G2API_SetBoneAngles(ent->ghoul2, 0, "bone_hinge", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y,
			POSITIVE_Z, POSITIVE_X, NULL, 100, cg.time);
		trap->G2API_SetBoneAngles(ent->ghoul2, 0, "bone_gback", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y,
			POSITIVE_Z, POSITIVE_X, NULL, 100, cg.time);
		trap->G2API_SetBoneAngles(ent->ghoul2, 0, "bone_barrel", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y,
			POSITIVE_Z, POSITIVE_X, NULL, 100, cg.time);
	...
	if (ent->currentState.bolt2 != ENTITYNUM_NONE)
	{
		...
		const centity_t* enemy = &cg_entities[ent->currentState.bolt2];
	...
	ent->frame_minus1_refreshed = cg.time;
	trap->G2API_SetBoneAngles(ent->ghoul2, 0, "bone_hinge", ent->turAngles, BONE_ANGLES_REPLACE, NEGATIVE_Y, NEGATIVE_Z,
		NEGATIVE_X, NULL, 100, cg.time);
}