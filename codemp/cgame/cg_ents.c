...
static void CG_General(centity_t* cent)
{
		...
		cent->currentState.NPC_class != CLASS_VEHICLE)
	{
		...
		if (cg.snap->ps.client_num == emp_own->currentState.number &&
			!cg.renderingThirdPerson)
		{
	...
	*/
	...
	if (cent->currentState.eFlags & EF_FORCE_VISIBLE && cg.snap->ps.fd.forcePowersActive & 1 << FP_SEE)
	{
		...
		{
			// A team game
			if (cg.predictedPlayerState.botclass == BCLASS_DESANN
				|| cg.predictedPlayerState.botclass == BCLASS_UNSTABLESABER
				|| cg.predictedPlayerState.botclass == BCLASS_SHADOWTROOPER
				|| cg.predictedPlayerState.botclass == BCLASS_TAVION
				|| cg.predictedPlayerState.botclass == BCLASS_ALORA
				|| cg.predictedPlayerState.botclass == BCLASS_REBORN)
			{
				ent.shaderRGBA[0] = 255;
			...
			else if (cg.predictedPlayerState.botclass == BCLASS_SBD
				|| cg.predictedPlayerState.botclass == BCLASS_ASSASSIN_DROID
				|| cg.predictedPlayerState.botclass == BCLASS_BATTLEDROID
				|| cg.predictedPlayerState.botclass == BCLASS_DROIDEKA
				|| cg.predictedPlayerState.botclass == BCLASS_SABER_DROID)
			{
				ent.shaderRGBA[0] = 100;
				ent.shaderRGBA[1] = 90;
				ent.shaderRGBA[2] = 160;
			}
			else if (cg.predictedPlayerState.botclass == BCLASS_JEDI
				|| cg.predictedPlayerState.botclass == BCLASS_JEDITRAINER
				|| cg.predictedPlayerState.botclass == BCLASS_JEDIMASTER
				|| cg.predictedPlayerState.botclass == BCLASS_LUKE
				|| cg.predictedPlayerState.botclass == BCLASS_KYLE)
			{
				ent.shaderRGBA[0] = 0;
				ent.shaderRGBA[1] = 120;
				ent.shaderRGBA[2] = 0;
			}
			else if (cg.predictedPlayerState.botclass == BCLASS_R2D2
				|| cg.predictedPlayerState.botclass == BCLASS_R5D2
				|| cg.predictedPlayerState.botclass == BCLASS_PROTOCOL)
			{
				ent.shaderRGBA[0] = 255;
				ent.shaderRGBA[1] = 255;
				ent.shaderRGBA[2] = 230;
			}
			else if (cg.predictedPlayerState.botclass == BCLASS_JAN)
			{
				ent.shaderRGBA[0] = 125;
				ent.shaderRGBA[1] = 255;
				ent.shaderRGBA[2] = 230;
			}
			else if (cg.predictedPlayerState.botclass == BCLASS_BOBAFETT
				|| cg.predictedPlayerState.botclass == BCLASS_ROCKETTROOPER
				|| cg.predictedPlayerState.botclass == BCLASS_MANDOLORIAN
				|| cg.predictedPlayerState.botclass == BCLASS_MANDOLORIAN1
				|| cg.predictedPlayerState.botclass == BCLASS_MANDOLORIAN2)
			{
				ent.shaderRGBA[0] = 85;
...
static void CG_Speaker(centity_t* cent)
{
	...
	cent->miscTime = cg.time + cent->currentState.frame * 100 + cent->currentState.client_num * 100 *
		Q_flrand(-1.0f, 1.0f);
}
...
static void CG_Mover(centity_t* cent)
{
	...
	*/
	...
	{
		ent.hModel = cgs.game_models[s1->modelindex2];
		if (s1->iModelScale)
		{
			//custom model2 scale
...
void CG_CalcEntityLerpPositions(centity_t* cent)
{
	...
	if (!cg_smoothClients.integer)
	{
		...
		if (cent->currentState.number != cg.client_num && cent->currentState.number < MAX_CLIENTS || cent->currentState.
			eType == ET_NPC)
		{
	...
	if (cent->interpolate && cent->currentState.pos.trType == TR_LINEAR_STOP
		&& (cent->currentState.number != cg.client_num && cent->currentState.number < MAX_CLIENTS || cent->currentState.
			eType == ET_NPC))
	{
		CG_InterpolateEntityPosition(cent);
		go_away = qtrue;
...
static void CG_FX(centity_t* cent)
{
	...
	if (s1->modelindex2 < FX_STATE_ONE_SHOT_LIMIT)
	{
		...
		// fx is single shot
		if (cent->muzzleOverheatTime == s1->modelindex2)
		{
			return;
		}

		cent->muzzleFlashTime = s1->modelindex2;
		cent->muzzleOverheatTime = s1->modelindex2;
	}
...
static void CG_AddCEntity(centity_t* cent)
{
	...
	if (cg.predictedPlayerState.client_num == cent->currentState.number && cg.predictedPlayerState.persistant[PERS_TEAM]
		== TEAM_SPECTATOR)
		return;
