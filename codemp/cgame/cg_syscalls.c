...
void trap_R_AddPolyToScene(const qhandle_t hShader, const int numVerts, const polyVert_t* verts)
{
	Q_syscall(CG_R_ADDPOLYTOSCENE, hShader, numVerts, verts);
}

void trap_R_AddPolysToScene(const qhandle_t hShader, const int numVerts, const polyVert_t* verts, const int num)
{
...
void trap_R_DrawRotatePic(const float x, const float y, const float w, const float h, const float s1, const float t1,
	const float s2, const float t2, const float a, const qhandle_t hShader)
{
	Q_syscall(CG_R_DRAWROTATEPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1),
		PASSFLOAT(s2), PASSFLOAT(t2), PASSFLOAT(a), hShader);
}
...
void trap_FX_PlayEntityEffect(const char* file, vec3_t org, matrix3_t axis, const int bolt_info, const int entNum,
...
void trap_FX_PlayEntityEffectID(const int id, vec3_t org, matrix3_t axis, const int bolt_info, const int entNum,
	const int vol, const int rad)
{
...
qboolean trap_G2API_GetBoltMatrix(void* ghoul2, const int modelIndex, const int bolt_index, mdxaBone_t* matrix,
	...
	vec3_t scale)
{
	return Q_syscall(CG_G2_GETBOLT, ghoul2, modelIndex, bolt_index, matrix, angles, position, frameNum, modelList,
		scale);
}
...
int trap_G2API_InitGhoul2Model(void** ghoul2Ptr, const char* file_name, const int modelIndex,
	...
	const int lodBias)
{
	return Q_syscall(CG_G2_INITGHOUL2MODEL, ghoul2Ptr, file_name, modelIndex, customSkin, customShader, modelFlags,
		lodBias);
}
...
{
	Q_syscall(CG_G2_COPYSPECIFICGHOUL2MODEL, g2_from, model_from, g2_to, model_to);
}
...
qboolean trap_G2API_AttachEnt(int* bolt_info, void* ghlInfoTo, const int toBoltIndex, const int entNum,
...
{
	Q_syscall(CG_G2_SETBOLTON, ghoul2, modelIndex, bolt_info);
}
...
void CGSyscall_R_AddPolysToScene(const qhandle_t hShader, const int numVerts, const polyVert_t* verts, int num)
{
