...
void CG_BloodTrail(const localEntity_t* le)
{
	...
	const int t2 = step * (cg.time / step);
...
static void CG_AddPuff(localEntity_t* le)
{
	...
	// fade / grow time
	const float c = (le->endTime - cg.time) / (float)(le->endTime - le->startTime);
...
static void CG_AddScaleFade(localEntity_t* le)
{
	...
	// fade / grow time
	const float c = (le->endTime - cg.time) * le->lifeRate;
...
{
	const refEntity_t* ent = &ex->refEntity;
...
void CG_AddScorePlum(localEntity_t* le)
{
	...
	for (int i = 0; i < numdigits; i++)
	{
		...
		re->custom_shader = cgs.media.numberShaders[digits[numdigits - 1 - i]];
		trap->R_AddRefEntityToScene(re);
	}
}
...
void CG_AddOLine(localEntity_t* le)
{
	...
	const float alpha = le->alpha + le->dalpha * frac;
