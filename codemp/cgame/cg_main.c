...
static void CG_GetOrigin(const int ent_id, vec3_t out)
{
	VectorCopy(cg_entities[ent_id].currentState.pos.trBase, out);
}

static void CG_GetAngles(const int ent_id, vec3_t out)
{
	VectorCopy(cg_entities[ent_id].currentState.apos.trBase, out);
}

static trajectory_t* CG_GetOriginTrajectory(const int ent_id)
{
	return &cg_entities[ent_id].nextState.pos;
}

static trajectory_t* CG_GetAngleTrajectory(const int ent_id)
{
	return &cg_entities[ent_id].nextState.apos;
}

static void _CG_ROFF_NotetrackCallback(const int ent_id, const char* notetrack)
{
	CG_ROFF_NotetrackCallback(&cg_entities[ent_id], notetrack);
}
