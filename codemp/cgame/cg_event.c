...
void CG_EntityEvent(centity_t* cent, vec3_t position)
{
	...
	case EV_JUMP:
		...
		if (cg.predicted_player_state.duelInProgress && (cg.predicted_player_state.client_num != es->client_num && cg.
			predicted_player_state.duelIndex != es->client_num))
		{
			break;
		}
	...
	case EV_ROLL:
		...
		if (cg.predicted_player_state.duelInProgress && (es->client_num != cg.predicted_player_state.client_num && es->
			client_num != cg.predicted_player_state.duelIndex))
		{
			break;
		}
