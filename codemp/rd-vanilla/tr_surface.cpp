...
void RB_SurfacePolychain(const srfPoly_t* p) {
	...
	for (i = 0; i < p->numVerts - 2; i++) {
		tess.indexes[tess.numIndexes + 0] = tess.num_vertexes;
		tess.indexes[tess.numIndexes + 1] = tess.num_vertexes + i + 1;
		tess.indexes[tess.numIndexes + 2] = tess.num_vertexes + i + 2;
		tess.numIndexes += 3;
	}
...
void RB_SurfaceTriangles(const srfTriangles_t* srf) {
	...
	for (i = 0; i < srf->numIndexes; i += 3) {
		tess.indexes[tess.numIndexes + i + 0] = tess.num_vertexes + srf->indexes[i + 0];
		tess.indexes[tess.numIndexes + i + 1] = tess.num_vertexes + srf->indexes[i + 1];
		tess.indexes[tess.numIndexes + i + 2] = tess.num_vertexes + srf->indexes[i + 2];
	}
...
static void DoCylinderPart(polyVert_t* verts)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		tess.vertexColors[tess.num_vertexes][0] = verts->modulate[0];
		tess.vertexColors[tess.num_vertexes][1] = verts->modulate[1];
		tess.vertexColors[tess.num_vertexes][2] = verts->modulate[2];
		tess.vertexColors[tess.num_vertexes][3] = verts->modulate[3];
		tess.num_vertexes++;
		verts++;
	}
...
static void RB_SurfaceCylinder()
{
	...
	for (i = 0; i < segments; i++)
	{
		...
		verts[0].modulate[0] = static_cast<byte>(e->shaderRGBA[0]);
		verts[0].modulate[1] = static_cast<byte>(e->shaderRGBA[1]);
		verts[0].modulate[2] = static_cast<byte>(e->shaderRGBA[2]);
		verts[0].modulate[3] = static_cast<byte>(e->shaderRGBA[3]);

		VectorCopy(lower_points[i].xyz, verts[1].xyz);
		verts[1].st[1] = 0.0f;
		verts[1].st[0] = detail * i;
		verts[1].modulate[0] = static_cast<byte>(e->shaderRGBA[0]);
		verts[1].modulate[1] = static_cast<byte>(e->shaderRGBA[1]);
		verts[1].modulate[2] = static_cast<byte>(e->shaderRGBA[2]);
		verts[1].modulate[3] = static_cast<byte>(e->shaderRGBA[3]);

		VectorCopy(lower_points[next_segment].xyz, verts[2].xyz);
		verts[2].st[1] = 0.0f;
		verts[2].st[0] = detail * (i + 1);
		verts[2].modulate[0] = static_cast<byte>(e->shaderRGBA[0]);
		verts[2].modulate[1] = static_cast<byte>(e->shaderRGBA[1]);
		verts[2].modulate[2] = static_cast<byte>(e->shaderRGBA[2]);
		verts[2].modulate[3] = static_cast<byte>(e->shaderRGBA[3]);

		VectorCopy(upper_points[next_segment].xyz, verts[3].xyz);
		verts[3].st[1] = 1.0f;
		verts[3].st[0] = detail * (i + 1);
		verts[3].modulate[0] = static_cast<byte>(e->shaderRGBA[0]);
		verts[3].modulate[1] = static_cast<byte>(e->shaderRGBA[1]);
		verts[3].modulate[2] = static_cast<byte>(e->shaderRGBA[2]);
		verts[3].modulate[3] = static_cast<byte>(e->shaderRGBA[3]);
...
void RB_SurfaceFace(srfSurfaceFace_t* surf) {
	...
	const unsigned int* indices = (unsigned*)((char*)surf + surf->ofsIndices);

	const int bob = tess.num_vertexes;
	glIndex_t* tess_indexes = tess.indexes + tess.numIndexes;
	...
	const int numPoints = surf->numPoints;

	//if ( tess.shader->needsNormal )
	{
		const float* normal = surf->plane.normal;
		for (i = 0, ndx = tess.num_vertexes; i < numPoints; i++, ndx++) {
			VectorCopy(normal, tess.normal[ndx]);
		}
	}

	for (i = 0, v = surf->points[0], ndx = tess.num_vertexes; i < numPoints; i++, v += VERTEXSIZE, ndx++)
	{
		byteAlias_t ba{};
	...
	tess.num_vertexes += surf->numPoints;
}
...
void RB_SurfaceGrid(srfGridMesh_t* cv) {
	...
	while (used < lod_height - 1) {
		...
		do {
			...
			irows = (SHADER_MAX_INDEXES - tess.numIndexes) / (lod_width * 6);
