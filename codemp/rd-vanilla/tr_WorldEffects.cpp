...
inline float WE_flrand(const float min, const float max)
{
...
public:
	...
	void AddWeatherZone(vec3_t mins, vec3_t maxs)
	{
		if (!mWeatherZones.full())
		{
			...
			const int arraySize = (Wz.mWidth * Wz.mHeight * Wz.mDepth);
			Wz.mPointCache = static_cast<uint32_t*>(Z_Malloc(arraySize * sizeof(uint32_t), TAG_POINTCACHE, qtrue));
		}
	}
...
public:
	...
	void Initialize(const int count, const char* texturePath, const int VertexCount = 4)
	{
		...
		{
			CWeatherParticle* part = &mParticles[particleNum];
	...
	void Update()
	{
		...
		*/
		...
		for (int particleNum = 0; particleNum < mParticleCount; particleNum++)
		{
			...
			bool partRendering = part->mFlags.get_bit(CWeatherParticle::FLAG_RENDER);
	...
	void Render()
	{
		...
		for (int particleNum = 0; particleNum < mParticleCount; particleNum++)
		{
			...
			{
				qglColor4f(mColor[0] * part->mAlpha, mColor[1] * part->mAlpha, mColor[2] * part->mAlpha,
					mColor[3] * part->mAlpha);
			}
