...
static	void R_LoadVisibility(const lump_t* l, world_t& world_data) {
	...
	world_data.novis = static_cast<unsigned char*>(Hunk_Alloc(len, h_low));
...
static void ParseTriSurf(const dsurface_t* ds, mapVert_t* verts, msurface_t* surf, int* indexes, const world_t& world_data, const int index) {
	...
	srfTriangles_t* tri = static_cast<srfTriangles_t*>(Hunk_Alloc(sizeof * tri + num_verts * sizeof tri->verts[0]
...
=================
*/
void R_FixSharedVertexLodError_r(const int start, const srfGridMesh_t* grid1, world_t& world_data) {
	int k, l, m, n, offset1, offset2;

	for (int j = start; j < world_data.numsurfaces; j++) {
		//
		srfGridMesh_t* grid2 = reinterpret_cast<srfGridMesh_t*>(world_data.surfaces[j].data);
		...
		if (touch) {
			grid2->lodFixed = 2;
			R_FixSharedVertexLodError_r(start, grid2, world_data);
...
=================
*/
void R_FixSharedVertexLodError(world_t& world_data) {
	for (int i = 0; i < world_data.numsurfaces; i++) {
		//
		srfGridMesh_t* grid1 = reinterpret_cast<srfGridMesh_t*>(world_data.surfaces[i].data);
		// if this surface is not a grid
		...
		R_FixSharedVertexLodError_r(i + 1, grid1, world_data);
	}
}
...
===============
*/
int R_StitchPatches(int grid1_num, int grid2_num, world_t& world_data) {
	int k, l, m, n, offset1, offset2, row, column;
	srfGridMesh_t* grid1, * grid2;
	float* v1, * v2;

	grid1 = reinterpret_cast<srfGridMesh_t*>(world_data.surfaces[grid1_num].data);
	grid2 = reinterpret_cast<srfGridMesh_t*>(world_data.surfaces[grid2_num].data);
	for (n = 0; n < 2; n++) {
		//
		if (n) offset1 = (grid1->height - 1) * grid1->width;
		else offset1 = 0;
		...
		for (k = 0; k < grid1->width - 2; k += 2) {
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->width - 1; l++) {
					...
					world_data.surfaces[grid2_num].data = reinterpret_cast<surfaceType_t*>(grid2);
					return qtrue;
				}
			}
			for (m = 0; m < 2; m++) {
...
===============
*/
int R_TryStitchingPatch(const int grid1_num, world_t& world_data) {
	int numstitches = 0;
	const srfGridMesh_t* grid1 = reinterpret_cast<srfGridMesh_t*>(world_data.surfaces[grid1_num].data);
	for (int j = 0; j < world_data.numsurfaces; j++) {
		//
		const srfGridMesh_t* grid2 = reinterpret_cast<srfGridMesh_t*>(world_data.surfaces[j].data);
		// if this surface is not a grid
		if (grid2->surfaceType != SF_GRID) continue;
		// grids in the same LOD group should have the exact same lod radius
		if (grid1->lodRadius != grid2->lodRadius) continue;
		...
		if (grid1->lodOrigin[2] != grid2->lodOrigin[2]) continue;
		//
		while (R_StitchPatches(grid1_num, j, world_data))
		{
			numstitches++;
		}
	}
...
===============
*/
void R_StitchAllPatches(world_t& world_data) {
	int stitched;
...
===============
*/
void R_MovePatchSurfacesToHunk(const world_t& world_data) {
	for (int i = 0; i < world_data.numsurfaces; i++) {
		//
		srfGridMesh_t* grid = reinterpret_cast<srfGridMesh_t*>(world_data.surfaces[i].data);
		// if this surface is not a grid
		...
		world_data.surfaces[i].data = reinterpret_cast<surfaceType_t*>(hunkgrid);
	}
}
...
static	void R_LoadSurfaces(const lump_t* surfs, const lump_t* verts, const lump_t* index_lump, world_t& world_data, const int index) {
	...
	for (int i = 0; i < count; i++, in++, out++) {
		...
		case MST_TRIANGLE_SOUP:
			...
			break;
		case MST_PLANAR:
			ParseFace(in, dv, out, indexes, world_data, index);
			num_faces++;
			break;
		case MST_FLARE:
	...
	R_StitchAllPatches(world_data);
#endif

	R_FixSharedVertexLodError(world_data);

#ifdef PATCH_STITCHING
	R_MovePatchSurfacesToHunk(world_data);
#endif
...
static	void R_LoadSubmodels(const lump_t* l, world_t& world_data, const int index) {
	...
	world_data.bmodels = out = static_cast<bmodel_t*>(Hunk_Alloc(count * sizeof * out, h_low));
...
static	void R_LoadFogs(const lump_t* l, const lump_t* brushes_lump, const lump_t* sides_lump, world_t& world_data, const int index) {
	...
	world_data.fogs = static_cast<fog_t*>(Hunk_Alloc(world_data.numfogs * sizeof * out, h_low));
...
void RE_LoadWorldMap_Actual(const char* name, world_t& world_data, const int index)
{
	...
	world_data.dataSize = static_cast<byte*>(Hunk_Alloc(0, h_low)) - start_marker;
