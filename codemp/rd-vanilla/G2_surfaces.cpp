...
surfaceInfo_t* G2_FindOverrideSurface(const int surfaceNum, surfaceInfo_v& surfaceList)
{
	// look through entire list
	for (auto& i : surfaceList)
	{
...
mdxmSurface_t* G2_FindSurface(const CGhoul2Info* ghlInfo, const surfaceInfo_v& slist, const char* surfaceName, int* surfIndex)
{
	...
	for (int i = slist.size() - 1; i >= 0; i--)
	{
		if (slist[i].surface != 10000 && slist[i].surface != -1)
		{
			...
			const mdxmSurfHierarchy_t* surfInfo = reinterpret_cast<mdxmSurfHierarchy_t*>(reinterpret_cast<byte*>(surfIndexes) + surfIndexes->offsets[surf->
				thisSurfaceIndex]);
...
void G2_SetSurfaceOnOffFromSkin(CGhoul2Info* ghlInfo, const qhandle_t renderSkin)
{
	...
	for (int j = 0; j < skin->numSurfaces; j++)
	{
		...
		{
			int	flags;
			const int surfaceNum = G2_IsSurfaceLegal((void*)ghlInfo->currentModel, skin->surfaces[j]->name, &flags);
...
static void G2_FindRecursiveSurface(model_t* currentModel, int surfaceNum, surfaceInfo_v& root_list, int* activeSurfaces)
{
	...
	const mdxmSurfHierarchy_t* surfInfo = reinterpret_cast<mdxmSurfHierarchy_t*>(reinterpret_cast<byte*>(surfIndexes) + surfIndexes->offsets[surface->thisSurfaceIndex]);
	...
	for (int i = 0; i < surfInfo->numChildren; i++)
	{
		...
		G2_FindRecursiveSurface(currentModel, surfaceNum, root_list, activeSurfaces);
	}
}

void G2_RemoveRedundantGeneratedSurfaces(surfaceInfo_v& slist, const int* activeSurfaces)
{
...
qboolean G2_SetRootSurface(CGhoul2Info_v& ghoul2, const int modelIndex, const char* surfaceName)
{
	...
	const auto mod_m = const_cast<model_t*>(ghoul2[modelIndex].currentModel);
	const model_t* mod_a = const_cast<model_t*>(ghoul2[modelIndex].animModel);
	...
	// first find if we already have this surface in the list
	const int surf = G2_IsSurfaceLegal(mod_m, surfaceName, &flags);
	if (surf != -1)
	{
		...
		for (int i = 0; i < ghoul2.size(); i++)
		{
			...
			if (ghoul2[i].mModelBoltLink != -1)
			{
				...
				if (static_cast<int>(ghoul2[bolt_mod].mBltlist.size()) <= bolt_num ||
					...
					ghoul2[bolt_mod].mBltlist[bolt_num].surfaceNumber == -1)
				{
...
int G2_IsSurfaceRendered(const CGhoul2Info* ghlInfo, const char* surfaceName, const surfaceInfo_v& slist)
{
	...
	int surfNum = G2_IsSurfaceLegal(const_cast<model_t*>(ghlInfo->currentModel), surfaceName, &flags);
	if (surfNum != -1)
	{//must be legal
		const mdxmHierarchyOffsets_t* surfIndexes = reinterpret_cast<mdxmHierarchyOffsets_t*>(reinterpret_cast<byte*>(ghlInfo->currentModel->mdxm) + sizeof(mdxmHeader_t));
		const mdxmSurfHierarchy_t* surfInfo = reinterpret_cast<mdxmSurfHierarchy_t*>((byte*)surfIndexes + surfIndexes->offsets[surfNum]);
		...
		while (surfNum != -1)
		{
			...
			const mdxmSurfHierarchy_t* parent_surf_info = reinterpret_cast<mdxmSurfHierarchy_t*>((byte*)surfIndexes + surfIndexes->offsets[
				surfNum]);

			// find the original surface in the surface list
			//G2 was error, above comment was accurate, but we don't want the original flags, we want the parent flags
			G2_IsSurfaceLegal(const_cast<model_t*>(ghlInfo->currentModel), parent_surf_info->name, &parent_flags);
