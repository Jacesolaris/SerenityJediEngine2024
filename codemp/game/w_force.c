...
{
	if (other->client && (other->client->ps.inAirAnim || other->client->ps.groundentity_num == ENTITYNUM_NONE))
	{
		return 1;
	}
	...
	else if (forcePower == FP_GRIP && other->client)
	{
		...
		case FORCE_LEVEL_2: //Can block if walking or running
			if (other->client->ps.inAirAnim || other->client->ps.groundentity_num == ENTITYNUM_NONE)
				return 1;
			break;
...
qboolean WP_CounterForce(const gentity_t* attacker, const gentity_t* defender, const int attackPower)
{
	...
	if (abilityDef >= 2)
	{
		...
		if (!walk_check(defender) || defender->client->ps.groundentity_num == ENTITYNUM_NONE)
		{
...
void ForceGrip(const gentity_t* self)
{
		...
		//don't grip someone who's still crippled
	{
		...
		self->client->ps.fd.forceGripentity_num = tr.entity_num;
	...
	if (trace_ent && trace_ent->takedamage)
	{
		...
		if (trace_ent->client)
		{
			...
			if (ForcePowerUsableOn(self, trace_ent, FP_LIGHTNING))
			{
				...
				const qboolean saber_lightning_blocked = saber_block_lightning(self, trace_ent);
				const qboolean hand_lightning_blocked = melee_block_lightning(self, trace_ent);
				...
				if (dmg && !saber_lightning_blocked && !hand_lightning_blocked)
				{
					...
					{
						G_Damage(trace_ent, self, self, dir, impact_point, dmg, DAMAGE_NO_KNOCKBACK,
							MOD_FORCE_LIGHTNING);
					}
					...
					if (trace_ent->s.weapon != WP_EMPLACED_GUN)
					{
							...
							&& trace_ent->health <= 35 && !class_is_gunner(trace_ent))
						{
							...
							gentity_t* tent = G_TempEntity(trace_ent->r.currentOrigin, EV_STUNNED);
							tent->s.owner = trace_ent->s.number;
						}

						if (trace_ent->client->ps.stats[STAT_HEALTH] <= 0 && class_is_gunner(trace_ent))
						{
							VectorClear(trace_ent->client->ps.lastHitLoc);
							VectorClear(trace_ent->client->ps.velocity);

							trace_ent->client->ps.eFlags |= EF_DISINTEGRATION;
							trace_ent->r.contents = 0;

							trace_ent->think = G_FreeEntity;
						...
						else if (trace_ent->client->ps.groundentity_num == ENTITYNUM_NONE && trace_ent->client->ps.stats[STAT_HEALTH] > 1)
						{
						...
						else
						{
							if (!PM_RunningAnim(trace_ent->client->ps.legsAnim)
								...
								&& trace_ent->client->ps.stats[STAT_HEALTH] > 1)
							{
				...
				if (trace_ent->client)
				{
					...
					else if (trace_ent->client->ps.electrifyTime < level.time + 400 && !saber_lightning_blocked && !
						hand_lightning_blocked)
					{
						//only update every 400ms to reduce bandwidth usage (as it is passing a 32-bit time value)
						trace_ent->client->ps.electrifyTime = level.time + 3000;
					}
						...
						&& trace_ent->client->ps.weapon == WP_SABER)
					{
							...
							&& !PM_InKnockDown(&trace_ent->client->ps))
						{
							...
							if (saber_lightning_blocked && !hand_lightning_blocked)
							{
								...
								switch (trace_ent->client->ps.fd.saber_anim_level)
								{
								case SS_DUAL:
									G_SetAnim(trace_ent, &trace_ent->client->pers.cmd, SETANIM_TORSO, BOTH_P6_S6_T_,
										SETANIM_AFLAG_PACE, 0);
									break;
								case SS_STAFF:
									G_SetAnim(trace_ent, &trace_ent->client->pers.cmd, SETANIM_TORSO, BOTH_P7_S7_T_,
										SETANIM_AFLAG_PACE, 0);
									break;
								case SS_FAST:
								case SS_TAVION:
								...
								case SS_DESANN:
								case SS_MEDIUM:
									G_SetAnim(trace_ent, &trace_ent->client->pers.cmd, SETANIM_TORSO, BOTH_P1_S1_T_,
										SETANIM_AFLAG_PACE, 0);
									break;
								case SS_NONE:
								default:
					...
					else if (trace_ent->client->ps.weapon == WP_NONE
						|| trace_ent->client->ps.weapon == WP_MELEE
						&& trace_ent->client->ps.fd.forcePower > 20
						&& in_front(self->client->ps.origin, trace_ent->client->ps.origin,
							trace_ent->client->ps.viewangles, -0.7f))
					{
					...
					else if (trace_ent && trace_ent->client && trace_ent->client->NPC_class == CLASS_GALAK)
					{
						if (trace_ent->client->ps.powerups[PW_GALAK_SHIELD])
						{
							//has shield up
							dmg = 0;
						}
...
int ForceShootDestruction(gentity_t* self)
{
	...
	if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_2)
	{
		...
		for (int e = 0; e < num_listed_entities; e++)
		{
			...
			// ReSharper disable once CppEntityAssignedButNoRead
			float dot;
			const gentity_t* trace_ent = entity_list[e];
...
void ForceJumpCharge(gentity_t* self, usercmd_t* ucmd)
{
	...
	if (!self->client->ps.fd.forceJumpCharge && self->client->ps.groundentity_num == ENTITYNUM_NONE)
	{
		return;
	}
...
qboolean ForceTelepathyCheckDirectNPCTarget(gentity_t* self, trace_t* tr, qboolean* tookPower)
{
	...
	if (targetLive && trace_ent->NPC)
	{
			...
			else if (trace_ent->s.weapon != WP_SABER && trace_ent->client->NPC_class != CLASS_REBORN)
			{
				...
				if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_2)
				{
					...
					if (trace_ent->s.weapon != WP_NONE)
					{
						...
						trace_ent->NPC->charmedTime = level.time + mindTrickTime[self->client->ps.fd.forcePowerLevel[
							FP_TELEPATHY]];

						G_SetAnim(trace_ent, &trace_ent->client->pers.cmd, SETANIM_TORSO, BOTH_SONICPAIN_HOLD,
				...
				else
				{
					...
					trace_ent->NPC->confusionTime = level.time + mindTrickTime[self->client->ps.fd.forcePowerLevel[
		...
		else if (trace_ent->client->playerTeam == self->client->playerTeam)
		{
			...
			if (trace_ent->client->ps.pm_type < PM_DEAD && trace_ent->NPC != NULL && !(trace_ent->NPC->scriptFlags &
				SCF_NO_RESPONSE))
			{
...
void ForceThrow(gentity_t* self, qboolean pull)
{
	...
	if (self->client->ps.groundentity_num == ENTITYNUM_NONE && self->client->ps.fd.forcePowerLevel[FP_PUSH] >
		FORCE_LEVEL_2
	...
	else if (self->client->ps.groundentity_num == ENTITYNUM_NONE && self->client->ps.fd.forcePowerLevel[FP_PUSH] <
		FORCE_LEVEL_3
		&& (self->s.weapon == WP_MELEE ||
			self->s.weapon == WP_NONE ||
			self->s.weapon == WP_SABER && self->client->ps.saber_holstered))
		...
		while (e < num_listed_entities)
		{
			...
			if (Q_stricmp(ent->classname, "sentryGun") == 0)
			{
					...
					tr.entity_num != ENTITYNUM_NONE)
				{
					...
					aiming_at = &g_entities[tr.entity_num];
				...
				if (pull)
				{
					...
					if (push_target[x]->client && VectorLength(push_dir) <= 256)
					{
						...
						if (!OnSameTeam(self, push_target[x]) && can_pull_weapon)
						{
							...
							if (tr.fraction != 1.0
								&& tr.entity_num == push_target[x]->s.number)
							{
								vec3_t vecnorm;
								vec3_t uorg;
...
static void wp_force_power_run(gentity_t* self, const forcePowers_t forcePower, const usercmd_t* cmd)
{
	...
	case FP_GRIP:
		...
		break;
	case FP_LEVITATION:
		if (self->client->ps.groundentity_num != ENTITYNUM_NONE && !self->client->ps.fd.forceJumpZStart ||
...
int WP_DoSpecificPower(gentity_t* self, const usercmd_t* ucmd, const forcePowers_t forcepower)
{
	...
	case FP_LEVITATION:
		...
		if (self->client->ps.groundentity_num == ENTITYNUM_NONE)
		{
			self->client->ps.fd.forceJumpCharge = 0;
			G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_1 - 50], CHAN_VOICE);
			//This only happens if the groundentity_num == ENTITYNUM_NONE when the button is actually released
		}
		else
		{
...
void WP_ForcePowersUpdate(gentity_t* self, usercmd_t* ucmd)
{
		...
		self->client->ps.forceHandExtendTime < level.time)
	{
		...
		else if (self->client->ps.forceHandExtend == HANDEXTEND_POSTTHROWN)
		{
			...
			else if (self->client->ps.groundentity_num != ENTITYNUM_NONE && !self->client->ps.forceDodgeAnim)
			{
				self->client->ps.forceDodgeAnim = 1;
				self->client->ps.forceHandExtendTime = level.time + 1000;
				G_EntitySound(self, CHAN_VOICE, G_SoundIndex("*jump1.wav"));
	...
	if (self->client->ps.fd.forceJumpCharge && self->client->ps.groundentity_num == ENTITYNUM_NONE && self->client->
		fjDidJump)
	{
	...
	else if (ucmd->upmove < 10 && self->client->ps.groundentity_num == ENTITYNUM_NONE && self->client->ps.fd.forceJumpCharge)
	{
	...
	if (self->client->ps.saberInFlight)
	{
		...
		{
			//
			if (&g_entities[self->client->ps.saberentity_num] != NULL && g_entities[self->client->ps.saberentity_num].s.
	...
	if (self->client->DodgeDebounce < level.time
		...
		&& self->client->ps.groundentity_num != ENTITYNUM_NONE //can't regen while in the air.
		&& walk_check(self))
	{
