...
int NAV_FindClosestWaypointForPoint(const gentity_t* ent, vec3_t point)
{
	...
	const int bestWP = trap->
		Nav_GetNearestNode((sharedEntity_t*)marker, marker->waypoint, NF_CLEAR_PATH, WAYPOINT_NONE);
...
int NAV_Steer(gentity_t* self, vec3_t dir, const float distance)
{
	...
	vec3_t deviation;
	trace_t tr;
	const float right_ang = dir[YAW] + 45;
	const float left_ang = dir[YAW] - 45;
...
qboolean NAV_CheckAhead(const gentity_t* self, vec3_t end, trace_t* trace, int clipmask)
{
	...
	const float radius = self->r.maxs[0] > self->r.maxs[1] ? self->r.maxs[0] : self->r.maxs[1];
	const float dist = Distance(self->r.currentOrigin, end);
...
qboolean NAV_Bypass(gentity_t* self, gentity_t* blocker, vec3_t blocked_dir, const float blocked_dist, vec3_t movedir)
{
	...
	const float yaw = vectoyaw(blocked_dir);

	//Get the avoid radius
	const float avoidRadius = sqrt(blocker->r.maxs[0] * blocker->r.maxs[0] + blocker->r.maxs[1] * blocker->r.maxs[1]) +
...
qboolean NAV_TrueCollision(const gentity_t* self, const gentity_t* blocker, vec3_t movedir, vec3_t blocked_dir)
{
	...
	const float speed = VectorNormalize2(self->client->ps.velocity, velocityDir);

	//See if it's even feasible
	const float dot = DotProduct(movedir, velocityDir);
...
qboolean NAV_ResolveEntityCollision(gentity_t* self, gentity_t* blocker, vec3_t movedir, vec3_t pathDir)
{
	...
	const float blocked_dist = VectorNormalize(blocked_dir);
...
int NAV_TestBestNode(const gentity_t* self, const int startID, const int endID, const qboolean failEdge)
{
	...
	if (trace.entityNum < ENTITYNUM_WORLD)
	{
		...
		if VALIDSTRING(blocker->classname)
		{
			...
			else if (blocker->targetname && blocker->s.solid == SOLID_BMODEL && (blocker->r.contents &
				CONTENTS_MONSTERCLIP || blocker->r.contents & CONTENTS_BOTCLIP))
			{
...
int NAV_MoveToGoal(gentity_t* self, navInfo_t* info)
{
	...
	int bestNode = trap->Nav_GetBestNode(self->waypoint, self->NPC->goalEntity->waypoint, NODE_NONE);
...
unsigned int waypoint_getRadius(gentity_t* ent)
{
	...
	{
		const unsigned int dist = waypoint_testDirection(ent->r.currentOrigin, 360.0f / YAW_ITERATIONS * i, minDist);
...
void SP_waypoint(gentity_t* ent)
{
	if (navCalculatePaths)
	{
		...
		const unsigned int radius = waypoint_getRadius(ent);
...
{
	const int radius = ent->radius ? (int)ent->radius | NAVGOAL_USE_RADIUS : 12;
	...
	G_FreeEntity(ent);
	//can't do this, they need to be found later by some functions, though those could be fixed, maybe?
}
