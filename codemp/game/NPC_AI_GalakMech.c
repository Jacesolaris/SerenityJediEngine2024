...
void NPC_GM_Pain(gentity_t* self, gentity_t* attacker, const int damage)
{
	...
	if (!self->lockCount && self->client->ps.torsoTimer <= 0)
	{
		...
		if (self->count < 4 && self->health > 100)
		{
	...
	else if (inflictor && inflictor->lastEnemy == self)
	{
	...
	if (!hitAlly4 && NPCS.NPCInfo->enemyLastSeenTime > 0)
	{
		if (level.time - NPCS.NPCInfo->enemyLastSeenTime < 10000)
		{
			if (!Q_irand(0, 10))
			{
				...
				float dist = DistanceSquared(impactPos4, muzzle);
...
void NPC_GM_StartLaser(void)
{
	if (!NPCS.NPC->lockCount)
	{
		...
		NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, TORSO_RAISEWEAP2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void GM_StartGloat(void)
{
	...
	NPC_SetSurfaceOnOff(NPCS.NPC, "torso_galakface_off", TURN_ON);
	NPC_SetSurfaceOnOff(NPCS.NPC, "torso_galakhead_off", TURN_ON);
	NPC_SetSurfaceOnOff(NPCS.NPC, "torso_eyes_mouth_off", TURN_ON);
	NPC_SetSurfaceOnOff(NPCS.NPC, "torso_collar_off", TURN_ON);
	NPC_SetSurfaceOnOff(NPCS.NPC, "torso_galaktorso_off", TURN_ON);
	NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, BOTH_STAND2TO1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void NPC_BSGM_Attack(void)
{
	...
	{
		//my enemy is dead
		if (NPCS.NPC->client->ps.torsoAnim == BOTH_STAND2TO1)
		{
			if (NPCS.NPC->client->ps.torsoTimer <= 500)
			{
				G_AddVoiceEvent(NPCS.NPC, Q_irand(EV_VICTORY1, EV_VICTORY3), 3000);
				NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, BOTH_TRIUMPHANT1START, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				NPCS.NPC->client->ps.legsTimer += 500;
				NPCS.NPC->client->ps.torsoTimer += 500;
			}
		}
		else if (NPCS.NPC->client->ps.torsoAnim == BOTH_TRIUMPHANT1START)
		{
			if (NPCS.NPC->client->ps.torsoTimer <= 500)
			{
		...
		else if (NPCS.NPC->client->ps.torsoAnim == BOTH_TRIUMPHANT1STARTGESTURE)
		{
			if (NPCS.NPC->client->ps.torsoTimer <= 500)
			{
				NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, BOTH_TRIUMPHANT1STOP, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				NPCS.NPC->client->ps.legsTimer += 500;
				NPCS.NPC->client->ps.torsoTimer += 500;
			}
		}
		else if (NPCS.NPC->client->ps.torsoAnim == BOTH_TRIUMPHANT1STOP)
		{
			if (NPCS.NPC->client->ps.torsoTimer <= 500)
			{
				NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, BOTH_STAND1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				NPCS.NPC->client->ps.legsTimer = -1;
				NPCS.NPC->client->ps.torsoTimer = -1;
			}
		}
		else if (NPCS.NPC->wait)
		{
			if (TIMER_Done(NPCS.NPC, "gloatTime"))
			{
				GM_StartGloat();
			}
			else if (DistanceHorizontalSquared(NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->enemy->r.currentOrigin)
	...
	if (NPCS.NPC->client->ps.torsoAnim == BOTH_ATTACK4 || NPCS.NPC->client->ps.torsoAnim == BOTH_ATTACK5)
	{
		shoot4 = qfalse;
	...
	else if (NPCS.NPC->lockCount) //already shooting laser
	{
		...
		else
		{
			...
			if (NPCS.NPC->client->ps.torsoTimer <= 0)
			{
				...
				NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, TORSO_DROPWEAP2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	if (move4 && !NPCS.NPC->lockCount)
	{
		//move toward goal
		if (NPCS.NPCInfo->goalEntity
			&& NPCS.NPC->client->ps.legsAnim != BOTH_ALERT1
			&& NPCS.NPC->client->ps.legsAnim != BOTH_ATTACK2
			&& NPCS.NPC->client->ps.legsAnim != BOTH_ATTACK4
			&& NPCS.NPC->client->ps.legsAnim != BOTH_ATTACK5
			&& NPCS.NPC->client->ps.legsAnim != BOTH_ATTACK7)
		{
			move4 = GM_Move();
		}
		else
...
void NPC_BSGM_Default(void)
{
	...
	{
		//armor present
		NPCS.NPC->client->ps.powerups[PW_GALAK_SHIELD] = Q3_INFINITE; //temp, for effect
		NPC_SetSurfaceOnOff(NPCS.NPC, "torso_shield", TURN_ON);
	}
	else
	{
