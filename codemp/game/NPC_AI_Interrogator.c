...
void Interrogator_Strafe(void)
{
	...
	if (tr.fraction > 0.9f)
	{
		...
		{
			// Find the height difference
			float dif = NPCS.NPC->enemy->r.currentOrigin[2] + 32 - NPCS.NPC->r.currentOrigin[2];
...
void Interrogator_Hunt(const qboolean visible, const qboolean advance)
{
	...
	const float speed = HUNTER_FORWARD_BASE_SPEED + HUNTER_FORWARD_MULTIPLIER * g_npcspskill.integer;
...
void Interrogator_Melee(const qboolean visible, const qboolean advance)
{
	if (TIMER_Done(NPCS.NPC, "attackDelay")) // Attack?
	{
		...
		if (NPCS.NPC->r.currentOrigin[2] >= NPCS.NPC->enemy->r.currentOrigin[2] + NPCS.NPC->enemy->r.mins[2] && NPCS.NPC
			->r.currentOrigin[2] + NPCS.NPC->r.mins[2] + 8 < NPCS.NPC->enemy->r.currentOrigin[2] + NPCS.NPC->enemy->r.
			maxs[2])
		{
...
void Interrogator_Attack(void)
{
	...
	qboolean advance = distance > MIN_DISTANCE * MIN_DISTANCE;
