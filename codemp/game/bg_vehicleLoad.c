...
typedef enum
{
	...
	VF_SHADER,
	// (cgame only) take the string, call trap->R_RegisterShader
	VF_SHADER_NOMIP,
	// (cgame only) take the string, call trap->R_RegisterShaderNoMip
	VF_SOUND,
...
static int vfieldcmp(const void* a, const void* b)
{
...
static qboolean BG_ParseVehWeaponParm(vehWeaponInfo_t* vehWeapon, const char* parmName, const char* pValue)
{
	...
	vehField_t* vehWeaponField = Q_LinearSearch(parmName, vehWeaponFields, numVehWeaponFields,
		sizeof vehWeaponFields[0], vfieldcmp);
	...
	case VF_STRING: // string on disk, pointer in memory
		...
		break;
	case VF_VECTOR:
		i_fields_read = sscanf(value, "%f %f %f", &vec[0], &vec[1], &vec[2]);
...
static int VEH_LoadVehWeapon(const char* vehWeaponName)
{
	...
	while (1)
	{
		...
		char* value = COM_ParseExt(&p, qtrue);
		if (!value || !value[0])
		{
...
static int veh_load_vehicle(const char* vehicle_name)
{
	...
	vehicleInfo_t* vehicle = &g_vehicleInfo[numVehicles];
...
static void BG_VehWeaponLoadParms(void)
{
	...
	*marker = 0;
	...
	char* tempReadBuffer = BG_TempAlloc(MAX_VEH_WEAPON_DATA_SIZE);
...
void BG_VehicleLoadParms(void)
{
	...
	*marker = 0;
	...
	char* tempReadBuffer = BG_TempAlloc(MAX_VEHICLE_DATA_SIZE);
