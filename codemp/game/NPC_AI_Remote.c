...
-------------------------
*/
void NPC_Remote_Pain(gentity_t* self, gentity_t* attacker, const int damage)
{
	SaveNPCGlobals();
	SetNPCGlobals(self);
	Remote_Strafe();
...
void Remote_MaintainHeight(void)
{
	...
	if (NPCS.NPC->enemy)
	{
		if (TIMER_Done(NPCS.NPC, "heightChange"))
		{
			...
			dif = NPCS.NPC->enemy->r.currentOrigin[2] + Q_irand(0, NPCS.NPC->enemy->r.maxs[2] + 8) - NPCS.NPC->r.
				currentOrigin[2];
...
void Remote_Hunt(const qboolean visible, const qboolean advance, const qboolean retreat)
{
	...
	float distance = VectorNormalize(forward);

	float speed = REMOTE_FORWARD_BASE_SPEED + REMOTE_FORWARD_MULTIPLIER * g_npcspskill.integer;
	if (retreat == qtrue)
	{
		speed *= -1;
	}
...
void Remote_Fire(void)
{
	...
	gentity_t* missile = create_missile(NPCS.NPC->r.currentOrigin, forward, 1000, 10000, NPCS.NPC, qfalse);
