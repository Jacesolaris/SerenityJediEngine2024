...
void Sentry_Fire(void)
{
	...
	const int which = NPCS.NPCInfo->burstCount % 3;
	switch (which)
	{
	case 0:
	...
	gentity_t* missile = create_missile(muzzle, forward, 1600, 10000, NPCS.NPC, qfalse);
...
void Sentry_Strafe(void)
{
	...
	if (tr.fraction > 0.9f)
	{
		...
		if (Q_irand(1, 2) == 1)
		{
			G_SoundOnEnt(NPCS.NPC, CHAN_AUTO, "sound/chars/sentry/misc/hover");
		}
	}
}
...
void Sentry_Hunt(const qboolean visible, const qboolean advance)
{
	...
	const float speed = SENTRY_FORWARD_BASE_SPEED + SENTRY_FORWARD_MULTIPLIER * g_npcspskill.integer;
