...
void InitSiegeMode(void)
{
	...
	if (BG_SiegeGetValueGroup(siege_info, team1, gParseObjectives))
	{
		...
		if (BG_SiegeGetPairedValue(gParseObjectives, "Timed", goalreq))
		{
			...
			{
				Com_Printf(
					"Tried to set imperial time limit, but there's already a rebel time limit!\nOnly one team can have a time limit.\n");
			}
			else
			{
...
void SiegeBroadcast_OBJECTIVECOMPLETE(const int team, const int client, const int objective)
{
	...
	gentity_t* te = G_TempEntity(nomatter, EV_SIEGE_OBJECTIVECOMPLETE);
...
void SiegeBroadcast_ROUNDOVER(const int winningteam, const int winningclient)
{
	...
	gentity_t* te = G_TempEntity(nomatter, EV_SIEGE_ROUNDOVER);
	te->r.svFlags |= SVF_BROADCAST;
...
void SiegeCheckTimers(void)
{
	...
	if (!gSiegeRoundBegun)
	{
		...
		{
			trap->SetConfigstring(CS_SIEGE_STATE, va("2|%i", gSiegeBeginTime - SIEGE_ROUND_BEGIN_TIME));
...
void SiegeItemUse(gentity_t* ent, gentity_t* other, gentity_t* activator)
{
	...
	if (ent->paintarget && ent->paintarget[0])
	{
		...
		const gentity_t* targ = G_Find(NULL, FOFS(targetname), ent->paintarget);
