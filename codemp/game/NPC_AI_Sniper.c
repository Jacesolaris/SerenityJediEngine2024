...
static qboolean Sniper_Move(void)
{
	...
	if (moved == qfalse)
	{
		...
		if (NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES && NPCS.NPCInfo->goalEntity && NPCS.NPCInfo->goalEntity ==
			NPCS.NPC->enemy)
		{
...
static void Sniper_CheckFireState(void)
{
	...
	if (!Q_irand(0, 1) && NPCS.NPCInfo->enemyLastSeenTime && level.time - NPCS.NPCInfo->enemyLastSeenTime < (5 - NPCS.
		NPCInfo->stats.aim) * 1000) //FIXME: incorporate skill too?
	{
