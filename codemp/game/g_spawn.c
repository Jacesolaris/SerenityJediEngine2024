...
void SP_trigger_airspace(gentity_t* self);
...
extern void SP_emplaced_eweb(gentity_t* ent);
...
spawn_t spawns[] = {
	...
	{"npc_BobaFett2", SP_NPC_BobaFett2},
	{"npc_BobaFett3", SP_NPC_BobaFett3},
	{"npc_BobaFett4", SP_NPC_BobaFett4},
	...
	{"terrain", SP_terrain},
	{"trigger_airspace", SP_trigger_airspace},
...
qboolean G_CallSpawn(gentity_t* ent)
{
	...
	const spawn_t* s = (spawn_t*)Q_LinearSearch(ent->classname, spawns, ARRAY_LEN(spawns), sizeof(spawn_t), spawncmp);
...
{
	return Q_stricmp(a, ((field_t*)b)->name);
}

void Q3_SetParm(int ent_id, int parmNum, const char* parmValue);
...
void G_SpawnGEntityFromSpawnVars(qboolean inSubBSP)
{
	...
	if (G_SpawnString("gametype", NULL, &value))
	{
		if (level.gametype >= GT_FFA && level.gametype < GT_MAX_GAME_TYPE)
		{
			...
			const char* s = strstr(value, gametypeName);
			if (!s)
			{
				ADJUST_AREAPORTAL();
				G_FreeEntity(ent);
...
void sje_set_entity_field(gentity_t* ent, const char* key, const char* value)
{
	...
	{
		// initializing correctly the fields with null value
...
// spawns the entity. Old function
void sje_spawn_entity(gentity_t* ent)
{
	// setting these to 0 so manual entities spawned by this function (used in quests, fo example) are not bugged by the spawnvars used by the last effect spawned through new entity system
	level.sje_spawn_strings_values_count[ent->s.number] = 0;
...
void sje_main_set_entity_field(const gentity_t* ent, const char* key, const char* value)
{
	...
	// see if this key already exists to edit it. If not, add a new one
	while (i < level.sje_spawn_strings_values_count[ent->s.number])
	{
		if (Q_stricmp(level.sje_spawn_strings[ent->s.number][i], key) == 0)
		{
			// found the key
			if (Q_stricmp(value, "sjeremovekey") == 0)
			{
				// remove the key
				// starts from the next key
				i += 2;

				// moves all keys after this one 2 positions to remove the key
				while (i < level.sje_spawn_strings_values_count[ent->s.number])
				{
					level.sje_spawn_strings[ent->s.number][i - 2] = G_NewString(
						level.sje_spawn_strings[ent->s.number][i]);
					level.sje_spawn_strings[ent->s.number][i - 1] = G_NewString(
						level.sje_spawn_strings[ent->s.number][i + 1]);

					i += 2;
				}

				// decrease the counter
				level.sje_spawn_strings_values_count[ent->s.number] -= 2;
			}
			else
			{
				// edit the key
				level.sje_spawn_strings[ent->s.number][i] = G_NewString(key);
				level.sje_spawn_strings[ent->s.number][i + 1] = G_NewString(value);
			}
...
// function to spawn entities used by entity system
void sje_main_spawn_entity(gentity_t* ent)
{
	int i = 0;
	int j = 0;
	char* value;
	...
	{
		G_ParseField(level.sje_spawn_strings[ent->s.number][j], level.sje_spawn_strings[ent->s.number][j + 1], ent);
		level.spawnVars[i][0] = G_NewString(level.sje_spawn_strings[ent->s.number][j]);
		level.spawnVars[i][1] = G_NewString(level.sje_spawn_strings[ent->s.number][j + 1]);
...
static void HandleEntityAdjustment(void)
{
	...
	const float rotation = DEG2RAD(level.mRotationAdjust);
...
void scriptrunner_run(gentity_t* ent);
...
void SP_worldspawn(void)
{
	...
	for (i = 1; i < LS_NUM_STYLES; i++)
	{
		...
		const int lengthGreen = strlen(text);
		...
		const int lengthBlue = strlen(text);
