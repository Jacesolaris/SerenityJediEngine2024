...
{
	const float turndelta = AngleDelta(NPCS.NPC->r.currentAngles[YAW], NPCS.NPCInfo->desiredYaw);

	if (fabs(turndelta) > 20 && (NPCS.NPC->client->NPC_class == CLASS_R2D2 || NPCS.NPC->client->NPC_class ==
		CLASS_R5D2))
	{
		const int anim = NPCS.NPC->client->ps.legsAnim;
		if (turndelta < 0)
		{
