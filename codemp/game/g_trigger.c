...
void Touch_Multi(gentity_t* self, gentity_t* other, trace_t* trace)
{
	...
	if (self->spawnflags & 2)
	{
		...
		{
			AngleVectors(other->r.currentAngles, forward, NULL, NULL);
		}
	...
	if (self->spawnflags & 4)
	{
		...
		if (other->client->ps.weaponTime > 0 && other->client->ps.torsoAnim != BOTH_BUTTON_HOLD && other->client->ps.
			torsoAnim != BOTH_CONSOLE1 || other->health < 1 ||
		...
		if (self->genericValue7)
		{
			...
			if (other->client->isHacking != self->s.number && other->s.number < MAX_CLIENTS)
			{
				//start the hack
			...
			if (other->client->ps.hackingTime < level.time)
			{
			...
			{
				//hack in progress
				// Added this code from below... It would never have gotten there before...
...
void SP_trigger_always(gentity_t* ent)
{
	...
	// added a wait time so it will be possible to save it in entity file (before it gets removed)
...
void AimAtTarget(gentity_t* self)
{
	...
	const float height = ent->s.origin[2] - origin[2];
...
void space_touch(gentity_t* self, const gentity_t* other, trace_t* trace)
{
		...
		&& other->client->ps.m_iVehicleNum >= MAX_CLIENTS)
	{
		...
		const gentity_t* veh = &g_entities[other->client->ps.m_iVehicleNum];
...
void trigger_visible_think(gentity_t* self)
{
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (!(player->client->ps.fd.forcePowersActive & 1 << FP_SEE) &&
			self->spawnflags & 2)
			continue;

		if (self->radius)
		{
			if (Distance(self->s.origin, player->client->renderInfo.eyePoint) > self->radius)
			{
				continue;
			}
		}
