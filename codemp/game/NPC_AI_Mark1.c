...
void Mark1Dead_FireRocket(void)
{
	...
	gentity_t* missile = create_missile(muzzle1, muzzle_dir, BOWCASTER_VELOCITY, 10000, NPCS.NPC, qfalse);
...
void Mark1Dead_FireBlaster(void)
{
	...
	gentity_t* missile = create_missile(muzzle1, muzzle_dir, 1600, 10000, NPCS.NPC, qfalse);
...
-------------------------
*/
void NPC_Mark1_Pain(gentity_t* self, gentity_t* attacker, const int damage)
{
	int newBolt;
...
void Mark1_FireBlaster(void)
{
	...
	gentity_t* missile = create_missile(muzzle1, forward, 1600, 10000, NPCS.NPC, qfalse);
...
void Mark1_BlasterAttack(const qboolean advance)
{
	if (TIMER_Done(NPCS.NPC, "attackDelay")) // Attack?
	{
		...
		if (chance == 1)
		{
			...
			NPCS.NPC->client->ps.torsoTimer = 0; // Just in case the firing anim is running.
		}
		else
		{
			if (TIMER_Done(NPCS.NPC, "attackDelay2")) // Can't be shooting every frame.
			{
				TIMER_Set(NPCS.NPC, "attackDelay2", Q_irand(50, 50));
				Mark1_FireBlaster();
	...
	{
		if (NPCS.NPC->client->ps.torsoAnim == BOTH_ATTACK1)
		{
			NPCS.NPC->client->ps.torsoTimer = 0; // Just in case the firing anim is running.
		}
		Mark1_Hunt();
...
void Mark1_FireRocket(void)
{
	...
	gentity_t* missile = create_missile(muzzle1, forward, BOWCASTER_VELOCITY, 10000, NPCS.NPC, qfalse);
...
void Mark1_RocketAttack(const qboolean advance)
{
	...
	{
		TIMER_Set(NPCS.NPC, "attackDelay", Q_irand(1000, 3000));
		NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, BOTH_ATTACK2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
		Mark1_FireRocket();
	}
	else if (advance)
	{
...
void Mark1_AttackDecision(void)
{
	...
	if (NPCS.NPC->enemy->health < 1 || NPC_CheckEnemyExt(qfalse) == qfalse)
	{
		NPCS.NPC->enemy = NULL;
		return;
	}
	...
	const qboolean advance = distance > MIN_DISTANCE_SQR;
	...
	const int blasterTest = trap->G2API_GetSurfaceRenderStatus(NPCS.NPC->ghoul2, 0, "l_arm");
	const int rocketTest = trap->G2API_GetSurfaceRenderStatus(NPCS.NPC->ghoul2, 0, "r_arm");
	...
	else // It should never get here, but just in case
	{
		...
		if (NPCS.NPC->die)
		{
			NPCS.NPC->die(NPCS.NPC, NPCS.NPC, NPCS.NPC, 100, MOD_UNKNOWN);
		}
	}
...
void Mark1_Patrol(void)
{
	...
	{
		G_Sound(NPCS.NPC, CHAN_AUTO, G_SoundIndex("sound/chars/mark1/misc/mark1_wakeup"));
