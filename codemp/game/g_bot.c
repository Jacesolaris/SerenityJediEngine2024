...
void g_load_arenas(void)
{
	...
	numdirs = trap->FS_GetFileList("mpscripts", ".arena", dirlist, 1024);
	dirptr = dirlist;
	for (i = 0; i < numdirs; i++, dirptr += dirlen + 1)
	{
		dirlen = strlen(dirptr);
		Q_strncpyz(filename, "mpscripts/", sizeof(filename));
	...
	// get all arenas from .arena files
	int numFiles = trap->FS_GetFileList("mpscripts", ".arena", filelist, ARRAY_LEN(filelist));
