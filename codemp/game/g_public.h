...
#define SVF_NOSERVERINFO		0x00000200	// don't send CS_SERVERINFO updates to this client// so that it can be updated for ping tools without
#define SVF_CAPSULE				0x00000400	// use capsule for collision detection instead of bbox
#define SVF_NOTSINGLECLIENT		0x00000800	// send entity to everyone but one client// (entityShared_t->singleClient)
#define SVF_OWNERNOTSHARED		0x00001000	// If it's owned by something and another thing owned by that something// hits it, it will still touch
#define	SVF_ICARUS_FREEZE		0x00008000	// NPCs are frozen, ents don't execute ICARUS commands
#define SVF_ADMIN				0x00020000		// client has admin privs
#define SVF_NONNPC_ENEMY		0x00040000
#define SVF_CUSTOM_GRAVITY		0x00400000	// Use personal gravity
#define SVF_BBRUSH			    0x04000000  // breakable brush
#define SVF_IGNORE_ENEMIES		0x10000000	// NPC ignores enemies
...
#define SVF_SELF_ANIMATING   	0x100000000	// Ent will control it's animation itself in it's think func
#define	SVF_LOCKEDENEMY			0x100000000	// keep current enemy until dead
#define	SVF_NPC_PRECACHE		0x100000000	// Tell cgame to precache this spawner's NPC stuff
#define	SVF_BROKEN      		0x200000000	// A broken misc_model_breakable
...
typedef struct gameImport_s {
	...
	int         (*SE_GetStringTextString)				(const char* text, char* buffer, int bufferLength);

	// ROFF
	qboolean(*ROFF_Clean)							(void);
	void		(*ROFF_UpdateEntities)					(void);
	int			(*ROFF_Cache)							(char* file);
	qboolean(*ROFF_Play)							(int ent_id, int roff_id, qboolean do_translation);
	qboolean(*ROFF_Purge_Ent)						(int ent_id);

	// ICARUS
	int			(*ICARUS_RunScript)						(const sharedEntity_t* ent, const char* name);
	...
	qboolean(*ICARUS_IsInitialized)					(int ent_id);
	qboolean(*ICARUS_MaintainTaskManager)			(int ent_id);
	qboolean(*ICARUS_IsRunning)						(int ent_id);
	qboolean(*ICARUS_TaskIDPending)					(sharedEntity_t* ent, int taskID);
	void		(*ICARUS_InitEnt)						(sharedEntity_t* ent);
	void		(*ICARUS_FreeEnt)						(const sharedEntity_t* ent);
	void		(*ICARUS_AssociateEnt)					(const sharedEntity_t* ent);
	...
	void		(*ICARUS_SetVar)						(int taskID, int ent_id, const char* type_name, const char* data);
	...
	void		(*Nav_AddFailedEdge)					(int ent_id, int startID, int endID);
	...
	void		(*G2API_SetGhoul2model_indexes)			(void* ghoul2, qhandle_t* modelList, qhandle_t* skinList);
	qboolean(*G2API_GetBoltMatrix)					(void* ghoul2, int modelIndex, int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, int frameNum, qhandle_t* modelList, vec3_t scale);
	qboolean(*G2API_GetBoltMatrix_NoReconstruct)	(void* ghoul2, int modelIndex, int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, int frameNum, qhandle_t* modelList, vec3_t scale);
	qboolean(*G2API_GetBoltMatrix_NoRecNoRot)		(void* ghoul2, int modelIndex, int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, int frameNum, qhandle_t* modelList, vec3_t scale);
	int			(*G2API_InitGhoul2Model)				(void** ghoul2Ptr, const char* file_name, int modelIndex, qhandle_t customSkin, qhandle_t customShader, int modelFlags, int lodBias);
	...
	void		(*G2API_SetBoltInfo)					(void* ghoul2, int modelIndex, int bolt_info);
	qboolean(*G2API_SetBoneAngles)					(void* ghoul2, int modelIndex, const char* boneName, const vec3_t angles, int flags, int up, int right, int forward, qhandle_t* modelList, int blendTime, int currentTime);
	qboolean(*G2API_SetBoneAnim)					(void* ghoul2, int modelIndex, const char* boneName, int startFrame, int endFrame, int flags, float animSpeed, int currentTime, float setFrame, int blendTime);
	qboolean(*G2API_GetBoneAnim)					(void* ghoul2, const char* boneName, int currentTime, float* currentFrame, int* startFrame, int* endFrame, int* flags, float* animSpeed, int* modelList, int modelIndex);
	void		(*G2API_GetGLAName)						(void* ghoul2, int modelIndex, char* fillBuf);
	int			(*G2API_CopyGhoul2Instance)				(void* g2_from, void* g2_to, int modelIndex);
	void		(*G2API_CopySpecificGhoul2Model)		(void* g2_from, int model_from, void* g2_to, int model_to);
	...
	qboolean(*G2API_SetRootSurface)					(void* ghoul2, int modelIndex, const char* surfaceName);
	...
	int			(*G2API_GetSurfaceRenderStatus)			(void* ghoul2, int modelIndex, const char* surfaceName);
	...
	qboolean(*G2API_RagPCJGradientSpeed)			(void* ghoul2, const char* boneName, float speed);
...
typedef struct gameExport_s {
	...
	void		(*ROFF_NotetrackCallback)			(int ent_id, const char* notetrack);
	...
	qboolean(*NAV_ClearPathToPoint)				(int ent_id, vec3_t pmins, vec3_t pmaxs, vec3_t point, int clipmask, int okToHitEnt);
