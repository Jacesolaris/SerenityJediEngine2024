...
void NAVNEW_PushBlocker(const gentity_t* self, const gentity_t* blocker, vec3_t right, const qboolean setBlockedInfo)
{
	...
	const float moveamt = (self->r.maxs[1] + blocker->r.maxs[1]) * 1.2; //yes, magic number
...
qboolean NAVNEW_DanceWithBlocker(gentity_t* self, const gentity_t* blocker, vec3_t movedir, vec3_t right)
{
	...
	if (blocker->client && !VectorCompare(blocker->client->ps.velocity, vec3_origin))
	{
		...
		const float dot = DotProduct(blocker_movedir, right);
		if (dot > 50.0f)
		{
		...
		*/
		if (!inGoalWP)
		{
			...
			else
			{
				...
				const int oldBestNode = bestNode;
...
qboolean NAV_MoveDirSafe(const gentity_t* self, const usercmd_t* cmd, const float distScale)
{
	...
	if (VectorCompare(self->client->ps.moveDir, vec3_origin))
	{
		...
		vec3_t fwd, right, fwdAngs;
		if (!cmd->forwardmove && !cmd->rightmove)
		{
			//not moving at all
			return qtrue;
		}
		VectorSet(fwdAngs, 0, self->r.currentAngles[YAW], 0);
