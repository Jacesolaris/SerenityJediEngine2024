...
// NPC_misc.cpp
//
extern void Debug_Printf(const vmCvar_t* cv, int debug_level, char* fmt, ...);
extern void Debug_NPCPrintf(const gentity_t* printNPC, const vmCvar_t* cv, int debugLevel, char* fmt, ...);
...
extern qboolean ValidEnemy(const gentity_t* ent);
...
extern int NPC_ShotEntity(const gentity_t* ent, vec3_t impactPos); //impactedPos = NULL
extern int NPC_HaveBetterWeapon(int weapon);
...
extern int NPC_FindCombatPoint(const vec3_t position, const vec3_t avoidPosition, vec3_t dest_position, int flags,
	float avoidDist, int ignorePoint); //ignorePoint = -1
