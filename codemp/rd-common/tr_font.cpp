...
using FontIndexMap_t = std::map<sstring_t, int>;
...
static bool Korean_ValidKSC5601Hangul(const byte _iHi, const byte _iLo)
{
...
static bool Korean_ValidKSC5601Hangul(const unsigned int uiCode)
{
...
static bool Japanese_ValidShiftJISCode(const unsigned int uiCode)
{
...
static bool Chinese_ValidGBCode(const byte _iHi, const byte _iLo)
{
...
static bool Chinese_ValidGBCode(const unsigned int uiCode)
{
...
static int Thai_ValidTISCode(const byte* psString, int& iThaiBytes)
{
	...
	if (psString[0] >= 160)	// so western letters drop through and use normal font
	{
		...
		for (i = 0; i < 3; i++)
		{
			...
			const int iIndex = g_ThaiCodes.GetValidIndex(CodeToTry.uiCode);
			if (iIndex == -1)
			{
...
//
static bool Thai_IsTrailingPunctuation(const unsigned int uiCode)
{
	return uiCode == '_';
}
...
static int Thai_CollapseTISCode(const unsigned int uiCode)
{
	...
	{
		const int iCollapsedIndex = g_ThaiCodes.GetValidIndex(uiCode);
...
{
	const auto psString = (const byte*)psText;	// avoid sign-promote bug
...
CFontInfo::CFontInfo(const char* _fontName)
{
	...
	m_bIsFakeAlienLanguage = strcmp(_fontName, "aurabesh") == 0;	// dont try and make SBCS or asian overrides for this
	...
	if (len == sizeof(dfontdat_t))
	{
		...
		{
			mHeight = mPointSize;
			mAscender = mPointSize - Round(static_cast<float>(mPointSize) / 10.0f + 2);	// have to completely guess at the baseline... sigh.
...
void CFontInfo::UpdateAsianIfNeeded(bool bForceReEval /* = false */)
{
	...
	{
		const Language_e eLanguage = GetLanguageEnum();

		if (eLanguage == eKorean || eLanguage == eTaiwanese || eLanguage == eJapanese || eLanguage == eChinese || eLanguage == eThai)
		{
			const int iCappedHeight = mHeight < 16 ? 16 : mHeight;	// arbitrary limit on small char sizes because Asian chars don't squash well
	...
	if (AsianGlyphsAvailable())
	{
		...
		if (iCollapsedAsianCode)
		{
			if (piShader)
			{
				...
				const int iOriginalCollapsedAsianCode = iCollapsedAsianCode;	// need to back this up (if Thai) for later
				...
				default:
				{
					m_AsianGlyph.s = static_cast<float>(iColumn) / static_cast<float>(m_iAsianGlyphsAcross);
					m_AsianGlyph.t = static_cast<float>(iRow) / static_cast<float>(iAsianGlyphsDown);
					m_AsianGlyph.s2 = static_cast<float>(iColumn + 1) / static_cast<float>(m_iAsianGlyphsAcross);
					m_AsianGlyph.t2 = static_cast<float>(iRow + 1) / static_cast<float>(iAsianGlyphsDown);
				}
				break;

				case eTaiwanese:
				{
					m_AsianGlyph.s = static_cast<float>(1024 / m_iAsianGlyphsAcross * iColumn + 1) / 1024.0f;
					m_AsianGlyph.t = static_cast<float>(1024 / iAsianGlyphsDown * iRow + 1) / 1024.0f;
					m_AsianGlyph.s2 = static_cast<float>(1024 / m_iAsianGlyphsAcross * (iColumn + 1)) / 1024.0f;
					m_AsianGlyph.t2 = static_cast<float>(1024 / iAsianGlyphsDown * (iRow + 1)) / 1024.0f;
				}
				break;
				...
				case eChinese:
				{
					m_AsianGlyph.s = static_cast<float>(1024 / m_iAsianGlyphsAcross * iColumn) / 1024.0f;
					m_AsianGlyph.t = static_cast<float>(1024 / iAsianGlyphsDown * iRow) / 1024.0f;
					m_AsianGlyph.s2 = static_cast<float>(1024 / m_iAsianGlyphsAcross * (iColumn + 1) - 1) / 1024.0f;
					m_AsianGlyph.t2 = static_cast<float>(1024 / iAsianGlyphsDown * (iRow + 1) - 1) / 1024.0f;
				}
				break;
				...
				case eThai:
				{
					...
					m_AsianGlyph.s = static_cast<float>(iGlyphXpos) / 1024.0f;
					m_AsianGlyph.t = static_cast<float>(1024 / iAsianGlyphsDown * iRow) / 1024.0f;
					...
					m_AsianGlyph.s2 = static_cast<float>(iGlyphXpos + iGlyphWidth) / 1024.0f;
					m_AsianGlyph.t2 = static_cast<float>(1024 / iAsianGlyphsDown * (iRow + 1) - 1) / 1024.0f;
	...
	if (!pFont->m_bIsFakeAlienLanguage)
	{
		if (GetLanguageEnum() == eLanguageSBCS)
		{
			if (pFont->m_iAltSBCSFont == -1) 	// no reg attempted yet?
			{
				...
				//
				const int iAltFontIndex = RE_RegisterFont(va("%s/%s", COM_SkipPath(pFont->m_sFontName), psLanguageNameSBCS));	// ensure unique name (eg: "lcd/russian")
				...
				if (pAltFont)
				{
					...
					//
					pAltFont->m_fAltSBCSFontScaleFactor = RoundTenth(static_cast<float>(pFont->GetPointSize()) / static_cast<float>(pAltFont->GetPointSize()));
					//
...
float RE_Font_StrLenPixelsNew(const char* psText, const int iFontHandle, const float fScale) {
	...
	while (*psText) {
		...
		if (uiLetter == '\n') {
			...
			const float i_pixel_advance = static_cast<float>(curfont->GetLetterHorizAdvance(uiLetter));

			float f_value = i_pixel_advance * (uiLetter > static_cast<unsigned>(g_iNonScaledCharRange) ? fScaleAsian : fScale);

			if (r_aspectCorrectFonts->integer == 1) {
				f_value *= static_cast<float>((SCREEN_WIDTH * glConfig.vidHeight)) / static_cast<float>((SCREEN_HEIGHT * glConfig.vidWidth));
			}
...
int RE_Font_StrLenPixels(const char* psText, const int iFontHandle, const float fScale) {
	return static_cast<int>(ceilf(RE_Font_StrLenPixelsNew(psText, iFontHandle, fScale)));
}
...
{
	const CFontInfo* curfont = GetFont(iFontHandle);
	if (curfont)
	{
		const float fValue = curfont->GetPointSize() * fScale;
...
void RE_Font_DrawString(const int ox, const int oy, const char* psText, const float* rgba, const int iFontHandle, const int iMaxPixelWidth, const float fScale)
{
	...
	while (*psText && !bNextTextWouldOverflow)
	{
		...
		case 32:						// Space
			...
			bNextTextWouldOverflow = iMaxPixelWidth != -1 && fx - fox > static_cast<float>(iMaxPixelWidth) ? qtrue : qfalse; // yeuch
			break;
		...
		default:
			...
			const float fThisScale = uiLetter > static_cast<unsigned>(g_iNonScaledCharRange) ? fScaleAsian : fScale;
			...
			const float fAdvancePixels = curfont->mbRoundCalcs ? Round(pLetter->horizAdvance * fThisScale) : pLetter->horizAdvance * fThisScale;
			bNextTextWouldOverflow = iMaxPixelWidth != -1 && fx + fAdvancePixels - fox > static_cast<float>(iMaxPixelWidth) ? qtrue : qfalse; // yeuch
			if (!bNextTextWouldOverflow)
			{
				...
				RE_StretchPic(curfont->mbRoundCalcs ? fx + Round(pLetter->horizOffset * fThisScale) : fx + pLetter->horizOffset * fThisScale, // float x
					uiLetter > static_cast<unsigned>(g_iNonScaledCharRange) ? fy - fAsianYAdjust : fy,	// float y
				...
				if (r_aspectCorrectFonts->integer == 1) {
					fx += fAdvancePixels
						* (static_cast<float>((SCREEN_WIDTH * glConfig.vidHeight)) / static_cast<float>((SCREEN_HEIGHT * glConfig.vidWidth)));
				}
				else if (r_aspectCorrectFonts->integer == 2) {
...
void R_ReloadFonts_f()
{
	...
	if (iFontToFind == g_iCurrentFontIndex) // found all of them?
	{
		...
		//
		for (size_t iFont = 0; iFont < vstr_fonts.size(); iFont++)
		{
#ifdef _DEBUG
			const int iNewFontHandle = RE_RegisterFont(vstr_fonts[iFont].c_str());
			assert(iNewFontHandle == static_cast<int>(iFont + 1));
#else
