...
void LoadJPG(const char* filename, unsigned char** pic, int* width, int* height) {
	...
	*/
	...
	out = static_cast<byte*>(Z_Malloc(memcount, TAG_TEMP_WORKSPACE, qfalse));
...
{
	const auto dest = reinterpret_cast<my_dest_ptr>(cinfo->dest);
...
void RE_SaveJPG(const char* filename, const int quality, const int image_width, const int image_height, byte* image_buffer, const int padding)
{
	...
	const auto out = static_cast<byte*>(Hunk_AllocateTempMemory(bufSize));
