	...
	JDIMENSION out_rows_avail)
{
	...
	const JDIMENSION old_next_row = post->next_row;
	(*cinfo->upsample->upsample) (cinfo,
		input_buf, in_row_group_ctr, in_row_groups_avail,
	...
	if (post->next_row > old_next_row) {
		const JDIMENSION num_rows = post->next_row - old_next_row;
	...
	const JDIMENSION out_rows_avail)
{
	...
	JDIMENSION num_rows = post->strip_height - post->next_row; /* available in strip */
	JDIMENSION max_rows = out_rows_avail - *out_row_ctr; /* available in output area */
		...
		if (need_full_buffer) {
			...
			post->whole_image = (*cinfo->mem->request_virt_sarray)
				((j_common_ptr)cinfo, JPOOL_IMAGE, FALSE,
					...
					(JDIMENSION)jround_up(cinfo->output_height,
						post->strip_height),
					post->strip_height);
#else
			ERREXIT(cinfo, JERR_BAD_BUFFER_MODE);
