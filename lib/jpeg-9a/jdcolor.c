...
ycc_rgb_convert(const j_decompress_ptr cinfo,
	const JSAMp_image input_buf, JDIMENSION input_row,
	JSAMPARRAY output_buf, int num_rows)
{
	const my_cconvert_ptr cconvert = (my_cconvert_ptr)cinfo->cconvert;
	const JDIMENSION num_cols = cinfo->output_width;
	/* copy these pointers into registers if possible */
	const register JSAMPLE* range_limit = cconvert->range_limit;
	const register int* Crrtab = cconvert->Cr_r_tab;
	const register int* Cbbtab = cconvert->Cb_b_tab;
	const register INT32* Crgtab = cconvert->Cr_g_tab;
	const register INT32* Cbgtab = cconvert->Cb_g_tab;
	SHIFT_TEMPS

		while (--num_rows >= 0) {
			const register JSAMPROW inptr0 = input_buf[0][input_row];
			const register JSAMPROW inptr1 = input_buf[1][input_row];
			const register JSAMPROW inptr2 = input_buf[2][input_row];
			input_row++;
			register JSAMPROW outptr = *output_buf++;
			for (register JDIMENSION col = 0; col < num_cols; col++) {
				const register int y = GETJSAMPLE(inptr0[col]);
				const register int cb = GETJSAMPLE(inptr1[col]);
				const register int cr = GETJSAMPLE(inptr2[col]);
	...
	JSAMPARRAY output_buf, int num_rows)
{
	...
	while (--num_rows >= 0) {
		...
		const register JSAMPROW inptr2 = input_buf[2][input_row];
		input_row++;
		const register JSAMPROW outptr = *output_buf++;
		for (register JDIMENSION col = 0; col < num_cols; col++) {
			const register int r = GETJSAMPLE(inptr0[col]);
			const register int g = GETJSAMPLE(inptr1[col]);
			const register int b = GETJSAMPLE(inptr2[col]);
			/* Y */
			outptr[col] = (JSAMPLE)
	...
	JSAMPARRAY output_buf, int num_rows)
{
	...
	while (--num_rows >= 0) {
		...
		const register JSAMPROW outptr = *output_buf++;
		for (register JDIMENSION col = 0; col < num_cols; col++) {
			register int r = GETJSAMPLE(inptr0[col]);
			const register int g = GETJSAMPLE(inptr1[col]);
			register int b = GETJSAMPLE(inptr2[col]);
			/* Assume that MAXJSAMPLE+1 is a power of 2, so that the MOD
			 * (modulo) operator is equivalent to the bitmask operator AND.
			 */
	...
	JSAMPARRAY output_buf, int num_rows)
{
	...
	while (--num_rows >= 0) {
		for (int ci = 0; ci < nc; ci++) {
			register JSAMPROW inptr = input_buf[ci][input_row];
			register JSAMPROW outptr = output_buf[0] + ci;
			for (register JDIMENSION col = 0; col < num_cols; col++) {
...
grayscale_convert(const j_decompress_ptr cinfo,
	const JSAMp_image input_buf, const JDIMENSION input_row,
	...
	JSAMPARRAY output_buf, int num_rows)
{
	...
	while (--num_rows >= 0) {
		const register JSAMPROW inptr = input_buf[0][input_row++];
		register JSAMPROW outptr = *output_buf++;
		for (register JDIMENSION col = 0; col < num_cols; col++) {
			/* We can dispense with GETJSAMPLE() here */
	...
	JSAMPARRAY output_buf, int num_rows)
{
	...
	const JDIMENSION num_cols = cinfo->output_width;
	/* copy these pointers into registers if possible */
	const register JSAMPLE* range_limit = cinfo->sample_range_limit;
	const register int* Crrtab = cconvert->Cr_r_tab;
	const register int* Cbbtab = cconvert->Cb_b_tab;
	const register INT32* Crgtab = cconvert->Cr_g_tab;
	const register INT32* Cbgtab = cconvert->Cb_g_tab;
	SHIFT_TEMPS
		...
		while (--num_rows >= 0) {
			...
			const register JSAMPROW inptr1 = input_buf[1][input_row];
			const register JSAMPROW inptr2 = input_buf[2][input_row];
			const register JSAMPROW inptr3 = input_buf[3][input_row];
			input_row++;
			register JSAMPROW outptr = *output_buf++;
			for (register JDIMENSION col = 0; col < num_cols; col++) {
				const register int y = GETJSAMPLE(inptr0[col]);
