 ...
 */
{
	...
	int ci;
	const int M = cinfo->min_DCT_v_scaled_size;
	...
	for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		ci++, compptr++) {
		const int rgroup = compptr->v_samp_factor * compptr->DCT_v_scaled_size /
 ...
 */
{
	...
	for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		...
		const int rgroup = compptr->v_samp_factor * compptr->DCT_v_scaled_size /
			cinfo->min_DCT_v_scaled_size; /* height of a row group of component */
		const JSAMPARRAY xbuf0 = mainp->xbuffer[0][ci];
		const JSAMPARRAY xbuf1 = mainp->xbuffer[1][ci];
		/* First copy the workspace pointers as-is */
		const JSAMPARRAY buf = mainp->buffer[ci];
 ...
 */
{
	...
	for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		 ...
		 */
		const JSAMPARRAY xbuf = mainp->xbuffer[mainp->whichptr][ci];
