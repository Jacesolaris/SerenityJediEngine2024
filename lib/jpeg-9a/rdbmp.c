...
/* This version is for reading 8-bit colormap indexes */
{
	...
	const register JSAMPARRAY colormap = source->colormap;
	...
	register JSAMPROW inptr = image_ptr[0];
	register JSAMPROW outptr = source->pub.buffer[0];
	for (register JDIMENSION col = cinfo->image_width; col > 0; col--) {
		const register int t = GETJSAMPLE(*inptr++);
...
preload_image(const j_compress_ptr cinfo, const cjpeg_source_ptr sinfo)
{
	...
	for (JDIMENSION row = 0; row < cinfo->image_height; row++) {
		if (progress != NULL) {
		...
		register JSAMPROW out_ptr = image_ptr[0];
		for (JDIMENSION col = source->row_width; col > 0; col--) {
