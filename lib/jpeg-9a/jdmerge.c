	...
	const JSAMPARRAY output_buf)
{
	...
	const INT32* Crgtab = upsample->Cr_g_tab;
	const INT32* Cbgtab = upsample->Cb_g_tab;
	SHIFT_TEMPS

		JSAMPROW inptr0 = input_buf[0][in_row_group_ctr];
	JSAMPROW inptr1 = input_buf[1][in_row_group_ctr];
	JSAMPROW inptr2 = input_buf[2][in_row_group_ctr];
	register JSAMPROW outptr = output_buf[0];
	/* Loop for each pair of output pixels */
	for (JDIMENSION col = cinfo->output_width >> 1; col > 0; col--) {
	...
	const JSAMPARRAY output_buf)
{
	...
	SHIFT_TEMPS
		...
		JSAMPROW inptr00 = input_buf[0][in_row_group_ctr * 2];
	JSAMPROW inptr01 = input_buf[0][in_row_group_ctr * 2 + 1];
	JSAMPROW inptr1 = input_buf[1][in_row_group_ctr];
	JSAMPROW inptr2 = input_buf[2][in_row_group_ctr];
	register JSAMPROW outptr0 = output_buf[0];
	register JSAMPROW outptr1 = output_buf[1];
