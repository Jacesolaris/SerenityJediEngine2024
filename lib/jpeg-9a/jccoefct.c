	...
	for (int yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row;
		...
		for (JDIMENSION MCU_col_num = coef->mcu_ctr; MCU_col_num <= last_MCU_col;
			...
			for (int ci = 0; ci < cinfo->comps_in_scan; ci++) {
				...
				const forward_DCT_ptr forward_DCT = cinfo->fdct->forward_DCT[compptr->component_index];
				const int blockcnt = MCU_col_num < last_MCU_col
					? compptr->MCU_width
					: compptr->last_col_width;
				const JDIMENSION xpos = MCU_col_num * compptr->MCU_sample_width;
				JDIMENSION ypos = yoffset * compptr->DCT_v_scaled_size;
...
compress_first_pass(const j_compress_ptr cinfo, const JSAMp_image input_buf)
{
	const my_coef_ptr coef = (my_coef_ptr)cinfo->coef;
	const JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
	int bi, ci, block_row, block_rows;
	JCOEF lastDC;
	...
	for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		...
		if (coef->iMCU_row_num == last_iMCU_row) {
			...
			const JDIMENSION MCUs_across = blocks_across / h_samp_factor;
			for (block_row = block_rows; block_row < compptr->v_samp_factor;
				block_row++) {
				thisblockrow = buffer[block_row];
				JBLOCKROW lastblockrow = buffer[block_row - 1];
				FMEMZERO((void FAR*) thisblockrow,
					(size_t)(blocks_across * SIZEOF(JBLOCK)));
				for (JDIMENSION MCUindex = 0; MCUindex < MCUs_across; MCUindex++) {
