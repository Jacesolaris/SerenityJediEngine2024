	...
	/* This version is for writing 24-bit pixels */
{
	 ...
	 */
	register JSAMPROW inptr = dest->pub.buffer[0];
	register JSAMPROW outptr = image_ptr[0];
	for (register JDIMENSION col = cinfo->output_width; col > 0; col--) {
...
#define PUT_4B(array,offset,value)  \
	...
	/* File size */
	const INT32 headersize = 14 + 40 + cmap_entries * 4; /* Header and colormap */
	const INT32 bfSize = headersize + (INT32)dest->row_width * (INT32)cinfo->output_height;
...
/* Write an OS2-style BMP file header, including colormap if needed */
{
	...
	/* File size */
	const INT32 headersize = 14 + 12 + cmap_entries * 3; /* Header and colormap */
	const INT32 bfSize = headersize + (INT32)dest->row_width * (INT32)cinfo->output_height;
...
finish_output_bmp(const j_decompress_ptr cinfo, const djpeg_dest_ptr dinfo)
{
	...
	for (JDIMENSION row = cinfo->output_height; row > 0; row--) {
		if (progress != NULL) {
		...
		register JSAMPROW data_ptr = image_ptr[0];
		for (register JDIMENSION col = dest->row_width; col > 0; col--) {
