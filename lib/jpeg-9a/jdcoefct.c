...
decompress_onepass(const j_decompress_ptr cinfo, const JSAMp_image output_buf)
{
	...
	for (int yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row;
		yoffset++) {
		for (JDIMENSION MCU_col_num = coef->MCU_ctr; MCU_col_num <= last_MCU_col;
			MCU_col_num++) {
			...
			for (int ci = 0; ci < cinfo->comps_in_scan; ci++) {
				...
				const inverse_DCT_method_ptr inverse_DCT = cinfo->idct->inverse_DCT[compptr->component_index];
				const int useful_width = MCU_col_num < last_MCU_col
					? compptr->MCU_width
					: compptr->last_col_width;
				JSAMPARRAY output_ptr = output_buf[compptr->component_index] +
					yoffset * compptr->DCT_v_scaled_size;
				const JDIMENSION start_col = MCU_col_num * compptr->MCU_sample_width;
...
consume_data(const j_decompress_ptr cinfo)
{
	...
	for (int yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row;
		yoffset++) {
		for (JDIMENSION MCU_col_num = coef->MCU_ctr; MCU_col_num < cinfo->MCUs_per_row;
			MCU_col_num++) {
			/* Construct list of pointers to DCT blocks belonging to this MCU */
			int blkn = 0;			/* index of current DCT block within MCU */
			for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
				compptr = cinfo->cur_comp_info[ci];
				const JDIMENSION start_col = MCU_col_num * compptr->MCU_width;
				for (int yindex = 0; yindex < compptr->MCU_height; yindex++) {
					JBLOCKROW buffer_ptr = buffer[ci][yindex + yoffset] + start_col;
...
decompress_data(const j_decompress_ptr cinfo, const JSAMp_image output_buf)
{
	...
	for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		...
		const JBLOCKARRAY buffer = (*cinfo->mem->access_virt_barray)
		...
		const inverse_DCT_method_ptr inverse_DCT = cinfo->idct->inverse_DCT[ci];
		JSAMPARRAY output_ptr = output_buf[ci];
		/* Loop over all DCT blocks to be processed. */
		for (int block_row = 0; block_row < block_rows; block_row++) {
			JBLOCKROW buffer_ptr = buffer[block_row];
			JDIMENSION output_col = 0;
			for (JDIMENSION block_num = 0; block_num < compptr->width_in_blocks; block_num++) {
...
smoothing_ok(const j_decompress_ptr cinfo)
{
	...
	for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		...
		const int* coef_bits = cinfo->coef_bits[ci];
		if (coef_bits[0] < 0)
			return FALSE;
