	...
	const JSAMPARRAY input_data, JSAMPARRAY* output_data_ptr)
{
	...
	const int h_expand = upsample->h_expand[compptr->component_index];
	const int v_expand = upsample->v_expand[compptr->component_index];
	...
	while (outrow < cinfo->max_v_samp_factor) {
		...
		const JSAMPROW outend = outptr + cinfo->output_width;
		while (outptr < outend) {
			const register JSAMPLE invalue = *inptr++;	/* don't need GETJSAMPLE() here */
			for (register int h = h_expand; h > 0; h--) {
				*outptr++ = invalue;
			}
		}
...
jinit_upsampler(const j_decompress_ptr cinfo)
{
	...
	for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		 ...
		 */
		const int h_in_group = compptr->h_samp_factor * compptr->DCT_h_scaled_size /
			cinfo->min_DCT_h_scaled_size;
		const int v_in_group = compptr->v_samp_factor * compptr->DCT_v_scaled_size /
			cinfo->min_DCT_v_scaled_size;
		const int h_out_group = cinfo->max_h_samp_factor;
		const int v_out_group = cinfo->max_v_samp_factor;
